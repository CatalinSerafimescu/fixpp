...
std::string make_logon_body() {
    return std::string("35=A\x01") + "34=1\x01" + "49=SENDER\x01" + "56=TARGET\x01" +
           "52=20260516-09:30:00\x01" + "98=0\x01" + "108=30\x01" + "553=user\x01" + "554=pass\x01";
}

// NewOrderSingle (~20 fields, ~20 occ)
std::string make_nos_body() {
    return std::string("35=D\x01") + "34=1\x01" + "49=SENDER01\x01" + "56=TARGET01\x01" +
           "52=20260516-09:30:00.000\x01" + "11=ORD001\x01" + "55=AAPL\x01" + "54=1\x01" +
           "38=100\x01" + "40=2\x01" + "44=150.25\x01" + "59=0\x01" +
           "60=20260516-09:30:00.000\x01" + "1=ACC001\x01" + "21=1\x01" + "110=0\x01" +
...
std::string make_nol_body(int num_orders) {
    std::string body = std::string("35=E\x01") + "34=1\x01" + "49=SENDER\x01" + "56=TARGET\x01" +
                       "52=20260516-09:30:00.000\x01";
...
std::string make_mdir_body(int num_entries) {
    std::string body = std::string("35=X\x01") + "34=1\x01" + "49=SENDER\x01" + "56=TARGET\x01" +
                       "52=20260516-09:30:00.000\x01" + "262=REQ001\x01";
...
std::string make_seclist_body(int num_strikes) {
    std::string body = std::string("35=y\x01") + "34=1\x01" + "49=SENDER\x01" + "56=TARGET\x01" +
                       "52=20260516-09:30:00.000\x01";
...
[[nodiscard]] std::size_t count_occurrences(std::span<const std::byte> frame) noexcept {
    ...
    while (i < frame.size()) {
        // skip tag digits
        while (i < frame.size() && frame[i] != EQ && frame[i] != SOH) {
            ++i;
        }
        if (i >= frame.size() || frame[i] != EQ) {
            break;
        }
        ++i;  // skip '='
        // skip value bytes until SOH
        while (i < frame.size() && frame[i] != SOH) {
            ++i;
        }
        if (i < frame.size()) {
            ++i;
