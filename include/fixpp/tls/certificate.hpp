...
struct Certificate {
    ...
    raw_der() const noexcept [[clang::lifetimebound]] { return raw_der_; }

    // Subject distinguished name (UTF-8 one-line representation).
    [[nodiscard]] std::string_view
    subject_dn() const noexcept [[clang::lifetimebound]] { return subject_dn_; }

    // Issuer distinguished name.
    [[nodiscard]] std::string_view
    issuer_dn() const noexcept [[clang::lifetimebound]] { return issuer_dn_; }

    // SAN DNS names (bounded by max_san_entries).
    [[nodiscard]] std::span<const std::string_view>
    san_dns_names() const noexcept [[clang::lifetimebound]] { return san_dns_names_; }

    // SAN URI names (bounded by max_san_entries together with san_dns_names).
    [[nodiscard]] std::span<const std::string_view>
    san_uris() const noexcept [[clang::lifetimebound]] { return san_uris_; }

    // SHA-256 of the raw DER bytes — the Pinset fingerprint key per /clarify Q4.
    [[nodiscard]] std::array<std::byte, 32>
    sha256() const noexcept { return sha256_; }

    // X.509 version (1, 2, or 3; v1 rejected by verify_peer).
    [[nodiscard]] int
    x509_version() const noexcept { return x509_version_; }
    ...
    not_before() const noexcept { return not_before_; }

    [[nodiscard]] std::chrono::system_clock::time_point
    not_after() const noexcept { return not_after_; }

    // Signature algorithm (rsa_pss / ecdsa / unspecified).
    [[nodiscard]] signature_algorithm
    alg() const noexcept { return alg_; }

    // RSA key size in bits — only meaningful when alg() == rsa_pss; else 0.
    [[nodiscard]] std::size_t
    rsa_key_bits() const noexcept { return rsa_key_bits_; }

    // ECDSA curve — only meaningful when alg() == ecdsa; else unspecified.
    [[nodiscard]] ecdsa_curve
    curve() const noexcept { return curve_; }
