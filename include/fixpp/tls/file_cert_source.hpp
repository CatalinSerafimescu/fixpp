...
public:
    struct Config {
        ...
        std::string
            chain_path;  // PEM bundle; intermediates only or leaf+intermediates (deduped on load).
        std::string private_key_path;  // PEM or DER; PKCS#8 or RFC1421-style.
        std::string
            ca_bundle_path;  // PEM bundle for CA trust anchors. May be empty for mtls_pinned.
        std::function<std::string()>
            password_cb;  // Optional; called once at construction-time. May be empty.
    ...
    [[nodiscard]] static core::expected_t<std::shared_ptr<cert_source>> make_file_cert_source(
    ...
    [[nodiscard]] asio::awaitable<core::expected_t<local_credentials>> load_credentials() override;
