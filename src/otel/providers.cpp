...
opentelemetry::sdk::resource::Resource make_resource(const OtelResourceAttributes& ra) {
    opentelemetry::sdk::common::AttributeMap attrs;
    if (!ra.service_name.empty()) attrs["service.name"] = ra.service_name;
    if (!ra.service_version.empty()) attrs["service.version"] = ra.service_version;
    if (!ra.deployment_environment.empty())
        attrs["deployment.environment"] = ra.deployment_environment;
    for (const auto& [k, v] : ra.extra) attrs[k] = v;
...
TracerProvider::TracerProvider(const OtelConfig& cfg) : impl_(std::make_unique<Impl>()) {
    try {
        if (cfg.tracer_factory_for_test) {
            ...
            impl_->provider =
                std::shared_ptr<opentelemetry::trace::TracerProvider>(sdk_provider.release());
        }
    } catch (...) {
...
opentelemetry::nostd::shared_ptr<opentelemetry::trace::Tracer> TracerProvider::get_tracer(
    std::string_view name) const {
    return impl_->provider->GetTracer(opentelemetry::nostd::string_view{name.data(), name.size()});
}
...
MeterProvider::MeterProvider(const OtelConfig& cfg) : impl_(std::make_unique<Impl>()) {
    try {
        if (cfg.meter_factory_for_test) {
            ...
            auto sdk_provider = opentelemetry::sdk::metrics::MeterProviderFactory::Create(
                /*views=*/std::make_unique<opentelemetry::sdk::metrics::ViewRegistry>(), resource);
            impl_->provider =
                std::shared_ptr<opentelemetry::metrics::MeterProvider>(sdk_provider.release());
        }
    } catch (...) {
...
opentelemetry::nostd::shared_ptr<opentelemetry::metrics::Meter> MeterProvider::get_meter(
    std::string_view name) const {
    return impl_->provider->GetMeter(opentelemetry::nostd::string_view{name.data(), name.size()});
}

void MeterProvider::shutdown() {
    if (auto* sdk =
            dynamic_cast<opentelemetry::sdk::metrics::MeterProvider*>(impl_->provider.get())) {
        sdk->Shutdown();
    }
}
