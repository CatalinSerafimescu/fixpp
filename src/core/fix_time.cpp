...
struct date_t { std::int32_t year; std::uint8_t month; std::uint8_t day; };
...
constexpr std::uint8_t days_in_month(std::int32_t year, std::uint8_t month) noexcept {
    static constexpr std::uint8_t dom[13] = {
...
constexpr date_t days_to_date(std::int32_t days) noexcept {
    ...
    std::uint32_t doe = static_cast<std::uint32_t>(z - era * 146097);
    ...
    std::uint8_t d = static_cast<std::uint8_t>(doy - (153 * mp + 2) / 5 + 1);
    std::uint8_t m = static_cast<std::uint8_t>(mp < 10 ? mp + 3 : mp - 9);
    y += (m <= 2);
    return date_t{y, m, d};
}

// Date → days since epoch.
constexpr std::int32_t date_to_days(std::int32_t y, std::uint8_t m,
                                     std::uint8_t d) noexcept {
    // Hinnant "days_from_civil".
    y -= (m <= 2);
    std::int32_t era = (y >= 0 ? y : y - 399) / 400;
    std::uint32_t yoe = static_cast<std::uint32_t>(y - era * 400);
...
[[nodiscard]] expected_t<std::span<char>>
    ...
    const auto sec_total =
        duration_cast<seconds>(since_epoch);
    // Sub-second nanoseconds (always non-negative: 0 ≤ ns < 1e9).
    const auto ns_rem =
        duration_cast<nanoseconds>(since_epoch - sec_total);

    // Days and time-of-day.
    const auto day_count =
        static_cast<std::int32_t>(sec_total.count() / 86400);
    ...
    const std::uint8_t HH = static_cast<std::uint8_t>(time_of_day / 3600);
    const std::uint8_t MM = static_cast<std::uint8_t>((time_of_day % 3600) / 60);
    const std::uint8_t SS = static_cast<std::uint8_t>(time_of_day % 60);
...
[[nodiscard]] expected_t<utc_time_point>
    ...
    if (year_i < 0) { return std::unexpected(error::wire_invalid_field_format); }
    ...
    if (day_i > days_in_month(year, month)) {
        return std::unexpected(error::wire_invalid_field_format);
    }

    // '-'
    if (*p++ != '-') { return std::unexpected(error::wire_invalid_field_format); }
    ...
    const std::int64_t epoch_sec =
        static_cast<std::int64_t>(epoch_days) * 86400LL
        + hh_i * 3600LL + mm_i * 60LL + ss_i;
