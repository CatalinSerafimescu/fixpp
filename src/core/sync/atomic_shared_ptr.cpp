...
std::size_t shard_index_for(const void* self) noexcept {
    std::uint64_t x = static_cast<std::uint64_t>(reinterpret_cast<std::uintptr_t>(self));
    x ^= (x >> 33U);
...
shard_guard::shard_guard(const void* self) noexcept : shard_index_(shard_index_for(self)) {
