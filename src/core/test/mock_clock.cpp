...
void complete(asio::any_io_executor ex, asio::any_completion_handler<void(std::error_code)> h,
              std::error_code ec) {
    asio::post(ex,  // NOLINT(performance-move-const-arg) - post accepts const ref
               asio::append([](asio::any_completion_handler<void(std::error_code)> hh,
                               std::error_code e) { std::move(hh)(e); },
                            std::move(h), ec));
}
}  // namespace

mock_clock::mock_clock(utc_time_point initial_utc, steady_time_point initial_steady,
        ...
        [this, deadline, bound_ex](auto handler) {
            ...
            bool fire_now = false;
            {
                ...
                if (deadline <= impl_->steady) {
                    ...
                    id = impl_->next_id++;
                    impl_->waiters.emplace(id,
                                           state::waiter{deadline, asio::any_io_executor{bound_ex},
                                                         std::move(handler)});
                }
            }
            if (fire_now) {
                ...
                // handler was NOT moved into waiters when fire_now==true (mutually exclusive
                // paths): fire_now=true iff deadline<=steady in lock block, in which case the
                // else-branch that moves handler into the map is skipped — so handler is still
                // valid here.
                // NOLINTBEGIN(bugprone-use-after-move,hicpp-invalid-access-moved,clang-analyzer-core.StackAddressEscape)
                asio::post(bound_ex,
                           asio::append([](auto h, std::error_code e) { std::move(h)(e); },
            ...
            if (cs.is_connected()) {
                cs.assign([this, id](asio::cancellation_type) {
                    ...
                    if (found) {
                        complete(std::move(w.ex), std::move(w.h), asio::error::operation_aborted);
                    }
                });
            }
        },
