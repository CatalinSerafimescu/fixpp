...
| ID | Source | Category | Title | FIX version(s) | Spec ref | Status | /specify | PR | Tests | Verified |
|---|---|---|---|---|---|---|---|---|---|---|
| T-001 | OFFICIAL | transport | TCP transport — initiator and acceptor roles | 4.0–5.0SP2 | [FIX-SL §4.3.1] Transport layer requirements | implementing | 012-2h-transport | TBD | `tests/transport/transport_smoke_test.cpp` + `src/transport/asio_tls_transport.cpp` (initiator path); acceptor role (US3 `asio_listener`) deferred per MVP scope to follow-on slice. | 012 MVP slice ships initiator path via `asio_tls_transport` + `make_asio_tls_transport(...)`; acceptor (`asio_listener` US3) deferred. Pending /speckit-verify + Gate B. |
| T-002 | OFFICIAL | transport | TLS over TCP — OpenSSL on both Linux and Windows (Schannel dropped; ASIO has no Schannel backend) | 4.0–5.0SP2 | [FIX-SL §4.3.1] Transport layer requirements; [FIXS §1.1] Scope | implementing | 012-2h-transport | TBD | `tests/transport/test_tls_handshake_pinset_rotation.cpp`, `tests/transport/test_compid_tls_identity_binding.cpp`, `src/transport/asio_tls_transport.cpp` (OpenSSL `SSL_CTX` configuration + `verify_peer_trampoline`) | 012 MVP slice consumes 011-shipped `SslCtxConfig` + `verify_peer` via `asio::ssl::stream<>`; SSL_OP_NO_EARLY_DATA waiver pending (constant not in OpenSSL 3.6.2 — needs `SSL_CTX_set_max_early_data(ctx, 0)` or reliance on ASIO default). Pending /speckit-verify + Gate B. |
| T-003 | OFFICIAL | transport | ASIO async I/O layer — non-blocking read/write with back-pressure | 4.0–5.0SP2 | [impl] implementation | implementing | 012-2h-transport | TBD | `tests/perf/test_transport_read_alloc_guard.cpp` (read-path zero-alloc dual gate), `tests/transport/test_inflight_exclusivity.cpp` (FR-007 exclusivity), `src/transport/asio_tls_transport.cpp` (`async_read_some` + `async_write` composed) | 012 MVP ships ASIO awaitable surface; back-pressure via FR-007 in-flight exclusivity. Pending /speckit-verify + Gate B. |
| T-004 | OFFICIAL | transport | Reconnect / exponential back-off — initiator retry on disconnect | 4.0–5.0SP2 | [FIX-SL §4.3.1] Transport layer requirements | implementing | 012-2h-transport | TBD | (US2 `ReconnectPolicy` deferred to follow-on slice per MVP scope) | 012 MVP scope: `ReconnectPolicy` value type + `defaults()` + `delay_for_attempt(n)` deferred (US2 T030-T033). FR-026 factory-caching contract for reconnect minting is documented in `src/transport/transport_factory.cpp`; FSM-side reconnect loop is session-Phase-4. Pending US2 slice. |
| T-005 | OFFICIAL | transport | Multi-session TCP acceptor — accept multiple connections on one port | 4.0–5.0SP2 | [FIX-SL §4.3.1] Transport layer requirements | implementing | 012-2h-transport | TBD | (US3 `asio_listener` deferred to follow-on slice per MVP scope) | 012 MVP scope: `Listener` interface + `asio_listener` impl deferred (US3 T034-T038). Pending US3 slice. |
...
| T-009 | OFFICIAL | transport | FIXS: Mutual TLS — CA pinning (server) + leaf pinning (client) | 4.0–5.0SP2 | [FIXS §2.4] Certificate Validation with CA Pinning | implementing | 012-2h-transport | TBD | `tests/transport/test_tls_handshake_pinset_rotation.cpp` (pinned + unpinned mtls_ca modes), `src/transport/asio_tls_transport.cpp` (`async_handshake` captures pinset snapshot ONCE per `[2g §6.5.1]`) | 012 MVP slice consumes 011-shipped `SslCtxConfig` + `verify_peer` + `Pinset`; pinned/unpinned profiles wired via `SslCtxConfig::profile`. Pending /speckit-verify + Gate B. |
| T-010 | OFFICIAL | transport | FIXS: Simple TLS — server-only auth (Star topology; client auth deferred to FIXA/FIX session) | 4.0–5.0SP2 | [FIXS §2.2] Mutual and Simple TLS protocol options | implementing | 012-2h-transport | TBD | `tests/transport/test_tls_handshake_pinset_rotation.cpp` (one_way_ca scenario cell), `src/transport/asio_tls_transport.cpp` | 012 MVP slice. `SecurityProfile::one_way_ca` wired through `SSL_CTX_set_verify(SSL_VERIFY_PEER, ...)` without `SSL_VERIFY_FAIL_IF_NO_PEER_CERT`. Pending /speckit-verify + Gate B. |
...
| T-039 | OFFICIAL | transport | FIXS: Certificate parameters — RSA 2048-bit min, ECDSA 256-bit, X.509 v2/v3, expiration validation at handshake | 4.0–5.0SP2 | [FIXS §3.4] Certificate parameters | implementing | 012-2h-transport (2h-owned wiring half) | TBD | `src/transport/asio_tls_transport.cpp` `verify_peer_trampoline` (consumes 011's `fixpp::tls::verify_peer` via `SSL_set_verify_callback`) | 012-2h-transport ships the wiring half — `SSL_VERIFY_PEER` trampoline dispatches into 011's `verify_peer(augmented_cfg, peer_chain)` with an augmented per-call view carrying the captured pinset snapshot per Appendix D §D.7. Pending /speckit-verify + Gate B. |
| T-040 | OFFICIAL | transport | FIXS: Secrets management — distribute private keys/PSKs/pinned-certs via approved channels (HTTPS, GnuPG, PKCS#12, postal, in-person); store securely; support rotation | 4.0–5.0SP2 | [FIXS §4.1] Sharing secrets | implementing | 012-2h-transport (2h-owned wiring half) | TBD | `tests/transport/test_load_credentials_seam13_witness.cpp` (8-cell witness exercising 011 `file_cert_source` through 012 session_executor), `src/transport/asio_tls_transport.cpp` (consumes `cert_source` via `make_ssl_ctx_config`) | 012-2h-transport ships the wiring half — `cert_source` is consumed by `asio_tls_transport` constructor via 011's `make_ssl_ctx_config`. Phase 7 seam #13 witness verifies the production `Session*` + `session_executor` integration discharges 011 Gate B F-1 carryover. Pending /speckit-verify + Gate B. |
