...
- **L-037-1 — `43`/`122` were appended AFTER the body fields in both resend-answer emitters (header-after-body placement) — the row's safety claim ("safe because both reference engines parse the header into a field map making field order irrelevant to inbound validation") was FALSE. SUPERSEDED 2026-09-11 by fixpp#419.** A strict peer — QuickFIX-J with `UseDataDictionary=Y` — validates standard-header field order and rejects a header field appearing after a body field (`Reject(35=3), SessionRejectReason(373)=14 "Tag specified out of required order"`), reproduced live against feature 089's conversation cells (fixpp initiator × QFJ 3.0.1 acceptor). The "field order is irrelevant" claim was **never checked against a strict peer**: the live QuickFIX-J acceptance witness this row's sibling **L-037-2** describes was itself deferred, and every interop cell that exercised fixpp's resend path ran the peer with `UseDataDictionary=N`. **Fixed:** both `build_sequence_reset_gapfill` (`src/session/admin_messages.cpp`) and `build_replay_frame` (`src/session/session.cpp`) now insert `PossDupFlag(43)=Y` + `OrigSendingTime(122)` at the standard-header/body boundary — before the first body field — instead of after the full body. The header's own internal order is unchanged (8/9/35/34/49/52/56 first). *(fixpp#419; supersedes 037 research D-3; `tests/session/test_resend_answer_field_order.cpp`.)*
