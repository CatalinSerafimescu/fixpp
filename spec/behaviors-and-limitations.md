...
- **L-012-2 — Acceptor saturation beyond 10²–10³ sessions/port is unbenched; a client arriving at a full backlog is not connected — whether it is refused or simply left pending is OS- and configuration-dependent.** v1.0 target deployments are 10²–10³ sessions per acceptor. `Endpoint::backlog` *is* forwarded to `listen()` and does bound how many connections the OS completes while the application is not accepting — witnessed by `tests/transport/test_listener_acceptor.cpp` cell 10, which reds when the forwarding is dropped.
