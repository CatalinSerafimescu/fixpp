...
**B-050-4 — `fixpp_session_close` of a once-established, now-reaped session is an idempotent `FIXPP_ERR_OK`; only a never-established session closes as `FIXPP_ERR_THREAD_SESSION_LIFECYCLE` (issue #151).** When `close` runs after the session is gone from the engine registry (`Engine::lookup(id)==nullptr`), that one observation conflates two lifecycle states: (a) the session was established at least once and has since been reaped (e.g. the peer disconnected first) — a normal, idempotent close, returns `OK`; (b) the session was opened but never started/connected — a deliberate lifecycle outcome, returns `THREAD_SESSION_LIFECYCLE`. The two are distinguished by a sticky per-session `ever_established` latch (set once on the first `onLogon`, never reset — `established` alone is cleared by `onLogout`, so it cannot tell them apart). Either way the handle is invalidated, so a subsequent `close` returns `FIXPP_ERR_INVALID_HANDLE`. Before this fix the reaped case returned `THREAD_SESSION_LIFECYCLE` (downgraded to `UNKNOWN` for a `consumer_minor<2` engine), surfacing as a spurious close error on a clean disconnect. **Status: fixed (issue #151); a public C-ABI return-contract change, landed before the 0→1 GA freeze.** *(`src/capi/session.cpp` `fixpp_session_close`; `src/capi/engine.cpp` `CapiApplication::onLogon`; witnessed `tests/capi/lifecycle_negative_test.cpp::CloseEstablishedThenReapedIsIdempotentOk` + the never-established sibling, end-to-end `tests/capi/send_recv_test.cpp`.)*
