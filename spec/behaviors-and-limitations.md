...
**B-054-2 — the three blocking binding wrappers (`session_close`, `session_send`, `engine_destroy`) release the GIL around the native call; the one bound C→Python trampoline (`fixpp_py_recv_trampoline`) reacquires it.** A documented, exhaustive GIL-discipline audit table in `fixpp.i` classifies every `%include`d C-ABI function release/hold; the bound-trampoline census states exactly one bound trampoline (the `toApp`/send callback is `%ignore`d/unbound). A local-only `FIXPP_PY_GIL_RELEASE_CANARY` build elides the release bands; a two-mode subprocess witness proves the teardown-vs-in-flight-recv-callback scenario completes in a normal build (GREEN, in-matrix) and deadlocks under the canary (RED, local-only). A subprocess-watchdog test pins that a raising inbound callback (staged concurrently against a blocking teardown) never deadlocks the engine. *(054 FR-001..004/011, SC-003/004/007; `bindings/python/fixpp.i`; `tests/test_gil_release_canary.py` + `test_callback_raise_watchdog.py` + `_gil_staging.py`.)*

**L-054-1 — `session_send` (and any blocking C-ABI call) from inside the inbound callback DEADLOCKS as-built; the binding guarantee is documentary, not enforced, in v1.0.** The as-built 050 `fixpp_session_send` blocks on the dispatch (`co_spawn(ioc_, …, use_future)` + `fut.get()`; `src/capi/session.cpp:284-286`, rule `session.h:255-258`); called from inside the inbound callback (which runs on the engine worker / session strand) the worker blocks in `fut.get()` waiting for the send coroutine to run on the *same* io_context/strand, which cannot progress. This is a **strand/io_context reentrancy deadlock — distinct from, and unaffected by, the 053 GIL-teardown deadlock** (which the PY-002 GIL release fixes) **and the close-from-callback deadlock** (`CallbackReentrantClose`/1204). It is a **current limitation tied to the blocking as-built**, NOT a permanent "forbidden by design": the `[2m §4.6]` "Session.send-from-callback is legal" claim was predicated on a *non-blocking strand-dispatch* `fixpp_session_send`; restoring real legality is a deferred **engine** item. **Mitigation: from the callback, `msg.clone()`/copy the field out and `queue.put()` it to drain + `session_send` on another thread (off the strand).** The binding-level guarantee stays **documentary** (the callback docstring states the hazard); active detection (`session._in_callback` + a pre-call `CallbackReentrantClose`-style raise) is **PY-004**. The normative `[2m]` design is amended at all four send-from-callback sites (§1.3 rule (2), §3.12, §6.5 carve-out table, §4.6 `CallbackReentrantClose` docstring) per Article XX. **Status: documented v1.0 behaviour.** *(054 FR-005; `.specify/2m-pybind.md` §1.3/§3.12/§6.5/§4.6; `bindings/python/fixpp.i` callback docstring.)*

**L-054-2 — the `python-bindings` Tier-1 matrix has no UBSan lane; 054 carries forward the 053 D-9 UBSan waiver.** The new typed-exception + watchdog + GIL-release-GREEN tests run under the `none`/`asan`/`tsan` legs; standing up a `python-bindings` UBSan lane is a PY-005-era CI item (CPython C-API aliasing noise). **Status: documented verify-risk note / carried waiver, NOT a silent drop.** *(054 plan Constitution Check IX §2; 053 D-9.)*
