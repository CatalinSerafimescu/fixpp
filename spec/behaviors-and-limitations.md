...
- **L-012-2 — Acceptor saturation beyond 10²–10³ sessions/port is unbenched; a client arriving at a full backlog HANGS rather than being refused.** v1.0 target deployments are 10²–10³ sessions per acceptor. `Endpoint::backlog` *is* forwarded to `listen()` and does bound how many connections the OS completes while the application is not accepting — witnessed by `tests/transport/test_listener_acceptor.cpp` cell 10, which reds when the forwarding is dropped.

  ⚠️ **Do not expect a fast failure for the overflow client.** Until 2026-09-01 this row said backlog overflow "yields OS-level RST" / "returns TCP RST / connection-refused per OS behaviour". That is **false on both platforms fixpp ships to**, and is deleted rather than re-scoped: measured on the Linux and Windows development hosts, the overflow client's `connect()` **does not complete at all** — no RST, no `ECONNREFUSED`. On Linux this follows from `net.ipv4.tcp_abort_on_overflow`, whose default `0` drops the SYN instead of resetting it.

  **Operator consequence:** a client arriving while the acceptor is saturated blocks until *its own* connect timeout expires instead of failing fast, so failover logic driven by connect *errors* will not trigger — it will stall. Clients that need prompt rejection must impose their own connect deadline; fixpp exposes no server-side knob that converts this into an RST. **Status: benchmark deferred; the RST claim is corrected and the backlog-forwarding behaviour is witnessed (2026-09-01).** *(FR-024; spec Edge Cases "Service-side acceptor saturation" / US3 scenario 3; issue #332.)*
