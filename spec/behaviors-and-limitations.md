...
**B-044-1 — Gate-B blocker (T039): tomlplusplus 3.4.0 aborts or invokes undefined behaviour on certain malformed TOML table headers, bypassing the loader's `noexcept` boundary.** The loader wraps the tomlplusplus parse call in a `try/catch(...)`, which is sufficient for normal parse errors. However, `TOML_ASSERT_ASSUME(...)` inside the parser's `parse_key()` method fires `assert()` → `abort()` (debug/ASan builds) or `__builtin_assume(false)` → UB (release/NDEBUG builds) on attacker-controlled malformed input such as `[ [key]]` (space before the second `[`). In the debug/ASan path `abort()` is not a C++ exception and therefore escapes the `try/catch`, violating FR-012 rule-9 (the central guarantee that every input yields either a `ConfigBundle` or a vector of diagnostics, never a fatal signal). In the release path `__builtin_assume(false)` is UB. Reproducer: `tests/config/fuzz/crashes/repro_toml_assert_assume.toml`. A fuzzer-discovered defect (T037 libFuzzer harness, ~20k runs). **Status: Gate-B blocker (T039) — not patched; fix design and validation are Gate-B work.** *(044 T037/T039; FR-012 rule-9; `src/config/toml_config_loader.cpp`; `tests/config/fuzz/crashes/repro_toml_assert_assume.toml`.)*
