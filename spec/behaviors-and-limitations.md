...
- **B-456-1 — a `fixpp::dict::table_view`'s population surface is sealed, and the type is non-assignable, once built.** Its sixteen population methods are private; the only way to reach them is `fixpp::dict::table_view_builder`, whose `[[nodiscard]] table_view build() &&` yields the view. Both `operator=` overloads are deleted, so a published view cannot be re-seated — seat a `std::optional<table_view>` with `emplace`, not with assignment. Copy- and move-CONSTRUCTION are unchanged and remain load-bearing (`wire::dictionary_driven_validator` holds its view by value) — so a view the holder does not declare `const` can still be moved from, and an `optional<table_view>` re-seated with `emplace` substitutes a new object at the same address (`B-456-2`). **Sealed means no population channel; it does not mean an immutable object.** This is a **source-breaking** change to a public C++ type; it is **not** a C-ABI change — `table_view` appears in no `include/fix/` header, so no C-ABI version moves. Every out-of-tree break is a compile error, loud and at the call site: `'add_valid' is a private member of 'fixpp::dict::table_view'`, or `object of type 'table_view' cannot be assigned`. There is no silent behaviour change and no runtime failure mode. *(fixpp#456, `.specify/456-table-view-seal.md` §5a; witness `tests/dictionary/table_view_seal_compile_test.cpp`.)*
- **B-456-2 — a `wire::dict_hooks` bundle latches `has_nonstandard_pair()` at build time, and that latch can still go stale through a re-seated `optional<table_view>`.** What changed is narrower than this row first stated. The seal closes every **population** channel, so the contents of a published view can no longer change in place. What remains observable is that a bundle latches against an **object at an address**: `for_table_view` stores `std::addressof(dict)`, and `std::optional<table_view>::emplace` — or any destroy-and-reconstruct in the same storage — substitutes a **different** view at that same address with the bundle none the wiser, every value fully specified. The two production holders of a non-`const` by-value view (`capi_internal.hpp`'s `owned_tv_`, seated in `reify.cpp` and `message_write.cpp`) are seated exactly once under `assert(!…has_value())` — enforcement **by construction plus a debug assert**, not by the type; `assert` is stripped under `NDEBUG`. `DictHooksCustomPair.ABundleIsASnapshotOfTheDictionaryItWasBuiltFrom` was deleted on the claim that its premise was unconstructible. That claim was wrong: the *population* half became unconstructible, the *identity* half did not. `DictHooksCustomPair.ABundleDoesNotFollowAReSeatedOptional` carries it now. Separately, a non-`const` view can still be **moved from**, which empties the source rather than changing it (§5b). §7's raw `opaque_dict_` lifetime hole is the same shape; the seal neither causes nor cures any of them. *(fixpp#456 §5c / §5d item 5. This row exists to record what stopped being observable; a green suite is not evidence that half was ever covered.)*
...
- **L-456-1 — `table_view_builder::build()` performs no consistency validation.** A builder can still produce an internally inconsistent table; the known shape is a group with members and no first field (`B-384-2`), reached by calling `add_group_member` without `set_group_first`. The seal makes the view immutable; it does not make it valid. *(fixpp#456 §5c. A consistency check inside `build()` was considered and left out of scope.)*
