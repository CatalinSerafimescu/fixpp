...
**L-050-z — `fixpp_engine_destroy` retains the full engine shell graph (including `sessions_`, `app_`, `ioc_`, `clock_`) in the `s_dead_shells` tombstone registry; this is an unbounded per-cycle leak under engine create/destroy churn.** The tombstone round-1 fix retains the *entire* `fixpp_engine` object, not just a tiny control block, because `sessions_` cannot be freed — each `fixpp_session_t*` the consumer holds points directly into `sessions_` storage, so freeing it would reintroduce the round-1 post-destroy UAF on stale session handles (ASan-witnessed by `PostEngineDestroySessionHandleIsInvalidHandle`); similarly `app_` cannot be freed because each `fixpp_session::slot` borrows into `CapiApplication::slots_`. Only `ioc_`/`work_guard_`/`clock_`/`engine_` are reclaimable — but splitting them into a resettable `State` object is a moderate refactor. Under the v1.0 contract engines are few and long-lived (`[2i §4.10]`): a disciplined consumer creates a fixed set of engines at startup and holds them for the process lifetime, so the retained graph is bounded per engine and unbounded only under unsupported create/destroy churn. **Status: waived for v1.0 (engines are process-lifetime per `[2i §4.10]`; churn unsupported); reclaiming `ioc_`/`clock_` deferred to a follow-up.** *(050 Gate B R3; `src/capi/engine.cpp` `s_dead_shells`; `capi_internal.hpp` SHELL RETAIN note.)*
