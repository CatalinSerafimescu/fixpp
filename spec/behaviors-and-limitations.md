...
**L-062-1 — nested typed reads of a group whose NumInGroup tag is REUSED with differing membership in the FIX dictionary return the wrong membership against `Dictionary::as_table_view()` until 063 (pre-existing Defect A).** The XML loader registers repeating-group membership globally, first-XML-occurrence-wins, keyed only by the NumInGroup tag (`src/dict/xml_loader.cpp:476-516`, `if (!group_index_by_no_tag_.contains(no_tag)) …`). FIX legitimately reuses a NumInGroup tag for different group definitions in different components — confirmed in `dictionaries/FIX44.xml`: tag **295** heads both `QuotCxlEntriesGrp` (earlier) and `QuotEntryGrp` (later, used by **MassQuote**), so `group_member_tags(295)` first-seen-resolves to the wrong (QuotCxl) variant and `MassQuote.quote_sets()[s].quote_entries()` returns size 0 against the real dictionary. Not reachable via the hand-built `table_view` the 062 witnesses use (they specify membership explicitly); reachable by any caller parsing MassQuote via `Parser{Dictionary::as_table_view()}`. **063 Defect-A fix:** per-context group registration (`(context, no_tag)`), with a census of every reused NumInGroup tag across FIX44/FIX42/FIX50SP2. *(062 US2 / T015 `NestedQuoteEntriesPerInstancePrices` GTEST_SKIP; findings 2026-07-05 §Defect A.)*

**L-062-2 — nested typed reads of an outer group instance that contains a nested group with >1 entries return TOO FEW entries until 063 (pre-existing Defect B — `OffsetTable::group()` not nesting-aware).** `OffsetTable::group()`'s instance-boundary walk uses a flat `seen_in_instance` duplicate-tag heuristic with NO nested recursion (`src/wire/offset_table.cpp:419-437`): when an outer instance contains a nested repeating group whose delimiter/fields legitimately repeat (>1 nested entries), the walk truncates the outer slice at the 2nd nested entry. Confirmed: a MassQuote QuoteSet[0] with 2 QuoteEntries yields a truncated outer slice stopping before the 2nd entry's `299`, so `quote_entries().size()==1` not 2. 062's nested MECHANISM faithfully re-slices whatever outer slice it is handed, so it is fed a truncated slice — single-entry-per-occurrence nesting is unaffected (no descendant-tag repeat within one occurrence), which is why the 062 depth-3-collision + dict-aware-trailing witnesses PASS while the multi-entry `NestedQuoteEntriesPerInstancePrices` is honestly `GTEST_SKIP()`'d. Pre-existing on `main` (062 only ADDED `build_nested_subview`/`nested_group_slices`, never touched `group()`). **063 Defect-B fix:** make the boundary walk recursively consume a nested group's extent (via its own count/delimiter) before resuming the outer detection. *(062 US2 / T015; findings 2026-07-05 §Defect B; needs Defect A's correct membership to know which tags are nested counts.)*

**L-062-3 — the entry `field_value(tag)` escape hatch span-scans the WHOLE entry slice (including a nested group's bytes) and returns the FIRST occurrence.** A tag that lives ONLY inside a nested group is returned by an outer entry's `field_value(tag)` as if it were an outer-entry field. This mirrors the whole-message `field_value` first-occurrence semantics (N3) and is a property of the untyped escape hatch only — the codegen-scoped TYPED accessors (`emit_scalar` / nested `group<c,G_c>()`) are membership-scoped and unaffected. Prefer the typed accessors when nested/outer disambiguation matters. *(062 N3; spec `field_value` note; whole-message first-occurrence precedent.)*
...
**L-063-2 — the C-ABI `fixpp_group_get_nested_group` positional read may include a trailing outer-level member in the LAST nested instance's field lookups (edge activated, and net-improved, by 063; correct fix deferred to a membership-aware C-ABI follow-up).** The C-ABI nested-group read (`src/capi/message_read.cpp:462-609`) slices a nested repeating group by a **positional, dictionary-membership-free** delimiter scan and closes the last nested instance at the **end of the outer occurrence's slice** (`:587-590`). Before 063, Defect B truncated the outer slice before a nested group's 2nd entry, so this multi-instance branch was unreachable (a multi-entry nested C-ABI read returned `nc=0`). 063's nesting-aware `OffsetTable::group()` makes the outer slice correct (spans all nested entries) — a **strict improvement** for the common case — but if a well-formed message carries an outer-group member **after** a multi-entry nested group (e.g. MassQuote `QuoteSetValidUntilTime(367)` after the `NoQuoteEntries(295)` group inside a QuoteSet), that trailing member's bytes fall inside the last nested instance's span, so `fixpp_group_get_field_*(nested, last_index, trailing_tag, …)` returns it (`FIXPP_ERR_OK`) instead of `FIXPP_ERR_TAG_NOT_FOUND`. **The C++ typed read path (`Dictionary::as_table_view()` → generated flyweights) is UNAFFECTED — it is membership-bounded and correct.** A correct C-ABI fix needs nested-group membership, which this positional path deliberately lacks; plan.md Round-2 explicitly rejected plumbing dict/`group_context` through the GA-frozen C-ABI cursor (a gratuitous rewrite), so the fix is a **membership-aware C-ABI follow-up feature**, out of 063's Defect-A/B scope. Pinned by the `GTEST_SKIP`'d witness `tests/capi/message_read_test.cpp::MessageReadGroup.NestedGroupLastInstanceExtentDoesNotAbsorbTrailingOuterMember` (un-skipped when the follow-up lands, mirroring the `:353` lifecycle). No exported C symbol / freeze change (SC-005 holds). *(063 T025 / Round-2 tasks-pin #5; decided 2026-07-07; `message_read.cpp:554-562` LCOV_EXCL rationale corrected.)*
