...
**L-045-2 — Syslog sink + the file-sink writability preflight are POSIX-only.** `kind="syslog"` resolves to a real `SyslogSink` only on a build where `<syslog.h>` is present (`FIXPP_HAS_SYSLOG`, POSIX); on a non-POSIX build it fails closed with `invalid_or_contradictory_selector` (loud, never silently skipped — FR-013). The file-sink directory-writability preflight uses POSIX `::access(W_OK)`. There is no Windows/MSVC preset in the matrix; a cross-platform build would need a portable writability probe. **Status: platform-conditional by design.** *(045 FR-013; `src/config/logger_resolver.cpp`; witnessed `SyslogBuildConditional` in `test_load_logger_negative.cpp`.)*
