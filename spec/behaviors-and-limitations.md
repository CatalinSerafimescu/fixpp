...
- **B-002-1 — The loader THROWS typed exceptions; a deliberate carve-out from the engine's noexcept/`expected_t` model.** `XmlLoader::load` returns a `Dictionary` by value and signals construction-time failure via `dict::xml_parse_error` / `dict::unknown_version_error` / `dict::xml_oom_error` (derived from `std::runtime_error`) — not `expected_t`. The hot-path `Dictionary` accessors are then all `noexcept`. Callers must wrap the load in try/catch. *(spec §1 Style note; AC-L2 / AC-L9; NFR-002-5.)*
...
- **B-003-5 — A FIXT.1.1 message that can't resolve its application version returns a distinct error, not "unknown msg type".** When `default_appl == Unknown` and a frame lacks `ApplVerID(1128)`, `dict::reify` returns `dict_unresolved_application_version` (NOT `dict_reify_unknown_msg_type`). *(AC-D5 / AC-D6; `contracts/reify_dispatch.hpp:42`.)*
...
- **B-010-3 — Duplicate `Logon` and out-of-scope admin (`ResendRequest`/`SequenceReset`) received in `Active` each emit exactly one `Reject(35=3)` — diverging from QuickFIX's refresh-on-dup-Logon convention.** `is_session_admin` excludes `"A"`/`"2"`/`"4"`, routing them through Reject per 005 FR-017 ("never silent no-op"). The dup-Logon-in-Active Reject is a deliberate defensive divergence intended to stay. *(FR-006/FR-008; SC-002.)*
...
- **L-010-1 — Out-of-scope-admin (`ResendRequest`/`SequenceReset`) handling upgrades from Reject to Process only when the deferred session-recovery feature lands.** The `Active×ResendRequest`/`Active×SequenceReset` cells currently Reject (marked `TODO(2e-recovery)`); the dup-Logon-in-Active cell stays Reject. **Status: deferred** (catalogue row 400). *(SC-002.)*
...
- **L-010-3 — `SessionConfig::store_factory` is a `shared_ptr` (a small 005 amendment) to make `SessionConfig` copy-constructible.** The per-Session "one MessageStore per Session" invariant is unaffected. **Status: wontfix.** *(FR-001a; `session_config.hpp:127`.)*
...
- **B-007-2 — `Engine::open` rejects a null `EngineConfig::clock` with `error::clock_not_set`, even if every session supplies a `clock_override`.** No implicit default-clock fallback at engine scope. *(FR-006; US2.1.)*
- **B-007-3 — `now()` is not promised monotonic; all elapsed/heartbeat/SendingTime measurements use `steady_now()`.** A benign NTP/admin backward wall-clock step never trips a SendingTime-threshold reject; `now()` is consulted only for wire-formatted and log/OTel timestamps. Diverges from engines that measure heartbeat intervals against wall-clock. *(FR-004; US2.3.)*
...
- **B-008-1 — The FileStore log grows monotonically until a reset epoch (size disk accordingly).** FileStore is append-only with no compaction, rotation, or size cap: each outbound message appends ≈ `payload + 64 B` and each inbound advance a 24 B counter record. The only reclamation is a reset epoch (`reset_on_{logon,logout,disconnect}` — all default `false` — or bilateral `141=Y`). Default config ⇒ monotone growth for the durable session lifetime (restart re-scans the full log); at 1k msg/s @ 512 B ≈ 50 GB/day. fixpp has no `SessionTime`-driven daily auto-reset. **Operator action:** size disk for one reset epoch and enable a reset knob or arrange external EOD resets. *(Fable `5.4`; `file_store.cpp:503-543`, `file_store_factory.cpp:174`.)*
- **B-008-2 — The *default* `MemoryStore` config is rejected at construction under the *default* 1 GiB cap, by design.** The frozen defaults (`inbound = outbound = 10'000` × 256 KiB `max_frame_bytes` ≈ 5 GiB worst case) deliberately exceed `EngineConfig::max_store_memory_per_session = 1 GiB`, so a default `MemoryStoreFactory::make()` returns `store_factory_failed` and the session does not open — the operator must raise the cap or lower `max_frame_bytes`. *(FR-014a / SC-004; US2 scenario 2a.)*
...
- **L-014-2 — The CompID-binding decision is proven on the live path only for the initiator; the acceptor still uses the test seam, so T-041 does not fully close in 014.** 014 wires the live identity into `authorize()` and proves it only on the live initiator reconnect path; acceptor binding-logic is proven solely via the `logon_peer_identity_override` test seam. The live acceptor path and test-seam removal — and full T-041 production closure — land with 015. **Status: deferred.** *(FR-008; spec §Assumptions "T-041 advances but does not fully close"; Out of Scope.)*
