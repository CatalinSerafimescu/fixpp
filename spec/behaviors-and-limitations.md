...
**L-056-4 — the shipped wheel does NOT wrap the binaries produced by the `linux-clang-*` / `linux-gcc-*` CI builds; it is an independent build, and it differs from them in three dimensions at once.** `python-wheel-build` compiles the engine from source inside a **manylinux_2_28** container with `gcc-toolset-14`, `build_type=Release`, `-o fixpp/*:with_otel=False` and static OpenSSL (`bindings/python/cibw-before-all.sh`), statically linking the whole engine plus `fixpp_capi` into `_fixpp.so`. The Tier 1 matrix legs that run the Python **test suite** build with **clang 22, Debug (+ sanitizers), and `with_otel` at its default `True`** (`conanfile.py`). Those bindings are a **test vehicle only — not a byte of them ships**; likewise the `packages-linux-{clang,gcc}-release` artifacts are the **C++** consumer deliverable and contain no Python. ⚠️ **(#254, 2026-08-09)** The vehicle is now the **six `linux` matrix legs** themselves — the separate `python-bindings` job was folded into them and deleted. That fold made "contains no Python" a property held **by construction rather than by accident**: `FIXPP_BUILD_PYTHON=ON` fires four unconditional `install()` rules, and the two `-release` legs build `fixpp-package`, so the payload would have entered those artifacts. It is suppressed by **`FIXPP_INSTALL_PYTHON=OFF`** on those six legs (the option defaults **ON**, preserving 056's LAY-1 in-tree install), guarded by a Configure-time assertion on the generated install script **and** by `fixpp::python::install-absent-witness`, a `DESTDIR` install that rejects the payload. ⚠️ Note the pre-existing witnesses could not have caught this: `run_package_contents_witness.cmake` checks archive formats and an export-derived count, so it is **structurally blind** to added root-level files. Whether the packages *should* ship Python is **#257**'s decision, not this row's. The consequence to plan against: **the artifact users actually run is exercised only by `python-wheel-test`'s install + import + functional subset across 3.10–3.13**, while the deep testing (sanitizers, full pytest) runs against a build that differs in compiler, optimisation level, and whether OTel is compiled in at all. A Python consumer therefore needs **only the wheel** — `pip install fixpp-…whl` on x86-64 Linux, glibc ≥ 2.28, CPython ≥ 3.10 — and never a fixpp C++ package. ⚠️ A binding module built from a `-release` package would be **version- and compiler-specific and NOT interchangeable with the `cp310-abi3` wheel**; do not mix them. **Mitigation: build the wheel packaging path in the `-release` legs against one pinned interpreter so the shipped shape gets more than an import test — tracked as its own issue. Status: documented provenance boundary.** *(056 FR-002; `bindings/python/{cibw-before-all.sh,pyproject.toml}`; tier1.yml `python-wheel-*` + `linux` matrix.)*
...
- **L-067-2 — a genuinely-binary `Data` field (Length+Data coupled) carrying SOH/control bytes cannot be emitted through the generated builder: `body_builder`'s string path fail-closes on `0x01`/control bytes, so the coupled Length+Data member accepts clean-text only.** 067 models a Length+Data pair (e.g. `EncodedTextLen(354)`/`EncodedText(355)`, `RawDataLength(95)`/`RawData(96)`) as ONE coupled `std::optional<string_view>` in `<Msg>Args` with the Length auto-derived at emit time (FR-007a), routed through `body_builder`'s string path — which rejects any value containing SOH/control bytes before a byte reaches `out` (pinned by `tests/session/test_067_builder_failclosed.cpp`). Clean-text Data (no control bytes) IS supported; arbitrary-bytes/binary Data content is cut for v1.0 (`body_builder` exposes no arbitrary-bytes API). **Fix (deferred follow-up):** a `body_builder` arbitrary-bytes emit path for coupled Length+Data — a v1.x demand-driven follow-on. *(067 FR-007a / spec Out-of-Scope "Binary Data-field content"; `tests/session/test_067_builder_failclosed.cpp`; 067 close-out; decided 2026-07-10; PR #185.)*
...
- **B-215-1 — the C-ABI commit path now REJECTS a repeating group whose exact `(msg_type, ancestor path, no_tag)` context is not declared, instead of resolving one from the dictionary-global store.** 083 T052 already required that `fixpp_msg_commit`'s group-grammar check "must NEVER fall back to the bare global `dict->group_first_field(e.tag)`" — a fallback there "would let the builder accept an order inbound validation rejects". It enforced that by not *writing* a bare call, but the three-argument `table_view::group_first_field(msg_type, parent_path, no_tag)` it did call applies exactly that fallback **internally** on a context miss (the DUAL-STORE INVARIANT block in `include/fixpp/dict/table_view.hpp`), so the rule was stated and not enforced, and the site had no way to observe that it had happened. The commit path now resolves through `group_first_field_exact`, which reports a context miss as `std::nullopt` rather than masking it, and a miss returns `FIXPP_ERR_TYPE_MISMATCH` — joining the pre-existing `tv == nullptr` case on the same fail-closed disposition.
  ...
  **Depth ≥ 17 nesting is a further, DISTINCT wrong-ancestor-axis case (Gate B r1 O1) — ARGUED, not witnessed.** Item 3 replaced the ancestor-chain carrier this file threads with `wire::group_context::pushed()`, which **saturates** at `kMaxGroupContextDepth` (16) rather than growing unbounded. On `origin/main`, the pre-item-3 carrier was an unbounded `std::vector<uint16_t>&`, so a query at nesting depth ≥ 17 built a span of length 17+ — and `group_ctx_equal::eq`'s four-iterator `std::equal` rejects on any length mismatch against the (already-16-clamped, at insert time) stored key, so that query was a **guaranteed miss**, unconditionally, falling back to the 3-arg accessor's bare-store answer. The new 16-saturating carrier **can** match a context `as_table_view()` registered at that same depth, because both the insert-side clamp (`make_group_ctx_key`) and the new query-side saturation truncate from the same end (drop the innermost/deepest ancestor once past 16). The two are therefore NOT behaviour-identical past depth 16 — a false equivalence claim previously argued in this record and in `src/capi/message_write.cpp:718`'s comment, corrected by this round (see `.specify/decisions/215-simplify-followups-verify.md` for the correction and the fixer's account of why no test accompanies it).

  **No fixture accompanies this row.** Constructing one requires a dictionary that REGISTERS a group at nesting depth ≥ 17 — no shipped FIX dictionary does, and none exists in this test suite before this round. Attempting to build one for this Gate B round surfaced a **separate, pre-existing, unrelated defect** (unchanged by this PR; confirmed via `git diff origin/main -- src/dictionary/dictionary_internal.hpp` — empty): `find_context_without_delim_record`'s FR-023 completeness-check probe (`src/dictionary/dictionary_internal.hpp`) walks a group's ancestor chain from the IMMEDIATE parent outward and stops once it has collected 16 entries — keeping the 16 **innermost** ancestors — while the REAL registration path (`Dictionary::as_table_view()`, `src/dictionary/dictionary.cpp`) walks the full chain and then `make_group_ctx_key` clamps by keeping the first 16 of the **outermost-first** array, i.e. the 16 **outermost** ancestors. At depth ≥ 17 these two clamps disagree by exactly one element (the outermost ancestor vs the innermost one), so the completeness check computes a probe key that does not match the real registered key and rejects the load with a spurious `"registered by as_table_view() but has no per-context delimiter record"` error — on a dictionary that IS complete. This blocks loading any depth-≥-17 fixture at all, independent of item 3's delta, so the divergence this row describes cannot currently be witnessed by any test. Fixing the completeness-check bug is out of this batch's scope (it is pre-existing 083 T041 code, untouched by fixpp#215) and is left as a discovered-but-unfixed defect for the orchestrator to triage separately.
...
- **B-339-1 — every `async_*` on a Transport in the `closed` state now returns `transport_already_closed` (98); `async_connect` and `async_handshake` previously returned `transport_already_connected` (97) there.** FR-006 states the post-close rule without exception — *"After `close()` returns, every subsequent `async_*` returns `transport_already_closed`"* — and `async_read_some` / `async_write` already honoured it on both transports. `async_connect` (TLS and plaintext) and `async_handshake` (TLS) did not: their one-shot guards tested `state_ != fresh` / `state_ != connected`, which `closed` also satisfies, so the closed case fell through to the one-shot answer. A consumer that branched on `97` to mean *"this Transport is live, you already connected it"* will now see `98` for a Transport that is finished, which is the distinction FR-006 exists to draw.

  **`close()` is not the only door into `closed`, and the delta is therefore wider than "post-`close()`".** The TLS transport also enters `closed` from inside `async_handshake` — re-derive the set with `grep -n 'state_ = state_t::closed' src/transport/asio_tls_transport.cpp` rather than trusting a count here. ⚠️ **Not every failed `async_handshake` gets there.** The PREFLIGHT rejections — the pre-handshake cancellation reap (`transport_handshake_cancelled`) and the unset/PSK profile rejection (`transport_psk_unsupported`) — return before any state write and leave the Transport `connected`, so a second call after one of those still takes the one-shot path and answers 97. Only the IN-PROTOCOL failures, which are the ones that have actually touched the OpenSSL exchange, reach `closed`. In that state, **`async_read_some` and `async_write` already answered 98 before this change** (their guards test `state_ != handshaken`, which `closed` satisfies), while `async_connect` and `async_handshake` answered 97. So this change did not invent a meaning for the failure state — it made the two remaining entry points agree with the two that were already there. All four are now uniform, which `AsioTlsTransportErrorPaths.PostFailedHandshakeEntryPointsReturnAlreadyClosed` measures, read and write legs included, precisely so "uniform" is a measured property and not a claim. Note the plaintext transport has no such second door: `close()` is the only writer of its `closed` state.

  **The one-shot contract is unchanged for the OPEN states**, and only for those: `async_connect` from `connected`/`handshaken`, and `async_handshake` from `handshaken`, still return `transport_already_connected` — witnessed by `test_inflight_exclusivity.cpp` cells 3 and 4. The practical reach of the widened case is small: `tls_transport.hpp` already tells the caller it MUST `close()` after a handshake abort, and B-012-2 has the reconnect path destroying the dead Transport and minting a fresh one, so a second call without an intervening `close()` is not a supported flow — but it was reachable and it did answer, so the answer is disclosed here rather than left to be discovered. *(FR-006; FR-007 as amended; issue #339. Two hostile-review rounds shaped this row: the first cut claimed the one-shot contract was unchanged full stop — it was not; the second cut then said "handshake failure" without excluding the preflight rejections — also not. Both corrections are above rather than summarised, because the distinction is the row's content.)*

  Witnesses, each seen RED at 97 before its green was believed: `AsioPlainTransport.PostCloseConnectReturnsAlreadyClosed`, `AsioTlsTransportErrorPaths.PostClose{Connect,Handshake}ReturnsAlreadyClosed` (one per site, each red under a mutant that removes only its own guard), plus the failed-handshake cell above (red under both TLS mutants, since it probes both guards — deliberately not part of that diagonal).
