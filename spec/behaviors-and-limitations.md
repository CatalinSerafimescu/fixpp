...
- **L-012-2 — Acceptor saturation beyond 10²–10³ sessions/port is unbenched; a client arriving at a full backlog is not connected — whether it is refused or simply left pending is OS- and configuration-dependent.** v1.0 target deployments are 10²–10³ sessions per acceptor. `Endpoint::backlog` *is* forwarded to `listen()` and does bound how many connections the OS completes while the application is not accepting at the depths cell 10 probes — its probe count is its discrimination ceiling, so the 10²–10³ depths this row is about are not witnessed — witnessed by `tests/transport/test_listener_acceptor.cpp` cell 10, which reds when the forwarding is dropped.

  ⚠️ **fixpp makes no guarantee about how the OS declines the overflow client.** Until 2026-09-01 this row said backlog overflow "yields OS-level RST" / "returns TCP RST / connection-refused per OS behaviour". That is **false as a guarantee** and is deleted rather than re-scoped: the overflow client may be reset, refused, or left pending indefinitely, per OS and per configuration (on Linux, `net.ipv4.tcp_abort_on_overflow` selects between reset and drop).
