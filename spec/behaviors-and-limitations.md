...
- **B-215-1 — the C-ABI commit path now REJECTS a repeating group whose exact `(msg_type, ancestor path, no_tag)` context is not declared, instead of resolving one from the dictionary-global store.** 083 T052 already required that `fixpp_msg_commit`'s group-grammar check "must NEVER fall back to the bare global `dict->group_first_field(e.tag)`" — a fallback there "would let the builder accept an order inbound validation rejects". It enforced that by not *writing* a bare call, but the three-argument `table_view::group_first_field(msg_type, parent_path, no_tag)` it did call applies exactly that fallback **internally** on a context miss (the DUAL-STORE INVARIANT block in `include/fixpp/dict/table_view.hpp`), so the rule was stated and not enforced, and the site had no way to observe that it had happened. The commit path now resolves through `group_first_field_exact`, which reports a context miss as `std::nullopt` rather than masking it, and a miss returns `FIXPP_ERR_TYPE_MISMATCH` — joining the pre-existing `tv == nullptr` case on the same fail-closed disposition.
  ...
  **No fixture accompanies this row — but the blocker that made one impossible is GONE (fixpp#264).** Constructing one requires a dictionary that REGISTERS a group at nesting depth ≥ 17, and no shipped FIX dictionary does. Attempting to build one for the fixpp#215 Gate B round surfaced a separate, pre-existing defect, filed as fixpp#264 and fixed since: `find_context_without_delim_record`’s FR-023 completeness probe walked a group’s ancestor chain from the IMMEDIATE parent outward and stopped once it had collected `kMaxGroupContextDepth` entries — keeping the INNERMOST 16 — while the registration path (`Dictionary::as_table_view()`) walked the full chain and let `make_group_ctx_key` keep the first 16 of the outermost-first array, i.e. the OUTERMOST 16. Past depth 16 the two keys differ by construction, so the completeness check missed a record that was present and rejected a COMPLETE dictionary at load with a spurious "registered by as_table_view() but has no per-context delimiter record". Both sides now walk one shared, UNCLAMPED `detail::group_parent_path` and clamp only in the key builders, so a depth-17 dictionary loads and its deepest context resolves (`LoaderDisposition.DeepAncestorChainLoadsAndResolvesDelimiter`, RED before that fix with exactly the message quoted above). What remains unwitnessed is THIS row’s OWN claim — the depth-≥-17 wrong-ancestor QUERY behaviour of the saturating carrier. The fixture now makes it constructible; no test yet exercises it.
