...
- **B-424-1 — a stored application message with no `SendingTime(52)` is replayed with `52` and `122` both set to the retransmission stamp.** Before #424 it was replayed with no `52` and an empty `122`. FIX StandardHeader, `OrigSendingTime`: *"If data is not available set to same value as SendingTime"*. Reachable only through a custom or corrupted `MessageStore`, because `Session::send()` always stamps `52` when a clock is present. *(fixpp#424; witness `Replay_NoStoredSendingTime_Emits52And122FromTheResendClock`.)*
- **B-424-2 — a stored application message whose replay frame cannot be built is folded into the surrounding `SequenceReset-GapFill` run, and `session_event_resend_slot_gap_filled{seq, code}` is appended to `Session::recent_events()`.** Before #424 the slot was skipped silently: no replay and no GapFill, so the peer's gap never closed (FIX-SL §4.8.3/§4.8.5 require a resend answer to eliminate every gap). The replay is built before the open gap run is flushed, so the slot extends that run instead of splitting it. **Reachability:** after B-420-1 the stored `52` is replaced, not duplicated, so the replay frame grows only by the replay tags themselves. A frame that fits the capture buffer therefore cannot overflow the replay buffer, which is how this path used to be reached. A Session with no clock replaying a stored frame that has no `52` does reach it (`wire_required_field_missing`). *(fixpp#424 D4a; `include/fixpp/session/session_event.hpp`; witnesses `Replay_NoClockAndNoStoredSendingTime_SlotIsGapFilled`, `Replay_UnbuildableSlot_JoinsTheSurroundingGapFillRun`.)*
