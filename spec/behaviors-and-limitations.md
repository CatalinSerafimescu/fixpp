...
- **L-215-1 — `SessionConfig::dictionary_view` cannot be checked against `SessionConfig::dictionary`, by construction.** Item 1 lets a caller that has already paid for `Dictionary::as_table_view()` hand the result to `Session::open()` rather than have it walk the same dictionary again (the C-ABI is the only producer today, and derives both fields from the same `shared_ptr`). `table_view` carries **no identity token back to its source `Dictionary`**, so a view built from a *different* dictionary is undetectable and would silently drive inbound parsing and validation from the wrong grammar. The field's doc comment states the derivation requirement; nothing in code enforces it, and adding an identity token to `table_view` is out of this batch's scope. Null (the default, and every non-C-ABI producer) is unaffected — `open()` builds its own exactly as before.
