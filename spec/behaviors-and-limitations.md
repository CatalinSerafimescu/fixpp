...
Feature 074 adds `dict::OrchestraLoader` — a pugixml-based native reader for the official FIX Orchestra machine-readable standard (`OrchestraFIXLatest.xml`, EP303) — producing a runtime `Dictionary` under the new distinct `session_version::vlatest` (read/dictionary tier only; wire application version maps to the existing `v50sp2`, no distinct ApplVerID). Sibling to `XmlLoader`; zero C-ABI change; XmlLoader and the nine QuickFIX dicts untouched. Codeset values+descriptions are preserved via an additive tag-keyed enum side-table on `dict_metadata_handle` + `Dictionary::enum_values(tag)` (orthogonal to the still-stubbed `enum_valid`).
