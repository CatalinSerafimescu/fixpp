...
- **B-456-1 — a `fixpp::dict::table_view` is immutable and non-assignable once built.** Its sixteen population methods are private; the only way to reach them is `fixpp::dict::table_view_builder`, whose `[[nodiscard]] table_view build() &&` yields the view. Both `operator=` overloads are deleted, so a published view cannot be re-seated — seat a `std::optional<table_view>` with `emplace`, not with assignment. Copy- and move-CONSTRUCTION are unchanged and remain load-bearing (`wire::dictionary_driven_validator` holds its view by value). This is a **source-breaking** change to a public C++ type; it is **not** a C-ABI change — `table_view` appears in no `include/fix/` header, so no C-ABI version moves. Every out-of-tree break is a compile error, loud and at the call site: `'add_valid' is a private member of 'fixpp::dict::table_view'`, or `object of type 'table_view' cannot be assigned`. There is no silent behaviour change and no runtime failure mode. *(fixpp#456, `.specify/456-table-view-seal.md` §5a; witness `tests/dictionary/table_view_seal_compile_test.cpp`.)*
- **B-456-2 — a `wire::dict_hooks` bundle latches `has_nonstandard_pair()` at build time, and that latch can no longer go stale.** The behaviour itself is unaltered: `for_table_view` still reads the flag once, when the bundle is built. What changed is that its precondition — the view does not move under a bundle already taken from it — is now enforced by the type rather than kept by convention, and is therefore **no longer observable through the public API**. The witness that demonstrated the stale case, `DictHooksCustomPair.ABundleIsASnapshotOfTheDictionaryItWasBuiltFrom`, is **deleted**, because its premise (mutate-after-publish) is unconstructible; the compile-time seal witness asserts the stronger property that replaced it. Its positive half — a bundle built *after* the pair was registered honours it — survives in `DictHooksCustomPair.FastPathsChangeNoAnswerForAnyTag`. *(fixpp#456 §5c / §5d item 5. This row exists to record what stopped being observable; a green suite is not evidence that half was ever covered.)*
