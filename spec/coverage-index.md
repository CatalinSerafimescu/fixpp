...
| FIX Orchestra / Rules of Engagement machine-readable format (D-011) — **read/dictionary tier PROMOTED → in-scope, delivered by 074-orchestra-native-reader (v1.0); live-wire validation PROMOTED → in-scope, delivered by 075-live-wire-enum-validation (v1.0); typed READ/reify/args/validator codegen tier PROMOTED → in-scope, delivered by 076-fix-latest-typed-codegen (v1.0)**; only the typed-`build_<Msg>`-BUILDER codegen / ApplExtID(1156)=303 / session-negotiation tiers remain deferred | FIX Orchestra spec (`[FIX-Orchestra]`) | Read/dictionary tier: DELIVERED in v1.0 by 074 (`dict::OrchestraLoader` → `session_version::vlatest`). Live-wire validation: DELIVERED in v1.0 by 075. Typed read/reify/args/validator codegen for all 181 FIX Latest classes: DELIVERED in v1.0 by 076 (`fixpp::vlatest`, opt-in `FIXPP_CODEGEN_FIX_LATEST`; non-circular completeness census V-1 ∘ V-1b). Remaining tiers: the typed `build_<Msg>` BUILDER codegen (deferred to a follow-up — 137 MB uncompilable Builders.hpp, Args-dedup redesign required) + ApplExtID(1156)=303 differentiation + session negotiation stay post-1.0 (constitution Article I §1, v0.8 amendment) | v1.2+ (typed-BUILDER-codegen / session-negotiation tiers only) |
