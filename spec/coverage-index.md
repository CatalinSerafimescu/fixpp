...
> **New/changed source (the coverage surface):**
> - `src/capi/dictionary.cpp` (NEW) — CA-011 `fixpp_dict_load_from_xml` (construction-time thunk over `dict::XmlLoader::load`; `CAPI_CONFIG_INVALID` on throw; `*out=NULL` every failure path) + `fixpp_dict_destroy` (full-critical-section process-global mutex: tag check → `dict.reset()` → `tag_=DEAD` → bounded dead-shell registry; NULL-safe; TSan-clean concurrent double-destroy).
