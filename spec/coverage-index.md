...
- `src/dictionary/xml_loader.cpp` `LoaderState::finalize()` — parent-chain walk rejects a nested==parent delimiter before `groups_` sort / before any `table_view`; `as_table_view()` stays non-throwing (FR-003/FR-005). **Ordering, corrected 2026-08-02 (Gate B r1 E1, fixpp#216):** relative to 083's FR-023 completeness invariant, also enforced in `finalize()`, this collision guard runs **SECOND, not first** — verified at source, FR-023's throw is at `xml_loader.cpp:1057` / `orchestra_loader.cpp:898`, this guard at `xml_loader.cpp:1134` / `orchestra_loader.cpp:958`. The two are still correctly **discriminated by catch type** (`group_delimiter_collision_error` derives `xml_parse_error`), which remains the right and sufficient discriminator for a test regardless of which runs first — a witness must not assume this guard fires earlier.
