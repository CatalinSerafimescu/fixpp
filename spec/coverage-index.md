...
| FIX Orchestra / Rules of Engagement machine-readable format (D-011) — **read/dictionary tier PROMOTED → in-scope, delivered by 074-orchestra-native-reader (v1.0); live-wire validation PROMOTED → in-scope, delivered by 075-live-wire-enum-validation (v1.0)**; only the typed-codegen / ApplExtID(1156)=303 / session-negotiation tiers remain deferred | FIX Orchestra spec (`[FIX-Orchestra]`) | Read/dictionary tier: DELIVERED in v1.0 by 074 (`dict::OrchestraLoader` → `session_version::vlatest`). Live-wire validation: DELIVERED in v1.0 by 075 (`wire::dictionary_driven_validator`/`table_view::enum_valid()`, generic across all ten dictionaries incl. FIX Latest). Remaining tiers: typed codegen for the 181 FIX Latest classes + ApplExtID(1156)=303 differentiation + session negotiation stay post-1.0 (constitution Article I §1, v0.7 amendment; row 4b) | v1.2+ (typed-codegen / session-negotiation tiers only) |
