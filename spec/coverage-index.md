...
|---|---|---|---|
| BeginString=FIXT.1.1 — session profile identification | §5.3.1 | S-020 | **033**: `BeginString=FIXT.1.1` accepted/emitted both roles; FIX.4.x sessions byte-identical no-op (FR-009/SC-002). Unit witnesses green (`test_fixt_logon_establishment.cpp` W1/W4); live interop (SC-004/SC-006) deferred to `/speckit-verify` self-run. |
| DefaultApplVerID(1137) on Logon — default app version | §4.3.7, §5.3.3 | S-025 | **033**: `backlog → done`. Emit/parse/enforce (FR-001–FR-006); missing-1137 → `Reject` (FR-003); unserviceable acceptor-side → `Reject` + Logout (FR-004a); `negotiated_version_profile()` (FR-005). Unit witnesses green (W1/W2/W3/W5); live deferred (SC-004/SC-006). |
