...
Wheel-suite port (T012): the only divergence from the in-tree source is ``dict_path``,
which resolves through the installed locator (``_wheeldict``) instead of a
repo-relative ``dictionaries/`` path, so the OO ports run against an installed wheel.
"""
