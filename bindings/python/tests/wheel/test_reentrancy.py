...
def test_engine_close_preflight_raises_without_closing_sibling(monkeypatch):
    ...
    # exactly what makes _Finalizable.__del__ act: at some
...
def test_session_close_step0_backstop_leaves_state_unmodified(monkeypatch):
    ...
    try:
        ...
        # _Finalizable.__del__ calls close() -> _close_impl -> session_close
        # via _close_impl's session_close(self._handle) call, and Engine.close() cascades the same way once per
        ...
        # (_close_impl's _CALLBACK_REENTRANT_CLOSE raise, fixpp.i's _make_error via strerror), whose 4.5.0 wrapper routes
