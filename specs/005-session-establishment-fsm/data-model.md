...
`enum class fsm_state : std::uint8_t` — `NotConnected=0`, `LogonSent`, `LogonReceived`, `Active`, `RecoveryPending`, `LogoutSent`, `Disconnected`. Single-writer on the per-session strand. The `RecoveryPending` state (Clarification Q1 / D-2) is entered from `Active` only; its **exit** transitions are an explicit deferred seam (later recovery feature).

### Transition matrix (`[FIX-SL §4.10]`; every cell defined — no UB; FR-001)

| From \ Event | open (initiator) | inbound Logon (valid) | inbound Logon (refused: BeginString/CompID/not-1st) | seqnum too-low (no PossDup) | seqnum too-high (gap) | inbound TestRequest | heartbeat/TR timer | inbound Logout | initiate Logout | close(terminal) / fatal |
|---|---|---|---|---|---|---|---|---|---|---|
| **NotConnected** | → LogonSent (emit Logon, seq=1, SendingTime) | → LogonReceived → Active (reply Logon, agreed HeartBtInt) | refuse, → Disconnected (no Active) | n/a | n/a | reject/ignore (`[FIX-SL §4.3]`) | n/a | → Disconnected (`[FIX-SL §4.6]`) | → Disconnected | → Disconnected |
| **LogonSent** | idem (idempotent) | → Active (validate HeartBtInt/CompID/BeginString) | refuse, → Disconnected | fatal Logout+disconnect | RecoveryPending (held) | (post-Active only) | Logon-timeout → Disconnected | → Disconnected (`[FIX-SL §4.6]`) | → LogoutSent | → Disconnected |
| **LogonReceived** | n/a | (already acked) | refuse, → Disconnected | fatal Logout+disconnect | → RecoveryPending | → emit Heartbeat(echo TestReqID) | → Active | → Disconnected | → LogoutSent | → Disconnected |
| **Active** | n/a | dup-Logon per `[FIX-SL §4.3]` (no silent re-establish) | n/a | **fatal** Logout(text)+disconnect (`[FIX-SL §4.5.3]`) | **→ RecoveryPending** (hold msg, surface, no ResendRequest) | → emit Heartbeat(echo) | emit Heartbeat / emit TestRequest / unanswered→unhealthy→disconnect | → emit Logout, → Disconnected | → emit Logout, → LogoutSent | → Disconnected |
| **RecoveryPending** | n/a | held (not counted) | n/a | held | held | held | held | → Disconnected (`[FIX-SL §4.6]`) | → LogoutSent | → Disconnected |
| **LogoutSent** | n/a | (drained) | n/a | n/a | n/a | n/a | graceful-close timeout (child slot) → Disconnected | → Disconnected (confirm) | idem (idempotent) | → Disconnected |
| **Disconnected** | `session_already_closed` | ignored | ignored | ignored | ignored | ignored | none | ignored | `session_already_closed` | idem (idempotent) |

`RecoveryPending` exits (on ResendRequest/SequenceReset/gap-fill completion) are intentionally **absent here** — the deferred recovery feature wires them; receipt of `ResendRequest`/`SequenceReset` admin types in 005 is a defined bounded transition consistent with the `RecoveryPending` seam (FR-017), never undefined.

## E3 — Sequence-number state

Next-expected **inbound** `seqnum_t` and next **outbound** `seqnum_t` (E4). Increment-by-one on accepted/sent admin messages, advanced under the durable-before-transmit `MessageStore` ordering (E6). Detect: in-sequence (advance), too-low + no `PossDupFlag=Y` (**session-fatal** Logout+disconnect, `[FIX-SL §4.5.3]`; PossDup S-010 out of scope → treated as the no-PossDup case), too-high (gap → `RecoveryPending`, message held/not-counted). Overflow at `seqnum_max` ⇒ **session-fatal, no wrap**, surfaced via `[2e §6.7] store_seqnum_overflow` through a session-level error callback (`[2e §7.6]` N6). Counter bookkeeping serialized by `fixpp::sync::async_mutex` (D-7; defence-in-depth, structurally zero contention).
...
| `session_seqnum_too_low` | 46 | FR-008, `[FIX-SL §4.5.3]` | session-fatal (no PossDup) |
| `session_recovery_pending` | 47 | Clarification Q1, FR-001 | gap held/surfaced (not fatal) |
...
| `session_invalid_config` | 52 | `[2d §4.5]` N-P2-3 / Session::open validation | configuration |

`session_already_closed` / cancellation outcomes (`dispatch_aborted`, `clock_sleeps_cancelled`) — **reuse the `[2d §6.5]`/`[2d §6.7]`-named variants**; 005 (first threading consumer) introduces the minimal set it exercises at the next contiguous slots, coordinated at Gate A (D-9 cross-doc note). Overflow ⇒ **reuse** `[2e §6.7] store_seqnum_overflow` (no new variant). Final slot numbers/list confirmed at Gate A; the table is the planned allocation, non-renumbering once published per `[const §X.4]`.
...
- **I-4** A `RecoveryPending`-held message is neither processed, counted, nor cause for disconnect; no ResendRequest/SequenceReset is emitted by 005 — seam #4.
