...
| `[const §XVI.4]` | `/analyze` mandatory post-`/plan` | Canonical Spec-Kit order is `/plan` → Gate A → `/tasks` → `/analyze` → `/implement`: `/speckit-analyze` **consumes `tasks.md`**, so it runs after Gate A converges **and after `/speckit-tasks`**, before `/speckit-implement`. |
...
- [x] Error slots T005-pinned at **66..76** (T005 2026-05-21; pre-impl 43..53 + 54..N draft was occupied by 006/007/008 merges, so the 005 set landed at the next free contiguous range per `[const §X.4]`; `[2d §6.5]/[2d §6.7]` triple — `clock_sleeps_cancelled=49`, `session_already_closed=52`, `dispatch_aborted=55` — already pinned by 007/2d, reused not duplicated; research/data-model D-9: `session_*`, `FIXPP_ERR_SESSION_*` 2i-owned coalescing)
...
- [x] Gate A converged round 3, 2 rewrites, user-signed-off 2026-05-18 (runs before `/tasks`)
- [x] `/analyze` drift check (post-Gate-A, **post-`/tasks`**, pre-`/implement` — `/speckit-analyze` consumes `tasks.md`); run 2026-05-18, no CRITICAL; A1/F1/F2/G1/I1 remediated in-bundle
