...
`[2d §4.5]` declares `heartbeat_interval`, `test_request_threshold`, `sending_time_threshold` (all `std::optional`) and `RejectPolicy reject_policy` but explicitly defers their **values** to the session-module spec (this feature). The **graceful-close (Logout) timeout** is *not* a `[2d §4.5]`-declared optional — it is a 005-owned fixed bound for the phase-1 `Clock::sleep_until` close window (D-6); its value is pinned here so FR-005/SC-005 are objectively gate-able. **Decisions:**
...
| `graceful_close_timeout` (Logout-confirmation wait; **not a `[2d §4.5]` optional** — 005-owned fixed bound, D-6) | **2 s** | QuickFIX `LogoutTimeout` default (`Session::DEFAULT_LOGOUT_TIMEOUT = 2`, QuickFIX C++/J unanimous); `[FIX-SL §4.6]` orderly logout. Bound source = the effective-clock seam (`Clock::sleep_until` on the steady deadline, D-5/D-6); on expiry the session force-disconnects → `Disconnected` and surfaces `session_logout_timeout` (slot 73 — T005 pinning post-006/007/008). Not a FIX-spec invariant. |

The first four are `SessionConfig`-tunable knobs (rules-of-engagement, not spec invariants — coverage-index `§3.1` note); the engine substitutes these 005-defined fallbacks at `Session::open` when the optional is `std::nullopt` (`[2d §4.5]` contract). `graceful_close_timeout` is the lone 005-owned **fixed** bound (no `[2d §4.5]` optional declared for it); if a later `[2d]`/`SessionConfig` revision adds the optional, the value is already pinned here.
