...
| `graceful_close_timeout` (Logout-confirmation wait; **not a `[2d §4.5]` optional** — 005-owned fixed bound, D-6) | **2 s** | QuickFIX `LogoutTimeout` default (`Session::DEFAULT_LOGOUT_TIMEOUT = 2`, QuickFIX C++/J unanimous); `[FIX-SL §4.6]` orderly logout. Bound source = the effective-clock seam (`Clock::sleep_until` on the steady deadline, D-5/D-6); on expiry the session force-disconnects → `Disconnected` and surfaces `session_logout_timeout` (slot 50). Not a FIX-spec invariant. |
