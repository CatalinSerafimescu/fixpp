...
### D-2 — FSM state model + the `RecoveryPending` seam (Clarification Q1, fixed)

**Decision:** 7 states — `NotConnected`, `LogonSent`, `LogonReceived`, `Active`, `RecoveryPending`, `LogoutSent`, `Disconnected` — with the `[FIX-SL §4.10]` transition matrix (every state×event cell defined; no implicit/undefined transition). `RecoveryPending` is entered **from `Active`** on a detected `MsgSeqNum`-too-high gap: the out-of-sequence message is **held** (not processed, not counted), the gap is **surfaced** via a documented callback/error (`session_recovery_pending`), the session is **not** disconnected, and **no ResendRequest/SequenceReset is emitted**. The transitions *out of* `RecoveryPending` are a **documented deferred seam** owned by the later "session recovery" feature (S-005/S-006/S-014).

**Rationale (OSS-grounded, 2026-05-17):** QuickFIX C++ (`Session.cpp` `doTargetTooHigh` → queue + `generateResendRequest`) and QuickFIX/J model the gap as a flag/`ResendRange` *on* `Active`; fix8 (`session.hpp` `SessionStates`) models it as an **explicit state** (`st_resend_request_sent`). All three hold/queue and none treats a gap as fatal or advances past it. The explicit-state model (fix8 precedent) is the cleanest phased-delivery seam: 005 ships entry+hold+surface with the exit edges absent-by-design, and the recovery feature adds only the exit transitions — no rework of `Active` semantics. Consistent with `[FIX-SL §4.8.2]`. (Caveat per memory `project_reference_engines_setup`: all 3 engines *implement* resend, so they inform FSM **shape**, not the deferred-recovery phasing — `[FIX-SL]` is primary for the phasing, which the spec already locked.)
...
- `session_seqnum_too_low` (fatal, no-PossDup, FR-008, `[FIX-SL §4.5.3]`)
- `session_recovery_pending` (gap held, surfaced — Clarification Q1, FR-001)
...
**Oracle:** `reference-engines/quickfixj/quickfixj-core/src/test/resources/quickfix/test/acceptance/definitions/server/{fix42,fix44,fix50}` @ QFJ_RELEASE_3_0_1 (version-partitioned; 481 `.def` files; the `<n><letter>_Name.def` numbering mirrors the `[FIX-TC]` scenario numbers). `tests/session/conformance/tc_*.cpp` are parameterized GTest scenarios authored from the **in-scope** `.def` subset; deferred `.def` cases are recorded in `coverage-index.md` as deferred-with-traceability to the named later "session recovery" feature (`[const §V.5]` explicit-scoped-subset).
...
| TC-002 | In-sequence + MsgSeqNum-too-low (fatal) | `2a_MsgSeqNumCorrect`, `2c_MsgSeqNumTooLow` | US2#1/#2, SC-003 |
| TC-002 (entry only) | MsgSeqNum-too-high → `RecoveryPending` entry/hold/surface | `2b_MsgSeqNumTooHigh`, `1a_ValidLogonMsgSeqNumTooHigh` — **assert held+surfaced+not-disconnected only; the resend completion is DEFERRED** | US2#3, SC-003 |
| TC-004 | Liveness (heartbeat / test request) | `4a_NoDataSentDuringHeartBtInt`, `4b_ReceivedTestRequest` | US3, SC-004 |
| TC-005 | Session-level Reject taxonomy | `7_*`, `14a_BadField`, `14b_RequiredFieldMissing`, `14c_TagNotDefinedForMsgType`, `14d_TagSpecifiedWithoutValue`, `14e_IncorrectEnumValue`, `14f_IncorrectDataFormat`, `14g_HeaderBodyTrailerFieldsOutOfOrder`, `2q_MsgTypeNotValid`, `2r_UnregisteredMsgType` | US5#1, SC-006 |
...
**DEFERRED-with-traceability (recorded in `coverage-index.md` → later "session recovery" feature):** TC-006 ResendRequest (`8_*`), TC-007 SequenceReset-GapFill (`10_*`), TC-008 SequenceReset-Reset (`11a/b/c_*`), TC-012 PossDup/PossResend (`2e_PossDup*`, `19a/19b_PossResend*`; S-010), TC-014 synchronize seqnums (scenario 9, optional), store-recovery / RefreshOnLogon (S-018), and the `RecoveryPending` **exit** completion of `2b_MsgSeqNumTooHigh`. Wire-level garbled/checksum/bodylength cases (`2d/3c_GarbledMessage`, `3b_InvalidChecksum`, `2m_BodyLengthValueNotCorrect`) are `wire/`'s (merged PR #68) — referenced, not re-tested here, except the session-layer "unknown MsgType / missing MsgSeqNum" slice under TC-005.
