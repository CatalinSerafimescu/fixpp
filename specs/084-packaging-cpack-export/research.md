...
**Per-version `fixpp::dict::<ver>` targets — proposed OUT of the export set.** *(New at Gate A round 1.)* They were previously included by default. Three verified facts argue against:
...
If they are kept anyway, they MUST be **enumerated explicitly** (never exported as a class), `vt11` MUST be dispositioned by name, and a check other than SC-002 must carry the "the per-version install interface is correct" claim — SC-002 structurally cannot, for the same-destination reason above.
...
| **Per-version `fixpp::dict::<ver>` targets** (R2) | Proposed out. If kept: enumerate by name, disposition `vt11` explicitly, and provide a check other than SC-002 for their install interface |
