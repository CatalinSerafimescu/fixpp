...
> ### The umbrella is not the whole export set — four sign-off members are OPT-IN by name; one is forced-in-closure only
>
> *(Added at the 2026-08-01 sign-off, because D1 = Option A exists to serve a consumer this section otherwise never mentions. Split at Gate A checklist audit, 2026-08-01, because the five were previously presented as one undifferentiated "opt-in, linked by name" class — true for four of them, not for the fifth.)* `fixpp::fixpp` links `fixpp_session`, which transitively pulls the measured eleven. It does **not** reach `fixpp_capi`, `fixpp_config_toml`, `fixpp_tap` or `fixpp_service` — `fixpp_session` links none of them. That is **correct, not a gap**: `src/config/CMakeLists.txt:5-6` records `fixpp_config_toml` as an *"OPT-IN target: no module in the core graph links this"*, and a C-ABI consumer deliberately does not want the C++ umbrella (Article IV §2 / `architecture.md:509`, where linking both fails the `tools/check_layers.py` lint). Those **four** are exported so they are **available by name**, not so the umbrella drags them in.
>
> **`fixpp_log_otlp` is different in kind: it is a fifth member with no consumer-facing reason to be linked by name.** It joins the export set only because `fixpp_config_toml` links it `PRIVATE` under a guard live in every OTel-ON configuration (`src/config/CMakeLists.txt:43-45`), a `$<LINK_ONLY:>` closure requirement (FR-008a) — the same mechanism that drags `fixpp_capi_objects` and `fixpp_dict_dispatch` in. No public header names `fixpp::log_otlp`, and nothing in this contract instructs a consumer to link it directly. It is exported because closure requires it, not because it is an opt-in surface — grouping it with the other four in one "OPT-IN, linked by name" sentence would be wrong.
>
> ```cmake
> find_package(fixpp REQUIRED)
...
> ```
>
> **⚠️ Implementation obligation — `EXPORT_NAME`, or the exported names contradict `architecture.md` §7.4.** `install(EXPORT … NAMESPACE fixpp::)` derives each imported name from the **target** name, and **`add_library(fixpp::X ALIAS …)` does not carry into an export**. Left alone, the export publishes `fixpp::fixpp_capi`, `fixpp::fixpp_config_toml`, `fixpp::fixpp_tap`, `fixpp::fixpp_service` — while `architecture.md:503` (the ground D1 Option A was decided on) says C-ABI consumers link **`fixpp::capi`**, and `include/fixpp/config/toml_config_loader.hpp:7-8` tells consumers to link **`fixpp::config_toml`**. `set_target_properties(<tgt> PROPERTIES EXPORT_NAME <short>)` is the mechanism that makes the installed names match the in-tree aliases; **`EXPORT_NAME` is set nowhere in the repository today** (census: `grep -rn EXPORT_NAME src/ cmake/ CMakeLists.txt` → 0). The same applies to every member with an alias — `fixpp::core`, `fixpp::session`, `fixpp::wire`, … (`src/core/CMakeLists.txt:10` and thirteen siblings). **There is no open convention choice to make**: every export-set member already carries an in-tree `ALIAS` (`fixpp::<short>`), so the convention is fixed by construction — `EXPORT_NAME` MUST be set to the `<short>` suffix of that target's *existing* alias, uniformly, for every member. A member exported with an `EXPORT_NAME` that does not match its existing in-tree alias is the detectable "half-applied" failure mode — checkable by comparing each installed imported name against the alias census above.
...
**DECIDED OUT — the per-version `fixpp::dict::<ver>` INTERFACE targets** *(decided at Gate A round 1, research R2; previously exported as a class. Not one of the three decisions routed to Gate A round 3 for user sign-off — D1/FR-018e/D3 — because the three grounds below settle it without a user tradeoff)*. Three verified grounds, in research R2: both install rules share the destination `${CMAKE_INSTALL_INCLUDEDIR}` (`CMakeLists.txt:323` and `:348`), so the installed generated headers are already reachable through the umbrella's single include root and these targets add **no post-install capability**; `fixpp_dict_vt11` (`cmake/Codegen.cmake:539`) would export with an install interface over `vt11/`, which is denylisted at `CMakeLists.txt:350` and therefore **absent**; and every extra member is additional closure risk. **Standing re-open trigger, not a live branch**: if a *future* change adds them, they MUST be enumerated by name — never exported as a class — `vt11` MUST be dispositioned explicitly, and a check other than SC-002 must carry the install-interface claim.
...
| `fixpp_service` | FR-012a, bound to D1: its only link edge is `fixpp_capi` (`src/service/CMakeLists.txt:29`), now a member | INTERFACE |
