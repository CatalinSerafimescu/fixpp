...
**Scale/Scope**: 6 configurations × {DEB, RPM, TGZ} or {ZIP}. Export set = **11 measured members** (research R2) **+ at least 6 derived** by the 2026-08-01 D1/FR-012a sign-off (`contracts/export-set.md` §2a) + the `fixpp::fixpp` umbrella. The per-version dict INTERFACE targets are proposed *out* (research R2). Include-interface rewrites across **13** `src/*/CMakeLists.txt` files carrying **14** targets — every file that declares the raw include path except `src/core/test/CMakeLists.txt`, whose subtree is the one FR-012a `exclude` (FR-002a). No new library code.
...
│                                    #   fixpp::dict::<ver> targets are proposed OUT of
│                                    #   the export set (R2) -- edit scope depends on that
...
5. The three implementation choices still open by design: the include-interface **rewrite form** (FR-002a), the `fixpp_transport` **`FILE_SET` disposition** (FR-002b), and the artifact-directory **retention rule** (Assumption 5 / SC-008).
