...
- [ ] T001 Re-verify the source anchors the bundle cites still hold (read, do not edit): `include/fixpp/core/sync/async_mutex.hpp` — the reaper `cancel_and_drain` (~1061-1233), `drain_latch_state` (~352-388), `drain_latch_ptr_` (:249), `active_acquirers_count_` (:244), the shipped `drain_latch_state::in_flight_resumptions_` wait (OLD inner field ~1188-1196, read by the shipped terminal condition — REMOVED by T003; NOT the NEW `in_flight_resumers_` member 048 adds), resume runner + E-3 post (~588-615), `release_ref` (~670-690), `on_cancel` (~731-744), `inherited_slot.assign` (:862) + `store_executor` exit (:849-857), reap result `sync_lock_aborted` (:1129-1130); and `include/fixpp/core/error.hpp` `sync_lock_alloc_failed=44`/`sync_lock_drained=46`. Record any line drift in a note; the design is anchored to these.
...
- [ ] T003 In `include/fixpp/core/sync/async_mutex.hpp` remove the cross-thread members: `drain_latch_ptr_` (:249) and the entire nested `detail::drain_latch_state` type (~352-388, its `concurrent_channel`/`notify`/`signal_release`/`signal_abort`/`async_wait`/`released_`/`aborted_`/`in_flight_resumptions_`), and the now-vestigial `active_acquirers_count_`. **CENSUS-COMPLETE (do NOT rely on the list — `grep -n active_acquirers_count_` and remove EVERY occurrence):** decl `:244` + comment `:22` + write sites `:776/781/793/835/850/869/887/907` (8 — the analyze pass found `:793` and `:887` were missing from an earlier draft) + reader `:1190` (the removed terminal condition). After removal, a `grep` for `active_acquirers_count_`/`drain_latch_ptr_`/`drain_latch_state` MUST return zero hits. (data-model.md member table; `[[feedback_census_all_handrolled_scanners_before_scoping_parse_fix]]`.)
...
- [ ] T026 [P] SC-003 contended-perf gate: re-run `bench/sync/bench_async_mutex_contended.cpp` (+ `bench_async_mutex_uncontended.cpp`) under `linux-clang-release` vs the committed `bench/baselines/sync/async_mutex_baselines.json` main baseline; assert ≤5% regression per [const §VIII.2] (soft/report-only this phase); record the numbers in the `/speckit-verify` decision doc. (Removing the feeder RMWs should be neutral-to-better vs main.)
