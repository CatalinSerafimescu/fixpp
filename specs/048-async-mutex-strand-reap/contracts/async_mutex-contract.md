...
- **INV-C (narrowed):** the `inherited_slot.assign` lock-setup site fails closed with `sync_lock_alloc_failed`; the resume `asio::post` remains a pre-existing OOM-terminate site, and the holder-yield post is a new site that replaces the shipped channel `async_wait` (same OOM class) — both deferred L-048. 048 does NOT claim a fully terminate-free lock path.
