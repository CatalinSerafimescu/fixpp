...
- **Immediate-destroy-after-reap (NEW, P1-1):** destroy the mutex immediately after `cancel_and_drain()` returns while reaped resumers are queued → no UAF (the `in_flight_resumers_==0` terminal condition held before return); ASan clean.
