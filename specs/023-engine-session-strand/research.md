...
> **ADDENDUM 2026-06-06 (implementation-time retarget, Codex-validated — `research/reviews/codex_023-engine-session-strand_gate_a_v8_retarget.md`):** the `listeners_`-write-vs-`stop()`-`clear()` target below is **not** an honest deterministic RED in the as-built engine: the PRE-EXISTING join-before-clear (`outstanding_counter_`) establishes `listeners_` write **happens-before** `clear()` (the loop holds the counter; `stop()` blocks at the join until the loop unwinds; a one-sided park only delays both — TSan sees no race, and a parked loop would *deadlock* the join, not race the clear). The HB-free control-plane races that actually exist post-US1 are: **(a)** public synchronous readers `acceptor_bound_endpoint()`/`lookup()` (off-strand) vs the accept-loop map write / `stop()` clear — no live peer, fixed by **D-SNAP (US3)**; and **(b)** `publish_entry` write vs `stop()` step-2 read of `entry.live_transport` (HB-free, before the join) — fixed by confining `stop()`'s reads to the control strand (**T018**). **V-8 is retargeted to (a)** (deterministic, no-peer; RED in US2, GREEN in US3 via the snapshot, converging with V-11). **T018 confines BOTH mutations AND `stop()`'s control-plane reads.** The one-sided-park reasoning below remains correct *in principle* (it is why a latch is wrong) but its chosen access (the join-ordered listener map write) was the wrong target. See tasks.md DD-2026-06-06.
