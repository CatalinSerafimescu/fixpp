...
| **XV.9** Banned (`std::mutex` in awaitable hdr) | core enum is in a mutex-free chrono header; `session_config.hpp` gains `#include <fixpp/core/fix_time.hpp>` (mutex-free) — verify the `session.hpp` awaitable closure is unaffected (`-L sync` / unfiltered Tier-1) | ⚠ WATCH-ITEM (verified safe in Gate A New-5; reconfirm at T003 / verify via `-L sync`) |
...
| **XVII.1** Gate A before /tasks | mandatory — ran after this plan, before `/speckit-tasks` | ✅ PASS (Gate A converged round 3, 2026-06-06; 2 rewrites; user-signed-off) |
...
tests/session/
├── fix_time_roundtrip_test.cpp (extend; target session_fix_time_roundtrip)  # nanos format/parse round-trip; lenient 1-9 widths; reject malformed.
└── test_sending_time_precision.cpp (NEW; target session_sending_time_precision)  # config emits nanos/micros; default millis byte-identity;
...
- CONVERGED round 3 (2026-06-06; 2 rewrites; user-signed-off) — converged submodule commit `62ee12e`. Reviews at `research/reviews/{codex,opus}_026-nanosecond-sendingtime_gate_a*`. Round-by-round log below.
