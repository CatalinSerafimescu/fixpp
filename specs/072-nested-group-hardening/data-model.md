...
- **`code()`**: **inherited from `xml_parse_error`** (`dict_xml_parse_failed`) — **no new `fixpp::core::error` variant is appended** (this feature touches neither `include/fixpp/core/error.hpp` nor `test_020_error_completeness.cpp`). `code()` is **non-virtual**, so a base-typed `catch(xml_parse_error&)` cannot discriminate the derived error via `code()`; discrimination is **by catch type** (`catch (dict::group_delimiter_collision_error&)`). See research D-A1. (No C-ABI `fixpp_error_t` impact either way.)
...
- **Lookup key vs stored child context (do not conflate)**: `table_view::group_member_tags(msg_type, parent_path, no_tag)` takes the queried group's own `no_tag` **separately** from `parent_path` (which excludes it). So group `555`'s members resolve under the **lookup key** `(msg_type="i", parent_path=[296,295], no_tag=555)`; the triple `[296,295,555]` is the **stored child context** minted onto the returned view for a *further* descent — NOT the key under which 555's own members resolve.
...
- **Context store**: `add_group_member_ctx(msg_type, path, no_tag, member)` (`:359`) / `set_group_first_ctx`. Populated by `as_table_view()`. **`msg_type` is the wire `MsgType` value** (e.g. `"i"` for MassQuote, FIX44.xml:943), not the message name — the runtime read queries by the wire value, so a witness must register under `"i"` (NEW-3 / research D-B6).
...
- **Part B witness**: hand-build a `table_view` where the context store — keyed `add_group_member_ctx("i", [296,295], 555, …)` — and the bare `555` store disagree for grandchild group `555`, so the pre-fix bare-fallback is observably wrong.
