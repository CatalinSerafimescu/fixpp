...
- **SC-002**: The generated FIX Latest builder source shrinks by an order of magnitude — from ~137 MB / ~53,590 structs to the read-tier order. The `/plan` census measured **578 distinct `(no_tag, signature)` pairs** (from 26,806 per-path group structs → a 46× collapse of the nested-group surface), comparable to the read tier's 524 flyweights whose header is ~9.7 MB; expected generated size ~10 MB order.
