...
The **read** tier already solved this identical problem: `emit_messages.cpp` emits each distinct repeating group **exactly once** per version as a `G_<no_tag>` flyweight in a shared `fixpp::<ns>::groups` namespace (v44 59, v50sp2 505, vlatest 524 distinct groups), and the resulting single-file `Messages.hpp` compiles (vlatest's is ~9.7 MB / 140 k lines). This feature applies the same **`no_tag`-keyed deduplication** to the builder emitter, then extends the deduplicated builder tier to **every typed version that carries application messages** (v42, v44, v50sp2, vlatest).
...
- **FR-001**: The builder emitter MUST emit each distinct repeating group's input Args structure **exactly once per version**, keyed by the group's `NumInGroup` tag (`no_tag`), into a shared per-version namespace — rather than once per message-path.
- **FR-002**: Each per-message `build_<Msg>` MUST reference the shared per-group Args structures, so a group/component reused across N messages yields **one** Args definition, not N.
- **FR-003**: The builder validation metadata for each group (required-field / group-count / entry-validation helpers and the `writer_traits` binding) MUST be emitted **once per shared group**, with no duplicate or ODR-conflicting definitions across messages.
...
- **FR-006**: The builder tier MUST be emitted for **every typed version that carries application messages** — v42, v44, v50sp2, and vlatest — using the existing application-message scope predicate. Versions with no application messages (vt11) MUST emit no builder output without error.
...
- **Repeating group (dedup unit)**: identified within a version by its `no_tag`; carries a member set (scalars + nested child groups). The unit of deduplication.
- **Group Args structure**: the per-group typed input struct a builder consumes for one group level; after this feature, defined once per `no_tag` per version.
...
- **SC-002**: The generated FIX Latest builder source shrinks by an order of magnitude — from ~137 MB / ~53,590 structs to the read-tier order (roughly ~10 MB and low-thousands of structs), a >10× reduction.
...
- **SC-004**: All existing v44 typed-builder tests pass against the regenerated golden, and generated builder output is byte-identical across repeated generation runs (determinism holds for every builder-bearing version).
...
- **`no_tag` uniquely identifies a group's member set within a version.** This is the invariant the read tier already relies on for its shared `G_<no_tag>` flyweights (which compile across all versions); the builder dedup inherits it. If it did not hold, the read tier would already miscompile.
- **Builders are emitted only for typed versions that carry application messages** — v42, v44, v50sp2, vlatest. vt11 (FIXT session layer) has only admin/session messages, so under the existing application-message scope rule it emits no builders. **Admin/session-message builders (e.g. a typed Logon builder) are OUT of scope** — the engine constructs admin frames internally. *(Flagged for `/speckit-clarify`: confirm vt11/admin builders stay out.)*
- **Each builder-bearing version emits its full application-message set** (the existing "all" families mode). The v44-specific `official` sub-mode (frozen 33) is retained for backward-compatibility of the existing v44 golden freeze; whether a families-style breadth control generalizes to other versions is a `/plan` design detail. *(Flagged for `/speckit-clarify`.)*
