...
**Independent Test**: A subprocess-watchdog pytest stages the **raising callback concurrently against a blocking teardown** (the recv callback blocks on a `threading.Event` so it is provably mid-flight, then raises; the main thread enters a blocking teardown — `engine_destroy`/`session_close` — and sets the Event), and asserts the subprocess completes within a hard timeout (no hang) — so the test **times out (FAILs) if the GIL-release bands were removed** (the worker could never acquire the GIL to run the raising callback). A bare raising callback with no concurrent teardown does NOT discriminate (it just prints and returns), so the staging is what pins the 053 fix.
...
- Q: Is the C-ABI-code→exception translation an exposed callable Python function, or inline in the SWIG out-typemap only? → A: An **exposed helper** — 054 ships the **module-level** `fixpp._map_to_class(code)` + public `fixpp.exception_for_code(code)` (the `[2m §4.6]` package form `fixpp.errors._map_to_class` is the deferred package alias → PY-005) the out-typemap also routes through — single source of truth, so the fallback path and exact-mapping coverage are testable directly with a synthetic code (SC-002/SC-006).
...
### Session 2026-06-26 (Gate A round 1)

- Q: Which translator surface ships in 054? → A: The **module-level** `fixpp._map_to_class(code)` + public `fixpp.exception_for_code(code)` (ratified as the Article XX Gate-A decision); the `[2m §4.6]` package form `fixpp.errors._map_to_class` is the **deferred package alias → PY-005** (a package restructure out of scope for this hardening slice). All spec sites that quoted the package form verbatim are reconciled to the module-level surface so the contract `/tasks` consumes is single-valued.
...
- **FR-013**: The new/updated pytest(s) MUST run in the Tier-1 `python-bindings` CI matrix (`none` / `asan` / `tsan`) and the matrix MUST stay green — this includes the **GREEN (pass-without-canary) leg** of the GIL-release witness, which runs in a normal build and is therefore **in-matrix**. Only the `FIXPP_PY_GIL_RELEASE_CANARY` build (the deliberate-deadlock RED leg, FR-004) is **local-only**: proven RED locally and documented, with the CI-automation waived under the same rationale as 053's SC-004 canary (no deliberate-hang/timeout job is added to the matrix).
...
- **Code→exception translator**: the single Python-reachable callable **`fixpp._map_to_class(code)`** (module-level; + public `fixpp.exception_for_code`) mapping a numeric `fixpp_error_t` to its block subclass (or the `FixppError` fallback); the `[2m §4.6]` `fixpp.errors._map_to_class` package form is deferred to PY-005. The SWIG out-typemap routes through it, and tests exercise it directly for full-mapping coverage and the fallback path.
...
- **SC-003**: The GIL-release witness is **two-mode and discriminating**: in a **normal (non-canary) build** the test runs the teardown-vs-recv-callback scenario and **completes GREEN in the in-matrix suite** (the pass-without-canary leg is actually witnessed, not skipped); in a `FIXPP_PY_GIL_RELEASE_CANARY` build it runs the same scenario in a subprocess and **hangs (hits its hard timeout) — RED, local-only** (the canary CI-automation stays waived, FR-013). If a hang-discriminating scenario is not constructible, the limitation is documented and the non-discriminating substitute is explicitly NOT presented as proof.
- **SC-004**: A raising Python callback never deadlocks the engine — the subprocess-watchdog test (the raising callback staged concurrently against a blocking teardown, so it would **time out if the GIL-release bands were removed**) completes within its hard timeout across repeated runs, and the interpreter is not corrupted.
...
- `[2m §4.6]` The authoritative typed exception hierarchy (root `FixppError` + one subclass per block + the five `BindingError` subclasses + the `_map_to_class` translator, realized **module-level** as `fixpp._map_to_class`; the `fixpp.errors.*` package form is deferred → PY-005) — **realized verbatim by FR-006/007/008**, and **extended** with `AppError` for the post-2m `[1400,1499]` block.
