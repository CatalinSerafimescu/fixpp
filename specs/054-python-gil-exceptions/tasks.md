...
- [ ] T018 Ensure the three new pytests (`test_exceptions.py`, `test_error_coverage.py`, `test_callback_raise_watchdog.py`) are discovered + run by the Tier-1 `python-bindings` ctest matrix legs (`none`/`asan`/`tsan`) — verify `bindings/python/CMakeLists.txt` registration picks them up (the canary RED leg stays out of the matrix). Run the full `python-bindings` suite green under each preset (one at a time, `-j2`). (FR-013/SC-005)
