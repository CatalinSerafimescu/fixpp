...
- **FR-004a**: The module MUST expose the establishment knobs the gold-reference loopback (`capi_loopback_support.hpp:155-173`) proves are required for a fresh pair to log on, and the round-trip MUST set them: an **explicit security profile** (`set_security`, `INSECURE_PLAIN_TCP` for the loopback — without it the `unset` sentinel is rejected at `Session::open()` per constitution Article XII §5 and the session CANNOT establish), **per-role `reset_on_logon`** (`set_reset_on_logon`, true for the initiator / false for the acceptor), the **heartbeat interval** (`set_heartbeat_seconds`), and the **reset-seqnum policy** (`set_reset_seqnum_policy`, `BILATERAL_LENIENT` — accepts the one-sided 141=Y the per-role `reset_on_logon` asymmetry produces). These call already-shipped 052 setters (no C-ABI change).
