...
1. **The peer announced itself.** The readback stream's first line is a `hello` carrying `engine_version`,
   `readback_protocol`, ⭐ **`typed_accessor_arm`** (C-12 — the compile arm's version this counterparty
   *build* was gated by; an image published before the arm existed omits it and the cell must FAIL), and
   the join keys `run_id` / `script_digest` / `config` / `counterparty_digest`.
...
| A peer announcing a `readback_protocol` **older** than the minimum declared for this cell (`plan.md` § *External obligations* → the capability-minimums row — ⚠️ **that artifact is what makes this arm constructible**: *older than X* cannot be built without X, and no X existed anywhere in this bundle before it) | cell FAILS — not skip, not pass | C-2 · FR-016a |
| ⭐ **Run a cell against an image pinned by digest that OMITS `typed_accessor_arm`** — i.e. a counterparty published *before* the typed-accessor compile arm existed | cell **FAILS** — not skip, not pass. ⚠️ **Assert the contrast, or the arm proves only that a broken image fails**: FR-016b's digest pin **RESOLVES and stays GREEN** on that same image — the pin cannot tell a gated build from an ungated one, which is why C-12 exists. ⚠️ Forced **miss** is the correct polarity here (the defect *is* an absent announcement); the spurious hit it cannot catch is the hand-written literal, armed in the next table | C-12 · FR-016a · FR-016b · FR-003b |
...
| The shim's intent renderer drops one declared field of a peer-originated step | cell FAILS, the diagnostic naming FR-008d, the step and the missing path — the peer's `sent` record lacks it | FR-008d |
| A peer-originated declared value containing a TAB | the renderer refuses and writes no intent file; cell FAILS before the conversation, naming the step and path — never a skip | FR-008d |
...
| A persisted bundle whose fixpp stream **drops the `disposition` record** for one peer `sent` record | pair extraction RED, the diagnostic naming that `script_step_id` — ⚠️ assert it is **not** read as `rejected`: *neither delivered nor rejected* is an extraction failure (data-model §13) | FR-012a · C-13 |
...
| The QuickFIX-J counterparty's charset in effect set to `UTF-8` (`CharsetSupport.setCharset`) before its startup assertion runs | the counterparty **refuses to run**, exit non-zero, with its **own** diagnostic naming `UTF-8` and `ISO-8859-1` — ⚠️ and GREEN with the assertion deleted, so the arm is shown to depend on it. The assertion's subject is `getCharset()`; one written on `getDefaultCharset()` (a literal) passes this arm's unmutated half and fails its forced half, which is the point of running both | C-11 · FR-017 |
...
| Seed a message the dictionary should reject — **the probe script's NewOrderSingle missing `TransactTime(60)`, on `CONV-PROBE-off` / `CONV-PROBE-on`** (spec.md FR-010a) — and run **both** validation arms as **TWO** `validator-positive-control` **runs** — one per arm, each with its **own `run_id`** (FR-010a) — and construct the control **pair** from them. ⚠️ **Two runs, not one**: E-7 requires `off_run_id ≠ on_run_id` with opposite `has_validator`, so a single control run cannot form a legal pair | the two arms' accepted-message sets differ, and the validation pair reports `diverged` against `expected_verdict: diverged` | the arms **diverge**, naming the message and the validator's objection. ⚠️ Identical sets are exactly what a **dead validator** produces. ⚠️ This is a **separate run outside the 32-slot inventory** — seeding it into the normal script would fail FR-010 on all 32 by construction | FR-010a · SC-004a |
...
| ⭐ **AN EMPTY `validation_pairs:` SECTION** — the artifact otherwise complete | the pairs the 16-pair inventory names are simply absent, and **nothing that ranges over runs or witnesses can see it**: π, the 32-slot equality and the witness gates are computed from the `runs:` ledger and the witness rows, neither of which the pairs section feeds | **E-7a RED** in the committed schema check. ⚠️ Force it against the **otherwise-complete** artifact, or the arm proves only that a broken bundle fails. ⛔ **E-7b CO-FIRES — this arm does NOT isolate E-7a**, because an empty section carries no control pair either; isolating E-7a would require an admissible control pair to be present, which an empty section cannot have. ⛔ **The claim that stood in the middle column — that *no obligation other than E-7a ranges over a pair's cardinality* — is DELETED, not corrected**: E-7b falsified it, and a replacement enumeration is falsified by the next obligation over this entity | E-7a · FR-012a · SC-009d |
| ⭐ **A validation pair built from two bundles where one peer→fixpp witness is `verdict: fail` (a wrong field value) and BOTH arms' dispositions say `accepted`** | the witness rows disagree with the dispositions: fidelity failed, the validator accepted | the pair is **`identical`**. ⚠️ An extractor reading `verdict` instead of the dispositions yields `diverged` here — a divergence no validator behaviour caused. Write this arm **before** the extractor | FR-010 · data-model §13 · C-13 |
...
| ⤷ **Cross-reference — the GREEN half of the UBSan plant arm** (§ *Forced-miss arms*): the same planted cell under `ubsan` with the runner's preset-derived environment **removed** | the cell **PASSES** — asserted at the arm named in the `Force` cell, where it proves the injected environment, not the plant, is what reddens the cell | FR-021a · FR-017 |
