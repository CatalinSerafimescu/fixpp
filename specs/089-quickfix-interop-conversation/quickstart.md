...
| Bypass the peer's **typed accessor**, filling `typed_reads` from generic enumeration | **The `fix_type` column.** Each `typed_reads` entry carries the field's FIX datatype **as the emitter resolved it from the cell's dictionary**; the **script does not declare it**, and the comparator resolves the expected value independently from its own `FIX44.xml`. Generic enumeration walks the message's field map and does **no dictionary lookup**, so a bypass yields `fix_type` absent or empty — and cannot fabricate it, having no source | RED naming the path. ⚠️ **A decimal-spelling observable was tried and REJECTED on verified evidence**: it needed `Price(44) = 100.1000`'s trailing zeros to reach the wire, and fixpp's decimal writer **strips trailing fractional zeros** (`src/core/decimal.cpp`, `to_chars`, step *AC-S4*). Both paths would have read `100.1` and the arm would have been **inert while looking green** | FR-003b · FR-018 |
