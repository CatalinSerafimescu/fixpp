...
| **E-7c** | ⛔ **FOR EVERY `authoritative: true` PAIR, BOTH REFERENCED RUNS MUST *STILL* BE `authoritative: true`, AND EACH REFERENCE OF A `conformance` PAIR MUST *STILL* BE THE RUN SELECTED FOR ITS `(cell_id, config)` SLOT** — the same two predicates E-7 evaluates at construction, **re-evaluated by the committed schema-check ctest on every CI run**. ⛔ **THE SCOPE IS LOAD-BEARING, and this is the normative statement of it** — `authoritative: true` pairs only, exactly as E-7a and §11 are scoped. data-model §10 **retains** superseded pairs in the section and defines a pair as `authoritative: false` **iff either referenced run has been superseded**, so an *unscoped* E-7c is RED on every **correctly recorded** superseded pair — i.e. permanently RED in the committed ctest from the first retry onward. ⛔ **And do NOT reach for the other repair**: weakening §10's **iff** to make an unscoped E-7c satisfiable would strand E-7a's `authoritative: true` scoping and reopen the duplicate-collapse hole one field over. ⚠️ **Scoped, this reads as a restatement of §10's iff, so say what it verifies**: §10 states the *invariant*; E-7c verifies the **producer actually upheld it** on the committed artifact. A promotion command that supersedes a run and forgets to demote the pair leaves a stale `authoritative: true` pair that resolves, reports complete, and is backed by a run the ledger says governs nothing. ⚠️ **This exists because `authoritative` is MUTABLE after a pair is written.** A retry that lands later supersedes a referenced run; E-7 runs **once**, when the promotion command constructs the pair, and does not re-run — so the exact failure mode E-7's predicate closes re-enters through the temporal door and nothing reports it. The ctest's pre-existing *"every `off_run_id`/`on_run_id` resolves to a `runs:` row"* does not catch it: a **superseded** row satisfies resolution. ⚠️ Both operands are in the committed file — the ledger and the pairs are sections of the same `witness_evidence.yaml`, and *the run selected for its slot* is already well-defined there by E-1c / W-3c — so this is not a new mechanism, it is the **same predicate at the host that re-evaluates it** |
...
| ⭐ **a pair STILL MARKED `authoritative: true`, one of whose referenced runs is LATER SUPERSEDED by a retry — and the PAIR IS NOT DEMOTED** — the ledger row is demoted to `authoritative: false` after the pair was constructed and E-7 passed, the retry's **own replacement `authoritative: true` RUN row IS present**, and **NO replacement PAIR was constructed** — the producer re-promoted the run and never rebuilt the pair | **E-7c RED** in the committed schema check. ⛔ **The fixture must break §10's iff, not honour it** — a pair the producer *did* demote is `authoritative: false`, outside scoped E-7c's population, and the arm would measure nothing. The defect being armed **is** the producer leaving the pair authoritative. ⚠️ **spurious hit**: a stale `authoritative: true` pair carries exactly the shape a correct one does — it resolves, it is well-formed, it is the slot's only claimant — so no forced-miss arm can see it. ⛔ **Both halves of the shape are what ISOLATE E-7c, and each closes a different co-fire — ask what ELSE reddens on this artifact**: (i) the **replacement run row must be present**, or the slot has zero `authoritative: true` conformance runs and **E-1c** co-fires; (ii) **no replacement pair may be written**, or the slot has *two* authoritative conformance pairs and **E-7a** co-fires. Assert **E-1c and E-7a both stay GREEN** on this artifact — exactly one authoritative pair claims the slot and the 32-slot equality is satisfied — so E-7c is the only obligation that fires. ⚠️ **and assert the committed check's PRE-EXISTING reference-resolution clause stays GREEN on the same artifact**: *"every `off_run_id`/`on_run_id` resolves to a `runs:` row"* is satisfied, because a **superseded row still resolves**. That contrast is the arm, and it is assertable on one committed artifact. ⛔ **Do not phrase it as "E-7 stays GREEN"** — E-7 carries the same `authoritative: true` predicate and would go RED here too; E-7's green was a fact about the state *when it fired*, not a property of the fixture, so asserting it would name a contrast that instantiates nothing |
