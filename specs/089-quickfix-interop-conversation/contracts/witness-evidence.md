...
| E-7 | ⛔ A **validation pair** (data-model §10) MUST name **two DISTINCT runs with OPPOSITE arms**: `off_run_id ≠ on_run_id`, the run named by `off_run_id` recorded `has_validator: false`, the run named by `on_run_id` recorded `true`, and both match the pair's `cell_pair`, `config`, `script_digest` and `kind`. A pair violating any part ⇒ **promotion RED**. ⚠️ **This is E-6's sibling, not E-6.** E-6 checks *one run's* attestation against *its own row*; nothing in it ranges over a pair's two referenced runs, so a degenerate pair reading both arms from one execution satisfies E-6 and reports `identical` across all 32 slots — a set equalling itself |
