...
| A validation pair whose `verdict` **disagrees with its own `accepted_off`/`accepted_on`** (equal sets, `verdict: diverged`; or unequal sets, `verdict: identical`) | **E-7 RED** — proves `verdict` is *derived*, not carried. ⚠️ **spurious hit**: a hard-coded `identical` reports the same shape a measured one does |
