...
| C-2 | `readback_protocol` ≥ **the minimum declared for this cell by the harness capability-minimums artifact** — `plan.md` § *External obligations* → the capability-minimums row is that operand's **normative home**, names the artifact and states its fail-closed rule. ⛔ **No minimum is written into this bundle**, here or anywhere else | older ⇒ cell **FAILS**. ⛔ **And so does a cell for which no minimum is declared** — an absent operand is a failure, never *"no minimum"*; see that row |
...
| # | Witness |
|---|---|
| C-2 | a `readback_protocol` **older** than the minimum declared for this cell (`plan.md` § *External obligations* → the capability-minimums row) ⇒ cell FAILS |
| **C-12** | a peer whose `hello` **omits** `typed_accessor_arm`, or announces one older than the minimum declared for this cell (`plan.md` § *External obligations* → the capability-minimums row) ⇒ cell **FAILS** — ⚠️ **and assert FR-016b's digest pin RESOLVES and stays GREEN on that same image**, which is the contrast that makes the arm discriminating: the pin alone cannot tell a gated build from an ungated one. ⚠️ **Plus the provenance half, which is where the vacuity is**: a counterparty source carrying a **hand-written** `typed_accessor_arm` literal, with the arm removed from its build, **MUST FAIL TO COMPILE** — otherwise the announcement is satisfiable without the arm and C-12 measures a string. ⛔ **IN BOTH LANGUAGES, AND THE TWO HALVES ARE NOT SYMMETRIC BY DEFAULT** — the C++ half fails because `#ifndef … #error` is a dependency **independent of the value expression** and therefore survives a fixture that replaces the value with a literal; a Java implication of the form *"the generated constant is absent ⇒ compile error"* does **not** survive it, because the fixture removes the only reference. The mechanism that makes the Java half fail is stated in `plan.md` § *External obligations* → the typed-accessor compile-arm row, which is its **normative home**; this row does not restate it |
