...
| `phase-9-harness/tools/run_interop_cell.py`, `phase-9-harness/quickfix-cpp/counterparty/interop_counterparty_main.cpp`, `phase-9-harness/quickfixj/.../InteropCounterparty.java`, `phase-9-harness/tools/test_run_interop_cell.py` | **FR-008d.** The shim renders every `originator: peer` message's declared `intent_fields` into a flat per-run intent file (`step_id` TAB `path` TAB `value`; a value containing TAB, LF or SOH is refused, not escaped) and passes its path to the counterparty; each counterparty parses it by hand and builds its peer-originated messages from it instead of source literals. A post-run check fails the cell when any `sent` record of either process lacks a path the script declares for its step, or carries a different value. The arms are `quickstart.md` § *Step 4*'s FR-008d rows | FR-008d · FR-018 |
