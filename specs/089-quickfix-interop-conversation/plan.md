...
> round's finding. **Re-derive instead**: walk every `## N.` section of `data-model.md` and check each
> resolves to a row in the table below or to a named exception in the next paragraph.
>
...
| the instrument was proven on **g++ + javac only**, for a 3-toolchain matcher | — | all three now run; **every config is a `linux-clang-*` preset**, so the proven compiler was the one the matrix never runs | `spec.md` Clarifications |
...
⚠️ **Three RED fixtures added, two of them spurious-hit arms** — a forced-miss arm cannot catch either the hard-coded verdict or the duplicate slot, because both report the same shape a correct result does.
