...
in `research.md` mandates the experiment: build each configuration (targeted, per below) while
sampling **both** numbers, and derive **both** predicates from what is observed. ⚠️ *Amended 2026-09-11
(user decision, `spec.md` Clarifications): the build is measured as the matrix performs it — incrementally,
in the existing per-configuration trees — not from clean.*
...
| ″ | The shim **computes** `script_digest` (lowercase-hex SHA-256 over the script bytes) and **compares** it against the value each side recomputes; a mismatch FAILS before the conversation (FR-024 (b): before the gtest is launched only where fixpp initiates) | FR-008c · FR-013b |
...
| ″ | ⭐ **Each cell's process runs with its configuration's test-preset environment (FR-021a).** Before launching a cell's gtest, read the `environment` of the test preset that configuration maps to from `library/CMakePresets.json` and pass it to the process — **read, never copied**, so the value #268 put in one place stays in one place. ⚠️ The runner launches the gtest binary directly and so bypasses ctest, the only reader of a test preset until now. ⛔ No option value is written in this row | FR-021a · FR-017 · FR-018 |
...
| `phase-9-harness/tools/emit_matrix.py` | **Invokes the promotion command once per configuration**, after that configuration's cells have run and before anything reclaims its build tree; a conversation cell whose promotion fails cannot keep `status: pass`. ⚠️ This is where T025's sequencing point lives: `run_interop_cell.py` runs **one** cell, so the per-configuration loop — and therefore *after a config's cells complete* — is `emit_matrix.py`'s. ⭐ **It is also the producer of the cell row** (data-model §5): `build_manifest` overlays each live cell's `status` and `config` onto the committed `cell_results.yaml` template rows, once per configuration run | FR-014b · E-1b · data-model §5 |
| `phase-9-harness/configs/conversation-{quickfix-cpp,quickfixj}-{acceptor,initiator}-tls.cfg.in` (**NEW** ×4) | The eight conversation cells' counterparty templates: `UseDataDictionary=Y` with a FIX 4.4 `DataDictionary` path. ⛔ The existing `*-tls.cfg.in` templates are **not** edited — the `PD-*` and idle-cadence cells name them | FR-002 · R-5 |
| `phase-9-harness/tools/test_run_interop_cell.py`, `test_promote_interop_evidence.py`, `test_emit_matrix.py` (**NEW**) | The runner-side guards' arms — the hello gate, the script-digest compare, the test-preset environment, the promotion join keys and the pass-withholding step — each with a forced miss run against the unmutated tree first (quickstart.md § *Step 4*'s three rules) | FR-017 · FR-018 |
...
| ″ | ⭐ **A test-only UBSan plant (FR-021a).** A hook, off by default and enabled only by an explicit test variable, that makes one cell's gtest execute undefined behaviour UBSan reports and print a marker line first. It exists only for the `quickstart.md` § *Step 4* arms — the forced miss under `ubsan` and the spurious-hit run under `normal` — and must be unreachable in every other run | FR-021a · FR-017 · FR-018 |
...
| `phase-9-harness/quickfixj/.../InteropCounterparty.java` | Assert `org.quickfixj.CharsetSupport.getCharset()` is **`ISO-8859-1`** at startup and fail loudly otherwise. ⚠️ *Not* `getDefaultCharset()`: in the pinned engine that method returns a hard-coded literal, so an assertion on it can never fail; the charset in effect starts as that default and `setCharset` changes it. `value_b64` is only reconstructible because that charset is a total bijection over all 256 byte values; under a non-bijective charset the raw bytes are unrecoverable at application level | C-7 · FR-004 |
| `phase-9-harness/quickfixj/src/test/java/io/fixpp/phase9harness/quickfixj/InteropCounterpartyCharsetTest.java` (**NEW**) | The charset assertion's forced-miss arm (`quickstart.md` § *Step 4*): charset in effect set to `UTF-8` ⇒ refusal with its own diagnostic; default charset ⇒ accepted; restores the global charset after each test. The assertion is a method `main` calls, so the arm tests the code `main` runs | C-11 · FR-017 |
| `phase-9-harness/tools/run_interop_cell.py`, `phase-9-harness/quickfix-cpp/counterparty/interop_counterparty_main.cpp`, `phase-9-harness/quickfixj/.../InteropCounterparty.java`, `phase-9-harness/tools/test_run_interop_cell.py`, `library/tests/interop/conversation/` | **FR-008d.** The shim renders every message's declared `intent_fields` into a flat per-run intent file (`step_id` TAB `originator` TAB `msg_type` TAB `path` TAB `value`, for the cell's combo in script step order; a value containing TAB, LF or SOH is refused, not escaped) and passes its path to the counterparty and to fixpp's conversation cell; each parses it by hand and builds the messages its own `originator` names from it, instead of source literals — except messages the engine generates in reply, which no application builds. A post-run check fails the cell when any `sent` record of either process lacks a path the script declares for its step (excluding `runtime_generated` paths), or carries a different value. The arms are `quickstart.md` § *Step 4*'s FR-008d rows | FR-008d · FR-018 |
| `library/tests/interop/conversation/probe_script.yaml` (**NEW**), `library/tests/interop/conversation/conv_cell_test.cpp`, both counterparties' conversation mode, `phase-9-harness/tools/run_interop_cell.py`, `phase-9-harness/tools/promote_interop_evidence.py` | **FR-010a's divergence probe gets a launch surface (T062).** The committed probe script (one peer-originated `D` whose intent omits `TransactTime(60)`); the cells `CONV-PROBE-off` / `CONV-PROBE-on` on combo C1, whose **registration** carries `kind: validator-positive-control` and `expected_verdict: diverged`; a fixpp cell flow that records the probe message's disposition and then ends the session (`Engine::stop()`); the counterparty driving the probe from its intent file like any other script; and the runner passing `kind` / `expected_verdict` from the registration into promotion. ⛔ The probe never occupies a conformance slot and never enters the census | FR-010a · SC-004a · E-7b · data-model §9 |
| `library/tests/interop/conversation/conv_cell_test.cpp`, `library/tests/interop/support/readback_jsonl.hpp`, `phase-9-harness/tools/promote_interop_evidence.py`, `phase-9-harness/tools/test_promote_interop_evidence.py` | **The validator's per-message disposition gets a source (T061).** fixpp's conversation cell writes one `disposition` record per inbound arrival its session delivered (`fromApp`/`fromAdmin`) or rejected (its own outbound Reject(35=3), observed through `toAdmin`, whose `RefSeqNum(45)` names the arrival), with the Reject's 45/373/371/58 on a rejection; `readback_jsonl.hpp` gains the writer. The promotion command's disposition extractor reads each run's **persisted bundle** — fixpp's dispositions joined to the peer's `sent` records on `(seq_num, direction, occurrence)` — and **replaces the fail-closed default**. A peer `sent` record joining no disposition, or two, is extraction RED naming its step; *neither delivered nor rejected* is never read as `rejected`. The extractor MUST NOT take witness rows as input. Both C-13 arms land with it | FR-010 · FR-012 · FR-012a · data-model §10 · data-model §13 · C-13 |
| `library/spec/behaviors-and-limitations.md` | **L-067-2's text corrected to the guard's real range.** It says `body_builder`'s string path fail-closes on *"`0x01`/control bytes"* and that *"clean-text Data (no control bytes) IS supported"*; the guard (`src/wire/body_builder.cpp`, `is_printable`) admits only `0x20–0x7E`, so no non-ASCII content is supported either. Found because `B-05`'s `0xff` could not be built; the limitation itself stays open as fixpp #418 | spec.md § *Conversation census* → `B-05` · C-11 |
| `library/ci/disk-preflight.sh`, `library/ci/test-disk-preflight.sh` (**NEW**) | ⭐ **Added at `/speckit-implement` under RC-B** — both files were named by T002/T005/T007 and specified by `contracts/disk-preflight.md`, but had no row here, so the unconditional rule was not met. The obligation is the contract's, not a new one: D-1–D-11 including D-9a's bootstrap, and every arm in § *Required RED arms*. ⛔ **No threshold is recorded in this row**: both required values come from R-1's measurement per configuration, written into the script's threshold block with their dates, and nowhere else | D-1 · D-9a · D-11 · FR-023a · R-1 |
| `library/.github/workflows/tier1.yml` (`ci-script-pins` job), `library/ci/test-tier1-python-policy.sh` | ⭐ **Added at `/speckit-implement` under RC-B** — T006 registers the gate's harness in `ci-script-pins`, per the contract's *Convention* line, which changes both files. The policy harness carries a mutant proving the job **invokes** `ci/test-disk-preflight.sh` rather than merely naming it; a registration without that mutant is a claim, not a check | FR-017 · FR-018 |
...
| **`checklists/requirements.md`'s struck three-config-vocabulary paragraph, under its "after `/speckit-clarify`" heading** | Struck in place rather than rewritten — the lines sit under a dated *"after `/speckit-clarify`"* heading and are a historical record of that session; striking preserves the record while removing the false present tense | same file |
