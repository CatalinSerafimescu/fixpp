...
| `library/.github/workflows/interop-smoke.yml` | The `IMAGE:` key is pinned to the **pre-089 digest before the counterparty image is republished**, so moving `:latest` is inert for consumers resolving it from the default branch. ⛔ **THIS ROW IS ONE CONSUMER, NOT THE WHOLE DECISION.** R-11 decides *"pin `interop-smoke.yml` **and the existing cells**"*, and the cells it names — R-5's `INTEROP_CP_CORRUPT_ADMIN`, the `PD-*` malformed-dup cells, everything FR-020 protects — run under `interop-matrix.yml`, the row below. This row alone was the whole table's answer for ten rounds, and a reader who took it as the decision pinned half of it. ⚠️ **An unpullable pin is a SKIP, and a skip is GREEN** (FR-016b): this workflow's counterparty-absent handling skips-with-reason and keeps the job green, so a green check is **not** evidence the pin works — confirm from the run's own **step log** that the smoke cell executed. ⚠️ **And a pin merged to the default branch does not reach work already in flight** — FR-026's condition on a `branches: ["**"]` pull-request trigger, which this workflow carries | FR-026 · R-11 · FR-016b |
| `.github/workflows/interop-matrix.yml` (**PARENT repo** · **NEW row**) | ⭐ **APPLIED UNDER RC-B, WHICH IS UNCONDITIONAL** — a file outside the bundle that this feature must change had **no row at all**, and that absence is precisely what let the narrower reading of R-11 survive in the row above. **(a) THE PIN**: its `IMAGE:` key names the mutable `:latest` and is pinned to the **pre-089 digest before the republish**, on the same rule and for the same reason as the smoke workflow — this is where the cells R-11 means by *"and the existing cells"* actually run. **(b) AND THE TENSION THE PIN CREATES IS RESOLVED IN THE SAME CHANGE, NOT AT EXECUTION TIME**: a workflow-level pre-089 pin also **starves 089's own cells** of the image they need, because the matrix is how they receive it. ⛔ **THE OVERRIDE MECHANISM IS NOT DECIDED, AND AN EARLIER REVISION OF THIS ROW CLAIMED IT WAS.** That revision said the mechanism *"is settled by `tasks.md` T004 and lands here, as an obligation"* and, in the same breath, that *"decide the mechanism"* left standing in a task scores **ABSENT** — while T004 read, and still reads, *"**Decide** the mechanism…"*. The row certified its own closure against a task that had not closed. ⚠️ **That is this repository's most expensive recorded defect class — a fix replacing a wrong claim with a new one — committed inside the batch written to close it**, which is why the claim is deleted rather than restated. **This obligation is OPEN**: no artifact in this bundle states a mechanism, and until one does, `interop-matrix.yml` MUST NOT be pinned to the pre-089 digest, because doing so starves 089's own cells of the image they need with nothing to override it. ⚠️ **The two live options are already enumerated in `spec.md` FR-026 and the choice between them is a standing decision, not an implementation detail** — see the residual note there on which one the open-PR reachability finding favours. **(c) FAILURE DIRECTION**: ⚠️ **an unpullable pin is a SKIP, and a skip is GREEN** (FR-016b) — this workflow and the smoke workflow both stay green when the image will not pull, so neither pin is evidenced by a green check. Evidence that each pinned cell **actually executed**, read from the run's own **step log**, never from `.conclusion`. **(d) ⛔ DO NOT COUNT THE CONSUMERS — RE-DERIVE THEM**: `grep -rln "fixpp-interop-counterparties" .github/workflows/ research/G19-fix-fpml-iso20022/library/.github/workflows/` enumerates every file naming the image in either repository, and the **discriminator** is that a *consumer* resolves the image at run time while `publish-counterparties.yml` **creates** it — so the publisher appears in that output and is not a consumer. A written-down count re-arms the moment a third consumer appears, which is the shape this bundle deletes everywhere else | FR-026 · FR-016b · R-11 · R-5 · FR-020 |
