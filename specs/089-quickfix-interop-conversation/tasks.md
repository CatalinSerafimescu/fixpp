...
- [ ] T054 [US3] Assert fixpp's typed read tier returns the exact field values the peer set when fixpp is the **acceptor** — closing the direction that today has no field-value assertion (FR-007 · SC-002) — ⏳ C1 exercises fixpp's typed read tier on all six peer→fixpp steps; the acceptor direction this task names is C2/C4, open
