...
- [ ] T003 ⛔ **The `interop-matrix.yml` half of this task is GATED ON T004 — read it first.** Pinning the matrix before the FR-026 option is confirmed starves 089's own cells with no override mechanism, and under FR-026's second option no matrix pin is wanted at all. The `interop-smoke.yml` half is unaffected. Pin **every** counterparty-image consumer to the pre-089 digest — ⛔ **enumerate them, do not count them**: `plan.md` § *External obligations* → the `interop-matrix.yml` row carries the re-derivation recipe and the discriminator that separates a consumer from the publisher; the two below are what that recipe returns today, closing the half of FR-026/R-11 that is currently open: `.github/workflows/interop-smoke.yml`'s `IMAGE:` key (**[INHERITED]** — already done on fixpp `089-pin-counterparty-digest`) **and** the parent's `.github/workflows/interop-matrix.yml:38`, which still names `:latest`. R-11 requires *"`interop-smoke.yml` **and the existing cells**"*, and the cells it names by name — R-5's `INTEROP_CP_CORRUPT_ADMIN`, the `PD-*` malformed-dup cells, everything FR-020 protects — run under `interop-matrix.yml`, not under the smoke workflow
- [ ] T004 Close the bundle gap T003 exposes, then resolve the tension it creates. ⛔ **`.github/workflows/interop-matrix.yml` has NO row in `plan.md` § *External obligations* at all** — it is a file outside the bundle that this feature must change, so the plan's own **RC-B** (*"§ External obligations is UNCONDITIONAL"*) requires one. That missing row is what let the narrower reading survive: the table's `interop-smoke.yml` row is **narrower than R-11**, which says *"`interop-smoke.yml` **and the existing cells**"*. Add the row, then resolve the tension: pinning `interop-matrix.yml` to the pre-089 digest also starves **089's own** cells of the image they need, since the matrix is how they receive it. ⛔ **T004 BLOCKS T003's matrix half — do not pin `interop-matrix.yml` before this task closes.** Pinning it to the pre-089 digest starves 089's own cells of the image they need, and there is no override mechanism: `plan.md` § *External obligations* records that obligation as **OPEN**, after an earlier revision of that row wrongly certified it closed against this task. ⚠️ **The decision is upstream of the mechanism**: `spec.md` FR-026 enumerates two options, and its residual note records that the second (publish under a new tag; move `:latest` only after FR-020's regression run is green) **dominates on the open-PR evidence and needs no matrix pin at all**, which would dissolve this task rather than resolve it. So confirm the FR-026 option **first**; only if option 1 is confirmed does a mechanism need deciding, and then state it as an obligation rather than as prose. ⚠️ `interop-matrix.yml` triggers on `pull_request` with `branches: ["**"]`, so every open PR resolves the tag at run time against its own base — a pin landing on `main` does not reach them, and the republish therefore cannot be made retroactively safe
...
- [ ] T099 Run FR-020's regression obligation: every existing passing cell outside the narrow business set continues to pass, or each change is characterised. ⛔ **This is the measurement the counterparty republish currently lacks** — the inherited work argues safety from the opt-in gate rather than measuring it (FR-020 · SC-008)
