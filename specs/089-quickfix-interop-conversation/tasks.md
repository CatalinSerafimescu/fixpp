...
- [ ] T061b [US4] Replace `_default_disposition_extractor`'s fail-closed default in `promote_interop_evidence.py` with the §13 join, reading each run's **persisted** bundle (never the witness rows): a peer `sent` record joining no disposition, or two, ⇒ extraction RED naming its `script_step_id`; *neither delivered nor rejected* is never `rejected`. Both C-13 arms (`quickstart.md` Step 4) — ⛔ **write the spurious-hit arm (witness `verdict: fail`, both dispositions `accepted` ⇒ `identical`) before the extractor**, and show each arm RED for its own reason
