...
- [ ] T053 [US3] Implement FR-003a's `sent` record on the **peer** side as the source of truth for the peer→fixpp direction. ⚠️ This is why FR-003a exists: the counterparty generates `OrderID`/`ExecID` at run time (`const int seq = ++id_counter_;` then `"ORD" + …` / `"EXC" + …`), so those values are knowable only inside the peer and **no declarative script can supply them** — ⏳ QuickFIX-cpp side done with C1; QuickFIX-J open
- [ ] T053a [US3] Implement FR-008d. (a) `run_interop_cell.py` renders every message's `intent_fields` from `conversation_script.yaml` into a flat per-run intent file (`step_id` TAB `originator` TAB `msg_type` TAB `path` TAB `value`, the cell's combo only, in script step order), refusing a value containing TAB, LF or SOH, and hands its path to the counterparty and to fixpp's cell; both counterparties parse it by hand and build each peer-originated message from it, replacing the hardcoded literals for every declared path. ⚠️ **The renderer and its flat-format reader land before T052**, whose fixpp cell builds the fixpp-originated messages from the same file. (b) A post-run check over **both** processes' `sent` records: every path the script declares for the record's step is present with the declared value, else the cell FAILS. Arms per `quickstart.md` § *Step 4* (FR-008d rows). ⚠️ Counterparty source change ⇒ inert in the matrix until publish #2, which is held until US3's counterparty work lands (user decision, 2026-09-11). ⚠️ **Id note**: `a`-suffixed on this bundle's own idiom (see T002a) — added 2026-09-11 implement — ⏳ (a) renderer + three readers + reader-agreement check (harness 4910f7a…7fc61b5, library 0b3f9d62); (b) check wired; the QuickFIX-cpp peer builds from the file (C1). QuickFIX-J consumer open
- [ ] T054 [US3] Assert fixpp's typed read tier returns the exact field values the peer set when fixpp is the **acceptor** — closing the direction that today has no field-value assertion (FR-007 · SC-002) — ⏳ C1 exercises fixpp's typed read tier on all six peer→fixpp steps; **C2 done** (84b5e5c6): typed read vs declared intent asserted explicitly for fixpp-as-acceptor, RED when fixpp's ExecType(150) typed read is forced; C4 (QuickFIX-J) open
