...
- [ ] T024 *(PARTIAL 2026-09-11: the E-1b core — both streams, `hello` + `terminal` on each, join keys, `terminal_state` — landed on parent `089-harness`; the completeness gate, evidence persistence, `evidence_digest`, the ledger and manifest rows, `authoritative`, the validation pairs and E-7 wait on T026's `witness_evidence.yaml` and `census.yaml`; plan.md's row for this file is the full list)* Create `phase-9-harness/tools/promote_interop_evidence.py` — **the named promotion command** (FR-014b), the **only** reader of a run artifact and the **only** writer of a `pass` row. It opens both streams, requires a `hello` **and** a `terminal` on each (E-1b), checks the join keys, and evaluates the corroboration
