...
| fixpp `089-pin-counterparty-digest` | `5ef6e88b` | FR-026's `interop-smoke.yml` digest pin |
...
⛔ **Do not merge the parent branch before T0-series completes.** `research.md` R-11 requires
existing consumers pinned to the pre-089 digest **before** the republish, and only one of the two
consumers is pinned today. See T003/T004.
...
- [ ] T003 Pin **both** counterparty-image consumers to the pre-089 digest, closing the half of FR-026/R-11 that is currently open: `.github/workflows/interop-smoke.yml`'s `IMAGE:` key (**[INHERITED]** — already done on fixpp `089-pin-counterparty-digest`) **and** the parent's `.github/workflows/interop-matrix.yml:38`, which still names `:latest`. R-11 requires *"`interop-smoke.yml` **and the existing cells**"*, and the cells it names by name — R-5's `INTEROP_CP_CORRUPT_ADMIN`, the `PD-*` malformed-dup cells, everything FR-020 protects — run under `interop-matrix.yml`, not under the smoke workflow
- [ ] T004 Close the bundle gap T003 exposes, then resolve the tension it creates. ⛔ **`.github/workflows/interop-matrix.yml` has NO row in `plan.md` § *External obligations* at all** — it is a file outside the bundle that this feature must change, so the plan's own **RC-B** (*"§ External obligations is UNCONDITIONAL"*) requires one. That missing row is what let the narrower reading survive: the table's `interop-smoke.yml` row is **narrower than R-11**, which says *"`interop-smoke.yml` **and the existing cells**"*. Add the row, then resolve the tension: pinning `interop-matrix.yml` to the pre-089 digest also starves **089's own** cells of the image they need, since the matrix is how they receive it. Decide the mechanism by which 089's cells override the workflow-level pin with their own FR-016b digest, and state it as an obligation rather than as prose. ⚠️ `interop-matrix.yml` triggers on `pull_request` with `branches: ["**"]`, so every open PR resolves the tag at run time against its own base — a pin landing on `main` does not reach them, and the republish therefore cannot be made retroactively safe
...
- [ ] T020 Add the full `INTEROP_FIXPP_*` env block to the **gtest's** environment in `phase-9-harness/tools/run_interop_cell.py`: `RUN_ID`, `CELL_ID`, `CONFIG`, `ARM`, `SCRIPT_PATH`, `SCRIPT_DIGEST`, `READBACK_PATH` (data-model §1). ⚠️ `INTEROP_CP_*` is `env=cp_env` on the counterparty process only — the gtest needs its own block
...
- [ ] T027 Extend `REQUIRED_FIELDS` in `tests/interop/cell_results_schema_check_test.py` **conditionally, on `kind: conversation` only — never globally**. An unconditional extension breaks the **59** `status: pass` rows already committed, none of which has an 089 run behind it
...
- [ ] T031 [US1] Configure the fixpp session for this feature's cells with the production `dictionaries/FIX44.xml` rather than the `make_minimal_dictionary()` sentinel, in `tests/interop/happy/hp_support.hpp` (FR-001)
...
- [ ] T040 [US2] Extend the cross-language golden fixture at `phase-9-harness/interop-readback-fixture/` from **two** emitters to **all three** (QuickFIX-cpp counterparty · QuickFIX-J counterparty · **fixpp**) and add **tag 1156** (C-7). ⛔ **Extend it, do not replace it** — the committed two-way half carries a re-derivation recipe and a proven can-it-fail check
...
- [ ] T052 [US3] Implement the eight conversation cells (4 combos × 2 arms) in `tests/interop/conversation/`, driven by `conversation_script.yaml`, running the whole repertoire **within a single session per cell** (FR-008 · FR-009)
...
- [ ] T072 [P] [US4] **Spurious-hit** arm: a `validation_pairs:` section carrying **15 of the 16** conformance pairs — the other 15 well-formed and E-7 passing on every one ⇒ RED on the inventory equality, not on pair well-formedness
...
- [ ] T075 [US5] Implement E-1a's manifest row identity `(cell_id, config)` — **32 rows**, retries never committed — and E-1c's *exactly one `kind: conformance` run with `authoritative: true` per `(cell_id, config)`*, retries carrying `authoritative: false`
...
- [ ] T086 [P] [US5] Forced-miss arms on the schema check: a falsified `cell_results.yaml` row (`pass`, no evidence) ⇒ RED (E-1); `pass` naming a **ledger entry that does not exist** ⇒ RED; a ledger `witness_count` differing from the census figure for that slot ⇒ RED; an `ENOSPC` abort recorded as `fail` ⇒ RED (E-5)
...
- [ ] T095 Execute the 32-run matrix (8 cells × 4 configs) per `quickstart.md` Step 3, sequencing the four **targeted** builds so the disk constraint is respected — four FULL builds cannot fit, four TARGETED builds can (FR-021 · FR-023a). Every configure/build is gated by `ci/disk-preflight.sh`
...
- [ ] T099 Run FR-020's regression obligation: every existing passing cell outside the narrow business set continues to pass, or each change is characterised. ⛔ **This is the measurement the counterparty republish currently lacks** — the inherited work argues safety from the opt-in gate rather than measuring it
- [ ] T100 [P] Forced-miss arm: the **59 pre-existing `pass` rows, unmodified** ⇒ the schema check stays **GREEN** — the control proving the conditional-field rule of T027 did not break FR-020
...
- [ ] T103 Forced-miss arm: run a cell against an image pinned by digest that **omits** `typed_accessor_arm` — a counterparty published *before* the compile arm existed ⇒ cell FAILS (C-12). ⚠️ This is precisely the pre-089 digest T003 pins, so the fixture exists by construction
- [ ] T104 Execute the counterparty republish in the order T003/T004 settle: pin both consumers → merge the parent counterparty change → read the new digest from the publish run's **step log**, never `.conclusion` → repoint 089's cells per FR-016b → confirm from the smoke job's **step log** that the cell actually executed. ⛔ An unpullable pin is a **skip**, and a skip is green
...
- **Setup (Phase 1)**: No dependencies — start immediately. T001 → T002 (thresholds before the gate). T003 → T004 (the pin exposes the tension it resolves).
