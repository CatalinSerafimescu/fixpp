...
- [ ] T053a [US3] Implement FR-008d. (a) `run_interop_cell.py` renders every message's `intent_fields` from `conversation_script.yaml` into a flat per-run intent file (`step_id` TAB `originator` TAB `path` TAB `value`), refusing a value containing TAB, LF or SOH, and hands its path to the counterparty and to fixpp's cell; both counterparties parse it by hand and build each peer-originated message from it, replacing the hardcoded literals for every declared path. ⚠️ **The renderer and its flat-format reader land before T052**, whose fixpp cell builds the fixpp-originated messages from the same file. (b) A post-run check over **both** processes' `sent` records: every path the script declares for the record's step is present with the declared value, else the cell FAILS. Arms per `quickstart.md` § *Step 4* (FR-008d rows). ⚠️ Counterparty source change ⇒ inert in the matrix until publish #2, which is held until US3's counterparty work lands (user decision, 2026-09-11). ⚠️ **Id note**: `a`-suffixed on this bundle's own idiom (see T002a) — added 2026-09-11 implement
