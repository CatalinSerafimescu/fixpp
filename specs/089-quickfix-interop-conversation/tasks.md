...
- [X] T061 [US4] Run each cell under both arms and assert the accepted-message sets are **identical**, with the assertion stating so explicitly (FR-010 · SC-004). ✅ **Source pinned 2026-09-11: data-model §13's `disposition` record** — fixpp's cell writes one per inbound arrival it delivered or rejected, and §10's extractor joins it to the peer's `sent` records. ⚠️ It is **not** the witness `verdict`: that is field fidelity, and a message the validator accepted with one wrong field would read as rejected — a spurious divergence. Depends on T061a + T061b *(2026-09-11: CONV-C1-off/-on and CONV-C2-off/-on run live against QuickFIX-cpp (config `normal`), promoted into a scratch ledger/evidence root via `promote_and_record`; both pairs constructed by the real extractor report `verdict: identical` with `accepted_off == accepted_on == {B-02,B-04,B-06,B-07,B-09,B-11}` — see session report for the full pair records)*
