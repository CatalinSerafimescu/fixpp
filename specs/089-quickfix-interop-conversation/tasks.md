...
- [ ] T017 **[INHERITED]** Verify and land the QuickFIX-J startup assertion that `org.quickfixj.CharsetSupport.getCharset()` is **`ISO-8859-1`** *(amended 2026-09-11: the task named `getDefaultCharset()`, a hard-coded literal an assertion can never fail on; the shipped code asserts `getCharset()`)*, failing loudly otherwise — `value_b64` is only reconstructible because of it. Its forced-miss arm (`quickstart.md` § *Step 4*, the charset row) is **committed as a test** — a one-off demonstration does not discharge it
