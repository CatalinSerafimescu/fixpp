...
- [ ] T061 [US4] Run each cell under both arms and assert the accepted-message sets are **identical**, with the assertion stating so explicitly (FR-010 · SC-004). ⛔ **First pin WHERE the validator's per-message disposition is recorded in the run artifact** — data-model §10 requires `accepted_off`/`accepted_on`/`dispositions` to be *extracted from the two run artifacts*, and no record in data-model §12's closed vocabulary carries a disposition. ⚠️ It is **not** the witness `verdict`: that is field fidelity, and a message the validator accepted with one wrong field would read as rejected — a spurious divergence. Until this is pinned, T024's pair construction **fails closed** (added 2026-09-11)
