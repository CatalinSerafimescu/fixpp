...
- **FR-018**: In addition to forced-miss arms, **every guard** MUST carry at least one **spurious-hit**
  ...
  **what changes** when the mutation is applied, as a value.
  ...
  | Fidelity witness (typed tier, FR-003b) | Bypass the peer's **typed accessor**, leaving generic enumeration intact | **The `fix_type` column.** Every `typed_reads` entry carries `fix_type` — the field's FIX datatype **as the emitter resolved it from the dictionary loaded for that cell** — and the **script does NOT declare it**; the comparator resolves the expected value independently from its own copy of `FIX44.xml`. A generic enumeration walks the message's own field map and performs **no dictionary lookup at all**, so a bypass yields entries with `fix_type` absent or empty ⇒ **RED naming the path**. And a bypass cannot fabricate it from the script, because the script does not carry it. ⚠️ **A decimal-spelling observable was tried and REJECTED on verified evidence**: seeding `Price(44) = 100.1000` and discriminating raw `100.1000` from typed `100.1` requires the trailing zeros to reach the wire, and fixpp's decimal writer **strips trailing fractional zeros** (`src/core/decimal.cpp`, `decimal_traits<pod_decimal>::to_chars`, step *AC-S4: strip trailing fractional zeros*). Both paths would have yielded `100.1` and the arm would have been **inert while looking green** — the exact class this table exists to remove |
