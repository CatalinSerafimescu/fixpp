...
- **S-030**: `std::optional<std::uint32_t> advertised_max_message_size` in `SessionConfig`. Outbound: emit `383=<value>` in our Logon when set. Inbound: capture the peer's advertised 383 into session state (observability, FR-007); hard-enforce **inbound-only**, gated on the **established** FSM state (FR-005 "established session"; no new state flag) — at the top of `on_inbound_frame` (`session.cpp:1961`), if the session is established and `frame.size()` exceeds OUR advertised max, disconnect with a distinct "negotiated max message size exceeded" reason. Distinct from and never weaker than the absolute `max_frame_bytes` framer backstop (`framer.hpp:21`, `framer.cpp` — unchanged).
- **S-037**: `std::vector<supported_msg_type>` in `SessionConfig` (`struct supported_msg_type { msg_direction direction; std::string msg_type; }`, `enum class msg_direction { send, receive }` rendering to `'S'`/`'R'` — fail-closed by construction; default empty ⇒ no group). Outbound: when non-empty, emit `NoMsgTypes(384)=k` + k contiguous `372`/`385` member pairs (RefMsgType then MsgDirection — 372 is the group delimiter per `FIX44.xml:286-289`) in config order in `build_logon`, honoring the bounded stack-buffer / fail-closed discipline. Inbound parse is already tolerated (proven by `test_066_arena_fit_test.cpp`).
...
- Round 1 applied 2026-07-12: Codex P1=1 P2=5 P3=1; Opus post-judging P1=1 P2=5 P3=2 (all 7 Codex findings CONFIRMED; +1 NEW P3 wire-position/body-placement finding). Rewrite addresses root causes: (RC1) design authored against natural (385,372) tag order instead of the FIX44 dictionary delimiter order (372 first) → all S-037 wire-order references corrected; (RC2) "every inbound frame in every state" S-030 enforcement placement contradicted FR-005 "established session" → re-gated on the established FSM state (no new flag) across research/data-model/contract/plan; (RC3) backstop-vs-N contradiction → single layered min(N, max_frame_bytes) rule; (RC4) raw-char MsgDirection let an off-enum 385 reach the wire → typed `msg_direction` enum, fail-closed by construction (research D-D decision flipped); (RC5) 464 assumed an inheritable malformed-header validator that does not exist → explicit value check on the Logon posture path with empty≡absent⇒production documented; (RC6) missing constitution-mandated Normative References section → added. Reviews: research/reviews/codex_070-fix44-closeout_gate_a_review.md, research/reviews/opus_070-fix44-closeout_gate_a_adversarial_review.md.
- Round 2 applied 2026-07-12: Codex re-review P1=0 P2=1 P3=1; fixes = (P2) research.md D-F S-030 enforcement reconciled to established-state gate; (P3) Normative References split into coverage-index refs vs dictionary anchors. Review: research/reviews/codex_070-fix44-closeout_gate_a_2_review.md.
