...
- **Inner negotiated bound (NEW, opt-in):** at the **top of `on_inbound_frame`** (`session.cpp:1961`), when `cfg_.advertised_max_message_size` is set **and the session is established**, if `frame.size()` exceeds that value, disconnect with a distinct "negotiated max message size exceeded" reason. Boundary: exactly `N` accepted, `N+1` disconnects (SC-003).

**Rationale.** The framer runs first and caps the outer envelope; the negotiated check is a stricter-or-equal per-session inner bound applied to an already-framed message. The check reads `frame.size()` — the total on-wire length — matching FR-005's "total on-wire length". FR-006 is structurally satisfied: the negotiated check is *additional* code on a separate `std::optional` gate; it never touches `framer.cpp` and cannot weaken the backstop.

**Enforcement timing — gated on the ESTABLISHED state (corrects the earlier "every state" framing).** FR-005 scopes enforcement to an *established* session. Enforcing on *every* inbound frame in *every* state is not defensible: an acceptor's first inbound frame is the peer's Logon, arriving before either side has advertised its `383`; disconnecting that as a "negotiated" violation is premature. Gating on "our Logon carrying `383` has been sent" is likewise insufficient — an initiator sends its Logon first, then has a pre-establishment window before the acceptor's Logon arrives, re-introducing the same premature-enforcement class. The correct gate is the FSM **established/Active** state (the same state boundary C-4.1 references), which requires **no new session-state flag**. Any pre-establishment oversize frame is already covered by the framer backstop.

**Edge case (spec Edge Cases / S-030).** A configured `N` MAY exceed the framer backstop — no clamp of `N` to the backstop is applied. The two limits compose by **min-semantics**: effective inbound cap = `min(N, max_frame_bytes)`. A frame > 256 KiB never reaches `on_inbound_frame` (the framer disconnects first), so the negotiated check only ever *tightens*.
...
**Decision (REVISED — Gate A r1).** The advertised `385` value is produced from a **typed `enum class msg_direction { send, receive }`** on `supported_msg_type::direction`; the builder renders `send`→`'S'`, `receive`→`'R'`. An out-of-enum direction cannot be constructed, so an invalid `385` can never reach the wire (fail-closed by construction). This **reverses** the initial raw-`char` decision recorded below, which is retained for the audit trail. The **FIX 4.4 conformant enumerators** are `S` (SEND) and `R` (RECEIVE) (`FIX44.xml:4997-5000`); the enum matches the existing `session_posture` typed-enum pattern.
...
**Design risk (RESOLVED by the revised decision).** Under the earlier raw-`char` design an operator could advertise a non-conformant value (e.g. `'0'`) that still "passed" the builder and emitted an off-enum `385` — FR-008 requires a *well-formed* group, so this was a latent conformance defect (Codex Gate A #5). The typed enum removes the failure surface entirely: an out-of-enum value is unrepresentable, so no advertise-side runtime guard or new failure disposition is needed — the invalid case simply cannot be expressed. The enum→char render (`send`→`'S'`, `receive`→`'R'`) is a single branch on a stack value inside the builder — zero-alloc, no heap. Inbound parse of a peer's 384 group is already tolerated (`test_066_arena_fit_test.cpp`) and out of scope for validation here.

**Alternatives rejected.**
- Raw operator `char` written verbatim, no validation (the ORIGINAL decision): rejected on Gate A — lets a mis-configured operator emit an off-enum `385`, contradicting FR-008's well-formed-group requirement.
- Raw `char` + an advertise-side runtime guard rejecting non-`{S,R}`: rejected — adds a *new* failure disposition (build error) that the typed enum makes unnecessary; the enum is the same pattern already used for `session_posture`, so it adds no novel surface.
...
- **S-030 negotiated-size exceeded → Disconnect with a distinct reason.** Because the check fires **once the FSM is established/Active** (after our Logon carrying `383=N` has been advertised), before frame interpretation, and applies to app + admin frames alike **only after establishment** — a pre-establishment inbound frame is NOT size-checked against the negotiated limit (the absolute `max_frame_bytes` framer backstop still governs pre-establishment) — the disposition is a straight `record_state_transition_(fsm_state::Disconnected)` with the distinct "negotiated max message size exceeded" reason — mirroring the session-fatal Disconnected transitions already used across the inbound path (e.g. `session.cpp:2498-2499`, `2504-2505`). No Logout is emitted for S-030 (the peer violated a size contract mid-stream; the framer/transport-level disposition is a disconnect, consistent with `wire_frame_too_large` handling).
