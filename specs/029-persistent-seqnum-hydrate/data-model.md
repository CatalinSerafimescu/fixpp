...
| W8 | One-shot fires exactly once + happens-before | `ensure_hydrated_` mutates once across initiator AND acceptor; a reconnect does NOT re-hydrate (INV-H3); **hydrate completes-before the first `check_inbound` on both roles** (New-4). The happens-before MUST be asserted on **observable state**, not single-threaded call order — e.g. read `next_inbound_unsafe()` at the moment the first inbound Logon reaches the gate and assert it already equals the hydrated value (so a coincidental ordering in a 1-thread harness cannot pass it, [[feedback_single_threaded_harness_masks_strand_races]]) — invariant-count regression ([[feedback_half_restructure_symmetric_api]]) |
...
| W14 | Hydrate **read**-failure fatal, no partial seed (SC-005 / FR-006 / C2.3) | inject a `next_seqnum(dir,false)` read failure on the hydrate path → the session transitions to Disconnected (fatal), the manager is **NOT** partially seeded (mutated only after BOTH reads succeed), and `hydrated_` is left `false` so the next reconnect retries (D-9/INV-H6). **Split** (proving no partial seed): (a) the **first** read (`inbound,false`) fails → no mutation at all; (b) the **second** read (`outbound,false`) fails after the first succeeded → still no mutation (the manager is written only after both values are in hand). Distinct from W6 (which is the inbound **write**/persist failure). |
