...
Today that contract is honoured on only **16 of 26** engine-originated admin emit sites. Ten sites
...
This feature closes the gap exhaustively in one pass. It changes only *observation* — the same
frames go out on the wire, byte-for-byte, in the same order; the only behaviour change is that a
registered `Application` now sees them and a throwing callback now tears the session down.
...
A host's `toAdmin` callback throws (a bug, or a deliberate assertion). For the 16 already-wired
sites this is caught, surfaced as `app_callback_threw`, and the session is terminally closed. For
the ten unwired sites the throw escapes an unguarded path — inconsistent and potentially
process-terminating. After this feature, a throwing `toAdmin` on **any** administrative emit site is
handled identically: surfaced as `app_callback_threw` and the session terminally closed.
...
`app_callback_threw` and the session ends in a terminal/closed state (no escape, no swallow).

**Acceptance Scenarios**:

1. **Given** a `toAdmin` that throws, **When** an engine-originated `Reject(35=3)` would be emitted, **Then** the operation surfaces `app_callback_threw` and the session terminally closes.
2. **Given** a `toAdmin` that throws, **When** the initiator Logon-ack `Logout` would be emitted, **Then** the operation surfaces `app_callback_threw` and the session terminally closes.
...
- **toAdmin / toApp ordering**: the callback fires **before** the frame is stored/transmitted (consistent with the 16 existing sites), so an observer sees the frame before the peer does and a throw prevents transmission of that frame's session continuing.
...
- **FR-003**: A `toAdmin` callback that throws during any newly-wired administrative emit MUST be surfaced as `app_callback_threw` and MUST cause the session to terminally close — identical to the throw-handling of the 16 already-wired sites. `toAdmin` remains **inspect-only / not vetoable** for administrative frames (consistent with current behaviour).
...
- **SC-003**: A `toAdmin` (administrative) or `toApp` (`35=j`) callback that throws on any newly-wired site results in `app_callback_threw` and a terminally-closed session — 100% of the newly-wired sites match the existing throw-handling contract.
...
- **Observation is ordered before transmission.** Consistent with the 16 existing sites, the callback fires before the frame is stored/transmitted, so a throw prevents that frame's normal continuation and an observer sees the frame no later than the peer.
