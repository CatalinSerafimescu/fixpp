...
| CI-2 | Builds the **single** Linux `cp310-abi3` wheel via `cibuildwheel` inside `manylinux_2_28_x86_64`, with the gcc-toolset ≥13 + in-container Conan profile + `-DPy_LIMITED_API=0x030A0000` (compile-only) + the `wheel.py-api = "cp310"` abi3 tag + SWIG `>=4.2,<4.5` + `with_otel=False` + static OpenSSL (research D-2/D-3/D-7). The `<4.5` cap is a temporary #296 safety bound and remains until that issue is root-caused. |
