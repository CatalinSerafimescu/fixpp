...
- **SUPERSEDED 2026-07-04 (feature 060-int128-decimal-compare — the different-exponent path only).** The v0.1 rejection was of an *unguarded* scale-to-common-exponent, which overflows even 128-bit at the full exponent delta (≈189 bits) — correctly rejected. A **guarded exact wide-integer compare is sound** and is now the shipped different-exponent path (the sentinel/sign/same-exponent-hoist and the O(1)/`noexcept`/no-alloc/total-order contract are unchanged; the same-exponent regime still uses the direct signed-mantissa hoist). Bound proof: after the zero filter, `k = |ae − be|`; **`k ≥ 19` ⇒ dominance, no multiply** (`|m|·10^19 > INT64_MAX ≥ |other|`); **`k ≤ 18` ⇒ one 64×64→128 widening multiply**, product `≤ (2^63−1)·10^18 < 2^123`, compared via a two-limb `(hi,lo)`. Portability is preserved by a **per-compiler primitive at the multiply instruction only** — `unsigned __int128` (GCC/Clang/clang-cl), `_umul128` (MSVC x64), `__umulh`+`a*b` (MSVC ARM64), portable 32-bit-limb `#else` — i.e. one algorithm, **no MSVC-vs-Clang algorithm split**; so "no `__int128`" is no longer a hard constraint (MSVC uses its own intrinsic, never `__int128`). Result-identity to this frozen digit-string algorithm is a hard Tier-1 differential-oracle gate. See `.specify/2a-decimal.md §6.3` (amended) + `specs/060-int128-decimal-compare/`.
