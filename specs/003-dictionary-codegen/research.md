...
**Re-`/plan` 2026-05-15 — three Gate A round-1 root causes RESOLVED in-bundle** (this supersedes the "blocked, locked at re-`/plan`" framing the prior draft carried; the exhaustion→re-`/plan` path per `plan.md ## Gate A`): **RC#1** — `version_profile`/`resolved_message_version`/`resolve_application_version` + `field_traits`/`decode_field` are now 003-owned (D-21); **RC#2** — decimal route re-derived from corrected 2c v1.4 (D-22); **RC#3** — the dict↔wire bridge edge resolved via the `arch §2.4` v0.2→v0.3 carve-out amendment (D-23, superseding the D-12 open item). The affected decision bodies were reconciled into their post-replan reality at the replan loop round 1 (not merely appended-to): **D-7** retitled to "003-owned `resolve_application_version`" with its decision body rewritten; **D-2** layer-edge bullet rewritten to point at D-23 (its pre-amendment "confirmed clean" wording is struck); **D-10** preserved (slots LOCKED, already accurate); **D-12** carries its RESOLVED-superseded-by-D-23 note. Provenance is retained via the convergence-log entries / RESOLVED notes; only the live decision text is reconciled.
...
- **Layer-edge classification (superseded by D-23 — RC#3 RESOLVED at re-`/plan` 2026-05-15).** The vendored header lives under `include/fixpp/wire/` and is *consumed by generated dictionary headers AND the hand-written `dict/` bridge headers as a compile-time template argument*, not via a runtime `dictionary → wire` link edge. The earlier "confirmed at /plan as not a new runtime layer edge" framing was **incomplete**: Gate A round 1 (Codex P1-3 / Opus root cause #3) showed the pre-amendment `arch §2.4` carve-out covered only the *generated* `fixpp::vXX::*` subset and did **not** sanction the hand-written `dict/reify.hpp`/`field_traits.hpp` `#include <fixpp/wire/...>` nor the R6 stub written into `include/fixpp/wire/`. **Now resolved by the `arch §2.4` v0.2→v0.3 bridge-surface carve-out amendment — see D-23** (the full bridge surface compiles against both modules, is not a `dictionary` module link edge, and introduces no cycle; the literal `dictionary→wire` whitelist edge was explicitly rejected as cyclic). `[2c §1.1]` / `[arch §4.2]` confirm typed messages "reuse 2b's primitives" as a header contract; D-23 is the load-bearing closure, not this bullet's pre-amendment wording.
...
## D-7: FIXT cross-vocabulary resolution = **003-owned `resolve_application_version`** (locks AC-D3 / AC-D4)

- **Decision (post-replan reality):** `dict::reify` peeks `MsgType(35)`; on a FIXT-admin hit dispatches via the FIXT switch; on miss reads `ApplVerID(1128)` (mapping the 2b/wire field-absent error → empty `string_view`) and calls the **003-owned** free function `dict::resolve_application_version(profile, appl_ver_id_value)`. This feature does **not** re-derive the resolution *algorithm* — it is locked verbatim by `2c §4.3` (`2c §4.3:457-501`) — but it **does own the materialised free function + `version_profile`**, since 002 deferred both and 2c assigned no shipping owner (see D-21). The `[2c §6.3]` worked example (Logon→vt11, NOS ApplVerID=9→v50sp2, NOS ApplVerID=6→v44 override, OrderCancelRequest no ApplVerID→v50sp2 default, Heartbeat→vt11) is the AC-D4 oracle in `tests/integration/fixt_cross_vocabulary.cpp` (seam #10b).
- **RESOLVED at re-`/plan` 2026-05-15 (Gate A round 1, Codex P1-1 / Opus root cause #1 — was "consume 002's `resolve_application_version`", now closed in-bundle as 003-owned; see D-21):** `version_profile` and `dict::resolve_application_version` are **NOT 002-shipped** — 002 explicitly deferred both (`specs/002-dictionary-xml-loader/contracts/version_profile.hpp:59-66`; `contracts/dictionary.hpp:19-21`; on-disk `include/fixpp/dict/version_profile.hpp` carries the two enums only — verified). 2c §4.3 publishes `resolve_application_version` as a free function precisely so `dict::reify` (which holds no `Dictionary`) can run it — **003 is the natural owner**, now materialised: `contracts/version_profile.hpp` (additive edit to the 002 enums-only file), data-model Entity 10, ACs AC-VP1..AC-VP6, the wire `ApplVerID(1128)`→C++ enum-mapping table (`2c §4.3:486-501`), error slots locked (D-10). One-resolution-truth holds (003 owns the single algorithm); `dict_unresolved_application_version`/`dict_unknown_appl_ver_id` mapped per AC-D6/AC-D7. AC-D3/AC-D4 are **no longer blocked**.
