...
- **CORRECTION (Gate A round 1, Codex P1-1 / Opus root cause #1):** `version_profile` and `dict::resolve_application_version` are **NOT 002-shipped** — 002 explicitly deferred both (`specs/002-dictionary-xml-loader/contracts/version_profile.hpp:59-66`; `contracts/dictionary.hpp:19-21`). The earlier "002-shipped, spec §A6" framing was the phantom-API premise Opus RC#1 names. 2c §4.3 (`.specify/2c-codegen.md:449-473`) publishes `resolve_application_version` as a free function precisely so `dict::reify` (which holds no `Dictionary`) can run it — i.e. **003 is the natural owner**. Resolution: 003 owns and ships `version_profile` + `dict::resolve_application_version` + the wire `ApplVerID(1128)` → C++ `application_version` enum-mapping table (2c §4.3, `.specify/2c-codegen.md:478`) with their own contract/files/ACs/tests — a material surface expansion **locked at re-`/plan`**, not this convergence pass (spec §8 "Upstream dependency audit"; `blocked_on_replan` front-matter). One-resolution-truth still holds (003 owns the single algorithm); the `dict_unresolved_application_version` / `dict_unknown_appl_ver_id` error paths are mapped per AC-D6/AC-D7.
...
- **Decision (REVISED — Gate A round 1, Codex P1-3 / Opus root cause #3):** the prior "no new runtime layer edge" green-check **does not hold** and is downgraded to an **open layer-amendment item for re-`/plan`**. (a) `dictionary → core` from 002 — unchanged, clean. (b) The `arch §2.4` carve-out ("Generated typed messages `fixpp::vXX::*` … compile against both modules") covers the **generated** `Messages.hpp`/`Reify.hpp`/`_dispatch` tree's `#include <fixpp/wire/...>`. It does **NOT** cover the **hand-written** public header `include/fixpp/dict/reify.hpp` (`contracts/reify.hpp:8` `#include <fixpp/wire/message_view_contract.hpp>`): `reify.hpp` is not a generated `fixpp::vXX::*` header, it lives in `dict/` which the `arch §2.3` whitelist (`.specify/architecture.md:113-118`) pins to `core` only, and an `#include` is a `check_layers.py` edge regardless of compile-time-template-argument framing. (c) This feature also *writes a new header into the 2b-owned `include/fixpp/wire/` tree* (the R6 stub). (d) The `fixpp-codegen` host-tool `fixpp::dict` link is a host-side build edge (`[const §III.5]`) — that part is clean. **Resolution required (NOT assertable at Gate A):** either an explicit `[const §XX]` / `arch §2.3` whitelist amendment adding the `dictionary(reify bridge) → wire(contract header)` edge with `check_layers.py` taught the carve-out, or relocate the contract header out of `dict/`'s public include path — decided at re-`/plan` + a design-doc/arch amendment, not this convergence pass.
- **Verification / status:** CI's `linux-clang-debug` runs `tools/check_layers.py`; **as currently written the `dict/reify.hpp → wire/` edge + the new `wire/` header write would NOT be clean** under the unamended `arch §2.3` whitelist. Recorded as an open item in `plan.md` `## Gate A`; spec NFR-003-8 / R6 narrowed accordingly.
