...
- Acceptance criteria are grouped AC-G (typed messages), AC-V (Fields/Validator/NormativeReferences), AC-R (reify bridge), AC-D (runtime dispatch), AC-X (version_registry shape), AC-C (CMake targets), AC-T (determinism/threading), **AC-VP (version_profile/resolve_application_version — RC#1, §4.8), AC-FT (field_traits/decode_field — RC#1, §4.8)** — each one testable, each tied to a `[2c §...]` anchor and/or a `[2c §9]` test seam.
...
- **Gate A rounds 1+2 (2026-05-15):** exhausted → re-`/plan` path. Three root causes (RC#1 unshipped+unassigned `version_profile`/`resolve_application_version`/`field_traits` surface; RC#2 inherited 2c §4.1.3/§4.7 `decimal_t::from_chars` incoherent with merged 2a/001; RC#3 open `dict/`→`wire/` layer amendment) were documented-as-blocking.
- **Re-`/plan` applied 2026-05-15 — all three RESOLVED in-bundle:** RC#1 → `version_profile`/`resolve_application_version`/`field_traits`+`decode_field` are now 003-owned (`contracts/version_profile.hpp` additive edit + NET-NEW `contracts/field_traits.hpp`; new ACs **AC-VP1..AC-VP6 / AC-FT1..AC-FT3** spec §4.8; six `core::error` slots locked 23–28; ApplVerID wire→C++ map). RC#2 → consume corrected `2c-codegen.md` **v1.4** (`[const §XX]`, commit 41dd8c1; AC-G4/AC-G4a/NFR-003-4 re-derived). RC#3 → `arch §2.4` **v0.2→v0.3** dual-compile bridge-surface carve-out (`[const §XX]`; no module cycle — the literal `dictionary→wire` edge rejected as cyclic) + `check_layers.py` exemption (clean). `blocked_on_replan` cleared (front-matter `replan_applied`); `/tasks` gated only until the fresh Codex Gate A converges. Full record: `plan.md` `## Re-/plan (RC resolution)` + `## Gate A`.
