...
- **Invariants:**
  ...
  - **I-1a.** Codegen emits, per typed message, one `template<> struct owning_message_traits<fixpp::<vXX>::Msg> { using type = fixpp::<vXX>::owning_<Msg>; };` specialisation and `static_assert(std::is_same_v<dict::owning_message_t<Msg>, fixpp::<vXX>::owning_<Msg>>)` — the canonical 2c v1.4 §4.8 (`.specify/2c-codegen.md:1456-1464`) form of the `dict::owning_message_t<Msg>` resolvent (`typename owning_message_traits<Msg>::type`; Entity 6). **Inherited 2c v1.4 text**, asserted by the codegen-shape golden as a compile-time shape-oracle check (AC-G7a; seam #18; Root cause #1, Option A).
...
- **Shape:** move-only type-erased handle. `version()` → `resolved_message_version`; `msg_type()`/`view()`/`field_value(tag)` per `[2c §4.8]`; `as<Msg>()` → borrowed `owning_message_t<Msg> const*` (the canonical 2c v1.4 §4.8 alias `typename owning_message_traits<Msg>::type`; equivalently 2c's prose owner name `owning_<Msg>`, which the trait resolves to the same type — AC-G7a/AC-R6) or `nullptr` on version/MsgType mismatch (AC-R6). No UB / no throw on mismatch.
- **Allocation:** the type-erased payload may add ≤ 1 allocation if heap-backed (small-buffer-optimised variant elides it below a published size threshold) — implementation-defined per `[2c §1.2]` / `[2c §4.8]` (L1197–1202), N-P2-4.
...
- **`owning_message_t<Msg>` definition (Gate A round 1 Opus N-P2-1; Root cause #1 Option A at replan loop round 2 — corrects the round-1 false 2c-attribution):** `template<class Msg> using owning_message_t = typename owning_message_traits<Msg>::type;` pinned in `contracts/reify.hpp` (with the `owning_message_traits` primary template), so `owning_message_t<v50sp2::NewOrderSingle> ≡ v50sp2::owning_NewOrderSingle`. **This is the canonical 2c v1.4 §4.8 form — inherited 2c v1.4 text:** 2c v1.4 §4.8 (`.specify/2c-codegen.md:1456-1464`) defines `owning_message_t<Msg>` as exactly `typename owning_message_traits<Msg>::type` via a per-message external-trait specialisation (`owning_message_traits<Msg>`) emitted alongside each `owning_<Msg>` class; it is **not** 003-derived/2c-underspecified and **not** a `Msg::owning_type` member alias (the round-1 attribution was false against the signed-off design doc and is corrected here). Codegen emits, per typed message, one `template<> struct owning_message_traits<Msg> { using type = fixpp::<vXX>::owning_<Msg>; };` specialisation (AC-G7a), pinned in **both** `contracts/generated_message.hpp` (the codegen shape oracle — forward-declares `owning_<Msg>`, pins the `owning_message_traits<NewOrderSingle>` specialisation, and `static_assert`s `owning_message_t<NewOrderSingle> ≡ owning_NewOrderSingle`) **and** `contracts/reify.hpp` (the primary template + the alias + the `reify_as` / `as<Msg>()` return type). The codegen-shape golden (seam #18, `flyweight_shape_test.cpp`) asserts the specialisation is present and correctly bound; a template that omits it is caught as a compile-time shape-oracle failure. 2c's prose name `owning_<Msg>` is the per-message concrete owner the trait resolves to — under the trait, `owning_<Msg>` and `owning_message_t<Msg>` denote the same type. This is the name-mangling/ADL surface every downstream consumer binds to — contract-pinned in the shape oracle, not deferred (AC-R1 / AC-G7a; `[2c §4.8]` L1456–1464, inherited).
