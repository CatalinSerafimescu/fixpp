...
Entities are the *generated artifacts* and the *runtime bridge values*. The `FieldRef`/`ComponentRef`/`GroupRef`/`Dictionary` types were shipped by 002 and are consumed here verbatim. **Correction (Gate A round 1, Codex P1-1 / Opus root cause #1):** `version_profile`, `dict::resolve_application_version`, and `<fixpp/dict/field_traits.hpp>` (`field_traits<T>`/`decode_field<T>`) were **NOT** shipped by 002 (002 deferred/never-shipped all three — spec §8 "Upstream dependency audit"); they are **003-owned blocking surface** added at re-`/plan` (new contract headers/ACs/tests; the wire→C++ `ApplVerID` enum-mapping table per 2c §4.3). Every shape is inherited from `[2c §4.7]`/`[2c §4.8]`/`[2c §4.9]`; this document records the invariants, the error mapping, and the PMR allocation accounting `/tasks` and Gate A consume.
...
- **Shape:** flyweight holding exactly one `wire::MessageView<wire::access_mode::Index> const&`. Static members `msg_type_v` (`constexpr std::string_view`), `version_v` (`constexpr application_version`). One `explicit ... noexcept` view-binding ctor (no validation). Per-field `[[nodiscard]] inline ... noexcept` accessor returning `expected_t<T>` (string/int/char via `dict::field_traits<T>`, `[2c §4.1.3]`; decimal via the `[2c §4.7]` decimal accessor body — **not** a `field_traits` specialisation: `[2c §4.1.3]` (`.specify/2c-codegen.md:268-271`) explicitly excludes the `decimal_t` case from `field_traits`, so the decimal route is cited to §4.7 only — **see ⚠ below**); view-returning accessors carry `[[clang::lifetimebound]]`.

  > **⚠ Inherited 2c defect (Gate A round 1, Opus root cause #2 — un-fixable by a bundle edit).** 2c v1.3 specifies the decimal route as `decimal_t::from_chars(fv->bytes())`, a symbol absent on merged 001/2a (only PMR-mandatory `decimal_traits<T>::from_chars(span,mr)` / `decimal<T>::parse(span,mr)` exist; 2a's Gate A removed the no-PMR form). Recorded, not papered over: resolution requires reopening 2c §4.1.3/§4.7; AC-G4/AC-G4a, NFR-003-4, and the PMR accounting below are blocked on that. The flyweight holds no `memory_resource*` (I-1 `sizeof == one pointer`), so a PMR-taking decimal accessor has no arena on the borrowed read path (Opus N-P1-1) — see PMR accounting note. Repeating groups → `wire::group_view<Leg>`-returning accessor; nested `Leg` flyweight with the same discipline. `field_value(uint16_t)` forwarder on the message and every nested group struct. `view()` bridge accessor.
- **Invariants:**
...
- **Dependency note (Codex P1-1 / Opus RC#1):** `version_profile` (the `reify` resolution input) is 003-owned blocking surface, NOT 002-shipped — see intro + spec §8.
...
- **Typed-accessor read path:** **0 allocations** — every accessor delegates to `wire::MessageView::get<Tag>()`, allocation-free Index mode (NFR-003-4; AC-T3; verified under `mallocnesia`, Linux). **⚠ Conditional / blocked (Gate A round 1, Opus N-P1-1, downstream of RC#2):** this holds for string/int/char unconditionally, but the **decimal** accessor's true contract is PMR-mandatory (`decimal_traits<T>::from_chars(span, mr)` — see Entity 1 ⚠). The borrowed flyweight holds no `memory_resource*` (I-1 `sizeof == one pointer`), so "0 allocations on the decimal read path" holds **only for a non-allocating `FIXPP_DECIMAL_T`** (e.g. `pod_decimal`); an allocating `FIXPP_DECIMAL_T` (`cpp_dec_float`, a supported substitution) would allocate per `price()` with **no arena in scope** and must use the arena-backed `owning_<Msg>` path (Entity 4). NFR-003-4's decimal arm + AC-G4a are blocked on the 2c §4.1.3/§4.7 reopen (RC#2).
