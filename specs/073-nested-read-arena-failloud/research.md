...
Formula (both exits): `alloc_failed = (table == nullptr) || (table->build_status().error() == out_of_memory) || table->group_slices_status(nested_no_tag).alloc_failed`, guarded by `slice_data != nullptr` (an absent group short-circuits to `false` at `:722-724` before either exit).

**Three origins, not two** (the third, mode (c), was found at `/speckit-implement` — the same `feedback_status_origin_must_cover_all_alloc_catch_sites` census gap that Gate-A round 1 fixed for mode (b), recurring one level deeper; not a design pivot, a more complete `bad_alloc`-catch census. See the Gate-B handoff note):
- **(a) sub-table build shell alloc failed** — `build_nested_subview` catches `std::bad_alloc` on `mr->allocate(sizeof(OffsetTable))` and returns `nullptr` (`offset_table.cpp:709-711`), so `table == nullptr` after a non-null `slice_data`.
- **(b) sub-table slice-materialization failed** — the sub-table **builds non-null**, but `OffsetTable::group_slices()` then exhausts the same null-upstream arena on its own `group_slices_.reserve()` / `push_back()` and hits its **own** `catch (std::bad_alloc) { return {}; }` (`offset_table.cpp:611-677`, the catch at `:674-675`), returning an empty span with `table != nullptr`. The original null-pointer-only predicate scored this `alloc_failed = false` → silent truncation survived on a present group.
- **(c) sub-table CONSTRUCTOR `build()` degraded on bad_alloc** — the shell alloc succeeds (cap ≥ `sizeof(OffsetTable)`), so `build_nested_subview` returns a **non-null** table, but the ctor's own `build()` then exhausts the arena and hits its `catch (std::bad_alloc)` (`offset_table.cpp:366-370`), which — because a `noexcept` ctor must not let `bad_alloc` escape — clears the table and sets `status_ = out_of_memory` rather than throwing or nulling. That degraded table's `group()` returns `fail(out_of_memory)` (`:507-508`), so `group_slices_status()` reports `{empty, false}` **with no throw at its own call site** — invisible to both mode (a) (non-null) and mode (b) (no throw). Surfaced by testing `table->build_status().error() == out_of_memory` explicitly at both exits. Scoped to `out_of_memory` only: a table degraded for a malformed-data reason (`err_offset_table_full` `:323`, `err_invalid_field_format` `:232`) is NOT arena exhaustion and MUST NOT fail-loud (preserves FR-007 disjointness).

**Repeated-read fail-loud, per mode**: mode (a) serves the cached null row; mode (b) re-materializes from the cached non-null row and re-throws (nothing cached in the sub-table's `group_index_` on the first throw). Mode (c) is different and more robust: the degraded table pushed a cached `{no_tag, start, count=0}` into `group_index_` on read 1, so read 2's `group_slices_status()` returns that cached count-0 span **without** re-throwing — read-2 fail-loud rides the persistent `build_status() == out_of_memory` check, not a re-throw.
...
**Disjointness (spec FR-007)**: `slice_data == nullptr` (absent, `:722-724`) → `alloc_failed = false`; a **non-null, `build_status()`-ok** table whose `group_slices_status()` returns a genuinely count-0 span **with no exception** (legit count-0 / zero-len slice) → `alloc_failed = false`. Only a null build (a), a caught `bad_alloc` in the sub-table's slice materialization (b), OR a sub-table whose ctor `build()` degraded specifically to `out_of_memory` (c) is a failure. A table degraded for a non-alloc (malformed-data) reason is NOT a failure.
...
5. **Second-loss** (D2 mode (b) discriminator): a cap tuned so the sub-`OffsetTable` **builds** (non-null) but its `group_slices()` `reserve`/`push_back` then exhausts the arena and hits the `:674` `catch (bad_alloc)` → assert fail-loud on **both** paths, **read twice** (read 1 = final exit; the non-null table is cached, so read 2 = cache-hit exit re-materializes → re-throws). A single read pins only the final exit and lets a fixer who forgets the cache-hit exit survive.
6. **Ctor-degraded** (D2 mode (c) discriminator, found at implement): a cap where `build_nested_subview`'s shell alloc **succeeds** (non-null table) but the ctor's own `build()` then exhausts the arena → `status_ = out_of_memory` (`:366-370`). `group_slices_status()` on that degraded table returns `{empty, false}` with **no throw** — invisible to modes (a)/(b). Read twice → both fail-loud via the persistent `build_status()==out_of_memory` check (read 2 rides the cached count-0 row, no re-throw). Mutation-proven RED against the 2-mode (a+b) formula, GREEN after adding the `build_status()==out_of_memory` OR term.

**Platform-robust mode pinning (implement-time hardening)**: `sizeof(OffsetTable)` (≈280 B on clang-debug) shifts under gcc-release / MSVC, so the cap band that selects mode (a) vs (b) vs (c) is **not** portable. The wire-level witness (T001, which holds the `OffsetTable` directly) therefore pins each mode by **introspection** — `table==nullptr` (a) / non-null+`build_status()`-ok+`group_slices_status().alloc_failed` (b) / non-null+`build_status()==out_of_memory` (c) — so a cap that lands in a different mode on another tier fails the assert LOUD rather than false-passing. The consumer witnesses (T006 C-ABI, T008 typed) cannot introspect the sub-table, so they use **wide-margin fixtures** (a large nested group whose nested alloc fails by a comfortable margin on every tier) and assert only the mode-agnostic contract (fail-loud + repeated-read + controls) ([[feedback_local_verify_clang_only_misses_gcc_release_ci_job]]).
