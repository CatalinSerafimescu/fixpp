...
| `BehindSide_KnobOn_AdmitsPeerResend_NoFatalDisconnect_Acceptor` + `_Initiator` | knob on + peer Logon MsgSeqNum too-high ⇒ NOT fatal at `check_inbound`; `next_inbound_` left at X (no `set_next_inbound`); admits the peer's `[X, peer_N-1]` resend in-sequence; **final `next_inbound_ == peer_N`** (NOT `X_logon+1` — peer_N set ≥1 frame beyond the Logon so the two diverge, I-NEX-5/I-NEX-12); no at-logon ResendRequest, both roles (one case per role) |
...
| `Suppression_KnobOn_NoAtLogonResendRequest_KnobOff_FatalOnTooHigh` | knob-on at-logon too-high tolerated, no ResendRequest; knob-off ⇒ existing fatal-on-too-high (013 regression guard) (I-NEX-5) |
