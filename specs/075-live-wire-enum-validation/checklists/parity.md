...
- [x] CHK006 - Is the corpus specified to cover **every fixpp Step-1 surface** (top-level, header, trailer, group member, nested group member, multi-value, degenerate whitespace, empty × each type arm, strict prefix), rather than only the surfaces the original theory said mattered? [Coverage, Spec §FR-018] — PASS: FR-018's corpus table enumerates exactly these surfaces. **UPDATED at T006 — now 13 rows**: row 6's header case re-based `PossDupFlag(43)` -> `MessageEncoding(347)` (a `STRING` header enum field, so both engines reach the enum arm), and `43=X` retained as `asserted: false` row 13 under the new **DV-5**.
...
- [x] CHK028 - Does **each** register row (**DV-1..DV-5**) carry all three of: an **argued disposition**, a **B-row**, and a **corpus row that measures it**? [Completeness, Spec §SC-009] — PASS: `contracts/enum-domain.md` C-6 table has non-empty Disposition, Corpus row and B-row columns for all **five** rows. **RE-AUDITED at T006**: **DV-5** is new (derived from measured golden output) and carries its argued disposition, corpus row **13**, and a B-row (T037 item (c2)). **DV-1's text was CORRECTED** there — QuickFIX rejects empty x `Char` with **6**, not 5. This checklist item is the reason DV-5 got a corpus row at all rather than living as prose.
