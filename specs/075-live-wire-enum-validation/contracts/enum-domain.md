...
| Field value is **empty**, field is typed **`Char`** (e.g. `Side(54)`) | **reject** — *not* via the enum check (it is skipped) but via `check_field_type`'s `size() != 1` arm | **5** | the tag — **DV-1** |
...
|---|---|---|---|---|---|---|
| **DV-1** | **empty × `Char`** (`54=`) | reject **5** — via `check_field_type`'s `size() != 1` arm (`validator.hpp:411-417`) | reject **5** — via `isFieldValue`'s `set.find("")` miss | **Parity by COINCIDENCE, via a different arm.** Documented explicitly so it is not *relied on*: the two engines agree here for unrelated reasons, and a change to either arm would silently break the agreement. `asserted: false` — we do not assert a coincidence. (FR-008) | 8 | ✅ |
