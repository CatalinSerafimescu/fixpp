...
| O-4 | Does the golden reveal a divergence **not** in the DV-1..DV-4 register? | **Phase-0.5 golden task.** By construction (FR-018/SC-009) such a divergence is a **defect** and blocks the feature until it is fixed or promoted to an argued register row. This row exists to make that outcome *expected and handled* rather than a surprise — the register is the golden's **output**, not the spec's input. |
