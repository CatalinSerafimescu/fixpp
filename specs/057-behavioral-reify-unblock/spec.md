...
- **Absent MsgType field (tag 35 not present)**: a frame with no tag 35 (a defensive/near-unreachable path — a validated frame always carries tag 35) returns `dict_reify_unknown_msg_type`. This is a **deliberate placeholder-retirement remap**: today this branch returns the now-retired `dict_reify_wire_body_not_ready`; post-057 it returns `dict_reify_unknown_msg_type` (an absent MsgType cannot name a known arm). Witnessed by an empty-/absent-35 view → `dict_reify_unknown_msg_type` (a witness distinct from the "empty-view succeeds" non-discriminating-green trap).
...
- **Allocation failure during deep copy**: the construction factory's deep-copy OOM surfaces as `dict_reify_oom`, propagated unchanged through the dispatch arm.
...
- **FR-007**: The build MUST provide a mechanism by which shipped `dict::reify()` (in `src/`) reaches the generated dispatch switches **without** any shipped `src/` source file `#include`-ing a build-tree-generated header (preserving arch §2.4 / NFR-003-8). The generated-header inclusion MUST be confined to a **build-tree-generated** translation unit (materialized into `${build}/_codegen/` at configure time, not a shipped `src/` file), so NFR-003-8 is satisfied literally without extending the arch §2.4 bridge carve-out.
- **FR-008**: The codegen emitter MUST emit runtime-dispatch arms that perform ONLY the known-MsgType membership test and, on a match, return an `owning_message_handle` via the pinned construction factory `detail::owning_message_handle_from_frame` (contract C-2), propagating that factory's OOM error (`dict_reify_oom`), rather than emitting the `dict_reify_wire_body_not_ready` placeholder. The arms no longer reference `owning_<Msg>` / `from_view()`; `from_view()` is reserved for `reify_as<Msg>` (D-5) and the decode-primitive context. Regenerated output MUST be byte-deterministic run-to-run (preserving B-003-3).
- **FR-009**: Error behavior for non-reifiable inputs MUST be preserved exactly: unknown MsgType (single- or multi-char, with no generated arm) → `dict_reify_unknown_msg_type`; unresolved application version → `dict_unresolved_application_version` / `dict_unknown_appl_ver_id`; allocation failure → `dict_reify_oom`. **One deliberate remap (not a "preserved exactly" case):** the **absent-MsgType** (tag 35 not present) defensive branch is retired from the placeholder `dict_reify_wire_body_not_ready` to `dict_reify_unknown_msg_type` (see Edge Cases); this is owned explicitly here because FR-009's "preserved exactly" covers only the enumerated non-reifiable inputs, not the absent-35 path.
...
- **FR-011**: Limitation **L-003-1** MUST be flipped to a **partial** unblock in `spec/behaviors-and-limitations.md`: runtime `dict::reify()` + `dict::reify_as<Msg>()` are shipped (they return live handles / concrete typed owners), but the `owning_message_handle::as<Msg>()` typed-downcast half remains **AC-R6-deferred (003 spec) / T059-stubbed**. The note MUST cite AC-R6 as the still-live deferred contract — not claim a bare "shipped." The 003 roadmap reference (spec §11 R6) and the catalogue status for the affected 003 row MUST be updated to reflect this partial unblock.
- **FR-012**: This feature MUST NOT add any new wire format, error code, public message-builder surface, C-ABI symbol, or dependency. (It wires existing generated code and existing decode primitives; the only new artifacts are: a **private** same-module declaring header `src/dictionary/reify_dispatch_bridge.hpp` — not a shipped/public `include/` header; a repo-checked-in CMake template `cmake/templates/reify_dispatch_bridge.cpp.in`; the build-tree-generated bridge translation unit it produces; and the private `fixpp_dict_dispatch_bridge` build target.)
- **FR-013**: This feature MUST NOT expand into the downstream per-message scope: it delivers the reify **mechanism** validated over a representative set of messages, not per-message typed builders nor full per-message read-test coverage of every A/M/P/N row nor all-version coverage (these per-message deliverables are tracked by downstream/other-feature requirements, not defined in this spec).
...
- **Dispatch bridge (new, internal)**: A **build-tree-generated** translation unit (materialized from a repo-checked-in CMake template into `${build}/_codegen/`) — the only TU that includes the `_dispatch/reify_dispatch_{fixt,application}.hpp` headers — plus a **private same-module declaring header** (`src/dictionary/reify_dispatch_bridge.hpp`, not a shipped `include/` header) that exposes an entry `dict::reify()` can call from shipped source. The subject of FR-007.
...
- **SC-005**: Regenerated codegen output is byte-identical run-to-run, and no shipped `src/` source file `#include`s a build-tree-generated header (arch §2.4 / NFR-003-8 preserved literally, WITHOUT extending the arch §2.4 carve-out — the sole build-tree includer is the build-tree-generated bridge TU) — verifiable by the `check_layers.py` gate, which still bites if `reify.cpp` includes a build-tree header.
- **SC-006**: Zero new wire/error/public-builder/C-ABI/dependency surface is introduced (FR-012), confirmed by diff review (the new private declaring header, CMake template, build-tree-generated bridge TU, and private `fixpp_dict_dispatch_bridge` target are internal build artifacts — none of them a wire/error/public-builder/C-ABI/dependency surface).
...
- **The 2b-unblock design is the dispatch-bridge approach** sketched in the `reify.cpp` R6 comments: a generated-aware TU that includes the `_dispatch/` headers, plus a declaring header to which `dict::reify()` delegates. The `/speckit-plan` decision (D-1) resolves this to a **build-tree-generated** bridge TU + a **private** same-module declaring header (`src/dictionary/`), so no shipped `src/` file includes a build-tree header and the arch §2.4 carve-out is not extended.
...
Per `[const §VI.5]`, the coverage-index / inherited-design entries that inform this spec. This is a build-architecture **mechanism** feature, so the references are largely `[impl]`-backed (no new FIX-spec normative section is introduced):

- `[arch §2.4]` (v0.3) / **NFR-003-8** — no shipped `src/**` translation unit may `#include` a build-tree-generated `_dispatch/`/`vXX` header. This layer-hygiene rule is the actual blocker the feature works around (FR-007, SC-005); enforced by `tools/check_layers.py`.
- `[003 spec §11 R6]` / **AC-R6** — the 003-deferred "2b reify unblock" roadmap item and its still-live deferred acceptance contract. The `owning_message_handle::as<Msg>()` typed-downcast half stays AC-R6-deferred / T059-stubbed; this feature lifts the runtime `dict::reify()` / `dict::reify_as<Msg>` half. Subject of FR-011.
- `[L-003-1]` — the limitation ("behavioral reify returns `dict_reify_wire_body_not_ready`") flipped to a **PARTIAL** unblock here (FR-011). `[L-003-2]` — runtime-XML-only versions fall through to `dict_reify_unknown_msg_type` (preserved, FR-009).
- `[impl]` 003 typed-message machinery — the `owning_<Msg>` flyweights, `owning_<Msg>::from_view` / `::view()` (Framer rebuild), `dict::reify` / `dict::reify_as<Msg>`, `resolved_message_version`, and the `fixpp::dict::dispatch` generated `_dispatch/reify_dispatch_{application,fixt}.hpp` switches — all consumed as-is (Assumptions).
- `[const §I.1]` — codegen-vs-runtime split; `dict::reify` runtime dispatch is generated for v42/v44/v50sp2/vt11. This feature makes that generated dispatch live.
- `[2c-codegen]` seam #15b — generated-arm coverage disposition (representative subset graded + exhaustive smoke nightly); the basis for excluding the inline `_dispatch/*.hpp` arms from the ≥95/85 coverage gate (plan Article IX).
- `[FIX50SP2] ApplVerID(1128)` — the per-frame application-version discriminator; `dict::resolve_application_version` derives the resolved application version from an explicit `1128` over a profile's `Unknown` default (US1 Acceptance Scenario 4, FR-003). `[B-003-5]` — an unresolvable application version returns the distinct `dict_unresolved_application_version` / `dict_unknown_appl_ver_id`, preserved (FR-009).
- `[B-003-3]` — regenerated codegen output is byte-deterministic run-to-run (FR-008, SC-005); `determinism_test` + build-graph-clean gates.
