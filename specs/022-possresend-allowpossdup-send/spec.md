...
**2026-06-14 (037-resend-reply-possdup-tags):** `build_replay_frame` under `allow_pos_dup=true` now suppresses stored `PossDupFlag(43)` and `OrigSendingTime(122)` before re-appending them, eliminating duplicate-tag emission when the stored frame already carried those tags. Refines the INV-5 structural-independence note: the auto-resend path's `43`/`122` append is now dedup-guarded so the replayed frame carries each tag exactly once. Default-path (`allow_pos_dup=false`) replay is byte-identical (FR-006/SC-003). See 037 FR-004/FR-005 and `spec/behaviors-and-limitations.md` B-037-2.
