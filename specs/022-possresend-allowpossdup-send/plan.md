...
| **VIII.5** Allocator | the scanner + excision copies surviving fields into ONE bounded named strip stack scratch (no heap; distinct from the framer's `body_buf`, which is built later from the stripped payload); binding INV-4 gate = mallocnesia LD_PRELOAD (`/speckit-verify` Step 6) over a **new `send_impl`-strip no-heap cell in `test_send_allow_pos_dup_strip.cpp`** — NOT `test_session_alloc_guard`, which covers only the 005-era seqnum + admin-build paths and never exercises `send_impl` | ✅ by design |
