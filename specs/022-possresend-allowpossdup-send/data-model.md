...
// send_impl). Additive default-valued public-header field ⇒ no ABI/behavior break.
...
A pure function of (`app_payload`, `allow_pos_dup`) producing the bytes handed to the framer. Inserted **after** the 020 opaque-payload validation, **before** SendingTime stamp + frame build. Operates on the validated, SOH-delimited payload using the existing boundary-anchored matcher.

**Preconditions (inherited from the 020 validation that ran first):**
- `app_payload` is non-empty and begins with `35=` (MsgType is field 0).
- `app_payload` is a sequence of well-formed `<tag>=<value>\x01` fields (else already rejected `app_payload_malformed=131`).

**Transform:**

| `allow_pos_dup` | Field present | Result |
|-----------------|---------------|--------|
| `false` (default) | `43=…\x01` and/or `122=…\x01` at a field boundary | excise the **complete** field(s); copy surviving fields into the send stack scratch in original order |
| `false` | neither present | no-op (payload passed through unchanged) |
| `false` | `43=`/`122=` substring **inside** another field's value (not at a boundary) | **not** matched, **not** excised (injection-safe — D2) |
| `true` | any | passthrough verbatim (no excision) |

**Invariants:**
- INV-1: `35=` (field 0) is never touched (43/122 are never field 0).
- INV-2: Only complete boundary-anchored `43=…\x01` / `122=…\x01` fields are removed — never a partial span, never a substring inside a value ([[feedback_delimiter_injection_verbatim_field_copy]]).
...
- INV-4: No heap allocation (excision uses the existing 4096-byte send stack scratch — D6).
...
- `has_boundary_token` (`session.cpp:2917`) — the boundary-anchored token primitive (read; the strip uses the same boundary rule).
- 020 opaque-payload validation (`send_impl`, `session.cpp:2900-2926`) — runs before the strip; owns fail-closed (slot 131).
