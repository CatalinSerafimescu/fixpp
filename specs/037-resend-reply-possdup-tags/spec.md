...
- **Emitting `43=Y` REQUIRES emitting `122`** (clarify reference sweep, QFJ `Session.java:2580-2592`): a strict peer with `RequiresOrigSendingTime=Y` rejects a `43=Y` frame whose `122` is absent (`BAD_TIME_REJ_REASON`), and when `122` is present it must be `≤ 52` — `122 == 52` satisfies that boundary. This is why FR-002 makes `122` mandatory alongside FR-001's `43`, not optional.
