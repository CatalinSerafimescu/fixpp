...
3. **The fixpp-emits-GapFill witness is `tests/interop/happy/hp_fix44_recovery_outbound_answer_test.cpp`** (`49=FIXPP_INIT` on the `35=4` reply, via 013's `build_sequence_reset_gapfill` path), golden-compared. It currently compares the GapFill under the **`{52,10}` admin profile** (`golden_diff.hpp:54` `admin_profile_excluded_tags()`). Because the new `122` **equals `52` (a live wall-clock timestamp)**, comparing it verbatim would make the golden non-deterministic across runs. **The required change is already supported by existing infrastructure**: `golden_diff.hpp:60-76` already defines a `{52,10,122}` profile (composed for the replay path, which already carries `122`). The GapFill comparison must switch from `{52,10}` to `{52,10,122}`, and the `hp_fix44_recovery_outbound_answer` golden(s) re-baked. `43=Y` is deterministic (always `Y`) and is correctly compared verbatim. The synthetic gate-bite cells (`hp_fix44_recovery_outbound_answer_test.cpp:95-107`, which mutate `123`) stay valid under the extended profile.

**Net**: SC-004's live/in-process GapFill witness already exists; the only new verification work is (a) the `{52,10}`→`{52,10,122}` profile switch on the GapFill comparison + golden re-bake, and (b) the new unit cells (data-model INV-1..4) + the live QFcpp/QFJ recovery re-run. No undiscovered test inversion.
