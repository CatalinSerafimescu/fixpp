...
| **VII §7 — Fuzz (parser-touching modules)** | `build_replay_frame` byte-scan copy loop | **N/A.** The replay edit is a skip-set widening (`{9,10}`→`{9,10,43,122}`) in an EXISTING frame copy loop — no new parse/decode path is introduced, and the input is fixpp's OWN previously-stored outbound frame (not adversarial peer input). The fuzz-relevant inbound wire parser lives in `wire/` with its pre-existing harness. The GapFill builder (`admin_messages.cpp`) is a serializer, not a parser. No new fuzz harness required. |
