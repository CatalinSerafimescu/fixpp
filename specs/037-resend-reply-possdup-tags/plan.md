...
- **DEFECT 1 (default path) — `build_sequence_reset_gapfill` (`admin_messages.cpp:905`) omits `PossDupFlag(43)=Y` and `OrigSendingTime(122)`.** A GapFill's `MsgSeqNum(34)` sits at/below the peer's expected number — exactly the too-low position where `43=Y` suppresses a too-low-seqnum kill; and a strict `RequiresOrigSendingTime` peer rejects a `43=Y` frame missing `122`. Fix: append `43=Y` and `122 = <the GapFill's own 52>` (the `sending_time` parameter the builder already receives — no new parameter). Both reference engines (QFcpp `Session::generateSequenceReset`, QFJ `Session.generateSequenceReset`) do exactly this.
- **DEFECT 2 (non-default path) — `build_replay_frame` (`session.cpp:1625`) unconditionally appends `43`/`122`**, so a stored frame that already carries them (the `allow_pos_dup=true` retain case) replays with **duplicate** `43`/`122` tags → strict-peer reject. Fix: skip stored tags `43`/`122` in the copy loop (alongside the existing `9`/`10` skip), **keeping the `52 → orig_sending_time` capture before the skip** so the engine's appended `122` still uses the stored `52`.

Both builders are `noexcept`, stack-only (`fixpp::wire::Writer` over `std::pmr::null_memory_resource()`), and emit through the same resend reply. This is a **pure wire-output correction**: no new public signature, error slot, config field, codegen, or C-ABI surface (FR-007). It amends the at-rest disposition of S-013 (013 FR-010 resend re-serialization); it is **not** C-103 chunked-resend (deferred).
...
- **DEFECT 1 changes the DEFAULT-path bytes** — every GapFill on every resend reply gains two tags. Verification MUST re-bake any banked golden containing a fixpp-emitted GapFill and re-run the live QFcpp/QFJ resend / received-reset cells (both roles) to confirm peers *accept* the now-marked GapFill (peers tolerate *absence* today; that is not evidence they accept *presence*).
...
**Testing**: GoogleTest unit cells parsing the emitted frame fields (count `43`/`122` occurrences, assert `122 == 52`); golden re-bake for the default-path GapFill; live QFcpp/QFJ resend cells re-run (both roles); ASan/UBSan over the session + admin-message suites as regression
...
|---|---|---|
| **VI — Spec Coverage Discipline (100% FIX Rule)** | Wire fields `43`/`122` on `35=4` | `PossDupFlag(43)` and `OrigSendingTime(122)` are existing catalogued header fields; `SequenceReset(35=4)` already exists. No new message/field — this corrects which fields an existing frame carries. Catalogue gains a traceability row; B&L gains a conformance row. PASS. |
...
└── session.cpp          # DEFECT 2 — build_replay_frame (:1625):
                         ...
                         #   the 52→orig_sending_time capture (:1653-1656) stays BEFORE the skip,
                         #   so the engine's appended 122 (:1666-1671) still uses the stored 52.
...
phase-9-harness/golden/  # RE-BAKE any banked golden where fixpp EMITS a GapFill (inspect the 4 candidates that contain 35=4:
...
specs/013-*  +  specs/022-*   # dated notes: 013 FR-010 GapFill now stamps 43/122 (amends S-013 at-rest disposition);
                              # 022's build_replay_frame INV-5 now dedups stored 43/122 (no history rewrite)
```
...
2. **`43=Y` requires `122`** — QFJ `Session.java:2580-2592` rejects a `43=Y` frame missing `122` under `RequiresOrigSendingTime`, and requires `122 ≤ 52` (so `==52` is the safe boundary). Both tags are emitted together; emitting `43` alone would be *less* conformant than today.
...
4. **DEFECT 2 fix preserves replay semantics** — skipping stored `43`/`122` then re-appending the engine's `43=Y` + `122=<stored 52>` matches QFJ's resend (`setField` replaces rather than duplicates; `122` = the stored original `52`, not any caller-supplied `122`). The `52` capture stays before the skip.
...
- [quickstart.md](./quickstart.md) — the resend-reply witness recipe (field-occurrence counting + `122==52`), the `allow_pos_dup` retain cell, the default-path byte-identity check, the golden re-bake list, and the live QFcpp/QFJ re-run.
...
_(pending — runs after `/plan`, before `/tasks`, per `.specify/pipeline.md`; populated by `/gate-a 037-resend-reply-possdup-tags`)_
