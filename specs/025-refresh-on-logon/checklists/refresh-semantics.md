...
- [x] CHK014 Is the no-heap / allocation discipline on the per-logon re-hydrate path captured as a requirement (the refresh path allocates zero on a ready-awaitable store), so it is a measurable gate and not an afterthought? [Measurability, data-model W8 / [const §VIII.5]] — PASS: data-model W8 specifies "zero allocations attributable to the refresh (non-allocating ready-awaitable store)"; plan.md Constitution Check article VIII.5 confirms "the re-hydrate reads are counter-only (no frame body, no new container); reuses 029's non-allocating hydrate path; no-heap witness on the per-logon re-hydrate"; tasks T040 is the dedicated witness.
