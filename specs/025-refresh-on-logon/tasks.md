...
- [X] T032 [US3] Write W6 RED then GREEN: `refresh_on_logon=true`, lenient, ACCEPTOR with persistent non-1 inbound `{in:37}`; cold first logon (`hydrated_==false`, `force` inert); feed a peer reset Logon `34=1,141=Y`; assert the inbound seed is withheld (`apply_inbound_seed=false` via `peer_sent_reset`), the `:1925` received-141 reset establishes `{1,1}`, the peer `34=1` is accepted (in-seq, not too-low), session reaches Active. — SC-006, FR-009, INV-RoL-5, contract C5.3.
...
- [X] T040 [P] Write W8: no-heap under mallocnesia on the re-hydrate apply step (`SeqnumManager::hydrate()`) proxy, matching 029 (knob-on, lenient, non-allocating ready-awaitable store) — zero allocations on the apply step; the full coroutine/store/reconnect path is not mallocnesia-witnessed (029-precedented waiver). [const §VIII.5].
