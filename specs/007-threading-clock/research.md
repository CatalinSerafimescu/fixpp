...
**Decision:** Define `fixpp::otel::trace_context` as a minimal 32-byte trivially-copyable POD — `std::array<std::byte,16> trace_id` + `std::array<std::byte,8> span_id` + `std::uint8_t flags` + padding (`[2d §1.2]`/§4.4). It is the value carried by `session_local<trace_context>` and the `EngineConfig::engine_trace_context` atomic snapshot.
...
**Decision:** Seams that exercise FSM-shaped behavior (3 executor-compat sequences, 9 heartbeat-window, 16 `direct_executor` reentrancy) are driven by a **deterministic scripted test-double FSM** in `tests/support/` consumed by the 2d fixture; they assert only **2d-owned properties** (strand serialisation, `mock_clock` determinism, two-phase cancellation, alloc/latency), never FIX FSM correctness. Seam 11 is realized as a **2d-scoped deterministic clock-injection corpus** at `tests/session/test_clock_injection_corpus.cpp` (relocated from the design-doc nominal `tests/conformance/test_corpus_mock_clock.cpp`); the full FIX-TC corpus `tests/conformance/` (TC-001..017) lands with `005`. The test-double FSM picks values 2d does not pin (e.g. heartbeat interval).
...
### D-9 — Two-phase close: child `cancellation_state` below the root; close-timeout owned by `005`

**Decision:** `Session::close(graceful)` opens a **child** `asio::cancellation_state` composed below the session root; the Logout `async_write` + its `Clock::sleep_until` timeout bind to the **child** slot (NOT pre-cancelled by the eventual root total). Phase 2 fires `cancellation_type::total` on the root only after phase 1 resolves (peer ACK | child timeout | child cancelled). `close(terminal)` skips phase 1. `partial` is excluded from the v1.0 `close_mode` enum (N-P1-3). The `close_timeout` *value* is a `SessionConfig` `std::optional<...>` placeholder owned by `005` (N-P2-1); 2d wires the *mechanism* only.

**Rationale:** Root cause #1 close — child-state composition makes the Logout exchange survivable independent of the eventual root total; `partial` had no well-defined per-component semantics across the {transport r/w, heartbeat, async_mutex, fromApp dispatch, store write, Logout} matrix and 2d declines to ship an underspecified parameter. Seams 4/5/12 exercise the matrix.

**Anchor:** `[2d §4.7]` / `[2d §6.5]` / root cause #1 / N-P1-3 / N-P2-1.
...
- **`[const §VII.7]` strictly N/A** — 2d is not parser-touching — but a libFuzzer cancellation-timing harness (seam 12) is shipped **voluntarily** per `[2d §9 seam 12]` Gate-A discretion (`[const §IX.4]` extended to threading-touching code).
...
### D-12 — Appendix D cross-doc edits applied by the orchestrator at sign-off, NOT by this feature

**Decision:** The cross-doc text amendments OWED at 2d sign-off are **not** applied by the 2d rewrite agent / this feature (per the `[2c App D]` precedent):
- `[arch §5.4]` Trace-context Storage-bullet rename (the v0.2 `strand_local`→`session_local` finalisation; `[2d Appendix D §D.1]`).
- `[2d §11]` NFR-015 row → `library/spec/feature-catalogue.md` + a `coverage-index.md` entry linking `[2d §4.1]`/`[arch §1.1]` → NFR-015.
- `[arch §11]` row-7 disposition `TODO → DONE`.
- 2k record-schema `clock_scope` drop-in (consumed by 2k at its sign-off).

The 2f-requested `[2d App D §D.1/§D.2/§D.3]` edits (the `Session::session_arena()` accessor etc.) are **already in the design-doc body** (Appendix C cross-doc entries) and are therefore **realized as the shipped 2d surface** by this feature — they are not a separate text-edit task. The orchestrator (parent session) applies the catalogue/architecture text at the sign-off commit; this bundle records the drop-in language only.
