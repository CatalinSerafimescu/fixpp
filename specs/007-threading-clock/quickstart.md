...
Seam → file map is the authoritative table in `plan.md` (Test-seam → file mapping). FSM-shaped seams (3, 9, 15, 16) and the clock-injection corpus (seam 11, `tests/session/test_clock_injection_corpus.cpp`) are driven by the **scripted test-double FSM** in `tests/support/` (Clarifications 2026-05-19 / D-5) — they assert 2d-owned properties only, never FIX FSM correctness. Seam 15 (`tests/core/test_third_party_clock_conformance.cpp`, the on-disk realization of SC-006) drives its "Logon→NewOrderSingle→cancel" session through the **same scripted test-double FSM** (third-party `Clock` derivative injected; FIX labels are test-double script labels, not real FSM output), asserting only `sleep_until` completion on the awaiter's bound executor + root-`total`/child-state cancellation + alloc-guard clean — **not** a real FIX session FSM (`005`-owned per `2d-threading.md:38`). `tests/conformance/` (FIX-TC TC-001..017) is **not** populated by this feature (owned by `005`).
...
Pipeline order (single in-bundle source of truth = the 5-stage `plan.md` Constitution-Check `[const §XVI.4]` row, which records it as the bundle-local Spec-Kit pipeline convention — `[const §XVI.4]:237` itself backs only `/analyze`-before-`/implement`, not the full sequence): **`/plan` → Gate A → `/tasks` → `/analyze` → `/implement`**. This sequence is byte-identical to the `plan.md` row; the command membership is `[const §XVI.1]`, the Gate-A-before-`/tasks` ordering is `[const §XVII.1]`, the `/analyze`-before-`/implement` step is `[const §XVI.4]:237`. The further `/simplify`, `/speckit-verify`, and `/gate-b` steps shown in this bundle are downstream implementation/orchestration context layered after `/implement`. `/speckit-simplify` runs between `/implement` and `/speckit-verify` (memory `feedback_speckit_simplify_before_verify`). `/gate-b` is mandatory pre-merge (`[const §XVII.2]`); author≠reviewer (`[const §XVII.3]`); preconditions = `/speckit-verify` non-RED + feature-completeness audit non-failing. The NFR-015 `feature-catalogue.md:227` row + `coverage-index.md:460` entry + `[arch §11]` row-7 `DONE` (`architecture.md:602`) + `[arch §5.4]` `session_local` rename (`architecture.md:395`) are **already present in the authority set as of `2d` v0.4 sign-off 2026-05-08**; only the catalogue Status-field promotion (`backlog → done`) is residual orchestrator bookkeeping at this feature's Gate-B merge (research D-12). The 2k `clock_scope` schema is consumed downstream by 2k at its own sign-off.
