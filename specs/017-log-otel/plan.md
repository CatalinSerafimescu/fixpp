...
2. **017 owns its build scaffold.** Pin OTel C++ SDK exactly to `opentelemetry-cpp/1.16.1` (a tagged release ≥ 1.12, the first stable logs API) in `conanfile.py`, add CMake link wiring + the `FIXPP_LOG_MIN_LEVEL` / `FIXPP_LOG_SPIKE_QUILL` options; quill recipe is optional/spike-only (FR-023).
...
**Primary Dependencies**: OpenTelemetry C++ SDK pinned exactly to `opentelemetry-cpp/1.16.1` (a tagged release ≥ 1.12, the first stable logs API; in `conanfile.py` per `[const §XV.17]`), `asio` (standalone; only for `Logger::async_flush` completion post + existing executor types), `std::format`/`std::vformat` (drain-thread formatting), POSIX `<syslog.h>` (SyslogSink), `quill/3.9.0` (OPTIONAL, `FIXPP_LOG_SPIKE_QUILL=ON` only, TS-13).
...
| `[const §XV.17]` | Third-party version pinning | OTel SDK pinned **exactly** to `opentelemetry-cpp/1.16.1` (tagged Conan-Center release, ≥1.12 stable logs API); quill pinned **exactly** to `quill/3.9.0` (spike-only, `FIXPP_LOG_SPIKE_QUILL=ON`) in `conanfile.py` (FR-023, R1/R4). | build review |
...
conanfile.py                # AMEND: OTel SDK opentelemetry-cpp/1.16.1 pin (a tagged release ≥1.12, the first stable logs API) (+ optional quill/3.9.0)
