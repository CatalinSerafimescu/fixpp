...
- **Decision**: Pin **exactly `opentelemetry-cpp/1.26.0`** (latest on conancenter, revision 2026-04-23; ≥ 1.12 for the stable logs API) in `conanfile.py` per `[const §XV.17]`, built with the FR-driven option set: **`with_abi_v2=True`** (API V2 — sets `WITH_ABI_VERSION_2=ON` / `OPENTELEMETRY_ABI_VERSION_NO=2` on the `api` component; user directive 2026-06-02), `with_no_deprecated_code=True` (pairs with abi_v2), **`with_prometheus=True`** (the recipe default is `False`; FR-017 / TS-11 hard-require the embedded `:9464` reader), `with_otlp_http=True` (recipe default; OTLP HTTP path for `OtlpLogSink`/`OtlpMetricExporter`, our `use_grpc=false` default). *(Refreshed 2026-06-02: the anchor's `1.16.1` was stale — ~10 minors behind; upstream `1.27.0` is not yet packaged on conancenter, so `1.26.0` is the frictionless latest.)* Dual export = **one `MeterProvider` with two `MetricReader`s via two `AddMetricReader()` calls**: (1) `PrometheusExporter` (a `MetricReader`, pull model), (2) a `PeriodicExportingMetricReader` wrapping `OtlpMetricExporter` (a `PushMetricExporter`). `OtelDualExportBuilder::build()` performs both registrations; a single `meter.add(counter, 1)` fans out to both readers via the SDK internally.
  - **API V2 watch-item**: with `with_abi_v2=True` the `otel/*` wrappers are written against the v2 tracer/metrics surface (the SDK signatures differ from the abi-v1 default the anchor implicitly assumed). No FR changes; verify the SessionSpans/exporters wrappers compile against the v2 API at scaffold time (T005).
  - **gRPC transport DEFERRED to Phase 5 (marker).** v1.0 sets `with_otlp_grpc=False` — the OTLP exporters (`OtlpLogSink`/`OtlpMetricExporter`) use the HTTP path (`use_grpc=false` default, `with_otlp_http=True`). When Phase 5 adds `grpc`, flip OTel to `with_otlp_grpc=True` and pin `grpc` **once via Conan**: OTel requires `grpc/[>=1.67.1 <2]` transitively, so the Conan resolver unifies it with our own `grpc` (1.78.1, Conan-latest) to a single copy — no rebuild. Avoid mixing a Conan grpc (OTel) with a FetchContent grpc (1.81.0) for us: two copies / ODR conflict unless OTel is rebuilt from source. (grpc ≥1.70 forces a C++17 consumer + `cpp_plugin=True` — free for us at C++23.) Recorded in `conanfile.py`.
...
- **Pinned**: `opentelemetry-cpp/1.26.0` (exact, latest on conancenter) with `with_abi_v2=True` + `with_prometheus=True` + `with_otlp_http=True`.
...
- **Quill Conan pin**: `quill/11.1.0` (latest on conancenter; refreshed 2026-06-02 from the anchor §10 Q5 `3.9.0`), pulled only when `FIXPP_LOG_SPIKE_QUILL=ON`. NB: quill 11.x is a major-API rewrite vs 3.x — the TS-13 spike harness (`bench/log_spike.cpp`, T045) is written against the 11.x API; it has no bearing on the shipping own-ring backend.

## R5 — MPSC ring memory-ordering + drain mechanics (TSan-correct)

- **Decision**: `write_sequence_` (`std::atomic<uint64_t>`, producers CAS `acq_rel`) and `read_sequence_` (`std::atomic<uint64_t>`, drain stores `release`, producers load **`acquire`** — #402, was `relaxed`), each `alignas(64)` on its own cache line. Producer = load-check-CAS: load `w`, load `r` (acquire), if `w-r >= capacity` increment `drop_count_` and return (drop_newest); else CAS `w→w+1`; on success own slot `w%capacity`, `memcpy` the `Record`, publish a per-slot sequence number; drain consumes in `read_sequence_` order and advances with a `release` store.
- **Rationale**: the overflow check **before** claiming the slot guarantees the drain never waits on a claimed-but-unwritten slot (no deadlock, anchor §4.3 key invariant). The `read_sequence_` load must be **acquire** (#402). ⚠️ **Corrected #402 — was `relaxed`.** The old rationale (a stale, under-advanced read only makes the ring look fuller ⇒ at worst an early `drop_newest` drop) is about LIVENESS and was the wrong axis: it says nothing about the ordering that REUSING a slot requires. The acquire pairs with the drain's release store so that a producer claiming `i + capacity` cannot overwrite a slot the drain is still copying (a TORN record). See the MPSC ring protocol header in `src/log/logger.cpp`. `read_sequence_` MUST be `std::atomic` (not plain `uint64_t`) or TSan fires on the multi-producer fullness read (anchor P1-2 fix).
...
|---|---|---|---|
| R1 | OTel SDK pin + dual export | `opentelemetry-cpp/1.26.0` pinned exactly (`with_abi_v2=True`/`with_prometheus=True`/`with_otlp_http=True`); two `AddMetricReader()` calls | no (pin committed) |
