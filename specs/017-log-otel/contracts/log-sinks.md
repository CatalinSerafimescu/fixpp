...
- **Defensive-only note (RC#4):** Both `emit()` and `flush()` are declared `noexcept` on the `Sink` interface (`[2k §4.4]`). The drain-thread try/catch is therefore **defensive-only** for the noexcept virtual call path — a conforming sink that throws through `noexcept` will call `std::terminate`, not enter the catch. Conforming sinks self-swallow errors internally (e.g. `file_sink.cpp` catch in `emit()`). Error slots 124/125 remain valid C-ABI codes: they are reachable via self-counting sinks (a sink that increments `sink_error_count` internally) and via the `open()`-failure path (`logger.cpp` marks a failed-open sink permanently disabled and routes the error through the same counter slot). Operators should not expect the drain try/catch to catch throws from conforming sinks — write conforming, self-silencing sinks instead.
