...
| `ResetOnLogon_Acceptor_ResetsIdempotentWith141` | C2.2, C5.1 | Seed non-1, `reset_on_logon=true`, feed an inbound Logon carrying `141=Y` → seqnums `{1,1}` AND **exactly one** observable `store_->reset()` I/O (the knob-driven pre-`check_inbound` reset arm; exactly one store reset). |
