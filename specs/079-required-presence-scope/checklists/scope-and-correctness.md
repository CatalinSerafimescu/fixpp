...
- [x] CHK016 Do FR-001/FR-005 (loader threads no component-AND, component treatment unchanged) and the census oracle's stronger full-ancestor-chain component-AND (US4/FR-009) stay non-contradictory — i.e. is it consistent that the loader keeps a component-required field while the oracle would drop a *synthetic* optional-component one? [Consistency, Spec §FR-005 vs §FR-009] — PASS: data-model §Message-level required set explicitly reconciles this as a designed safety-net asymmetry, not a contradiction — the oracle is deliberately stronger than the shipped rule so an over-require surfaces as RED.
