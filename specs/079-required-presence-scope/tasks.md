...
- [ ] T017 [US4] NEW per-group required-member census (Contract 1a) in the same `required_scope_census_test.cpp` — TWO legs (distinct contracts): **(1) context store (PRIMARY)** `group_required_members(msg_type, parent_path, no_tag)` == the walker's per-context required set, exact set-equality both directions, for every `(msg_type, parent_path, no_tag)` in all 10 dicts **except FIX42** (**FIX42 carve-out per SC-003a / Contract 1a — L-066-1/#196**: FIX 4.2 group counts are `INT`-typed so `dictionary.cpp:358` leaves the context store empty; assert FIX42 context-store-**empty** instead — a pin that flips when #196 lands — while the raw-XML oracle DOES see FIX42's structural `<group>` members); **(2) bare store** `group_required_members(no_tag)` == the global first-seen variant (fallback contract — NOT required to equal every context; e.g. FIX44 tag 295 `{}` vs `{299}`). Also census the shipped **maximum** per-group required-member count across all 10 dicts (RC5 — pins the small-count assumption so the dynamic-width check cannot silently regress). Prove RED by an injected/omitted per-context required member (on a non-FIX42 dict).
