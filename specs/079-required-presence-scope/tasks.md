...
- [X] T016 [US4] Prove the message-level census RED on **two** witnesses (Contract 1 RED-proof obligation) — these two RED captures are the **load-bearing claim of the entire feature**; they MUST be PERFORMED and their RED output CAPTURED (not asserted/assumed — [[feedback_sanitizer_canary_must_be_proven_red]]), each reverted after capture: (a) revert the `in_group` gate in `expand_field_list` → group-member leak restored → census RED (this is ALSO the repo-wide discrimination proof for T006/T007/T008/T010 — capture which messages/tags flip); (b) inject a synthetic **non-header/trailer** optional-component-`required='Y'` field → the full-component-AND oracle drops it → census RED. **⚠️ SUPERSEDED (Gate B r1 F1/T020):** the "loader keeps it" / "0 optional-component sites" premise is stale — T020 threads message-level component-AND into the loader (so a synthetic optional-component field is now dropped loader-side too) and the census found 4 REAL sites (which became the actual pre-fix RED source). T016(a) in_group-revert remains the live RED discriminator; T016(b) synthetic demonstrates the mechanism but is no longer a divergence post-T020. Paste both actual RED ctest excerpts into `.specify/decisions/079-required-presence-scope-verify.md`; the orchestrator re-checks these captures directly (a subagent "went RED" claim does not satisfy this task).
