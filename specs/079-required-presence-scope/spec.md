...
- Q: Is the fix scoped to optional-group leakage only, or also optional-component over-require (issue fix item 3, componentRef AND)? → A: **Both** — group-member leakage AND optional-component over-require; the enclosing-container AND-rule is threaded through all three projections (xml_loader, orchestra_loader, codegen ir.cpp). This is the *over*-require direction and is distinct from L-067-1's *under*-require (required-component) parity case.
...
- A required field inside a *required* component (not a group) — must stay top-level-required (QuickFIX AND-rule / L-067-1 under-require direction; the fix must NOT drop these).
- A required field inside an *optional* component — must be dropped from the message level (the over-require direction corrected here); if that same component is `required='Y'` in one message and optional in another, the derivation is per-message.
- A required field inside a component nested under another component where any link on the path is optional — dropped (AND-composition down the full path, not just the immediate parent).
...
- **FR-001**: The message-level required-field set derived by the runtime dictionary loaders MUST contain only fields effectively required at the top level of the message. A field that is `required='Y'` solely inside a repeating group MUST NOT appear in it; a field that is `required='Y'` inside an **optional** component (or a component that is itself reached only through an optional component) MUST NOT appear in it.
...
- **FR-005**: The message-level required-set derivation MUST honor the full QuickFIX AND-rule for *component* usage in **both** directions: a field is message-level-required iff it is `required='Y'` **and** every enclosing component-usage on the path to the message root is `required='Y'` **and** it is not enclosed by any group. A field required inside a *required* component stays message-level-required (the L-067-1 / under-require direction, unchanged); a field required inside an *optional* component is dropped from the message level (the over-require direction, corrected here).
...
- **FR-007**: The generated typed `validate_<Msg>` validators MUST agree with the runtime validator on accept/reject for every affected message. Because component scope is in scope (Clarifications 2026-07-18), the codegen IR projection (`ir.cpp`) MUST thread enclosing component-usage AND-composition so the over-require sites in the generated v50sp2 / vlatest validators are corrected (issue fix item 3), with the attendant validator-golden / determinism regeneration. The *exact set* of over-require sites (the issue estimates ~6) MUST be enumerated empirically during planning by running the affected frames through the generated validators — not assumed — and whether the typed tier also needs a *group*-scope change (the issue claims it already checks group members per-entry) MUST be settled by the same empirical check and recorded. The census (FR-009) MUST cover the codegen IR projection — never silently omit it.
...
- **SC-003**: The non-circular census reports exact set-equality (0 extra, 0 missing tags) for the message-level required set of every message in all 10 dictionaries, for both the runtime derivation and the codegen IR projection — and is proven to fail (RED) when a group-scoped **or** optional-component-scoped required is deliberately reintroduced.
...
- **SC-008**: The over-require sites in the generated v50sp2 / vlatest typed validators (issue fix item 3; exact set enumerated at planning) are cleared — the generated validators no longer require a field that is required only inside an optional component/group — with the validator golden regenerated deterministically and byte-stable across runs.
...
- The codegen IR leg (FR-007) **is in scope** for the optional-component over-require correction (Clarifications 2026-07-18). What remains empirical at planning/Gate A is the *exact set* of generated-validator over-require sites (issue estimates ~6) and whether the typed tier also needs a group-scope change — both measured by running the affected frames through the generated validators (e.g. a sides-less TradeCaptureReport), not assumed.
