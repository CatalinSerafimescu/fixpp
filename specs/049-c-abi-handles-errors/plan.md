...
**Performance Goals**: `fixpp_strerror` / `fixpp_version` / `fixpp_library_version` are O(1) and **zero global-heap allocation** (static string literals / value-typed PoD), per `[2i §4.4]` / `[2i §4.5]`.
...
src/capi/
├── error.cpp       # NEW — translate(error)→fixpp_error_t (116 enumerators→~30 coalescing) + fixpp_strerror (static string literals) + downgrade translate(code, consumer_minor)
