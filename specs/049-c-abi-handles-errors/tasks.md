...
- [X] T011 [US2] Create `src/capi/error.cpp` (NEW): `fixpp_capi::detail::translate(fixpp::core::error) noexcept` — a **total** `switch`, **no `default`** (`-Wswitch`), 116 arms per data-model E-3, with override groups (`session_*`/`log_*`/`otel_*`/`app_*`/`out_of_memory`) → `FIXPP_ERR_UNKNOWN` each carrying a `// publish FIXPP_ERR_SESSION_* in Feature B` / deferral comment (L-049-2); `fixpp_capi::detail::translate_for_consumer(fixpp_error_t, uint16_t consumer_minor) noexcept` downgrade (`introducing_minor > consumer_minor → UNKNOWN`, E-5); `fixpp_strerror` static `const char* const` table [planning snapshot — realized as a `switch` over published codes returning static string literals; see research.md D-1], out-of-range/undefined → "unknown error", zero global-heap alloc. Add `error.cpp` to `src/capi/CMakeLists.txt`. Make T009 GREEN. (FR-007/008/009/017, research D-1/D-2/D-3)
