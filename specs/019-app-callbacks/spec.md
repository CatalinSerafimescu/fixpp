...
A library user is notified when a session is created and when it becomes / ceases to be established: `onCreate` after the engine creates the session object and `Session::open()` initializes its executor but **before** first Logon processing/emission (so `onCreate` is already on the session strand — the executor is valid only post-`open()`), `onLogon` when the session reaches `Active`, and `onLogout` when it leaves the established state (graceful or terminal). These notifications let the user gate origination (send only after `onLogon`) and release per-session resources (on `onLogout`).
...
- **Wiring point** *(refined — research D3, plan §VIII.5/§XV.15)*: callbacks execute **on** the session's serialized executor; the inbound (`fromAdmin`/`fromApp`), lifecycle (`onCreate`/`onLogon`/`onLogout`), and emit (`toAdmin`/`toApp`) callbacks are invoked **directly** at the existing on-strand sites (no per-message `asio::post`, preserving zero-alloc §VIII.5 and no-drop §XV.15); the post-based `Session::dispatch_app_callback` seam is used **only** by the any-thread public `Engine::send` to reach the strand. This feature adds the public `Application` surface and its engine wiring; it does not introduce a second executor or change the session threading model.
