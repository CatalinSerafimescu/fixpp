...
- [X] T017 [P] `tests/session/test_application_strand.cpp` — no two callbacks for one session run concurrently (TSan; INV-2 — the `exec_` strand is the real guarantee, `dispatch_guard` is the debug-only check); the Engine drains all dispatched callback work + any `Engine::send`-posted-but-not-run work before a `Session` is destroyed (`stop()`→`close(terminal)`+join-before-registry-clear), no callback runs against a freed session (ASan; FR-012/INV-3); `Engine::send`'s post holds the session keepalive so `stop()` racing a post is UAF-free (ASan); re-entrant `send` from inside an on-strand callback does not deadlock. (SC-005.) DONE: 5 tests pass debug+ASan. NoConcurrentCallbacks/DrainBeforeDestroy/EngineSendKeepAlive/ReentrantSendNoDeadlock/NullApplicationZeroDelta. TSan-gated (NoConcurrentCallbacks) + ASan-gated (DrainBeforeDestroy, EngineSendKeepAlive) confirmed ASan green. T019 fix (scope-before-close) is the correctness load-bearer.
- [X] T018 [P] `tests/session/test_application_throw.cpp` — a throwing user callback at **every** site (`fromApp`/`fromAdmin`/`toApp`/`toAdmin`/`onCreate`/`onLogon`/`onLogout`) ⇒ the engine catches at the dispatch boundary, logs, clears the re-entrancy guard, terminal-closes the session, records `error::app_callback_threw`, and never propagates the exception inward or corrupts session state. (FR-011.) DONE: 7/7 cells pass debug+ASan. Each cell: throw caught, session terminal-closed (not Active post-drain), second session works (engine not corrupted). toAdmin cell note: for initiator (default role), toAdmin fires in open() via emit_initiator_logon_() — open() itself returns error (correct behavior, throw caught).

### Implementation

- [X] T019 Harden drain + keepalive in `src/session/engine.cpp` — `stop()` drains posted-but-not-yet-run `Engine::send` work; the registry holds a strong session ref outliving posted work; the T007 throw wrapper is applied at all 7 callback sites; an `application==nullptr` zero-delta regression witness confirms SC-006/INV-1 (existing session/engine suites unchanged). (FR-011/FR-012/FR-014; SC-005/SC-006.) DONE: (1) Root cause found + fixed: callback_dispatch_scope was held ACROSS co_await close(terminal) at 6 call sites in session.cpp — onLogout (fired inside close→record_state_transition_) tried to re-acquire the scope while it was held → debug assert. Fix: nested-scope pattern so cs drops BEFORE close(terminal)/record_state_transition_ at every fromApp/fromAdmin/toApp/onCreate call site. (2) Engine::send co_spawn now resets total cancellation so stop()'s total-cancel propagates into Session::send. (3) Keepalive (shared_ptr<Session> kl) confirmed load-bearing: session alive via kl even if registry_.clear() races the post. (4) NullApplicationZeroDelta test added in test_application_strand.cpp (SC-006/INV-1). Full regression 89/89 pass.
