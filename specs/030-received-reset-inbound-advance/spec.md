...
3. **Given** an acceptor with a **persistent** store and `reset_on_logon=false`, **When** the peer sends `Logon(34=1, 141=Y)` and the durable store reset **fails** (fault-injected), **Then** the session disconnects and the store error propagates, and the persist-to-2 write-through is NOT reached — so the store retains its last-good durable value (a valid INV-H1 lower bound; never advanced past it). *(This is the discriminating witness for FR-010: it falsifies the "INV-H1 holds because the reset is fatal-when-persistent" claim if the reset were ever swallowed on a persistent store. Note: a transient in-memory `store=N > manager=1` during fatal teardown is harmless — the manager is discarded before restart; the guarantee is that persist-to-2 never runs after a failed reset, so the durable store is never advanced past its last-good value.)*
...
2. **Given** an initiator with a **persistent** store whose peer Logon-ack carries `141=Y`, **When** the durable store reset **fails** (fault-injected), **Then** the session disconnects and the store error propagates and the persist-to-2 write-through is NOT reached — the store retains its last-good durable value (symmetric to US1 scenario 3, FR-010).
...
- **FR-010**: On a **persistent** store, a received-141 durable reset failure MUST be **fatal** — the session disconnects and the store error propagates — on BOTH arms (acceptor + initiator). It MUST NOT be swallowed-and-proceed. This guarantees persist-to-2 runs only after a known-good reset, so the store is never advanced past its last-good durable value (never `store = N+1`, the over-persist that skips on restart). On a reset failure the store retains its last-good value `N` — a valid INV-H1 lower bound (≤ true position); restart re-hydrates the manager from it and 013 recovery resyncs, so no inbound is ever skipped. (A transient in-memory `store=N > manager=1` on the fatal teardown path is lower-bound-preserving and harmless — the manager is discarded before any restart.) On a **non-persistent** store the reset stays effectively `logged` (it cannot meaningfully fail) and the session does not disconnect on a received-141 reset. This amends the 024 FR-001/C2.6 I-07 logged-then-proceed contract for the persistent received-141 sub-case (was: stay-Active under a swallowed store-reset failure). Implementation: pass `store_is_persistent_ ? reset_disposition::fatal : reset_disposition::logged` to the shared `reset_seqnums_to_one_durable(disposition)` helper on both arms; the initiator arm (currently a hand-rolled `reset_to_one()` + swallowed `(*store_).reset()`) is consolidated onto that same helper for symmetric disposition semantics ([[feedback_half_restructure_symmetric_api]]).
...
**Fault-injection witness (the FR-010 soundness proof, both arms):** a separate witness per arm with a **persistent** store seeded to N>1 (last-good durable value) and a fault-injected store-reset failure (`fail_next_reset()`) MUST assert (i) the session **Disconnected** + `reset_call_count()==1`, AND (ii) `store == N` (last-good value retained — persist-to-2 was **not** reached, so the store was never advanced past its last-good durable value). The seed to N>1 makes assertion (ii) genuinely discriminating: a trivial fresh store (N=1) cannot distinguish the fatal path from a successful reset + persist-to-2 (both leave store==1); with N=37, `store==37` proves only the fatal path. This is the witness that makes the "INV-H1 holds because the reset is fatal-when-persistent" claim falsifiable — without it the soundness claim is unwitnessed (the 029 W9b proxy-gap lesson, [[feedback_witness_asserts_named_postcondition_not_proxy]]).
...
- **SC-002**: Both role witnesses pass — the acceptor discriminating triple (next_inbound==2 AND reply.MsgSeqNum==1 AND reply.789==2, 027-on) AND the initiator witness (next_inbound==2 + harm-repro). Each asserts INV-H1 directly on the store (`store == manager == 2`). The **fault-injection witness** (persistent store seeded to N=37, store-reset failure → Disconnected + `reset_call_count()==1`, persist-to-2 not reached → `store==37`) passes on both arms (FR-010 soundness proof — the seed N=37 makes assertion (ii) genuinely discriminating).
- **SC-003**: The blast radius is updated in one pass and the full 6-preset verify matrix (incl. all sanitizers) is green: (a) the **8 value-pins** across features 013/024/027/029/025 (and `reset_seqnum_policy_matrix`) that pin the off-by-one *value* flip to the corrected expectations — three acceptor policy-matrix pins (`next_inbound` 1→2); the **initiator** policy-matrix pin `BilateralStrict_Initiator_CountersResetToOne` (`next_inbound` 1→2); the 029 hydrate **W9b acceptor** pin (two sub-assertions: `next_inbound` 1→2 AND `durable_inbound` 1→2); the **initiator durable INV-H1 twin** `INV_H1_Initiator_PeerAck141_NoOverPersist` (`durable_inbound` 1→2); the 027 `AcceptorReplyReceived141` reply `789` 1→2; and the **025 refresh-on-logon W6** acceptor pin `W6_Acceptor_KnobOn_PeerResetLogon_InboundSeedWithheld` (two sub-assertions: `next_inbound` 1→2 AND `call_count` 2→3 — the persist-to-2 write-through adds one `next_seqnum` store call, a **store-interaction-count** pin invisible to a value-grep). The final two (initiator INV-H1 twin + W6) were **discovered at implement-time via a clean-build full ctest, NOT in the original 7-pin analysis** which undercounted the shared received-141 path's footprint; (b) the merged 024 *contract* witness `ResetOnLogon_Off_Inbound141_StoreFailure_StillActive` (`tests/session/test_reset_on_lifecycle.cpp`) is **split** — under FR-010 the persistent variant now asserts **Disconnect**, and a NEW sibling witness with a `yields_persistent_store()==false` factory retains the **stay-Active** characterization for non-persistent stores. (**8 value-pins + 1 contract-witness split = 9 pins total**; authoritative count from a clean-build full ctest — incremental builds gave false-greens that masked two pins, see the implement-time note.)
...
Eight existing tests currently **pin the defective value** (next-expected-inbound = 1, `789=1`, or a store-call count). These are stale pins of the off-by-one and must flip to the corrected value; each must be individually re-read to confirm it pins *this* case and not a distinct one. (The original analysis listed six; items 5 and 6 below — the initiator INV-H1 durable twin and the 025 W6 refresh-on-logon pin — were discovered at implement-time via a clean-build full ctest, after incremental builds gave false-greens that masked them. **The authoritative pin-finder is a clean-build full ctest, not the static analysis** — the received-141 path is a shared observable driven by tests across 013/024/025/027/029.)
...
4. `reset_seqnum_policy_matrix` (`tests/session/test_reset_seqnum_policy_matrix.cpp`) — `BilateralStrict_Initiator_CountersResetToOne` (×1): `next_inbound` 1 → 2. This is the **initiator** off-by-one value-pin — it drives the same `peer_ack_sent_reset_flag` initiator arm that FR-009 corrects, so under 030 its `EXPECT_EQ(next_inbound_unsafe(), seqnum_t{1})` flips to 2 (matching the FR-009 initiator witness for the identical path).
5. `persistent_seqnum_hydrate` (`tests/session/test_persistent_seqnum_hydrate.cpp`) — `INV_H1_Initiator_PeerAck141_NoOverPersist` (×1): `store->durable_inbound == 1` → 2 (the `EXPECT_LE(store, manager)` stays valid as 2≤2). **The initiator durable INV-H1 twin of the W9b acceptor pin (item 2)** — the 029 over-persist witness on the initiator `peer_ack_sent_reset_flag` arm. Under 030 the consumed seq-37 reset-ack Logon is a surviving net-advance, so the dedicated arm-local persist takes store==manager==2 (equality, not over-persist). **Implement-time blast-radius finding (NOT in the original 7-pin analysis):** the analysis listed the acceptor W9b durable pin but missed its initiator symmetric twin — the same symmetric-twin undercount class flagged at Gate A R3 ([[feedback_half_restructure_symmetric_api]] applied to test PINS, not just production). Caught by the full debug suite after the US2 initiator hunk, not by the per-suite target run.
6. `refresh_on_logon` (`tests/session/test_refresh_on_logon.cpp`) — `W6_Acceptor_KnobOn_PeerResetLogon_InboundSeedWithheld` (×1 test, **two** sub-assertions): `next_inbound == 1` → 2 (the consumed seq-1 reset Logon survives under 030, even with `refresh_on_logon=true`; the store-37 inbound seed stays withheld) **AND** `call_count == 2` → 3 (the 030 persist-to-2 write-through adds one `next_seqnum(inbound,true)` store call). **The `call_count` sub-assertion is a store-INTERACTION-count pin — a value-grep cannot see it** (the analysis missed it; only a clean-build full ctest surfaced it). Justified: the 030 restore+persist runs on this acceptor received-141 arm exactly as on the non-refresh path (refresh only adds the withheld force-hydrate). Discovered at implement-time after a clean rebuild exposed the false-green that had masked it.

### Contract-amendment pin (a different category from the value-pins)

A **9th** pin is the merged 024 *contract* witness — it pins a behavior, not a counter value, so it is tracked separately from the 8 value-pins above:

7. `reset_on_lifecycle` (`tests/session/test_reset_on_lifecycle.cpp`) — `ResetOnLogon_Off_Inbound141_StoreFailure_StillActive` injects `fail_next_reset()` on the received-141 path and asserts the session **stays Active** under the 024 FR-001/C2.6 I-07 logged-then-proceed contract. Its `StoreDoubleFactory` does **not** override `yields_persistent_store()`, and the base default is `true` (`include/fixpp/session/message_store_factory.hpp`), so the test runs on a **persistent** store. Under FR-010 the fatal-when-persistent flip changes its outcome Active→Disconnected. **Split into two:** the persistent variant flips to assert **Disconnect** (+ error propagated); a NEW sibling witness with a `yields_persistent_store()==false` factory **retains** the stay-Active characterization for non-persistent stores. This is the **024 FR-001/C2.6 I-07 contract amendment** (persistent received-141 reset failure now disconnects; non-persistent keeps stay-Active).
