...
- [X] T008 [US1] In `tests/session/test_reset_on_lifecycle.cpp`, add the **fault-injection** witness (FR-010): persistent store seeded to N=37 AFTER `open()` (before the Logon feed) + `fail_next_reset()` → assert (i) session **Disconnected** + `reset_call_count()==1`, AND (ii) persist-to-2 NOT reached → `store==37` (last-good value retained; proves the failed reset did NOT corrupt the store; `store==38` would mean persist-to-2 ran). Seed AFTER open() so the one-shot hydrate already ran at seqnum_min=1 (for the acceptor the inbound seed is withheld on the received-141 path anyway, but the after-open pattern is safer in general).
...
- [X] T014 [US2] In `tests/session/test_reset_on_lifecycle.cpp`, add the initiator **fault-injection** witness (FR-010, symmetric to T008): persistent store seeded to N=37 AFTER `open()` (the initiator hydrates the manager at open(); seeding before would set manager=37, making the peer's Logon-ack at 34=1 read too-low → different fatal path) + `fail_next_reset()` → Disconnect + `reset_call_count()==1`, persist-to-2 not reached → `store==37`.
...
- [ ] T028 Live interop close-out (SC-001) — re-run the acceptor received-141 cell vs **both** QuickFIX-cpp and QuickFIX-J: assert the session reaches Active with **zero** fixpp `ResendRequest` and the peer's seq-2 message accepted (skip-without-counterparty guard for CI). This is the true close-out of the live-found defect; unblocks the other acceptor feature cells. **STATUS 2026-06-10 — CELL AUTHORED + skip-guarded (committed); only the live RUN deferred to Item 1 (advisor-corrected split: author now, run later):** Created `tests/interop/happy/hp_fix44_received_reset_test.cpp` — `ReceivedResetAcceptor` × {QFcpp, QFj}: fixpp acceptor `reset_on_logon=false` (013-only received-141 path), in-process witnesses (a) FSM Active, (b) outbound>1, (c) **next_inbound==2** (the 030 SC-001 correction; pre-030=1→spurious ResendRequest). Registered in `tests/interop/CMakeLists.txt` (`interop_030_received_reset_test`), `cell_results.yaml` (RR-QFcpp/QFj-acc, `skip:counterparty-unavailable`/live), `cell_results_schema_check_test.py` EXPECTED_IDS (exact-set GREEN 9/9), and `MATRIX.md`. Compiles + SKIPS cleanly with reason (no counterparty) — the committed prior-feature posture (027/029 cited their cell, Item-1 RUNS it). **Deferred (only the live run):** `FIXPP_TLS_FIXTURE_DIR` + running QFcpp/QFJ counterparties (Item-1 live golden-capture, [[project_live_quickfix_hostile_scenarios]]) → captures `RR-*-acc-fix44-received-reset.fix`. Fix correctness fully established in-process + 6-preset matrix + 100% branch coverage; the live run is end-to-end confirmation.
