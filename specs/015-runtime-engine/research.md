...
3. rebinds outbound so `transport_send_` writes to the live accepted transport (see the ordering hazard below) — `install_reconnected_transport` does NOT touch `transport_send_`, and `transport_send_` is otherwise frozen at `open()` (`session.hpp:530-534`);
