...
1. Construct the `Session` + `co_await open()` — `open()` performs session/FSM/executor/clock/arena setup but, for an **engine-driven initiator**, **does NOT emit the initial Logon at open time** (the open()-time Logon-emit at `session.cpp:447-518` is valid only under the per-session-direct model). Exact open()-restructure spelling (a defer-emit mode vs. extracting the Logon-emit into a method the connect loop calls) locked at implement; the REQUIRED behavior is: no Logon on the wire before the transport is live.
...
- **Public surface (SC-010)**: `Session::drive_reconnect()` + `Session::live_transport()` (mirroring the public `attach_accepted_transport`).
