...
- **SC-010**: The public surface delta is limited to the documented runtime-engine additions and the seam removal (the `Engine` type, the `SessionId` value type, `Session::attach_accepted_transport`, the `Engine::acceptor_bound_endpoint` accessor, error slot 121, and the removed `logon_peer_identity_override` seam — nothing more).
...
- **Reuse of 014's per-session wiring**: the initiator connect path reuses 014's realized `drive_reconnect_attempt` loop and live `handshake_result.peer_id → authorize()` binding; 015 wraps it in the public engine and adds the symmetric acceptor path.
