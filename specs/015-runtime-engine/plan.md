...
**Constraints**: caller-supplied executor, no engine-owned threads (clarify Q3); per-session strand (`[const §XI.4]`); **no `std::mutex` in awaitable headers** (`[const §XV.9]` — `register_session()` before `start()`, then registry access confined to the single injected executor); ASIO **total-cancellation** teardown for `stop()` (`[const §XI.2]`, `[[feedback_asio_cospawn_total_cancellation_default]]`); append-only error slots (`[const §X.4]`); fail-CLOSED authorization on BOTH the static and dynamic acceptor paths (`[const §XII]`)
...
- **Article XI.4 (per-session strand)**: ✓ — read-pump dispatches `on_inbound_frame` on the session strand (FR-004); registry access stays on the single injected executor (E-5), no cross-session shared mutable state.
