...
- [ ] T016 [US2] Implement the **initiator live connect path** (Option A — single connect+pump; **multi-cycle reconnect-respin DEFERRED** per Clarifications 2026-05-31 / E-1a: `close(terminal)` is permanent + 014 has no tested multi-cycle reconnect; symmetric with US1's single-peer acceptor; documented Gate-B note). **TDD — write the RED test first.**
