...
- [X] T023 **DONE** — feature-completeness audit GREEN. **FR coverage 14/14:** FR-001→T005/T006/T017/T018; FR-002→T004/T006/T017(d); FR-003→T012/T016; FR-004→T015/T016(e); FR-005→T012; FR-006→T013; FR-007→T013/T009; FR-008→T013; FR-009→T020/T021/T019; FR-010→T022; FR-011→T006/T018/T017; FR-012→T013/T015 (inherits 013/014 dispositions, adds none); FR-013→scope-guard/T015; FR-014→T010/T012. **SC coverage 11/11:** SC-001→T008, SC-002→T009, SC-003→T014, SC-004→T017, SC-006→T019/T020/T021, SC-007→T022, SC-011→T010 — all GREEN now; **SC-005 + SC-008 are verify-gated** (full ASan/UBSan/TSan matrix runs at `/speckit-verify` step 12, NOT Polish — green claim deferred there per `[[feedback_gateb_full_sanitizer_before_signoff]]`). **SC-009 confirmed (grep):** zero `Application`/config-parse/`extern "C"`/pybind in `engine.{hpp,cpp}`. **SC-010 confirmed (grep):** public delta = exactly `Engine` + `SessionId` + `attach_accepted_transport` + `drive_reconnect()` + `live_transport()` + `acceptor_bound_endpoint` + `engine_managed` + slot 121 + removed seam — nothing more; `attach_accepted_transport` public is the recorded `/simplify` (step 11) tightening follow-up, not a completeness gap. **Catalogue:** T-041 = `done`, slot 121 recorded (T022). **Note:** the `coverage-index.md` per-feature merge-narrative block is a close-out (pipeline step 19) artifact carrying the squash hash — added at merge like the 013/012/011 blocks, NOT a Polish deliverable (PR pending); the `§4.4` coverage row already maps T-041. Depends T022.
- [X] T024 **DONE** — walked quickstart.md against shipped headers. Structural claims all MATCH: ctor `Engine(asio::any_io_executor, EngineConfig)` (engine.hpp:169) ↔ `Engine(io.get_executor(), std::move(ecfg))`; `register_session → expected_t<void>` (:184); non-blocking `start()` + `co_await stop()`; connect-then-Logon (initiator emits POST-connect via `drive_reconnect`→`install_reconnected_transport` rebind, read-pump on `live_transport()`); acceptor accept→handshake→bounded-first-frame→`reversed_from_logon`→`attach_accepted_transport`→deliver→authorize, no-match→slot 121. **ONE drift fixed:** the T-041-closure section cited stale gate-site anchors `:1048`/`:1864`/`:1913`; as-built live-identity arms are `session.cpp:1167` (acceptor) / `:1978` (initiator) and the seam arm is gone — corrected the line + anchored on the structural `live_peer_id_.has_value() && is_mtls` arm rather than a drift-prone line number (Gate-A fabricated-anchor discipline).
- [X] T025 **DONE** — incremental `linux-clang-debug` build (`-j2`) exit 0, then the **unfiltered** Tier-1 `ctest` (NOT `-R/-L 015`-scoped) on the clean tree: **354/354 passed, 0 failed, 132s**. The #132 `codegen-build-graph-check` git-cleanliness gate **passed** (tree clean). Confirms no awaitable-corpus include-edge / `[const §XV.9]` regression from the US4 `session.cpp`/`session.hpp`/`session_config.hpp` edits — the whole corpus is green, not just the name-scoped 015 subset. (Full ASan/UBSan/TSan matrix still runs at `/speckit-verify` step 12 for SC-005/SC-008.)
- [X] T026 **DONE** — `codegraph sync` from the submodule → "Already up to date" (Polish changed no source — last code change was US4 `0c7e46d`, synced then); `codegraph status` = 549 files / 59,242 nodes, healthy.
