...
- Q: How deep does the required-presence validator check — top-level message fields only, or also repeating-group entry required fields? → A: **Also group-entry required fields** — validation recurses into repeating-group entries. The emitter emits NEW level-scoped required-presence tables for both levels (a top-level body table + per-occurrence group tables), generated from the same shared IR `FieldRef.rule` as the read side. The existing top-level `<Msg>_rules` are NOT reused — they are header-polluted and level-flattened (rejected prior art, see FR-006a/R3).
...
**Why this priority**: Validation-drift-by-construction is a core Emitter-Lite benefit — the emitter emits NEW level-scoped, header-excluded required-presence tables (a top-level body table + per-occurrence group tables) generated from the same shared IR `FieldRef.rule` the read side uses, so they cannot drift from the dictionary. The existing top-level `<Msg>_rules` are NOT reused — they are header-polluted and level-flattened (rejected prior art, FR-006a/R3). One generic runtime routine over the NEW tables gives required-presence validation. It is a **separate `validate()` step**, NOT part of `commit()`: `commit()` must stay a pure serializer byte-identical to the 061 exemplars (which perform no dictionary validation), so gating serialization on validation would break the headline shape-oracle equality. Lower than P1 because builders serialize without it, but it closes the fail-closed contract for callers that opt in.
...
- **Component-usage required-ness NOT enforced**: `validate_<Msg>` enforces field-level `required='Y'` exactly as the loader carries it in `FieldRef.rule`. FIX dictionaries model many mandatory fields via a `<component required='Y'>` USAGE tag while the component-internal `<field>` is `required='N'` (e.g. `Instrument required='Y'` on NewOrderSingle, but `Symbol(55) required='N'` inside `Instrument`). The loader carries the component-internal required-ness, so such fields are Optional in the generated tables and their absence is NOT flagged. This is inherited from the existing loader (shared with the read-side `Validator.hpp` tables), not introduced here; component-presence validation is out of scope for v1.0. Recorded as a Behaviors & Limitations row.
...
- **Top-level body required-presence table** — new per-message table this feature emits: `group_no_tag==0`, `rule==Required`, header/trailer tags excluded by **provenance** (`VersionIR::header_trailer_tags`, recursively resolved through `<component>`/`<group>` refs; the 8-tag `{8,9,10,34,35,49,52,56}` framer floor is a defensive subset). Distinct from the existing header-polluted, level-flattened `Validator.hpp` `<Msg>_rules` (which this feature does NOT reuse).
...
- **The existing `<Msg>_rules` tables are rejected prior art, NOT an input.** They are header-polluted and level-flattened (FR-006a/R3); this feature does NOT consume them — it emits its own NEW level-scoped, header-excluded required-presence tables from the shared IR `FieldRef.rule` (drift-free by shared-IR provenance, not by table reuse).
...
**Art VI §3 canonical-format disposition (T029(c), 2026-07-10):** the 33 write rows above cite the `spec/coverage-index.md` app-message rows at **message-level** DocAbbrev granularity (`[FIX44]`/`[FIX50SP2]`), NOT section-granular `[DocAbbrev §X.Y.Z]` refs. This is a **deliberate `[impl]`/design-authority disposition** per Constitution Art VI §3 — consistent with the pre-existing project-wide convention that the entire FIX44 application-message domain carries message-level refs in the coverage-index — **not an omission**. The byte-exact canonical format 067 reproduces is fixed by the frozen 061 shape-oracle + QuickFIX goldens + `body_builder` (the "Design authority" block above), which serve as the authoritative format reference in lieu of section-granular spec citations. Cross-recorded as an HTML-comment disposition in `spec/coverage-index.md` immediately before the "Application Messages — Trade" section.
