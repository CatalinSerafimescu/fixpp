...
2. **Top-level fields sorted by tag ascending (R1)** — served by the tag-sorted `m.fields` run: for each present top-level scalar, `bb.field(tag, *args.member)` (Bool via the `'Y'`/`'N'` char overload; Length+Data as a coupled unit — FR-007a); for each present group (`nullopt` optional group ⇒ skipped entirely), at its `No<G>` tag position open `bb.group_begin(no_tag, delim_tag)`, loop the span emitting each instance's members in **declaration member order** via `entry->set_*`, recursing for nested groups, then `bb.group_end`. The `delim_tag` and member order for each group come from THIS message's occurrence in `MessageIR.group_order` (the declaration-order XML walk, §3 / research R9) — NOT the tag-sorted `m.fields`, NOT a version-wide plan; the same `no_tag` (e.g. `NoMDEntries(268)`) has delimiter 269 in W but 279 in X. The per-occurrence required SET (used by `validate_`, §2.2) comes from `m.fields` filtered by `group_no_tag` (order-independent).
...
**Per-message, NOT per-`no_tag`-global**: the required SET is emitted per (message, occurrence), keyed like 063's `(msg, parent-path, no_tag)`, resolved at codegen time from static per-message IR (`m.fields` filtered by `group_no_tag ∧ rule==Required` — order-independent, so the tag-sorted run serves it). The same `no_tag` yields a different required set in different messages — e.g. `NoMDEntries(268)` requires `MDEntryType(269)` in W (`FIX44.xml:3023-3024`) but `MDUpdateAction(279)` in X (`FIX44.xml:3060-3061`). A single version-wide `<Group>_required` keyed on `no_tag` alone would over-reject one and under-reject the other. The delimiter and member ORDER used by `build_` (§1.2/§3), by contrast, are order-dependent and therefore come from `MessageIR.group_order` (the declaration-order XML walk, research R9), NOT from the tag-sorted `m.fields`.

**Provenance**: both tables derive purely from IR `FieldRef.rule` + `FieldRef.group_no_tag` off THIS message's `m.fields` (R2) as the **normal** source. NOT reused from `Validator.hpp`'s flat header-polluted `<Msg>_rules` (R3), and NOT from the read emitter's version-wide `MemberMap` union (R7).

**Caveat (RC#3 — dedup collapse)**: `append_run` tag-sorts + tag-dedups the per-message run (`xml_loader.cpp:695-702`), so a tag present at ≥2 levels within one message can collapse to a single `FieldRef` and drop from a level's set. A mandatory /tasks census (plan.md N3) enumerates these; any affected required set is instead sourced from **the codegen-local declaration walk, extended to carry per-member `rule` (and a top-level declaration set)** — NOT `group_order`, which carries member ORDER not `rule` and cannot recover a collapsed *top-level* field.
...
**Delimiter tag** for `group_begin(no_tag, delimiter_tag)` = the group's first DECLARED member in THIS message's occurrence, taken from `MessageIR.group_order` (`feedback_group_delimiter_from_groupref_not_tagsorted_members`), supplied by the emitter at codegen time (author-supplied contract of `body_builder`; no runtime wire→dict edge). Because it is per-message declaration order, W's `NoMDEntries` delimiter is `MDEntryType(269)` and X's is `MDUpdateAction(279)` from the same `no_tag`; deriving it from the tag-sorted `m.fields` would wrongly pick 269 for X (269 < 279) — the RC#7 defect.
...
| INV-ORDER | this feature (R1/R9) | top-level tag-ascending (from tag-sorted `m.fields`); group-entry **declaration** member order (from `MessageIR.group_order`, NOT `m.fields`); instances follow their `No<G>` tag position. |
