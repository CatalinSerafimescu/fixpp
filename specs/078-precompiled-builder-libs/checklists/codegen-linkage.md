...
- [x] CHK004 Are requirements defined for where `builder_registry` lives after the split, and which artifact each migrated gate re-points to (`all.hpp` vs the per-plan `groups/` dir)? [Completeness, data-model Entity 5/8] — PASS: Entity 5 pins `builder_registry` to `all.hpp`; Entity 8 explicitly calls out the two distinct re-point targets (`builder_registry`→`all.hpp`, `G_<no_tag>Args`→per-plan `groups/<Plan>.hpp`), consistent with the T016/T017 dispositions.
- [x] CHK005 Is the golden artifact requirement redefined as a file *set* (not a single header) with an explicit inventory? [Completeness, FR-010, research R6] — PASS: FR-010, research R6, data-model Entity 7, and completeness-and-golden.md "Golden set" section all enumerate the same explicit file inventory (per-plan `groups/<Plan>.hpp` set + umbrella `groups.hpp`+`validators/traits.hpp`+per-message 5-file set+`all.hpp`).
...
- [x] CHK014 Is SC-001's "proportional-to-closure, materially below the monolith" claim anchored to the measured monolith baseline (~3.6 GiB RSS) and routed to a concrete witness (compile-bench record), not a hard ctest gate? [Measurability, SC-001 (AMENDED), research R9, L-078-1] — PASS: SC-001 cites the baseline inline and the amended proportional-to-closure figures (v44 order-of-magnitude MET; v50sp2/vlatest median ~7.9×/common ~4.2×/dense ~2.6×, not order-of-magnitude); R9's SC→witness table routes it to the 003/T046 compile-bench decision-record convention (explicitly NOT Article VIII); T023 implements it, `.specify/decisions/078-precompiled-builder-libs-verify.md` records the measurement.
...
- [x] CHK021 Is the "shared group headers (`groups/<Plan>.hpp`) included from many per-message headers" case covered by an include-once requirement? [Edge Case, FR-012] — PASS: FR-012, Entity 1 invariants (`#pragma once`), T029 explicitly tests the FR-012 include-once property via `all.hpp`.
...
- [x] CHK023 Are requirements defined so an admin-only / no-builder version (vt11) yields a coherent absence (no emitted target/aggregator) rather than a broken build? [Edge Case, spec §Edge Cases, A7] — PASS: T014 asserts no `<ver>/all.hpp`/`messages` dir is emitted for vt11/v42; `Codegen.cmake:550` builds targets only for v44/v50sp2/vlatest.
