...
1. **Builder ⟂ validator (SC-005):** the `build_` include graph **and object** never reach validator traits/symbols — the builder `.hpp`/`.builder.inl`/`.builder.cpp` include only trait-free `groups.hpp`. Emitter invariant + `nm` witness + R2a probe.
