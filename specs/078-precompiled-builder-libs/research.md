...
1. **SC-005 / FR-005** — a builder-only consumer must carry **zero** `validate_`/traits machine code. *(Note: unreferenced `inline` template specializations are not ODR-used and emit **no machine code**, so merely including a traits header in a builder-only TU does not by itself break SC-005 machine-code — the machine-code leak is the co-located-object problem, R1/root-cause-#1. What placing traits in the builder-included `groups.hpp` breaks is the **R2 invariant + SC-001 parse cost**: every builder slim header would parse the whole validator trait surface.)*
...
- The shared traits stay `inline` (explicit specializations → one definition across TUs), so linked-lib + force-inlined validators that share a plan collapse to one definition — ODR-safe mixing.
- Because neither `validators/traits.hpp` nor the per-message validator surface is reachable through the builder include graph, a builder-only include never pulls trait machinery → SC-005 + SC-001 hold.
...
**Decision.** A documented macro `FIXPP_BUILDERS_HEADER_ONLY` selects builder-inline mode; a separate `FIXPP_VALIDATORS_HEADER_ONLY` selects validator-inline mode; per-message override macros (e.g. `FIXPP_BUILDERS_HEADER_ONLY_<Msg>` / `FIXPP_VALIDATORS_HEADER_ONLY_<Msg>`, or a small opt-in `#include "messages/<Msg>.builder.inl"` directly) let a client force-inline a chosen subset per side. `messages/<Msg>.hpp` branches per side: default → `extern` declaration (link mode); builder-inline set → `#include "<Msg>.builder.inl"`; validator-inline set → `#include "<Msg>.validator.inl"`. Because the inline surfaces are **split**, force-inlining `build_<Msg>` pulls **only** `<Msg>.builder.inl` (+ `groups.hpp`) and no validator trait (SC-005 holds on the inline path too). Mixing (link the bulk, inline a few) is ODR-safe because (a) the `.inl` inline `build_`/`validate_` are the standard "inline definition may appear in multiple TUs, must be identical" and (b) the shared group-plan traits are inline in one header (`validators/traits.hpp`, R2). Validated by the R2a probe extended into the FR-007 test (both a builder-side and a validator-side mixing witness).
