...
- **C-ABI ≡ C++ typed path** on the represented **depth-1** layout: both exclude `T` from `N`'s extent (SC-005), because both call `consume_group_extent` via the same primitive under the same `{msg_type,[outer]}` context. Verified by **BOTH** FR-011 witnesses [clarify 2026-07-09]: **(a)** a direct `as_table_view()` witness (inline-XML `load_from_string` in `message_read_test.cpp`) for the nested-extent arithmetic, AND **(b)** an engine-loopback witness (via `capi_dict066_loopback_support.hpp`, in a dict066-style loopback target) for production dispatch-path context threading (the 063 Gate-B RC#1 empty-`msg_type` class). C7 requires both — the loopback witness pins the exact parked-P1 regression seam and must not be narrowed away. (Depth-≥2 equivalence excluded — C5a.)
