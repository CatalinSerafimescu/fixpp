...
**Approach**: delete the hand-rolled scanner and delegate nested-instance slicing to the existing membership-aware primitive `OffsetTable::nested_group_slices` (062), which bounds every instance via the same context-threaded `consume_group_extent` walk (063) that makes the C++ typed path correct — so the two paths agree by *sharing the primitive*, not by re-deriving membership (FR-005). The only new input the C-ABI cursor lacks is the nested-descent **context**; carry it as one `group_context` field on the **internal** `fixpp_group` cursor (zero public-ABI) plus one convenience overload on `OffsetTable` (public C++, non-ABI). This is the membership-aware C-ABI follow-up feature that 063 `plan.md:106` explicitly deferred; the change is a net deletion + primitive reuse, answering 063 Round-2's "gratuitous rewrite" objection (research Decision 5).

## Technical Context

**Language/Version**: C++23. **Primary surface**:
- `src/capi/message_read.cpp` — `fixpp_group_get_nested_group` (`:462-615`): replace Phase-1/Phase-2 hand-rolled scanner with a `nested_group_slices` call + absent-vs-empty presence probe (research Decision 1/6); `fixpp_msg_get_group` (`:336-380`): seed `grp->group_ctx = view->offsets().stored_group_context().pushed(group_tag)`.
...
- **Appendix A mandatory triggers**: **C-ABI surface** (behavioral correction of a C-ABI read; no symbol change) AND **Wire/parser-adjacent** (nested-group extent semantics) → run the mandatory controls: `/speckit-clarify` (done — scan performed, no critical ambiguity; the degradation fork resolves by-construction, research Decision 3), Codex **Gate A** (pending — `## Gate A` below), `/speckit-analyze` (pending — after `/tasks`, per `plan → Gate A → tasks → analyze`), user `/plan` sign-off (pending). Full **Gate B** before merge.
...
**Post-design re-check** (after Phase 1): the two surface additions — the internal `fixpp_group::group_ctx` field and the `OffsetTable::nested_group_slices` convenience overload — are both **non-C-ABI**: the cursor struct is internal (public header only forward-declares it), and the overload is public C++ recompiled with the library. The C-ABI exported surface is byte-identical. Recorded in Complexity Tracking. No unresolved clarifications.
...
src/capi/message_read.cpp     # fixpp_msg_get_group (:336): seed grp->group_ctx = offsets().stored_group_context().pushed(group_tag)
...
**Structure Decision**: single-library layout; the change is confined to the C-ABI read layer (`src/capi/`) plus one inline convenience overload on `OffsetTable` (`include/fixpp/wire/`). No new modules, no codegen/goldens, no C-ABI symbol change.

## Complexity Tracking

> Two surface additions, both **non-C-ABI**, both required to reuse the shared membership primitive (the alternative — a bespoke C-ABI membership scan — is the greater complexity and is FR-005-forbidden).
