...
| `OverBound_SmallBoundSeam_SkipStoreButTransmit` (fault-injection, **>256-byte `35=A` frame** via `store_then_emit_test_access()`) | drives the dead over-bound branch **by injecting an over-bound frame against the real 256 bound**; asserts (a) **no cleartext persisted**, (b) the **wire frame still carries the real 554**. *(c) a resend over that skipped seqnum → SequenceReset-GapFill (not a masked verbatim replay) is **inherited, dead-defensive design behavior 034 does not change** — proven by the resend store-walk: a stored `35=A` (admin) or absent slot is classified non-app at `src/session/session.cpp:4815-4818` and folded into a SequenceReset-GapFill at `:4835-4841` (built by `build_sequence_reset_gapfill`, fired via `fire_to_admin_`). Not re-proven inside T010 (T010 comment lines 1213-1217). The `test_recovery_admin_span_gapfill.cpp` GapFill tests are illustrative of GapFill emission for resend ranges but exercise the short-store branch (`:4788`), not the stored-`35=A` classification.)* | C2 step-2 / I-07 / N1 (earns the over-bound BRDA) |
