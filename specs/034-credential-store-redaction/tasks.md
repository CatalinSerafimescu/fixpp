...
- Test-seam **accessor** `store_then_emit_test_access()`: `include/fixpp/session/session.hpp` (under `FIXPP_TEST_HOOKS`); the bound itself is a fixed `static constexpr 256` (not under FIXPP_TEST_HOOKS)
...
- [X] T004 Add the `kMaxMaskableLogonBytes` bound in `include/fixpp/session/session.hpp` as a **fixed `static constexpr 256`** (no compile-time override); the dead over-bound branch is exercised via the `FIXPP_TEST_HOOKS`-gated `store_then_emit_test_access()` frame-injection accessor (no public `SessionConfig`/ctor/template surface — Art. X; precedent: `kRpBufSize`). (R3 / N1)
...
- [X] T007 [US1] Implement the role-independent `open()`-time credential-length guard (extend the 033 FQ-1 FIXT-config validation in `src/session/session.cpp`): reject config when `username.size() + password.size() >= kMaxMaskableLogonBytes` (a one-directional sufficient-condition floor, no Logon-overhead accounting) — fires for BOTH initiator and acceptor open() (FR-004 both roles; makes the over-bound branch production-unreachable). The **primary** over-bound unreachability proof is `build_logon`'s fail-closed at the 256-byte buffers (FR-004 both roles). (C3 / N2)
...
- [X] T010 [P] Fault-injection witness `OverBound_SmallBoundSeam_SkipStoreButTransmit` in `tests/session/test_credential_store_redaction.cpp`: via the `FIXPP_TEST_HOOKS` `store_then_emit_test_access()` accessor feeding a **>256-byte `35=A` frame** against the real bound, drive the dead over-bound branch and assert (a) no cleartext persisted, (b) the wire frame still carries the real 554. T010 proves only (a)+(b). *(c) a resend over that seqnum → SequenceReset-GapFill (not a masked verbatim replay) is **inherited design behavior** proven by the resend store-walk classification (`session.cpp:4815-4818` classifies an absent slot or stored `35=A` as non-app; `:4835-4841` folds it into a GapFill run) — not proven by T010 or the recovery unit tests (T010 comment lines 1213-1217). The `test_recovery_admin_span_gapfill.cpp` GapFill tests are illustrative of GapFill emission but exercise the short-store branch (`:4788`), not the stored-`35=A` classification.* Earns the over-bound BRDA (no §IX.1 waiver). (N1 / C2-step-2 / I-07)
