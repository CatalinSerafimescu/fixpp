...
- **FR-008**: Masking MUST NOT allocate heap memory on the store-persistence path (self-imposed zero-allocation persist-path discipline, aligned with the `[const §XV.1]` per-message heap-avoidance banned pattern).
