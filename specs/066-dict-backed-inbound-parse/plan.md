...
| Reify owning handle carries the dictionary; dict-backed re-frame — threads a dict through `owning_message_handle_from_frame` (`reify.hpp:87`, takes no dict today) + the owning-handle pimpl + the build-tree dispatch bridge | FR-007: reify copies bytes and re-frames dict-free, losing the source's membership; the owning handle outlives the session so it needs an OWNED membership source (spike-confirmed the factory carries no dict). | Documenting reify divergence, or deferring to a follow-up — both rejected by the user (chose "both clone + reify in 066" after the spike surfaced this larger surface). |

## Phase notes
- **Phase 0** (done): research.md — the dict-free-parse root cause + evidence; the reuse-the-existing-dict-backed-Parser mechanism; the strict-membership/Orchestra reconciliation; clone/reify lifetime; validator-parse assessment; regression-risk + real-dispatch test strategy. **Pre-Gate-A spike (research Decision 8, grade-1): the dict-backed inbound parse breaks 0/147 existing session+capi+interop tests; the 065 depth-2 typed-gap concern is disproven (3-level typed read passes dict-backed); reify's larger propagation surface confirmed (factory takes no dictionary).**
