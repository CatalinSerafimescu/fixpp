...
- [X] T019 [P] FR-010: amend the **L-063-2** row in `spec/behaviors-and-limitations.md` (the "C++ typed read path is unaffected" claim is false on the shipped path until 066), and confirm the issue **#179** amendment comment (posted 2026-07-09) reflects the final resolution.
- [X] T020 [P] Run `quickstart.md` end-to-end validation (§1 RED reproduction → §7 065 prerequisite note).
- [X] T021 [P] **Catalogue close-out**: flip every 066-owned OFFICIAL row in `spec/feature-catalogue.md` → `done` (with PR/evidence ref) AND add/update its matching `spec/coverage-index.md` entry. (Gate-B precondition, Article XVII §8.)
