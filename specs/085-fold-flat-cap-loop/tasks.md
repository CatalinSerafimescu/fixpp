...
- [ ] T024 [P] **Catalogue close-out**: flip every feature-owned OFFICIAL row in `spec/feature-catalogue.md` to `done` (with the PR / evidence ref) AND add/update its matching `spec/coverage-index.md` entry. Also re-point the submodule `CLAUDE.md` open-issue count and in-flight pointer: **#214 closes with this feature** and **#220 is already open** (filed 2026-08-03 during `/speckit-specify`). Write the **post-merge** figure, not the mid-feature one: open issues stand at **9** right now (`196, 209, 211, 213, 214, 215, 217, 218, 220`), and closing #214 leaves **8**. Compute it at close-out rather than copying this number — the count in `CLAUDE.md` has already gone stale once (its own note records an earlier revision claiming "6" that was wrong).
