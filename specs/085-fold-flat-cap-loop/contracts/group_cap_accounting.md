...
| `group_slices_reserve_bound()` (`:599-634`) | Not reached by this change. ⚠️ **DELETED by #389** — the estimator's bound was computed from the WIRE delimiter while the loop it had to dominate re-split with the DICTIONARY one. 085's "not reached" verdict stands as history; the site no longer exists. |
