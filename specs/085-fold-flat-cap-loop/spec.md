...
- **FR-003a**: The dict-free cap's **trailing-field false positive** — its final segment extending to `entries_.size()`, so top-level fields after the group count toward one instance and can trip `err_group_too_large` on a group far below the cap — MUST be recorded as a named limitation in `spec/behaviors-and-limitations.md`, and MUST be filed as its own GitHub tracking issue before this feature closes out. It MUST NOT be repaired here. The filed issue MUST state that the shape is **reachable, not latent**: dict-free construction is a public, documented constructor (`OffsetTable(frame_view const&, memory_resource*)`, `include/fixpp/wire/offset_table.hpp:125`, described as "Dict-FREE construction" at `:161`), and `group()` is already exercised through it today (`tests/wire/offset_table_test.cpp:150-157`). *(Clarified 2026-08-03; reachability verified at source, 2026-08-03.)*
...
- **SC-010**: The dict-free trailing-field false positive (FR-003a) is recorded as a named limitation row **and** carries an open GitHub issue number cited from that row, before close-out. A row without an issue number does not discharge this criterion.
