...
- [X] T014 [US3] **DISCHARGED LOCALLY on MSVC x64 (windows-msvc-debug, `/mnt/c/temp/fixpp` sandbox — `project_tier2_windows_port`) 2026-07-04**: built `decimal.cpp` (exercising the `_umul128` `#elif` branch) + ran `decimal_compare_diff_oracle_test` **11/11** (corpus + 7-row witness matrix incl. hi-limb 2^64 / k-boundary + antisymmetry/transitivity), `decimal_mul_u64_wide_test` + `_portable` **4/4**, `DecimalCompare.*` AC-C1..C5 **16/16** — the `_umul128` wide-multiply yields **bit-identical ordering to the frozen reference on MSVC**. The local build ALSO caught a real cross-platform bug (the mul-test golden used `unsigned __int128`, unsupported on MSVC — fixed `5ff12dde`). Full 3-lane `windows-msvc-{debug,release,asan}` still runs on the PR's `run-tier2` CI as belt-and-suspenders (the KEY-LESSON debug≠release/asan variance is OOM-test skips, irrelevant to decimal compare). Local leg T012 (signatures vs MS docs) also DONE.
