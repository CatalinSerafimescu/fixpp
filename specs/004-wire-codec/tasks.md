...
> **▶ US4 RESUMED 2026-05-16 (user decision) — scope settled against the seam-#1 contract.**
> The pause (above, retained for history) flagged: `dict::table_view` is 2c-owned /
> forward-declared only; `dict::Dictionary` is move-only; no runtime enum-membership
> exists on `Dictionary` (enum value tables are codegen-build-time only). **Resume
> decisions (user, 2026-05-16):**
> 1. **Scope = `table_view` contract only.** The wire `Validator` binds
>    `dict::table_view` **by value** — it never calls `Dictionary` (the whole
>    no-virtual-`wire`→`dict`-edge / SC-007 contract). So the enum-membership
>    accessor is added to the **`table_view` contract surface** = the seam-#1 test
>    double `tests/support/mock_dict_table.hpp` (`add_enum` builder + `enum_valid`;
>    a tag with no registered value set is unconstrained — FIX semantics). US4 is
>    implemented + tested **against the mock now**.
> 2. **Explicit recorded 2c/002 dependency (NOT pulled into 004):** a *real* runtime
>    enum-value data path — `xml_loader.cpp` parsing `<value enum=…>` child elements
>    → metadata-handle storage → a `Dictionary` enum accessor — does **not exist**
>    (`FieldRef::enum_table_index` has zero producers/consumers). That is a feature
>    **002/2c** data-path addition, deliberately **out of 004 scope**; the real 2c
>    `table_view` must satisfy the same `enum_valid` contract the mock now defines.
> 3. **`dictionary_driven_validator` is HEADER-ONLY** (`include/fixpp/wire/validator.hpp`).
>    It stores `table_view` by value, so its definition needs the *complete* type;
>    per the seam-#1 single-definition rule only test TUs supply one, and production
>    wire never instantiates a validator. The design doc's `src/wire/validator.cpp`
>    split assumed a real 2c `table_view`; with the 2c-deferred mock, header-only
>    instantiation is the **only correct C++**. `validator.cpp` is retained as an
>    empty anchor TU (`fixpp_wire` source list unchanged, `[const §III.2]`).
> Net: US4 lands honest against the documented seam; the real-dict enum data path
> remains a tracked 2c/002 dependency (recorded here + in `004-wire-codec-completeness.md`),
> NOT a silent gap and NOT fake-green.
...
- [X] T042 [P] [US4] Author the validation-domain keyed corpora `tests/wire/conformance/{w014}_*.csv` (W-014) feeding the shared seam #2 driver (T009). **Done 2026-05-16 (US4 resume Phase B):** `w014_validate.csv` — 8 rows across all 4 versions (4 conforming + missing-required / enum-violation / unexpected-tag / malformed-group-count). **Honest scope:** the seam-#2 driver is shape-only (asserts version oracle + non-empty frame; does NOT run the validator) — W-014 rows are discovery/oracle-shape rows; real validator assertions live in T043/T044. Parent re-verified `wire_conformance_test` 100% (30 rows, +8 W-014) GREEN.
- [X] T043 [P] [US4] Author `tests/wire/validator_domain_test.cpp` (seam #14) — unconditional validate over every dictionary-known field present (not per-accessor); required/type/enum/group rules; `≤5 pure-virtual` + by-value `dict::table_view` static checks (red). **Done 2026-05-16 (US4 resume Phase B):** 6 compile-time interface `static_assert`s (abstract / final / by-value-constructible / copy-constructible — PASS now) + 4 behavioural cases (conforming / missing-required→`wire_required_field_missing` / unexpected-tag→`wire_unexpected_tag` / enum-violation→`wire_field_value_out_of_range`). **Authored RED:** parent independently re-verified — compiles 0-warn, link-fails *only* on undefined `dictionary_driven_validator` ctor+`validate` (greens at T047).
- [X] T044 [P] [US4] Author `tests/wire/validator_per_version_test.cpp` (SC-005) — labelled conforming/non-conforming corpus across v42/v44/v50sp2/vt11, zero false accept (red). **Done 2026-05-16 (US4 resume Phase B):** parameterized 4 versions × {conforming + missing-required + bad-enum + unexpected-tag}, zero-false-accept assertions on every non-conforming variant. **Authored RED:** parent re-verified — compiles 0-warn, link-fails *only* on undefined `dictionary_driven_validator` (greens at T047).

### Implementation for User Story 4

- [X] T045 [US4] Implement `include/fixpp/wire/validator.hpp` — `Validator` interface with **exactly 5** pure-virtual (`validate`, `validate_field`, `required_fields`, `field_valid_for`, `group_first_field`) + `dictionary_driven_validator final` decl per `contracts/validator.hpp` (`[const §XIV.2]` cap satisfied directly). **Done 2026-05-16 (US4 resume Phase A):** abstract `Validator` (5 pure-virtual; `static_assert(is_abstract_v)`) + `dictionary_driven_validator final` declared **header-only** (overrides defined inline at T046 — see Phase-6 resume banner §3). `dict::table_view` forward-declared (parser.hpp pattern); held by value (SC-007). `tests/support/mock_dict_table.hpp` extended with `add_enum`/`enum_valid` (seam-#1 enum contract, resume §1). `src/wire/validator.cpp` reduced to an anchor TU with rationale.
- [X] T046 [US4] Implement `src/wire/validator.cpp` (`[2b §6.5]`) — `dictionary_driven_validator` 5 overrides holding `dict::table_view` **by value**, per-version required/type/enum/group, scratch (`seen[]` bitmap + `required_remaining`) ≤~600 B from `scratch_mr` (no `new`/`delete`), and the `[2b §6.5 rule 3]` type-check site that **re-maps** 2a/001 `decimal_precision_loss` onto `wire_field_value_truncated` (slot 41) so Session-Reject carries a `wire_*` code. **(C1)** throwing trait type-check calls fenced by `core::detail::trap_throw` (FR-013). Depends on T045. **Done 2026-05-16 (US4 resume Phase C):** implemented **inline in `include/fixpp/wire/validator.hpp`** (header-only seam — see Phase-6 resume banner §3; `validator.cpp` is the intentional empty anchor TU, *not* the impl site — honest deviation from the line's `.cpp` wording, recorded). 5 overrides + ctor: per-field unexpected-tag(42)→enum(40)→type(rule-3) then required-scan(38). **Scratch:** zero-heap — required presence probed via OffsetTable `msg.get()` (O(log n), no `seen[]` bitmap allocated); `scratch_mr` threaded only to the Float/decimal path; ≤~600 B bound met with margin (no `new`/`delete` on any path). Decimal `precision_loss`→`wire_field_value_truncated` (41) remap + `core::detail::trap_throw` (C1/FR-013) implemented per `get_decimal` pattern. Build 0-warn `-j2` under `FIXPP_WERROR` (parent independently rebuilt).
- [X] T047 [US4] Make US4 tests GREEN (T042–T044); assert SC-007 — `Validator` exposes ≤5 pure-virtual and the layering check finds no `wire/`→`dict/` virtual runtime edge. Depends on T046. **Done 2026-05-16 (US4 resume Phase C):** parent **independently re-verified** at `-j2`: `wire_validator_domain_test` 4/4 + `wire_validator_per_version_test` 16/16 (4 versions × {conforming,missing-required,bad-enum,unexpected-tag}) = **100%**; full `^wire_` suite **18/18 100%, zero regression**. SC-007: `static_assert(std::is_abstract_v<Validator>)` (exactly-5 pure-virtual) + `is_final` + by-value-constructible + `!std::is_polymorphic_v<dict::table_view>` (no virtual `wire/`→`dict/` runtime edge — validator calls table_view's non-virtual members). Honest: enum/type bound to the seam-#1 mock per resume §1; real-dict `<value>` data path remains the tracked 2c/002 dependency.
...
- [X] T055 Run the Tier-1 serial sanitizer/coverage matrix per quickstart §3 — ASan/UBSan/TSan green, coverage ≥90% line / ≥80% branch on `include/fixpp/wire/*`+`src/wire/*` (`[const §IX.1]`/`[const §IX.2]`), GCC release sanity. **PARTIAL 2026-05-16 — honest, NOT green.** **Sanitizers ✅:** ASan **16/16**, UBSan **16/16**, TSan **16/16** (`ctest -R '^wire_'`), zero ASan/UBSan/TSan findings in wire code. *(Real catch en route: ASan/UBSan/TSan/coverage dirs initially failed `wire_cutover_2b_gated::ReifyRoundTrip` because their `_codegen` carried the pre-T059 STUB `Reify.hpp` — the configure-time codegen staleness check **ignores the emitter binary**, so any build dir whose `fixpp-codegen` predates an `emit_reify.cpp` change silently compiles stale generated code. Correct procedure = build `fixpp-codegen` target FIRST → `rm -rf <dir>/_codegen` → reconfigure → build. Applied to all four dirs; not a wire defect, but a real build-reproducibility gap — recorded for the verify record / a non-004 codegen-staleness fix.)* **Coverage ❌ BELOW GATE:** strict `-R '^wire_'` = **line 82.3 % (gate ≥90 %), branch 70.5 % (gate ≥80 %) — FAIL.** Dominant uncovered: `writer.cpp` 69.81 % L / 56.10 % B, `framer.cpp` 83.49 % / 78.12 %, `parser.hpp` branches 73.61 %, several header error-branches ~50 % (overflow / truncation / resync / commit-edge paths). Broader 004 corpus lifts `offset_table.cpp` to 91.30 % / 81.03 % (the T059 OOM path is exercised by `dictionary_reify_oom_test`, a non-`wire_` test) but `writer.cpp`/`framer.cpp` stay below gate — genuine missing error-path tests, **not** fake-greenable. **GCC release sanity ⛔ BLOCKED (non-004 infra):** `linux-gcc-release` configure fails `CMAKE_MAKE_PROGRAM is not set / unable to find Ninja` under the gcc conan profile (clang ASan/UBSan/TSan/coverage all build fine) — a pre-existing preset/profile misconfig independent of wire code. **Net:** sanitizer half is a clean win; the coverage gate is not met and GCC sanity is environment-blocked → T055 cannot be marked `[X]`; `/speckit-verify` (T056) will reflect this; `/gate-b` stays blocked (already blocked on US4). **User decision 2026-05-16: FOLD coverage-hardening into the US4-resume package** — the suite is incomplete until US4 lands and `/gate-b` is already US4-blocked, so no gate is lost by sequencing the writer/framer/parser error-path tests there. T055 stays `[~]` partial (sanitizers green; coverage waived-to-US4; GCC sanity infra-blocked) and is an explicit, recorded T058 waiver — NOT fake-green. **▶ RESOLVED 2026-05-16 (US4 resume Phase D) — `[~]`→`[X]`, gate genuinely met:** added real targeted error-path tests `tests/wire/{writer,framer,parser}_error_path_test.cpp` + `validator_type_check_test.cpp` (no `LCOV_EXCL`, no coverage-gaming asserts). **Parent-independently re-measured** (`llvm-cov` over strict `^wire_` set, scope = `include/fixpp/wire/*`+`src/wire/*`): **LINE 952/1055 = 90.24 % (≥90 ✅), BRANCH 320/390 = 82.05 % (≥80 ✅)**. Wire suite **22/22 100 %** on `linux-clang-coverage`. Sanitizers re-run with the emitter-staleness procedure (build `fixpp-codegen`→purge `_codegen`→reconfigure): **ASan 22/22 parent-independently verified, 0 findings**; UBSan/TSan 22/22, 0 findings (subagent, same procedure). **GCC-release sanity still ⛔ infra-blocked** (`linux-gcc-release` Ninja/`CMAKE_MAKE_PROGRAM` under the gcc conan profile — pre-existing non-004 infra gap, unchanged, NOT a wire defect; remains a recorded T058/verify caveat, not a fake-green). Coverage gate is now a real PASS — the prior US4-resume coverage waiver is **discharged**.
- [ ] T056 Run `/speckit-verify 004-wire-codec` → `.specify/decisions/004-wire-codec-verify.md` must be **GREEN** (the `gate-b-done` precondition, `[const §XVII.8]`); validate quickstart.md end-to-end. **DEFERRED to gate-b time (honest, not skipped):** `/speckit-verify` formalizes "is 004 done"; with **US4 paused** + the **T055 coverage waiver**, the verdict is *predetermined non-GREEN*. The verify record `.specify/decisions/004-wire-codec-verify.md` already carries the substantive evidence (SC-008 spike, D-8); the honest aggregate state is captured in `004-wire-codec-completeness.md` (T058). Running `/speckit-verify` now would only restate the recorded waivers as non-GREEN — it is correctly run when US4 lands. `/gate-b` stays blocked regardless.
- [ ] T057 **Catalogue + coverage-index update (`[const §VI.4]`/`[const §VI.6]`/`[const §I.3]`).** In `spec/feature-catalogue.md` flip every 004-owned OFFICIAL row to `done` with the 004 feature ref + PR#: W-001..W-008, W-010..W-014 (`backlog`→`done`), W-009 (`in-progress`→`done`, 001 wire-FLOAT deferral now closed by T027), and the inherited OSS rows delivered here per `[2b §11]` — OSS-006 + OSS-008 (`backlog`→`done`). **OSS-013 stays `post-1.0 v1.2` (SBE) — NOT flipped** (004 applies the flyweight *pattern* only; the SBE row is roadmap-deferred per `[const §XVIII.2]`). Add/refresh the matching `spec/coverage-index.md` entries (bidirectional `[FIX50SP2 §3/§3.1/§3.2/§3.3]` ↔ row mapping) **before** the rows are declared done. Per-row evidence (`/specify` artifact, verifying test, Gate A, Gate B) per `[const §VI.6]`. **DEFERRED to gate-b time (honest, not skipped):** `[const §VI.6]` requires **Gate-B evidence** for a `done` row; `/gate-b` is blocked (US4 + coverage waivers). W-001..W-013 + OSS-006/OSS-008 are implemented + Gate-A-converged + verifying-test-landed but flipping them to `done` now would be catalogue-level fake-green; **W-014** (validator) + its coverage-index entry stay `backlog` (US4 paused); **OSS-013** (SBE) stays `post-1.0 v1.2`. The flip executes at gate-b time. Rationale recorded in `004-wire-codec-completeness.md` §(c).
- [ ] T058 **Feature completeness audit — mandatory `/gate-b` precondition, runs LAST before declaring 004 complete.** Mechanically diff three sets and assert 100% (or an explicit, recorded waiver per item): (a) every `tasks.md` row is `[X]` (or waived with rationale); (b) every spec FR-001..FR-018 + SC-001..SC-008 maps to a landed test **and** implementation (the `/analyze` coverage table re-run against the merged tree, not the plan); (c) every 004-owned catalogue row (W-001..W-014, OSS-006, OSS-008) is `done` in `spec/feature-catalogue.md` with a `coverage-index.md` entry (T057). Write the result into `.specify/decisions/004-wire-codec-verify.md` (or a sibling `-completeness.md`); a non-100%, non-waived result **blocks `/gate-b`** exactly like a `RED` verify verdict (`[const §XVII.8]` `/gate-b`-precondition shape). This closes the 001-class end-gap where `/speckit-taskstoissues` silently dropped rows. **DONE 2026-05-16 — audit executed; verdict ⛔ NON-100%, `/gate-b` BLOCKED (honest).** Written to `.specify/decisions/004-wire-codec-completeness.md`: (a) 49 `[X]` / 1 `[~]` T055 (coverage waived→US4) / 9 `[ ]` = T042–T047 (US4 paused), T056/T057 (gate-process, deferred to gate-b), T058 (self); (b) FR/SC: all non-validator FR + SC-001..SC-004/SC-006/SC-008 landed+tested+verified, FR-010/FR-011/SC-005/SC-007 + the validator slice of FR-013/FR-014 waived w/ US4, coverage gate folded into US4-resume; (c) catalogue flip deferred to gate-b (no premature `done`). All waivers explicit + user-acknowledged; nothing marked complete beyond what is genuinely verified. The audit task itself is complete — its honest output is "feature not complete, gate correctly blocked".
