...
- [~] T031 [US1] Make all US1 tests GREEN (T011–T020); run the quickstart §9 cutover sanity — `grep -rn 'message_view_contract' include/fixpp/dict tests/` shows only the kept re-export shim and **zero references to the frozen-stub surface** remain anywhere in the tree (SC-006). Depends on T023–T030. **PARTIAL — honest 2026-05-16**: US1 tests + the FLOAT-accessor cutover leg GREEN (`ctest -R '^wire_|codegen_|dictionary_'` 100%); stale frozen-stub comments reconciled in `reify.hpp`/`field_traits.hpp`. **The SC-006 003-reify round-trip half is NOT green and is NOT fake-greened**: `cutover_2b_gated_test::DISABLED_ReifyRoundTripOnRealMessageView` stays `DISABLED_` (would be RED, not GREEN — see T059). T031 closes only when **T059** lands. SC-006 is therefore **partially** satisfied (FLOAT leg ✅, reify round-trip ⛔ → T059) and is a `/gate-b` blocker until T059 (T058 audit will catch this).

- [ ] T059 [US1] **Carved out 2026-05-16 (was invisibly folded into T029/T031).** Make the 003 `dict::reify` *round-trip* real on the cutover surface (SC-006 completion): (a) change the **`fixpp-codegen` emitter** so generated `<vXX>/Reify.hpp` `owning_<Msg>::from_view` no longer returns the `dict_reify_wire_body_not_ready` R6 stub but performs the real owning-message deep-copy (frame bytes → caller PMR arena → reconstructed `wire::MessageView<Index>`, values survive source-arena reset); (b) implement the matching `owning_message_handle` SBO/polymorphic owner in `src/dictionary/reify.cpp` (retire the `pimpl_` R6 stub); (c) flip `tests/wire/cutover_2b_gated_test.cpp::DISABLED_ReifyRoundTripOnRealMessageView` to an enabled, real round-trip test (parse → `reify_as<v44::NewOrderSingle>` → typed field readback byte-identical) and remove the `#ifndef FIXPP_R6_WIRE_BODY_READY` skips in `tests/dictionary/reify_test.cpp`; (d) re-run the quickstart §9 sanity. Depends on T024, T028, T029. **This completes SC-006 and is a hard `/gate-b` blocker** (T058 audit precondition, `[const §XVII.8]`).

**Checkpoint**: US1 parse path + the **type-surface** cutover are functional & **independently testable via the `frame_view_factory`** — real `MessageView`/`OffsetTable`/`field_view`, the FLOAT-accessor cutover leg GREEN, 001 deferred work unblocked. **The 003-reify round-trip half of SC-006 is carved into T059** (codegen `from_view` + owning deep-copy) and is not yet green. **This is the MVP critical path; T059 is its honest remaining completion.**
