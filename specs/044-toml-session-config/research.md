...
| `transport` (tls/plaintext) | engine `default_transport_factory` (by `kind()`) + per-session `reconnect_endpoint` | `make_asio_tls_transport_factory(cfg, ssl_cfg)` / `make_asio_plain_transport_factory(cfg)` | **ready** (see D-6, D-6a) | **host/port → `SessionConfig::reconnect_endpoint` ONLY** (the connect target; `scheme` picks the factory `kind()`) |
...
- One **engine-shared** `default_transport_factory` per `kind()` serves **all** sessions of that scheme; each session's distinct endpoint lives in its own `reconnect_endpoint`. The earlier "host/port → `Transport::Config`" half of D-5 is dropped.
