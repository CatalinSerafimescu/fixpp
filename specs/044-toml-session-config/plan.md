...
| **VII — Testing/TDD** | PASS (fuzz required) | TDD mandatory; negative-file battery + equivalence + parity tests. **§VII.7 fuzzing (parser-touching MUST, Gate-B blocker):** the loader is a TOML parser front-end, so a **libFuzzer harness over malformed TOML (T037)** is required — it directly validates the feature's central guarantee that no malformed input ever terminates (every throwing parse/construction site → a `LoadDiagnostic` via the noexcept-boundary wrapper, FR-012/rule 9). ≥10-min corpus run at Gate B. **NOT waived** (evaluated at `/analyze` D1; analyzer flagged the bare-PASS omission). |
