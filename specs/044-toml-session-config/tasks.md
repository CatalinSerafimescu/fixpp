...
- [X] T026 [P] [US3] **Selector-kind matrix** in `tests/config/test_load_selectors.cpp` + fixtures: each kind resolves to the requested built-in (assert the concrete factory/store/transport type or behavior); `store=memory` reuses the existing `MemoryStoreFactory`; `transport=plaintext` builds the plain factory with `kind()==plaintext`.
- [X] T027 [P] [US3] **Divergent-cert multi-session test**: two `[[session]]` with different cert/profile → each gets a freshly-minted `transport_factory_override` with `use_count()==1` (never a shared override); a session matching the engine default uses the shared `default_transport_factory` (D-6a).
- [X] T028 [P] [US3] **Deferred-selection + FR-011 guard cells**: `dictionary.kind="version"`, `dialect_overlay`, `security_profile.kind="mtls_pinned"` each → `recognized_not_yet_supported_step2`; `mode=direct_executor` without attestation fails closed (US3 AC-4).

### Implementation for US3

- [X] T029 [US3] Harden the selector resolver registry in `src/config/selector_resolver.cpp` so each `seam`+`kind` maps to its built-in with the legal-set diagnostic on miss; wire the divergent-cert `transport_factory_override` minting path (one-owner, use_count==1). Make T026–T028 pass.
...
- [X] T030 [P] [US4] **Parity-table test** in `tests/config/test_quickfix_parity_table.cpp`: enumerate the QuickFIX-cpp `SessionSettings` establishment keys **from the cloned reference source** (`reference-engines/quickfix-cpp`, per D-10 — NOT from memory); assert each maps to a documented 044 key or an explicit out-of-scope/deferred disposition (schedule keys = out-of-scope, with rationale). Emit the parity table doc artifact (SC-004).
- [X] T031 [P] [US4] **Multi-session defaults test** in `tests/config/test_load_multisession_defaults.cpp` + fixture: a `[default]` + multiple `[[session]]` file; each session inherits defaults, honors per-session overrides, and an override of a key absent from `[default]` is valid (SC-005).

### Implementation for US4

- [X] T032 [US4] Finalize the parity-key spellings (already pre-aligned to QuickFIX cfg) + author the parity-table doc; ensure the `[default]`-merge (T009) satisfies T031. Make T030/T031 pass.
...
- [X] T033 [P] **Catalogue + coverage-index** (Article VI §4 — NAMED pre-merge task): add the `[const §XV.16]` **design** row for 044 to `spec/feature-catalogue.md` (design row, NOT `OFFICIAL`) AND a design-choice note to `spec/coverage-index.md` (both files). Run the catalogue-consistency check. **Also reconcile the two implementation-discovered deviations noted above.** DONE: row T-043 added to new "Design-Blessed Features" section in feature-catalogue.md; coverage note added to coverage-index.md after 043 entry; B-044-1 (T039 Gate-B blocker), L-044-1 (reject_policy step-1 limit), L-044-2 (redundant dual-diagnostic) added to behaviors-and-limitations.md. No catalogue consistency-checker script exists in tools/ — the ctest suite does not include one. `/speckit-analyze` re-run RECOMMENDED before Gate B (data-model E-6 was changed to add reject_policy to the deferred set — a SPEC-FIXED-class edit; the change is contained to data-model prose and the B&L limitation row, no new FR/SC).
- [X] T034 [P] Run `quickstart.md` end-to-end as an example check (load the example file + host-completion stub compiles/loads); add to `examples/` if a runnable example is warranted (`[const §XIX.3]`). DONE (with drift fix): confirmed against `happy_full.toml` fixture + selector resolver source. Three drifts found and fixed in `quickstart.md`: (1) engine-level selectors (`[clock]`, `[store]`, `[dictionary]`, `[cert_source]`) were incorrectly placed under `[default]` (`[default.clock]` etc.) — the resolver reads them from `root_tbl` directly, NOT from the `[default]` sub-table (selector_resolver.cpp lines 51/125/208/298); moved to ROOT level; (2) cert_source key names were wrong: `leaf_path`/`private_key_path`/`ca_bundle_path` → `cert_file`/`key_file`/`ca_file` (confirmed by `happy_full.toml` lines 30-32 + resolver lines 261-263); (3) per-session `[session.cert_source]` removed — with a root-level engine-default cert_source the session's override entry would only trigger the D-6a divergence scan, not be read as the engine-default (session-level certs are divergence-only). Host-completion C++ stub verified against real API: all field names and signatures match. No `examples/` program added (not warranted — 5 test binaries cover load path; see note above). Coverage-index production-surface also corrected: removed non-existent `merge_defaults.cpp` file (merge_defaults is a static fn in toml_config_loader.cpp); added `loader_internal.cpp/hpp`.
- [X] T035 [P] Add the `tests/config/CMakeLists.txt` registrations for all five test binaries + labels; confirm `ctest -R '^config'` selects them. DONE: all 5 binaries already registered with LABELS "config;044". `ctest -L config` selects 6 tests (the 5 new 044 tests + `transport_asio_plain_transport_config` which also carries "config" in its labels from 043). All 5 044-labeled tests pass. RECONCILIATION: the tasks.md wording "`-R '^config'`" is INCORRECT — test names are `test_load_*`/`test_quickfix_*` (not `config*`), so `-R '^config'` matches nothing. The canonical selector is `-L config` (by label). This note supersedes the `-R '^config'` wording.
- [X] T036 `/speckit-verify` gate prep: confirm coverage ≥95/85 on `src/config/` + `include/fixpp/config/` (the loader is synchronous cold-path; no new concurrency), and the 6-preset matrix is green. (Runs in `/speckit-verify`, not here — this task is the checklist hook.) **HOOK NOTE: deferred to the `/speckit-verify` step per the task's own scope; the config tests (happy/negative/selectors/multisession/parity) are green on linux-clang-debug. The full 6-preset matrix + lcov DA/BRDA coverage measurement run in `/speckit-verify`.**
- [X] T037 **libFuzzer harness** (analyze D1 / `[const §VII.7]` — parser-touching MUST, Gate-B blocker) in `tests/config/fuzz/fuzz_toml_loader.cpp`: feed arbitrary bytes to `load_toml_config(tmpfile, LoadOptions{exec,res})` and assert it NEVER terminates/UB — every input yields either a `ConfigBundle` or a `vector<LoadDiagnostic>` (the central FR-012/rule-9 guarantee: no throw escapes the noexcept boundary). Seed corpus from the US1/US2 fixtures; wire into the fuzz CI target; ≥10-min corpus run at Gate B. (Logically validates US2's fail-closed promise.) **DONE: harness at tests/config/fuzz/fuzz_toml_loader.cpp; wired in tests/config/fuzz/CMakeLists.txt + top CMakeLists.txt FIXPP_BUILD_FUZZ guard; 5 seeds in tests/config/fuzz/corpus/toml_config_loader/. Smoke (20k runs/9s) found a genuine loader defect (ESCALATED — see Escalations): TOML_ASSERT_ASSUME in tomlplusplus debug builds fires assert() on malformed table headers (e.g. "[ [key]]"), bypassing the try/catch noexcept wrapper → process abort, violating FR-012/rule-9. In release, TOML_ASSERT_ASSUME expands to __builtin_unreachable() (UB) on the same path. Reproducer at tests/config/fuzz/crashes/repro_toml_assert_assume.toml. Defect NOT patched — fix requires an orchestrator design decision (see Escalations). Seed corpus in tests/config/fuzz/corpus/toml_config_loader/ does NOT include the reproducer (known-crashing inputs must not be seeds).**
- [X] T038 [P] **FR-021 migration note** (analyze E1): author a short section (in `quickstart.md` or `book/migration_from_quickfix.md`) stating explicitly that moving configuration into text **relocates type/spelling errors from build time to load time**, and that the mitigation is strict load-time validation (FR-012–FR-018) with per-key fail-closed diagnostics. This is the FR-021 documentation obligation, currently uncovered. **DONE: "Migration note: build-time → load-time error relocation (FR-021)" section appended to `specs/044-toml-session-config/quickstart.md`.**
- [X] T039 **[GATE-B BLOCKER — fuzzer-found FR-012/rule-9 defect]** tomlplusplus 3.4.0 aborts/UBs on certain malformed TOML instead of routing through the noexcept error path. Reproducer: `tests/config/fuzz/crashes/repro_toml_assert_assume.toml` (malformed table header `[ [key]]`). Mechanism: `parse_key()` (`parser.inl:3037`) fires `TOML_ASSERT_ASSUME(...)` on attacker-controlled input → **debug/ASan**: `assert()`→`abort()` escapes `load_toml_config`'s `try/catch` (abort is not an exception); **release/NDEBUG**: `__builtin_assume(false)` = UB. Verified reproducible (deadly signal up through `load_toml_config:349`). **Why a pre-define alone fails:** under `NDEBUG` toml++ `#undef`s any pre-defined `TOML_ASSERT` (preprocessor.hpp:1174-1177) and routes `TOML_ASSERT_ASSUME → TOML_ASSUME → __builtin_assume` (`TOML_ASSUME` has NO `#ifndef` guard). **Recommended fix (advisor):** a SINGLE shim header `src/config/toml_include.hpp` that, before `#include <toml++/toml.hpp>`, does `#undef NDEBUG` (save/restore) so the overridable `TOML_ASSERT` path is active in all builds, and `#define TOML_ASSERT(expr)` to **THROW** a catchable exception (NOT no-op — throwing unwinds cleanly into the existing catch as a `parse_error` diagnostic and never continues past the violated invariant; no-op would continue past it = a memory-safety question). **ALL config TUs must include the shim, never `<toml++/toml.hpp>` directly** (ODR: `#undef NDEBUG` changes inline/template bodies — mixing shimmed and un-shimmed TUs is UB). Currently 2 direct include sites: `src/config/mappers.hpp:19`, `src/config/toml_config_loader.cpp:26`. **Validation bar (Gate-B-scale, do NOT shortcut):** reproducer must yield a `parse_error` diagnostic (not a signal); then a ≥10-min ASan fuzz from the seeded corpus with zero crashes, repeated under BOTH a non-NDEBUG and an NDEBUG/release build (the pre-define-wiped path). Caveat to confirm: throwing `TOML_ASSERT` is only safe if no toml++ `noexcept` accessor asserts — the post-parse code uses only safe accessors (`value<>`/`get`/`as_table`+nullcheck), so only the (non-noexcept) parse-path asserts are reachable; confirm before trusting. Deferred from this `/implement` session per advisor: the harness (T037) is delivered; the fix's correctness rests on long fuzz validation in two build modes, which is Gate-B work and too delicate to rush at session tail. **DONE (2026-06-20):** implemented exactly as recommended — new `src/config/toml_include.hpp` shim (`#undef NDEBUG` save/restore + throwing `TOML_ASSERT` raising `std::logic_error`); both direct include sites (`mappers.hpp:19`, `toml_config_loader.cpp:26`) re-pointed to the shim (covers all 3 toml++-instantiating TUs: loader + scalar_mappers + selector_resolver via mappers.hpp; fuzz harness includes only the public header, no toml++ bodies). **Validation (full bar, not shortcut):** (1) RED confirmed first — unfixed asan fuzzer aborts at `parse_key():parser.inl:3037` on the reproducer; (2) reproducer GREEN post-fix in **both** asan/Debug AND release/`-O3 -DNDEBUG` (the `__builtin_assume` UB path) — executes clean, routes to a diagnostic; (3) ≥10-min ASan fuzz from the 238-seed corpus, **zero crashes**, in BOTH modes (asan/Debug 941,446 runs/601s; release/NDEBUG 2,158,938 runs/601s); (4) post-parse-accessor reachability confirmed — corpus replay covers 724 blocks deep (valid seeds drive past parse into mappers/selector_resolver), so the throw-from-noexcept-accessor risk surface is exercised across millions of runs with no terminate. Regression witness added: `tests/config/test_load_negative_battery.cpp::T039_TomlInternalAssert_NoTerminate` + fixture `tests/config/fixtures/neg_toml_assert_table_header.toml` (the `[ [key]]` header) — PASSES; full config suite (6 tests) green under asan with the shim (no ODR/link/behavior regression).
