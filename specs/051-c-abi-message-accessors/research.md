...
**Occupancy-gate mechanic (verified, `tools/check_capi_occupancy.sh`).** Check A asserts a per-symbol `EXPECTED` map (`[SYMBOL]=numeric`) equals the parsed `error.h` `#define`s AND the `error_codes_v1.txt` audit; Check B asserts the `[2i §4.3]` block-count table matches expected per-block counts. The gate counts **by symbol-value + by block-count, NOT by prefix-pattern** — so adding `FIXPP_ERR_SESSION_*`/`FIXPP_ERR_APP_*` into `[11,99]` does **not** trip a "prefix mismatch"; the co-update is: +5 `EXPECTED` entries (values 11–15) + 5 `error.h` defines + 5 audit rows + bump the §1.1 cross-cutting count 11→16 (+ Check B's expected cross-cutting count if hardcoded). Mechanically clean.

**The one real cost of `[11,99]` (for Gate A): semantic.** The `[0,99]` block is labelled `FIXPP_ERR_CAPI_*` (cross-cutting **sentinels** — `NULL_HANDLE`/`INVALID_HANDLE`/`TYPE_MISMATCH`/…). `session_invalid_argument`/`app_do_not_send` are session/app **domain** errors, not boundary sentinels — placing them here **relabels** the block to "cross-cutting + session/app". That is the GA-permanent trade-off Gate A weighs.

**Alternatives.** New dedicated Phase-4 block `[1400,1499]` — cleaner domain separation + headroom + avoids the relabel, but it is the last block in the stated ≤1500/15-block budget, plants a Phase-4 block in a Phase-2-keyed layout, and needs a genuinely new gate accounting term. Extend 2d THREAD `[300,399]` — rejected (splits the family; app codes in a threading block). All recorded as considered; `[11,99]` is the mechanically-lowest-risk default but the placement is **explicitly Gate-A-reviewable on the real diff** (the semantic relabel vs the new-block costs are a judgment call, not a settled fact).
