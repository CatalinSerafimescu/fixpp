...
**Note**: the ~7.5 MB `OrchestraFIXLatest.xml` is not cached locally (spike scratchpad was disposable) — fetching it from `FIXTradingCommunity/orchestrations @ 236d4a405…` is a real implementation task requiring network. The sha1 `26f60db1c1f52d169d3b6825ac68800abf487fde` is the **spike's grade-1 recorded sha1 of the OFFICIAL `OrchestraFIXLatest.xml`** (7,525,180 bytes; spike-and-plan doc line 36 — NOT the separate `OrchestraFIXLatest_relabeled.xml`). It is "unverified" only in that this planning session did not re-fetch and recompute it. The gating first task (FR-007 / SC-007) therefore **fetches the official file, computes its sha1, asserts it equals this pinned `26f60db1…` value as a supply-chain integrity check** (mismatch → STOP and investigate — do not silently proceed on a wrong/tampered file), and records it into `UPSTREAM.txt`.
