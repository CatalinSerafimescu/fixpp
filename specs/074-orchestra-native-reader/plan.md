...
**Technical approach** (from the three Phase-0 exploration passes, consolidated in [research.md](./research.md)): mirror the `XmlLoader` facade→`LoaderState`→`finalize()`→`dict_metadata_handle` shape; reuse the `field_data_type` enum, `dict_metadata_handle`, `Dictionary`, `as_table_view`, `table_view` context machinery, and the `dict::` error base classes as the single data model (no parallel model). Supply Orchestra-specific analogues **only** for (a) the root/version gate (`<fixr:repository version="FIX.Latest_EP303">` → `session_version::vlatest`) and (b) the datatype-token table (Orchestra `<fixr:datatype>` names → `field_data_type`). **One additive extension to the shared handle (discovered at /speckit-implement):** FR-002 requires codeset **values + descriptions** to be preserved and observable, but the existing `dict_metadata_handle` has **no** enum-value storage (`table_view::enum_valid`/`add_enum` are Phase-1 stubs with no backing store, `FieldRef.enum_table_index` unused). So this feature adds a small **additive, tag-keyed** enum side-table to `dict_metadata_handle` + a read-only `Dictionary::enum_values(tag)` accessor (new public `EnumValueRef{value,description}`), populated only by the Orchestra reader. It is orthogonal to validation — `enum_valid` stays a stub, so XmlLoader-produced dictionaries are byte-identical (empty enum store) and the T024 no-regression pin holds. `FieldRef` is **unchanged** (tag-keyed store, not via `enum_table_index`), so reify/C-ABI are untouched. Error strategy: derive `orchestra_parse_error : public dict::xml_parse_error` (reuse `code()`, discriminate by catch type — the `072` `group_delimiter_collision_error` precedent), zero C-ABI churn.
