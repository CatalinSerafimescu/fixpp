...
| Vendored source | `dictionaries/orchestra/OrchestraFIXLatest.xml` (+ `LICENSE`, `NOTICE`, `UPSTREAM.txt`) | Apache-2.0, pinned `236d4a405…`; sha1 **provisional/unverified** (`26f60db1…` carried from the spike scratchpad) — compute + record the true sha1 from the fetched official file (FR-007/SC-007), do **not** assert against the pre-filled constant before first fetch. EP303. |
