...
| `<fixr:codeSets>/<fixr:codeSet>` + `<fixr:code value= name= …>` | **additive tag-keyed enum side-table** in `dict_metadata_handle` (`enum_values_` flat store of `EnumValueRef{value,description}` + `enum_runs_` sorted-by-tag index), queried via new read-only `Dictionary::enum_values(tag)` | Flatten codeset into the referencing field's value list (FR-002): `value` = `<fixr:code value=>` bytes, `description` = `<fixr:code name=>` (the QuickFIX-`description`-equivalent symbolic name; richer `<documentation>` synopsis is the deferred "richer dial"). A field's `type=` naming a codeSet resolves `FieldRef.type` to the codeSet's base `type=` datatype **and** attaches that codeset's values to the field's tag. Shared-codeset naming abstraction dropped. **Additive** (discovered at /speckit-implement — the existing handle had no enum storage; `enum_valid` stays a Phase-1 stub, so XmlLoader dicts keep an empty enum store and stay byte-identical). `FieldRef` unchanged (tag-keyed, not via `enum_table_index`). |
...
| `EnumValueRef` + `Dictionary::enum_values(tag)` | `include/fixpp/dict/dictionary.hpp` (EDIT) | **Additive enum surface (discovered at /speckit-implement — FR-002).** `struct EnumValueRef{std::string_view value; std::string_view description;}` + `[[nodiscard]] std::span<EnumValueRef const> enum_values(std::uint16_t tag) const noexcept`. Read-only; empty span for tags with no codeset or for XmlLoader-produced dicts (empty enum store). Orthogonal to `enum_valid` (unchanged stub). |
| `enum_values_` / `enum_runs_` + `enum_values_impl(tag)` | `src/dictionary/dictionary_internal.hpp` (EDIT) + `src/dictionary/dictionary.cpp` (EDIT) | **Additive handle storage (FR-002).** `std::pmr::vector<EnumValueRef> enum_values_` (flat, `string_view`s into the frozen `name_pool_`) + `std::pmr::vector<EnumRun{tag,start,count}> enum_runs_` (sorted by tag, binary-searched). Populated only by `OrchestraLoader::finalize()`; XmlLoader leaves both empty (byte-identical dicts → T024 no-regression holds). |
