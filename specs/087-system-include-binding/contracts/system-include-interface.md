...
| 5 | **missing reply** | reply-side | **two mandatory sub-cases**, both against a copy of a real reply directory: *(i)* delete the per-target `target-<name>-*.json`; *(ii)* delete the **whole reply directory**. Invoke the shipped script against each, passing the same install prefix the original configure used | **`MISSING_REPLY`** | red naming the missing artifact — **not** read as "no includes", and distinct from #6's token. **Both sub-cases are required**: they exercise different lookup failures — a directory that exists but holds no reply for this leg, and a directory that is not there at all — and §2a says the realistic failure is the *second*, a query that was never created. *(Marked mandatory at the step-9 checklist audit 2026-08-05; the sub-cases were enumerated but not required, the same shape that left row 6a's missing-leg case undemonstrated until Gate A round 2.)* |
...
| 7 | **carrier deleted** | package-side | **two mandatory sub-cases**: *(i)* delete the **new 087 target** `probe_system_include_contract` from `tests/consumer/CMakeLists.txt`; *(ii)* delete `tests/consumer/compare_system_includes.cmake` and **leave the target** | **build failure by name** | with the target gone: `ninja: error: unknown target 'probe_system_include_contract'` (Ninja's phrasing — *not* Make's "No rule to make target"). With only the script gone: the target's own command fails. **Both sub-cases are required** — they discharge two different R7 guards (#3 by-name requirement, #4 script deletion), and *(i)* alone leaves #4 unproven, which is the guard that covers deleting the `leg-set` mode along with the file. **Deleting an 086 target would re-prove an 086 obligation, not this one** *(marked mandatory at the step-9 checklist audit 2026-08-05)* |
