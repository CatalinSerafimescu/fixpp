...
- **invocation** (#6a): drive the shipped comparator wrongly — **no tree or reply mutation at all**. All
  **four** of #6a's sub-cases are plain `cmake -P` calls: two are `compare` argument faults (unknown `leg`,
  empty `expectation`), and the other two are pure `leg-set` calls because contract C-6.4 requires the
  leg-set assertion to be a **separately invocable mode** of `compare_system_includes.cmake` rather than
  logic buried in the carrier's declaration. Without that, "one leg missing" and "a leg duplicated" could
  only be induced by editing `tests/consumer/CMakeLists.txt` — which this class forbids.
...
|---|---|---|---|---|
| 1 | expectation | *before the **service** leg's correct expectation exists*, declare the **service** leg as `include/service-iface` only, omitting `include/capi` — **with the `capi` expectation already at its measured value** (contract §5's demonstration-#1 box: the carrier runs `capi` first and short-circuits, so an absent or wrong `capi` expectation reds that leg instead and the service comparison is never reached) | `LEAK` | red naming `include/capi` — the **vacuity proof**, taken *before* the gate is ever green. `LEAK` direction by construction so it cannot be confused with #3 |
| 2 | package | the exact diff at contract §5's demonstration-#2 box against `src/capi/CMakeLists.txt:110-112` — **not** a `PRIVATE`→`PUBLIC` keyword flip, which is not the same edit; `:125-128` is **not** touched | `LEAK` | red naming the entries the reverted interface adds. **Qualitative:** the set gains the umbrella include root and third-party roots; **record the count you observe** — no expected count is stated, and none has been measured on a reverted `fixpp::capi` |
...
| 5 | reply | **two mandatory sub-cases**, both in a **copy**: *(i)* delete the per-target reply file; *(ii)* delete the whole reply directory | `MISSING_REPLY` | red **naming the missing artifact** — must not read as "no includes", and distinct from #6 |
...
| 6a | invocation | **four sub-cases, all `cmake -P`**: compare mode with an unknown `leg`; **leg-set** mode over **one** result file (missing leg); leg-set mode over the **same result file twice** (duplicated leg); compare mode with an **empty `expectation` argument**, every other argument correct | `LEG_ERROR` | red naming the offending leg — or the empty expectation — **distinguishable from `INPUT_ERROR`**: a corrupt reply and a mis-driven carrier are different defects. Makes C-6.4's "exactly two legs" observable. **All four are mandatory**: the missing-leg case is the one C-6.4 exists for (a `capi`-only comparator reporting green) and is *not* discharged by the unknown-leg case; the empty-expectation case reds **before the reply is located**, so run it against a *correct* reply — it is the only demonstration of what makes `data-model.md` I3 a runtime property rather than a property of the declared literal |
| 7 | package | **two mandatory sub-cases**: *(i)* delete the **new 087 target** `probe_system_include_contract`; *(ii)* delete `compare_system_includes.cmake` and keep the target | build failure | `ninja: error: unknown target 'probe_system_include_contract'` (Ninja's phrasing — *not* Make's "No rule to make target"); with only the script gone, the target's own command fails. Deleting an 086 target instead would re-prove an 086 obligation |
