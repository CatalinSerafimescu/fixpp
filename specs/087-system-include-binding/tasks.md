...
- [X] T025 [P] [US2] `.github/workflows/tier2.yml` — the same step, after `:323` and before `:360`, **with `shell: bash`**: this file's `ctest` steps are `shell: cmd`, and its own `packaging` assertion at `:371-384` already switches to bash for exactly this. **Model tier 2 on its own step, not on tier 1's.** Covers `:363` and `:389`
