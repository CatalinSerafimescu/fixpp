...
**Total: 44 tasks** (T001–T044). Tests are not optional in this feature — the gate *is* the deliverable.
...
- [ ] T001 Export `CCACHE_DIR=/mnt/wsl/fixppbuild/ccache` (unset by default it silently uses `~/.cache/ccache`) and confirm headroom with `df -h /mnt/wsl/fixppbuild` — a Debug tree measures 22–31 GB; build `-j2` maximum, per `quickstart.md` §0
- [ ] T002 Create the durable evidence directory `$FIXPP_087_EVIDENCE` (default `~/fixpp-087-evidence`, **not** `/tmp`) per `quickstart.md` §0
- [ ] T003 Run the profile-matched `conan install ... -of build/linux-clang-release` **before** `cmake --preset` — the preset hardcodes `build/linux-clang-release/conan_toolchain.cmake`, which does not exist in a fresh worktree (086 T003) — then configure and build `-j2`
- [ ] T004 `cmake --install build/linux-clang-release --prefix /tmp/fixpp-stage-087` into an **emptied** prefix (`cmake --install` does not remove stale files, so a dirty prefix makes a later comparison pass falsely). Any prefix works — the comparison is prefix-relative (I1, C-3)
- [ ] T005 Capture the pre-feature baseline in `$FIXPP_087_EVIDENCE/`: `ctest --test-dir build/linux-clang-release --no-tests=error --output-on-failure` → `ctest-before.txt` **with its exit code**, recorded **per-test status** (a name-set diff cannot see a red); plus `ctest --test-dir build/linux-clang-release -L consumer -N` → expect `Total Tests: 1` (the FR-014 baseline), and the option guards `CMakeLists.txt:290` (`FIXPP_BUILD_CODEGEN_TOOL` default ON, overridden nowhere), `:401`, `:421` (the sole `LABELS consumer`)
...
- [ ] T017 [US1] **Demonstration #3 — drop, reply-side (`DROP`).** `cp -r` a **real** reply directory produced by a real configure; in the **copy**'s `target-probe_service_positive-*.json` delete one entry from `compileGroups[].includes[]`; invoke the **shipped** `compare_system_includes.cmake` against the copy, passing the **same install prefix the original configure used** (without it C-3's prefix-relative comparison cannot be performed). Expect red naming the deleted entry as expected-but-absent — reachable **only** because C-1 asserts equality, not containment. "Remove an entry from the observed side" is **not** achievable by editing the tree: `run_consumer_witness.cmake:46` wipes and reconfigures the sub-build every run
- [ ] T018 [US1] **Demonstration #4 — reclassified, reply-side (`RECLASSIFIED`, and that token ALONE).** In a **copy**, flip one entry's `isSystem` `true`→`false` leaving **both paths identical**; invoke the shipped script against the copy with the original prefix. Expect red naming the path and both classifications, with **no** accompanying `LEAK` or `DROP` — C-1 stage 1 claims the path-matched pair and removes it, leaving stage 2 nothing. **This row is why C-1's staging is normative**, and it is the **only** demonstration that exercises FR-003a's classification leg: `isSystem` is uniformly `true` in the passing state, so no happy-path run varies it and a comparator that parsed `path` and discarded `isSystem` would satisfy every other row
...
- [ ] T020 [US2] **Demonstration #5 — missing reply (`MISSING_REPLY`), two sub-cases.** In a **copy** of a real reply directory: (a) delete the per-target `target-<name>-*.json`; (b) separately, delete the whole reply directory. Invoke the shipped script against each with the original install prefix. Expect red **naming the missing artifact** — not read as "no includes", and distinct from #6's token
...
| requirement | tasks |
|---|---|
| FR-001, FR-002 | T006, T007, T008 |
| FR-001a | T009, T010, T022(ii) |
| FR-003, FR-003a | T014, T017, T018 |
| FR-004 | T008 (complete token set, both directions named) |
| FR-005 | T008, T020 |
| FR-006 | T011, T023 |
...
- **Never hard-code a reply filename.** `target-<name>-<config>-<hash>.json` — the hash changes every
  configure (C-5, I5).
- **Pass the install prefix to every direct `compare` invocation** in T017–T022: the copied replies preserve
  absolute paths from the staged tree, and a standalone `cmake -P` inherits no CMake variables.
