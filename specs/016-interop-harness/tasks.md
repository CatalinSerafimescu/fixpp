...
>
> **All-TLS reconciliation (2026-06-01, FR-025):** fixpp ships TLS-only (no plaintext transport; `SecurityProfile` ∈ {`mtls_ca`,`mtls_pinned`,`one_way_ca`}). Every live cell therefore runs over TLS with a server-auth `one_way_ca` baseline profile; the original "plain-TCP" cells are reconciled to TLS (plain-TCP is unimplementable and adds no session-layer coverage). T015 ceases to be a separate "TLS variant" driver — TLS IS the baseline (T010–T014). The QuickFIX counterparties must run as SSL acceptors/initiators with a cert the `one_way_ca` profile trusts (parent-harness config obligation).

- [ ] T009 [P] [US1] Author the golden transcripts `tests/interop/happy/golden/HP-*.fix` for the 5 FIX 4.4 TLS event chains (capture from a paired run, check in byte-exact per `golden-transcript-format.md`); SOH rendered `\x01`, both directions interleaved with `>`/`<` markers. **NOTE:** a golden is by definition a *captured* artifact — it requires a live fixpp↔QuickFIX paired run, which needs the parent harness + a running SSL-configured counterparty. Until that exists, this task is **deferred to the first paired capture** (a parent-harness runtime step); the in-repo deliverable now is the `golden/` directory + `FORMAT.md` documenting the format. Do NOT hand-fabricate transcripts (a fabricated golden is a lying artifact — it would diff against fixpp's own output and prove nothing).
- [X] T010 [P] [US1] Driver `tests/interop/happy/hp_fix44_logon_hb_logout_test.cpp` — Logon → idle Heartbeat → TestRequest/Heartbeat → Logout; assert FSM disconnected both sides, seqnum deltas, counterparty terminal (received `Logout(35=5)` / orderly close, FR-007), golden match. `spec_ref [FIX-SL §4.3/§4.5.1/§4.5.5/§4.6]` (this is the smoke cell, FR-022)
- [X] T011 [P] [US1] Driver `tests/interop/happy/hp_fix44_testrequest_echo_test.cpp` — active+idle Heartbeat + TestRequest echo. `spec_ref [FIX-SL §4.5.1/§4.5.5]`
- [X] T012 [P] [US1] Driver `tests/interop/happy/hp_fix44_reject_invalid_admin_test.cpp` — invalid admin message → wire-conformant `Reject(35=3)` with RefTagID/RefMsgType. `spec_ref [FIX-SL §4.5.4]`
- [X] T013 [P] [US1] Driver `tests/interop/happy/hp_fix44_seqnum_recovery_test.cpp` — counterparty higher-numbered message → fixpp drives ResendRequest / SequenceReset-GapFill, session resynchronizes without fatal disconnect (live via 013). `spec_ref [FIX-SL §4.5.3/§4.8.1/§4.8.2/§4.8.5]`
- [X] T014 [US1] Driver `tests/interop/happy/hp_fix44_disconnect_reconnect_noreset_test.cpp` — abrupt disconnect → reconnect with `ResetSeqNumFlag=N`, sequence continuity preserved, no spurious reset; uses the finite reconnect policy from T008 + a `stop()` watchdog bound (FR-004). `spec_ref [FIX-SL §4.4.2/§4.4.3/§4.8.6]` (depends on T008)
- [X] T015 [US1] TLS profile coverage + mTLS deferral bookkeeping (REFOLDED 2026-06-01 — see the all-TLS reconciliation note above; TLS is no longer a separate "variant" since every baseline cell T010–T014 is already TLS). This task now: (a) confirm the all-TLS baseline cells carry the `spec_ref [FIXS §3.2/§3.4]` TLS reference; (b) record the `one_way_ca` server-auth baseline profile as the counterparty-satisfiable choice; (c) register `mtls_mutual` (client-cert) as `deferred:v1.1-mtls` with a note (FR-025) in `MATRIX.md`. No separate driver file.
- [X] T016 [US1] Down-peer regression cell `tests/interop/happy/hp_down_peer_stop_watchdog_test.cpp` — a *separate* (non-matrix) FR-028 cell proving `Engine::stop()` returns within a stated bound when the initiator targets a never-accepting peer; the watchdog IS the assertion, MUST NOT silently pass on a hang (builds on T008)
- [X] T017 [US1] Register the deferred placeholder rows in `tests/interop/happy/MATRIX.md` (matrix manifest): business cells `deferred:app-messages` (FR-005), FIX 5.0 SP2 / FIXT.1.1 cells `deferred:fixt-routing` (FR-003), Fix8 happy-path cells `deferred:fix8-revisit` (FR-009) — each present with rationale, none executed; every axis covered ≥ once or carries a deferred-to-vN.x note (FR-008)
...
- [X] T018 [US2] Create `tests/interop/thorny/CORPUS-INDEX.md` — the in-repo provenance index (issue ref, URL, state, category, priority bucket → test file); MUST NOT match `no-research.yml` patterns (not `*-decisions.md`); record the per-engine closed-tail cap (FR-010/FR-011, R2). DONE: bounded worklist (P1 C-001..C-007 + P2/P3 + known-limitation table), cap = the parity-audit-enumerated closed issues, append-only + disposition rule, open-issue `watch:` follow-on note.
- [X] T019 [US2] Derive the bounded executable worklist from `../phase-9-harness/manifest/scenarios.yaml` (closed-with-fix tail, capped) into descriptor stubs under `tests/interop/thorny/<category>/` bucketed `P1/P2/P3` (FR-012); the open-issue `watch:` bucket is explicitly out of this feature (follow-on note in CORPUS-INDEX.md). DONE: source corrected to the mined closed-tail in `phases/phase-9/parity-{quickfixj,quickfix-cpp}.md` (scenarios.yaml is the pre-seeded happy-path list per FR-010; it carries no upstream-issue provenance); `recovery/_descriptor.md` + `framing/_descriptor.md` stubs.
- [X] T020 [P] [US2] Encode the P1 (release-blocking) corpus scenarios `tests/interop/thorny/<category>/<engine>-issue-NNN_test.cpp` for the recovery/sequencing categories (SequenceReset/GapFill, ResendRequest edges, Logon/Logout race, persistence-recovery); each cites `spec_ref` (FR-018) and a `differentiator` flag where fixpp is spec-correct vs a buggy upstream (FR-015). DONE: `recovery/qfj-750-logout-seqnum-mismatch_test.cpp` (C-004 — surfaced a real spec-defensible DIVERGENCE: too-high Logout → gap-recovery precedence, not QFJ-750's disconnect-on-gap) + `recovery/qfj-271-sequencereset-large-gapfill_test.cpp` (C-005 — no inbound-reorder-queue recursion). C-001..C-003 covered-by-parity (US3 T023/T024/T025), not re-encoded.
- [X] T021 [P] [US2] Encode the P1 corpus scenarios for the framing/validation categories (BodyLength/checksum, field-validation, encoding, reject, repeating-group, Heartbeat); same provenance + spec_ref + differentiator discipline. DONE: `framing/qfj-603-unsupported-beginstring_test.cpp` (C-006) + `framing/qfj-721-non-logon-first-message_test.cpp` (C-007, no-NPE differentiator). All 4 thorny witnesses GREEN (interop suite 14/14).
- [X] T022 [US2] Wire the corpus disposition rule into the scenarios: a failing `P1` MUST be `pass` OR `known-limitation:<open-tracking-issue>` (FR-014); confirm append-only construction is the initial population (FR-013 governs later releases, data-model E3). DONE: disposition rule + append-only + release-gate semantics recorded in CORPUS-INDEX.md; known-limitation table maps the deferred-by-design GAPs (S-010/S-017/S-018/S-020-FIXT/app-message/nanos/latency-knob) to tracking refs.
...
- [X] T023 [P] [US3] Witness `tests/interop/parity/resend_abort_on_failing_write_test.cpp` — QFJ-646: resend aborts when transport `send()` returns false mid-resend (Bucket-2, still open upstream); plumbing exists → COVERED
- [X] T024 [P] [US3] Witness `tests/interop/parity/replay_subsumes_reorder_queue_test.cpp` — Bucket-4 model confirmation: assert fixpp's store-replay recovery subsumes QuickFIX's reorder-queue protocol outcome (simultaneous bidirectional ResendRequests, remove-queued-on-SequenceReset, large-queue); default no reorder-queue impl. If the witness FAILS, stop and surface a scoped session-layer finding (do NOT silently absorb — R-prod/2, leaves 016 a separate feature)
- [X] T025 [P] [US3] Witness `tests/interop/parity/inbound_sequencereset_arms_test.cpp` — confirm the S-023/#90 inbound-SequenceReset `NewSeqNo >/=/<` arms cover the audit rows; cite #90 tests → COVERED (extend only if a row is uncovered)
- [X] T026 [US3] Disposition update — flip the closed GAP rows to COVERED with test citations in `phases/phase-9/unit-test-parity-matrix.md` (parent doc): T023/T024/T025 + the already-closed EncryptMethod=0 / integer-overflow rows (cite #91 witnesses); leave Bucket-3 (PossDup/FIXT routing/5.0SP2/app-message) as `deferred:by-design` with rationale consistent with `deferred:fixt-routing` (FR-003); acceptor HeartBtInt-echo (Bucket-1) included only if cheap (FR-016/FR-017, SC-003)
...
- [X] T027 [US4] Create `.github/workflows/interop-smoke.yml` — run the single smoke cell `HP-QFcpp-init-fix44-logon-hb-logout` (T010) on the normal build only, `ubuntu-latest`, gating PRs touching the transport/session/interop surface; counterparty absent ⇒ skip-with-reason (FR-022/FR-023). Enforce the SC-005 budget: the smoke cell MUST complete in **≤ 120 s wall-clock** (job step timeout) — an over-budget run is a gate failure, not a flake (maps SC-005). DONE: light per-PR tier on `linux-clang-release`, paths-filtered to the transport/session/tls/interop surface, builds+runs only `interop_hp_fix44_logon_hb_logout_test` with a `timeout-minutes: 2` (≤120 s) run step.
- [X] T028 [US4] Emit a per-cell `cell_result` manifest from the suites (a `tests/interop/cell_results.schema-check` test or a generated artifact) honoring the contract schema — every matrix/corpus cell present incl. each `deferred:*` row as `status: n/a` (per-cell completeness rule); the strict `skip:<reason>` (FR-023 counterparty-unavailable) vs `matrix_disposition: deferred:<tag>` disambiguation is preserved (contract `parent-harness-gate-contract.md`). DONE: `tests/interop/cell_results.yaml` (18 happy live + down-peer + 4 deferred + 10 corpus + 3 parity) + `cell_results_schema_check_test.py` (9 invariants incl. n/a⇔deferred, skip-only-on-live, P1 block rule, per-axis completeness), registered as ctest `interop_cell_results_schema_check` (GREEN).
- [X] T029 [US4] Confirm every `interop-happy` + `interop-thorny` ctest target builds + runs clean under the ASan/UBSan and TSan presets locally (`-j2`, one preset at a time, `AskUserQuestion`-gated per `[const §XVII.7]`); a sanitizer-only failure is a real failure (FR-019/FR-020); record results for the parent `interop-full-matrix` named check (the release-prep full matrix itself runs in the parent). **FR-021 guard**: assert the `interop_*` ctest targets do NOT co-compile or link any counterparty engine source (only fixpp's process carries sanitizer flags); the counterparty-runs-as-unmodified-binary half is a parent-harness obligation — document it as inherited in `KNOWN-LIMITATIONS.md` (T030) if no in-repo assertion is feasible. **FR-021 guard DONE**: the submodule contains ZERO QuickFIX/Fix8 counterparty source (only fixpp's own `quickfix_compat` cfg-loader); `interop_*` targets link only `interop_support` + `fixpp_*` + GTest — structurally no counterparty source is sanitizer-instrumented; counterparties run as unmodified parent-harness binaries (documented in `KNOWN-LIMITATIONS.md`, T030). **Sanitizer run DEFERRED to `/speckit-verify`** (AskUserQuestion 2026-06-01 → defer; verify runs the full 6-preset matrix over the interop targets — avoids a double sanitizer build).
...
- [X] T030 [P] [US5] Assemble the documented-known-limitations source `tests/interop/KNOWN-LIMITATIONS.md` — each corpus `known-limitation` (FR-014) + the §VII.6 residual pointer + any TLS/down-peer caveats, each with its tracking issue, for the badge's limitations list (FR-024, SC-007). DONE: corpus known-limitation table (S-010/S-017/S-018/S-020-FIXT/app-message/nanos/latency-knob/chunked-resend), §VII.6 NewOrderSingle→ExecutionReport open-residual pointer (FR-027/SC-008), all-TLS/one_way_ca + mTLS-deferral + down-peer-L2 caveats, FR-021 inherited counterparty-unmodified note, exact counterparty versions for the badge.
- [X] T031 [US5] Confirm the per-scenario goldens (`tests/interop/happy/golden/`) + the CORPUS-INDEX are structured as archivable release artifacts the parent `interop-badge-emit` links; the badge text (`Interop verified against QuickFIX-cpp v1.16.0 / QuickFIX-J 3.0.1`) names exact `{name, version, commit}` (FR-024, contract `parent-harness-gate-contract.md`). DONE: `golden/FORMAT.md` defines the archivable per-scenario `HP-<id>.fix` shape (byte-exact, one-per-scenario; captures deferred to first paired run, T009); `thorny/CORPUS-INDEX.md` + `cell_results.yaml` are archivable result manifests; badge `{name, version}` pinned in `KNOWN-LIMITATIONS.md` (commit is the parent's runtime pairing property).
...
- [X] T032 [Polish] Record the `[const §VII.6]` business-message residual (FR-027/SC-008) in `spec/behaviors-and-limitations.md` (parent `book/`-adjacent spec dir) + the feature catalogue — the session-only badge does NOT discharge `Logon → NewOrderSingle → ExecutionReport → Logout`; forward-pointer to A-001/A-006; zero 016 artifact claims the business flow ran. DONE: `spec/behaviors-and-limitations.md` L-016-1 (§VII.6 open residual) + L-016-2 (all-TLS/mTLS-deferral) + B-016-1 (FR-018 spec-reconciliation / qfj-750 divergence); catalogue 016 note (T036).
- [X] T033 [Polish] Coverage disposition for the Option-1a production touch — add a coverage-index entry for the `SessionConfig` reconnect-policy field + bounded-connect lines and an Article IX §1 95/85 assessment note seeding `/speckit-verify`'s `verify.md` (T008 touched modules). DONE: `specs/016-interop-harness/VERIFY-SEED.md` (T008 touched files + per-arm 95/85 assessment + carried sanitizer-matrix deferral); the coverage-index entry is the `spec/coverage-index.md` "016 production-touch ledger" (T036) citing `reconnect_policy_witness_test.cpp` + the down-peer watchdog → S-014/S-032/FIX-SL §4.4.
- [X] T034 [P] [Polish] Run the `no-research.yml` guard grep over `tests/interop/**` (reject `^research/`, `^decisions/`, `*-decisions.md`, `opus_plan.md`, `SYNTHESIS.md`) — confirm CORPUS-INDEX + witnesses pass; the sweep analysis stays in the parent (R2). DONE: 0 forbidden matches across `tests/interop/**`.
- [X] T035 [Polish] Feature-completeness audit — map every task ↔ FR-001..028 / SC-001..008 ↔ catalogue rows; 100% or explicitly waived (the `/gate-b` precondition per `[[feedback_feature_completeness_gate]]`). DONE: `specs/016-interop-harness/COMPLETENESS.md` — FR-001..028 (23 done / 5 deferred-parent-or-verify / 0 waived) + SC-001..008 (5 done / 3 deferred / 0 waived); 100% dispositioned, /gate-b completeness precondition satisfied.
- [X] T036 [Polish] Update `feature-catalogue.md` + `coverage-index.md` for 016 (S-row interop dispositions, parity-row flips, the §VII.6 residual) per `[[feedback_feature_completeness_gate]]`. DONE: `spec/feature-catalogue.md` 016 tests-only note + standalone re-confirmation evidence appended to S-006/S-014/S-023 (NO status flipped — live interop is parent-side); `spec/coverage-index.md` 016 production-touch ledger; parity-row flips already landed in the PARENT `phases/phase-9/unit-test-parity-matrix.md` (commit `56bffdc`). (T035/T036 authored by Codex, source-reviewed: citations real, no overclaim/fabrication.)
- [X] T037 [Polish] Run quickstart.md validation — the parity standalone run + the smoke cell + the verify step all execute as documented. DONE: §1 parity standalone GREEN (3/3 `interop-parity`); §2 smoke cell SKIPS-with-reason (4 cells, 0 silent-pass — FR-023); §4 `/speckit-verify` is the documented next pipeline step (seeded by VERIFY-SEED.md).
