...
- [ ] T001 Create the `tests/interop/` directory structure (`support/`, `happy/golden/`, `thorny/`, `parity/`) per plan.md Project Structure
...
- [ ] T003 [P] Implement `tests/interop/support/counterparty_probe.hpp` — probe a counterparty binary/port at test start; absent ⇒ produce a `skip_reason` (GoogleTest `GTEST_SKIP() << reason`), never silent-pass (FR-023, data-model E6)
