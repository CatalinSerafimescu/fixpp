...
1. **Reconnect-path prerequisite (FR-004/FR-028).** The 015 down-peer carry-forward (CLAUDE.md L2) confirms there is **no `SessionConfig` reconnect-policy field** and the engine connect is not promptly cancellable, so a *reliable* live reconnect cell + a provable `stop()` is **not** free. **DECIDED at `/speckit-tasks` (2026-06-01): Option 1a** — scope-in the **small, bounded** production change: wire a `SessionConfig` reconnect-policy field + bound/cancel the connect (`tasks.md` T007 RED witness → T008 prod change). Its touched lines fall **unconditionally** under Article IX §1 95/85 with a `verify.md` assessment + a coverage-index entry (T033); this discharges the 015 down-peer carry-forward. (Option 1b — restrict v1.0 reconnect cells to an acceptor-side shape and defer initiator-reconnect — was the rejected alternative.)
...
Because Option 1a is scoped-in, the Article IX §1 touched-module gate applies (it is **not** N/A by construction); the `verify.md` 95/85 assessment for the reconnect-policy touched lines is mandatory (T033). The "near-zero production surface" framing is therefore **bounded, not zero**.
...
- **Article IX.1 (coverage 95/85 touched modules)**: ✓ with **one scoped-in production touch + one pinned escape hatch** (Gate A round-1, R-prod; reconnect path DECIDED Option 1a at `/speckit-tasks`). This feature predominantly adds tests, not modules; the parity tests *raise* coverage on existing `wire/`/`session/`/`store/` modules. The production-touch paths: **(1)** the FR-004/FR-028 reconnect-policy field is **scoped-in (Option 1a, DECIDED)** — its touched lines fall **unconditionally** under 95/85 with a mandatory `verify.md` assessment + a coverage-index entry (T033); **(2)** R-parity Bucket-4 — if the witness fails, the fix **leaves 016 as a separate feature** with its own coverage, never silently absorbed. The touched-module gate is therefore **live (not N/A)** for the reconnect-policy lines; `verify.md` records the 95/85 assessment.
