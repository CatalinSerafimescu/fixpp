...
- [X] T012 [US1] RED tests (witnesses, one per violation class → reason): header-out-of-order→14; unexpected-tag→2; required-field-missing→1; type-nonconformant (type arm)→5; Float garbage value→5 (T009a remap); conformant message → dispatched; seqnum NOT advanced on reject (C-2/C-3) **[at-expected no-advance SUPERSEDED 2026-09-14 by fixpp#423 — see B-423-1]**. **NOTE**: `decimal_precision_loss`→reason=6 arm is dead code with `FIXPP_DECIMAL_T=pod_decimal` — `pod_decimal::from_chars` never returns `decimal_precision_loss`; the arm is a forward-looking guard for fixed-precision decimal types; waived per [const §IX.1] (unreachable arm).
...
- [X] T024 Coverage — assessed in `/speckit-verify` (Step 4). New files: field_type.hpp 100%/87%, table_view.hpp 80%/100%, reject_reason_map.hpp 76%/83% (misses = the documented-waived reason-6 + fail-closed-default arms). Every new branch exercised by its targeted witness (wire_*→reason arms T012; default-off skip T015/T016; enabled-pass T012; Logon-arm rows T013; clock-gate reject+success T017; fail-closed config T003; Float remap W5a). Full sanitizer regression on the changed surface: ASan 73/73, UBSan 73/73, TSan 68/68, all clean. One waiver-with-proof: SC-003 reason-6 (unreachable with pod_decimal — L-041-3).
