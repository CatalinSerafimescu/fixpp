...
**Decision**: The app-send path MUST produce a frame in which (i) **MsgType(35) is the third wire field** (immediately after `9=BodyLength`), (ii) **BodyLength(9) is digit-only / unpadded**, and (iii) the **CheckSum(10) is valid**. It MUST ALSO validate the opaque payload fail-closed before consuming a seqnum (see the opaque-payload note below; FR-016). Recommended implementation (for `/implement`, not done here): route app sends through `wire::Writer` (which already produces field-3 MsgType ordering, digit-only BodyLength via `commit()`'s memmove, and the checksum), or correct the manual `send_impl` framer to hoist the payload's leading `35=…` into header position AND write digit-only BodyLength.
...
Both write into a caller `std::span<std::byte>` via `wire::Writer` (building into a local stack scratch buffer and copying into the caller `out` only on full success — atomic; on error the returned span is absent and `out` is unspecified, INV-4), return `expected_t<std::span<std::byte>>` of the written body, and emit **only** the app body (lead `35=…`, then business fields — **no** `8/9/34/49/52/56/10`; the engine stamps those). Numeric fields are `const fixpp::decimal_t&`, serialized via `decimal_t::format(std::span<std::byte>)`.
