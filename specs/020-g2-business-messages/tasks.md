...
- [X] T010 [US1] Opaque-payload fail-closed validation in the app-send path (`Engine::send`→`send_impl`), **before** seqnum assignment/store: require exactly one leading `35=`; reject empty payload / duplicate `35=` / any embedded session header-or-trailer tag (`8/9/34/49/52/56/10`) with `error::app_payload_malformed` (131); no transmit, no seqnum consumption. Makes T007 INV-8 green. (FR-016.)
