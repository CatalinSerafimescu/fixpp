...
5. **Given** a build request omitting a required field, **When** the user invokes the builder, **Then** it returns a typed error and produces no frame.
...
- **Missing required field on build** — the typed builder returns a typed error and emits no frame (US1 AS5); it never produces a partial/malformed `35=D`/`35=8`.
- **Malformed / required-field-missing inbound business message** — when an inbound `35=D`/`35=8` fails typed-field validation, the engine path reuses 019's `BusinessMessageReject (35=j)` (catalogue A-014) so the peer is told why; the session survives.
...
- **Decimal/qty/price formatting** — Price (44), OrderQty (38), LeavesQty (151), CumQty (14), AvgPx (6) are typed as `core::Decimal` and serialize via its canonical formatting (no locale-dependent or scientific notation); round-trip fidelity is asserted.
- **Timestamp fields** — TransactTime (60) MUST be a valid UTCTimestamp the counterparties accept (same formatting contract as SendingTime(52), which 016 F1 proved real engines validate).
- **Field ordering** — the body field order MUST satisfy both reference engines' tolerance (header tags first where required); ordering is fixed by the builder, not the caller.
...
- **FR-004**: The builder MUST NOT emit session header tags that the engine send path already stamps (`8`/`9`/`34`/`49`/`52`/`56`), and MUST emit business fields in an order both reference engines accept.
- **FR-005**: The builder MUST reject a build that omits any required minimal field, returning a typed error and producing no frame (fail-closed; no partial/malformed emission).
...
- **FR-007**: Numeric fields (Price, OrderQty, LeavesQty, CumQty, AvgPx) MUST be typed as **`core::Decimal`** (001) on the public builder/accessor API and MUST serialize in `Decimal`'s canonical, locale-independent decimal form that both reference engines parse, round-tripping to the originated value.
- **FR-008**: TransactTime (60) MUST serialize as a valid UTCTimestamp accepted by the reference engines (same formatting contract as SendingTime).
- **FR-009**: An inbound business message that fails typed-field validation MUST drive the existing `BusinessMessageReject (35=j)` path (catalogue A-014) so the peer receives a business-level reject and the session survives.
...
- **FR-014**: The feature MUST update catalogue rows **A-001** and **A-006** to `done` and record the discharge of the `[const §VII.6]` business-message interop clause (interop roadmap FR-027/SC-008), with feature-catalogue + coverage-index updates.
...
- **NewOrderSingle (35=D, A-001)** — a client order instruction. Minimal fields: **ClOrdID (11)** client order id (string); **Symbol (55)** instrument (string); **Side (54)** buy/sell (enum char); **OrderQty (38)** quantity (`core::Decimal`); **OrdType (40)** order type (fixed to Limit `2` for v1.0); **Price (44)** limit price (`core::Decimal`, always required given Limit); **TransactTime (60)** order time (UTCTimestamp).
- **ExecutionReport (35=8, A-006)** — an execution/ack of an order. Minimal fields: **OrderID (37)** exchange order id (string); **ExecID (17)** execution id (string); **ExecType (150)** execution event type (enum char; round-trip emits Filled `F`); **OrdStatus (39)** order status (enum char; round-trip emits Filled `2`); **Symbol (55)** instrument (string); **Side (54)** buy/sell (enum char); **LeavesQty (151)** open quantity (`core::Decimal`); **CumQty (14)** filled quantity (`core::Decimal`); **AvgPx (6)** average fill price (`core::Decimal`).
- **Responding counterparty Application** — a reference-engine-side `Application` (QuickFIX-J and QuickFIX-cpp) that, on receiving a `NewOrderSingle`, emits one `ExecutionReport`. Test/harness-side only; not a fixpp production entity.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: A user can construct and send a NewOrderSingle and an ExecutionReport using only named typed fields — zero hand-assembled wire bytes — and read both back through typed accessors (closes catalogue A-001/A-006).
...
- The 019 `Application` + `Engine::send` surface and its strand/keepalive/lifetime contract (L-019-3, L-015-4) are sufficient to originate and observe these messages; no new engine concurrency surface is introduced by this feature.
