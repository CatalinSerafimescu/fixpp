...
- **FR-015**: The feature MUST record the two forward obligations as tracked deferrals (NOT implemented in v1.0), captured in `spec/behaviors-and-limitations.md` and the deferred-work registry:
  - **FR-015a**: full FIX 4.4 field/group coverage for NewOrderSingle/ExecutionReport beyond the minimal set.
  - **FR-015b**: all-protocol-version coverage (FIX 4.2 / FIX 5.0 SP2 / FIXT.1.1) of these business messages, **scheduled as a post-v1.0 obligation**.
- **FR-016**: The application-message send path MUST validate the opaque `Engine::send` payload **fail-closed before** assigning a seqnum or transmitting (this applies to all 019 opaque callers, not only the typed builders): exactly **one** leading `35=` is required; an empty payload, a duplicate `35=`, or any embedded session header/trailer tag (`8`/`9`/`34`/`49`/`52`/`56`/`10`) MUST be rejected with the new `error::app_payload_malformed` (slot **131** — next free after 019's 129/130); on rejection there is **no transmit and no seqnum consumption**. Additionally, a payload whose final field is not SOH-terminated, or a payload where the leading `35=` MsgType value is empty (`35=` immediately followed by SOH), MUST also be rejected with the same `error::app_payload_malformed`.
