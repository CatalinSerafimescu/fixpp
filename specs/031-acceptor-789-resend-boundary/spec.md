...
1. **Given** a fixpp acceptor with the `789` knob enabled, fixpp's next-outbound at `N` (`N > 2`), and a peer initiator advertising `789 = X` with `X < N` (before the reply Logon is counted), **When** the Logon exchange completes, **Then** fixpp proactively resends exactly the range `[X, N-1]`, emits no `ResendRequest`, and reaches the established state.
2. **Given** the genuine-gap case above, **When** fixpp's emitted frames are inspected, **Then** the resend covers exactly `[X, N-1]` and never re-uses the sequence number consumed by the reply Logon.
...
- **FR-004**: The system MUST NOT emit any frame whose sequence number equals a sequence number already consumed by the reply Logon; every sequence number fixpp emits during and after the Logon exchange MUST be strictly monotonically increasing.
...
- **SC-003**: Across the Logon exchange and any honor-driven resend, **no two** frames emitted by fixpp share a sequence number (strict monotonicity) — verified by an automated test inspecting emitted frame sequence numbers; today the in-sync case violates this by re-using the reply-Logon sequence number.
