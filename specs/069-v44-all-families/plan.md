...
- Post-`/analyze` precision-fix 2026-07-11 (no re-Gate-A): the round-2 "message-instance precise" enumeration was short **7** FIX44-present `msgcat='app'` new messages the operative "full 83-set DELIVERED" clause already authorizes set-wise — **A-021 (`AH`/`AI`/`AJ`), N-001 (`BC`/`BD`), A-025 (`v`/`w`)**. `/speckit-analyze` (F1/E1) caught the illustrative enumeration + coverage-index flip-list undercounting by 7; enumeration (Sync Impact + §7 text above), spec Normative References, and `tasks.md` T021/T022 corrected. **Operative delivered scope (83) is unchanged — this is a traceability/precision clarification, not a scope change, so no third Gate A round is required** (user decision 2026-07-11). Root cause: `feature-catalogue.md` lists N-001 as `5.0–5.0SP2` though `BC`/`BD` are physically FIX44 `msgcat='app'` (F2 — catalogue version column to be corrected).
