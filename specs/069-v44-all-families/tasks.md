...
- [ ] T021 Run `/speckit-verify` (Tier-1 serial sanitizer matrix — ASan/UBSan/TSan + debug) on the codegen consumers + differential harness; confirm `capi_freeze.sha256` / `c_api.h` unchanged (FR-012/C6). Writes `.specify/decisions/069-v44-all-families-verify.md`.
