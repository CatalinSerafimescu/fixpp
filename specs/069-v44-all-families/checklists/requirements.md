...
- Message count: the spike's "~86" was a **msgtype-based** figure (retained only as the compile-cost basis). The authoritative in-scope set is **83 = 85 `msgcat='app'` minus {BE, BF}** (33 OFFICIAL + 50 new), dictionary-derived via `msgcat`, keeping the requirement robust to dictionary revisions. BW/BX/BY are absent from FIX44; XMLnonFIX `35=n` is `msgcat='admin'`, never emitted.
