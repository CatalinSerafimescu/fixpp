...
This feature closes that gap on the **one representative v44 namespace**: it widens the existing writer-emitter to generate the same typed artifacts for **all in-scope** FIX44 application messages (the 50 not-yet-typed families), so the legacy typed-ergonomics chapter is complete before the successor Orchestra / FIX-Latest direction. A measure-first spike (`research/G19-fix-fpml-iso20022/remaining-work/v44-all-families-measure-spike.md`) confirmed emission is mechanical from the existing dictionary model, with a bounded build-time cost and no architectural blocker.
...
- **A-021** the Quote/RFQ family present in FIX44 — `RFQRequest (35=AH)` / `QuoteStatusReport (35=AI)` / `QuoteResponse (35=AJ)`.
- **A-025** the SecurityList family — `SecurityTypeRequest (35=v)` / `SecurityTypes (35=w)` / `SecurityListRequest (35=x)` / `SecurityList (35=y)`.
...
- **R-001..R-005** Registration / indication family — `RegistrationInstructions (35=o)` / `RegistrationInstructionsResponse (35=p)` and the `R-00x`-row indication variants per the coverage-index (`IOI (35=6)`, `Advertisement (35=7)`, `News (35=B)`, `Email (35=C)`).
- **N-001** the Network-status family (`msgcat='app'`) — `NetworkCounterpartySystemStatusRequest (35=BC)` / `NetworkCounterpartySystemStatusResponse (35=BD)`. Distinct from the excluded N-002/N-003 session-FSM pair below.
...
- `[research/G19-fix-fpml-iso20022/remaining-work/v44-all-families-measure-spike.md]` — the measure-first spike (build-cost basis; msgtype-based 86 count).
