...
| **XV §1 — no per-message heap alloc on hot path** | `store` is on the send path | **RESOLVED in Phase 0 (decision #1) — the fix is a strict allocation IMPROVEMENT.** §XV.1 verbatim (`constitution.md:209`) is **hot-path-scoped**. Apples-to-apples probe: the **shipped** `post(pool,use_awaitable)`+rebind that `FileStore::store` uses today = **4 global `new`/op** (and never reaches the pool — the inert bug); the nested-`co_spawn` fix = **1 global `new`/op** (genuinely on the pool). The merged-and-gate-passing store already allocs 4/op and the 008 seam-14 gate (`tests/perf/test_store_alloc_guard.cpp`) imposes **no** zero-global-alloc bar on `FileStore::store` (only `MemoryStore::store` Test 1 + `FileStore::retrieve` Test 2) ⇒ §XV.1 "hot path" operationally excludes `FileStore::store`. So 4×→1× is **not** a regression. Idiom is project-blessed (`asio_tls_transport.hpp:45–51`). `MemoryStore` stays zero. |
