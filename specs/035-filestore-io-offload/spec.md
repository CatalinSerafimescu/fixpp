...
- **FR-004**: The per-method cancellation-result contract (008 **FR-020** / **SC-006**) MUST be preserved exactly: cancellation winning **before** a method's linearisation point completes the awaitable with `store_cancelled` and **no** state change; cancellation winning **after** completes normally with the operation's value and the new state durable. Because the blocking syscall on a worker thread is not interruptible mid-call, "before the linearisation point" has **two** observation points the implementation MUST honour: (i) at the `async_mutex` acquire (no `co_spawn` issued, no syscall), and (ii) at **child pickup on the `file_io_executor`, before the syscall begins** — i.e. cancellation arriving after the writer mutex is held and the child is enqueued but before the pool starts the syscall (no linearisation has occurred). Both (i) and (ii) MUST yield `store_cancelled` with **no durable effect** (matching 2e §4.3.2: *"cancellation aborts pending I/O at the next `file_io_executor` scheduling point"*). Cancellation arriving **after the syscall has begun** → the syscall runs to completion and cancellation is observed (if at all) only when the outer await resumes on the strand, linearised at the durable transition — never a torn or partial durable state, and a durable success is never lost. A deterministic test for sub-case (ii) (cancel-after-enqueue-before-pickup, on a stalled/saturated executor) is required.
...
- **Cancellation cannot interrupt a syscall mid-flight.** A blocking `pwrite`/`fdatasync` on a worker thread is not an asio suspension point; the FileStore cancellation contract is therefore decided around the **start of the syscall** — observable at two pre-syscall checkpoints (mutex-acquire and child-pickup-before-syscall; FR-004) — consistent with the linearisation-at-durable-transition rule in §6.1.4.
