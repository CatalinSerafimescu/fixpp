...
- **SC-007**: Teardown with in-flight offloaded I/O produces **no** UAF and **no** use-of-joined-pool under ASan/TSan. Two complementary witnesses: (a) Engine/terminal-close: `Engine::stop()` (`close(terminal)`) returns only after every in-flight Session-reachable `store()` offload completes, verified by the shutdown-ordering seam (`test_store_shutdown_ordering.cpp` Test 6) — the app-owned pool is then safe to join; (b) graceful-close flush: `Session::close(close_mode::graceful)` invokes the A1 `flush_for_session_close` typed-thunk (`session.cpp:1258–1264`) and `co_await`s it to completion (C1, never detached) — the flush is structurally joined before the session closes. [Note: `Engine::stop()` uses `close(terminal)` at every call site (`engine.cpp:517,947,1042,1085,1366`); `terminal` skips phase-1 (`session.cpp:1241`) and therefore does NOT invoke `flush_for_session_close`. This is not a gap — terminal close sacrifices the graceful flush by design; the graceful path is reached via `Session::close(graceful)` only.]
