...
- [ ] T004 [P] [US1] `bindings/python/tests/test_lifetime.py`: seam #3 (Session outlives `engine.close()` → `send` raises `ObjectLifetime` 1202) and seam #8 (inbound `Message` stashed past callback return / past `session.close()` → accessor raises `ObjectLifetime`); both runnable under the ASan leg. Assert the exact numeric code + no dereference. (SC-001, FR-003/FR-004; AC1/AC2/AC3)
- [ ] T005 [P] [US1] `bindings/python/tests/test_subinterpreter.py`: constructing `fixpp.Engine(config)` from a PEP 554 sub-interpreter raises `fixpp.SubInterpreterRejected` (1201) before any native engine is created. (FR-018, SC-007; contracts C-7)

### Implementation for User Story 1

- [ ] T006 [US1] `Engine` wrapper in `bindings/python/fixpp_oo.py`: `__init__` creates the native handle via the flat `engine_create` AND rejects a non-main CPython interpreter → `SubInterpreterRejected` (1201) before native creation; `open_session(...)` (wraps `fixpp_session_open`, tracks the child in a `_sessions` `weakref.WeakSet`), `start()`; all public methods guard with `_ensure_live()`. (FR-001, FR-002, FR-018, FR-006-init; data-model E-1)
- [ ] T007 [US1] `Session` wrapper in `bindings/python/fixpp_oo.py`: constructed by `Engine.open_session`; holds `_handle`, a **strong** `_engine` ref UP (Py_INCREF — parent cannot be GC'd first), a `_messages` `weakref.WeakSet`, `_application=None`, `_in_callback=False`; `send(msg)` and accessors guard with `_ensure_live()`; `register_application(app)` registers via the `fixpp.i` hand-wrapper (userdata = this `Session`). (FR-001, FR-002, FR-005, FR-006-init; data-model E-2)
- [ ] T008 [US1] `Message` wrapper (two flavours) in `bindings/python/fixpp_oo.py`: `_handle`, `_dead`, `_is_inbound`, a **strong** `_session` ref UP (Py_INCREF); accessors (`get_string`/`get_int`/… as needed) guard with `_ensure_live()`; `set_*` on an inbound message short-circuits to `CapiError(code=4)` (`_is_inbound`, distinct from `_dead`); constructed instances add themselves to `Session._messages`. (FR-001, FR-002, FR-005, FR-012-distinction; data-model E-3; contracts C-1)
- [ ] T009 [US1] `Dictionary` + `Application` wrappers in `bindings/python/fixpp_oo.py`: `Dictionary.load_xml(path)` (wraps `fixpp_dict_load_from_xml`) / `destroy()` guarded by `_ensure_live()` (E-5); `Application` base class exposing `fromApp(self, session, msg)` (v1.0 inbound-only per D-1). (FR-001/FR-002; data-model E-4/E-5)
- [ ] T010 [US1] Restructure the inbound trampoline + register hand-wrapper in `bindings/python/fixpp.i`: make the registration `userdata` the **INCREF'd owning `Session`** (reach the callable via `session._application`); in the trampoline build the inbound `Message` wrapper (engine-owned, `_is_inbound=True`, `own=0`), register it in `session._messages`, pass it to the callback, and **arm `msg._dead=True` before the GIL is released at callback return** so a stashed reference read afterward raises `ObjectLifetime`. (FR-004 — **closes L-053-1**; FR-001 substrate for US2; data-model E-3 inbound lifecycle; contracts C-2)
- [ ] T011 [US1] SC-004 regression gate: confirm the existing flat-substrate tests (`test_roundtrip.py`, `test_exceptions.py`, `test_error_coverage.py`, `test_gil_release_canary.py`, `test_callback_raise_watchdog.py`, `test_smoke.py`) stay green unchanged after the OO layer + trampoline restructure, and the T004/T005 tests pass (ASan leg for T004). (SC-001 non-concurrent, SC-004)
