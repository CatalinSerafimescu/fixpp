...
**Advisory open lock** (FR-013): `flock(LOCK_EX | LOCK_NB)` (Linux) / `LockFileEx(LOCKFILE_EXCLUSIVE_LOCK | LOCKFILE_FAIL_IMMEDIATELY)` (Windows) at open; second opener gets `store_factory_failed`. **Scope restriction added v0.5 per `[2e §D.5]` (Gap 2 close):** supported only on filesystems where these primitives provide effective cross-process exclusive-lock semantics for every host; NFS / SMB / FUSE / cluster filesystems are **unsupported** for v1.0; `make()` does NOT detect or warn — operator attestation required.

**CompID filesystem-safety validation** (added v0.5 per `[2e §D.4]` — Gap 1 close): `FileStoreFactory::make()` MUST validate `sender_comp_id` / `target_comp_id` before composing the filename and before opening the file or taking the advisory lock — reject (a) empty strings, (b) path separators (`/` on Linux; `/` or `\\` on Windows), (c) NUL bytes, (d) `.` / `..` segments, (e) control characters in `[0x00, 0x1F]` or `0x7F`, (f) values producing a path component exceeding `NAME_MAX` (`pathconf(_PC_NAME_MAX)` on Linux; `MAX_PATH` minus directory prefix on Windows) — with `store_factory_failed`. Validation uses primitive `string_view::find_first_of` / `find`; `std::filesystem::path` constructors are not invoked until validation has passed (preserves `noexcept`).
...
**Store-object allocation contract** (added v0.5 per `[2e §D.6]` — Gap 3 close): the `expected_t<std::unique_ptr<MessageStore>>` return type commits v1.0 to **`std::default_delete<MessageStore>`** destruction. The concrete store object MUST be destructible via `delete static_cast<MessageStore*>(p)`. Factory implementations that PMR-allocate the store object itself MUST wrap deallocation into a `std::default_delete`-compatible path (typical pattern: static `operator delete` overload on the concrete store class + `std::pmr::polymorphic_allocator::new_object` for allocation). A `std::unique_ptr<MessageStore, CustomDeleter>` return type is NOT supported in v1.0; reserved for post-v1.0 per `[const §X.4]`. The `mr` PMR resource threaded into `make()` and the FR-026 / FR-027 PMR contracts govern only the store's **internal storage** (slab / ring / framing scratch / index), not the store object's own allocation/deallocation.
...
| I-16 | `FileStoreFactory::make()` takes an `flock` (Linux) / `LockFileEx` (Windows) advisory exclusive lock on the live log; second opener returns `store_factory_failed`. **Scope restriction per `[2e §D.5]` (Gap 2, v0.5):** supported only on filesystems where these primitives provide effective cross-process exclusive-lock semantics for every host; NFS / SMB / FUSE / cluster filesystems are unsupported (no detection or warning at `make()` — operator attestation required). Additionally, **CompID filesystem-safety per `[2e §D.4]` (Gap 1, v0.5):** `make()` MUST reject `sender_comp_id` / `target_comp_id` containing path separators (`/` on Linux; `/` or `\\` on Windows), NUL, `.` / `..` segments, control chars `[0x00, 0x1F]` / `0x7F`, empty strings, or values producing a path component exceeding `NAME_MAX` — with `store_factory_failed` **before** any file is opened or lock taken; mirrored at `cfg_loader` as defense in depth. | E7 | Edge case test in seam 2 / seam 10 (second-opener variant); seam 2 sub-cases per `[2e §9]` N-3 cover the CompID-validation rejects. |
