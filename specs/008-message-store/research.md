...
**Decision:** Probe at T002 (a configure-time check `tools/cmake/check_crc32.cmake`); decision tree:

1. If C++ stdlib provides `std::crc32` (none does today as of 2026-05-20 against Clang 22 / libc++ / libstdc++ — confirmed via `__cpp_lib_*` SD-6 feature-test macros none exist) — use it.
2. Else, add `crc32c/1.1.2` (Google's BSD-3-Clause SSE4.2 / ARM CRC32 instructions when available, software fallback otherwise) as a Conan dependency.

The choice is local to `FileStore`; nothing else in the codebase consumes a CRC32.

**Rationale:** Per-record CRC32 (FR-009) is the torn-write detector on the FileStore restart scan (FR-012) and inside the sentinel record (FR-009: `magic | version | session_triple_hash | crc32`). Performance matters — a 10⁴-message warm-up at default 200-byte frames computes 10⁴ CRC32 values during open. SSE4.2 / ARM CRC32 instructions are ~10× faster than the byte-table software loop; `crc32c/1.1.2` gives both with one Conan row. A hand-rolled CRC32 would force us to implement the platform detection + dispatch, which is exactly what `crc32c` provides — same BSD-3-Clause license compatibility with AGPL per `[const §V.3]` (no LGPL).
...
**Decision:** **One per-`FileStore`-instance** ASIO `asio::io_context` (or `asio::thread_pool` of size 1; deferred to T-impl) with one worker thread. All `pwrite` / `fdatasync` / `rename` / `MoveFileExW` work is posted there; completions rebind to the session strand via `[2d §6.5]` `cancellable_dispatch`.

Per-instance (not per-session, not engine-wide) because:

- Cross-thread races on the log file handle (the file descriptor / `HANDLE` is shared between `pwrite` and `fdatasync`) are avoided when a single executor owns the handle — `[2e §10] Q10` records this as DECIDED.
- A per-engine `file_io_executor` would serialise all sessions' I/O through one thread (latency hazard with N sessions).
- A per-session `file_io_executor` would burn N threads at idle (resource hazard).
...
**Rationale:** Mirrors 007's `system_clock_source`'s per-session `steady_timer` slot pool keyed by `Session*` — same lifetime-binding logic, same `[arch §5.4]` storage-class shape, same `[const §VIII.5]` zero-global-heap discipline (the `io_context` / `thread_pool` is constructed once at `FileStore::open` from `store_arena` PMR, NOT on every call).
...
## D-8 — `[2d §4.5]` / `[2d §4.7]` cross-doc amendments already applied at 007 merge
...
**This feature does NOT re-amend `[2d]`.** It realizes:

- The factory's pure-virtual `make(...)` method that returns `expected_t<std::unique_ptr<MessageStore>>` — i.e., it **extends in place** the 007 stub by adding the virtual method to the existing class (NOT a new class).
...
## D-9 — `[arch §11]` Q3 + `coverage-index.md:76` + `feature-catalogue.md` OSS-002 / COM-009 + `EngineConfig::max_store_memory_per_session` — owed by this feature

**Decision:** All four edits ship **inside this feature's merge bundle** as part of FR-014a / FR-037 / FR-038 / FR-039 (hybrid ownership per Clarifications Session 2026-05-20 Q4).

**Verified anchors at plan time:**

- `.specify/architecture.md:598` reads `| 3 | QuickFIX-compat shim for synchronous MessageStore impls — feasible or document as known incompatibility | 2e | Phase 2 validates [SYN §3.2 Q7] |` — to be amended to `CLOSED in 2e v0.3: Path B only (documented incompatibility); see [2e §4.8.A]. v0.2's Path A subset wrapper retired in round 2 per Codex C-R2-P2-1 escalation.`
- `spec/coverage-index.md:76` reads `| §4.8 | Message recovery | Y | S-014 | — |` — to be amended to `| §4.8 | Message recovery | Y | S-011, S-012, S-013, S-014 | — |` (note: store-side discharges S-011/S-012/S-013; S-014 stays `backlog` for `005`).
- `spec/feature-catalogue.md:240` (OSS-002 row) — to be updated with the v0.3 Path B verdict (no runtime adapter ships; documented incompatibility + `cfg_loader` config translation only).
- `spec/feature-catalogue.md:332` (COM-009 row) — to gain a forward-compat note pointing at `[2e §10] Q2` (post-v1.0; row stays `backlog`).
- `include/fixpp/core/engine_config.hpp` adjacent to line 119 (`default_store_factory` field) — to gain `std::size_t max_store_memory_per_session = 1ULL << 30;`.

**The S-011 / S-012 / S-013 row Status promotion (`backlog → done`) is the orchestrator's step-19 close-out at this feature's Gate-B merge** per the 007 NFR-015 / `[2c App D]` precedent (D-12 below). This feature ships the **structural cross-doc edits**; it does NOT ship the Status promotion in the spec body (which is the Gate-B-merge close-out's signal that the row's implementation is real, not just declared).
...
- **Propagation to factories:** at engine-open time, both `MemoryStoreFactory::make()` and `FileStoreFactory::make()` receive a reference to (or `EngineConfig*` pointer at) the engine's resolved `max_store_memory_per_session` so `make()` can validate without requiring the user to re-pass the value. The minimal mechanism: the factories' constructors take a `const EngineConfig&` (or pointer), and `make()` reads `cfg.max_store_memory_per_session` to validate `(inbound_capacity + outbound_capacity) * max_frame_bytes` against the cap; on overflow, `make()` returns `store_factory_failed`. Storage-DoS construction guard per FR-014 / SC-004 / `[2e §1.2]` / round-2 N9 / C-R2-P1-3 close.
...
**Decision:** The `feature-catalogue.md` rows S-011 / S-012 / S-013 Status field flip (`backlog → done`) is **orchestrator step 19** at this feature's Gate-B merge, applied by `.specify/pipeline.md` step 19. This feature does NOT promote in the spec body; the spec ships the **structural cross-doc edits** (FR-037 / FR-038 / FR-039) but leaves the Status field alone until the orchestrator's step-19 close-out.

S-014 stays `backlog` (FSM half is `005`'s).

The orchestrator's step 19 also records the PR / Tests / Verified linkage for S-011 / S-012 / S-013 (PR number, test target names, `/speckit-verify` decision-doc reference).

**Rationale:** Same pattern 007 used for NFR-015 (`feature-catalogue.md:227` was promoted at 007's Gate-B merge per the `[2c App D]` precedent). The user's feedback memory `feedback_pipeline_mark_done_step` explicitly flags this as the canonical close-out path and notes the recurring forgotten-mark hazard; pipeline step 19 is the explicit mitigation.
...
**Orchestrator-applied (step 19, Gate-B merge):**

| Edit ID | File | Line(s) | Action | Owner |
|---------|------|---------|--------|-------|
| O-1 | `spec/feature-catalogue.md` | 28 (S-011), 29 (S-012), 30 (S-013) | Status `backlog → done` + PR / Tests / Verified linkage | orchestrator |

**Post-merge SHA tracking:** the orchestrator step-19 close-out records the submodule SHA at merge and updates the parent repo's submodule pointer; the catalogue `Verified` column gets the `.specify/decisions/008-message-store-verify.md` short reference.
