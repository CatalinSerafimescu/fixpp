...
//
// specs/008-message-store/contracts/cfg_loader.hpp
//
...
// [2e §4.8.A.1] (user callback re-entry deadlock, hidden std::mutex,
// unbounded blocking on sync I/O, mismatched threading model, cancellation
// token mismatch) compose; a static_assert-wrapped "safe subset" cannot
// enforce them against user attestation. v1.0 ships Path B (documented
// incompatibility + migration recipe) + cfg_loader (config translation).
//
// The §4.8.A.3 1-page migration recipe lives in
// book/migration_from_quickfix.md (FR-031). The compile-time Path B guard
// (FR-032) lives in seam 11.
#pragma once
...
// extracts FileStorePath (and the other FileStore-relevant keys per QuickFIX
