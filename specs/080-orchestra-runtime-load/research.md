...
- Its input, `EngineConfig.dictionaries`, is populated with multiple dictionaries only by the TOML config path (`selector_resolver.cpp:371`). `fixpp_engine_create` (`src/capi/engine.cpp:186-193` — note: distinct from `src/session/engine.cpp:108` cited above) builds its `EngineConfig` setting only executor/clock/application and **never** populates `dictionaries` — a C-API engine's registry is empty, so the FIX50SP2+FIX-Latest collision is **unreachable via the C-API**.
