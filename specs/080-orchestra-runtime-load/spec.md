...
**Input**: User description: "Expose Orchestra / FIX-Latest runtime dictionary loading to non-C++ consumers (the C-API and TOML config) via root-element sniff-and-dispatch, without adding new public C-API symbols; turn the dual-dictionary registry abort into a clean config-layer error." *(Note: the "turn the dual-dictionary registry abort into a clean config-layer error" portion of this original request was **descoped by Gate A round 1** — the collision is unreachable via any config surface; see the moot Clarifications banner above and the behaviors-and-limitations note. The delivered scope is the root-sniff acquisition path only.)*

## Clarifications

### Session 2026-07-19

> **MOOT after Gate A round 1 (2026-07-19):** both questions below shaped the dual-dictionary collision leg, which Gate A descoped — the collision is unreachable via any config surface (TOML reads a single `[dictionary]` table → at most one dictionary; the C-API never populates the engine registry). See plan.md `## Gate A` and the behaviors-and-limitations note below. Retained for audit per Article XX §1.
...
- **FR-008**: A single-dictionary FIX-Latest-only configuration MUST load successfully.
...
- **Contract-widening (must be documented, not silent)**: This is a deliberate widening of the shipped *behavior* of `fixpp_dict_load_from_xml` (052) and of the TOML `dictionary.path` contract. An input (an Orchestra repository document) that previously returned a config-invalid error now loads successfully. This MUST be called out in the spec/plan and captured as a behaviors-and-limitations note. It is **not** a C-ABI change at all: no exported symbol is added or changed, no `fixpp_error_t` value is added, no byte-frozen header is edited, and dispatch is purely internal — so the abidiff/`nm` symbol golden and the `capi_freeze.sha256` header freeze are untouched.
- **Behaviors-and-limitations note (close-out obligation)**: TOML config cannot name two dictionaries (a single `[dictionary]` table resolves to at most one dictionary), and the C-API never populates the engine's multi-dictionary registry. The FIX50SP2 + FIX-Latest `version_registry` `std::abort` therefore remains reachable **only** via a direct-C++ `EngineConfig.dictionaries` caller that bypasses both config surfaces, and is retained by design as the fail-loud backstop (074 L-074-1). No config-layer collision pre-check is added.
- **Frozen-header docs obligation (close-out, pinned — see quickstart.md ABI gate)**: Because `fixpp_dict_load_from_xml` now also accepts an Orchestra `<fixr:repository>` root but its declaring header `include/fix/c_api/dict.h` is byte-frozen (line 3 of `tools/capi_freeze.sha256`), a **non-frozen** public docs / API-reference note MUST be added at close-out stating that the entry point accepts both `<fix>` and `<fixr:repository>` roots, and that the frozen `dict.h` Doxygen prose ("FIX XML data dictionary / wraps `XmlLoader`") is **retained verbatim for ABI stability**. This obligation is pinned so it cannot be skipped.
- **Out of scope**: Full multi-version `version_registry` re-keying (ApplExtID(1156)=303 differentiation) that would let FIX50SP2 and FIX Latest genuinely coexist. Multi-version coexistence remains tracked under REMAINING-WORK row 4b.
...
- **Version registry**: the existing structure that maps a session/application version to its dictionary; it retains its current fail-loud `std::abort` on a dual-dictionary collision as the direct-C++ backstop (074 L-074-1). It is unchanged by this feature — no config surface can express the colliding pair (see the behaviors-and-limitations note), so no pre-check is introduced.
...
- **SC-005**: The C-ABI is unchanged — the `nm` exported-symbol golden (`tests/abi/golden/fixpp_capi_symbols.txt`) and the header byte-freeze (`tools/capi_freeze.sha256`) both pass without regeneration, and `FIXPP_C_ABI_VERSION` stays `1.5.0`. The only additive surface is one new public C++ `dict::load_any` symbol. The frozen-header docs obligation (Contract & Compatibility Notes) is satisfied at close-out.
...
- The Orchestra loader (`dict::OrchestraLoader`) and its fail-closed parse-error behavior from 074 are reused as-is; this feature adds no new loader logic beyond sniff-and-dispatch.
...
- FIX Latest continues to occupy the shared `v50sp2` version slot (074's L-074-1); this feature does not change version-slot keying.
