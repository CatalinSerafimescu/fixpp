...
> **⚠️ Post-Gate-A scope note (2026-07-16):** This document predates the builder descope and the Messages-only golden decision. Where it describes a typed `build_<Msg>`/`validate_<Msg>` builder tier, an app-subset builder coverage predicate, or a multi-file/extended checked-in golden, that is **superseded** — see `spec.md` Clarifications → Session 2026-07-16 (builder descope) and `contracts/build-and-verification.md` V-4/V-7 (golden = `vlatest_Messages.golden.hpp` only + relative byte-diff) for the as-shipped design. Inline `[superseded 2026-07-16]` markers below give the as-landed detail per line; treat this banner as covering everything not individually annotated.
