...
- [x] CHK001 Are the per-message artifact kinds emitted for the universal surface (typed args, readback accessors, `dict::reify()` participation) enumerated so a missing kind is detectable? [Completeness, data-model Entity 2] — PASS: data-model Entity 2 enumerates exactly 4 universal kinds (typed args struct, readback accessors, `dict::reify()` participation, `version_v` constant); contract C-1 mirrors the same enumeration. **SUPERSEDED 2026-07-16:** the originally-planned 2 app-subset kinds (`build_<Msg>`, `validate_<Msg>`) are DEFERRED to a follow-up feature (see spec.md Clarifications → Session 2026-07-16) — not part of this PR's delivered surface. The 4 universal kinds ARE delivered for all 181.
...
- [x] CHK005 Is the Option-A surface split — read/reify/args/readback for **all 181** vs `build_<Msg>`/`validate_<Msg>` for the **application subset** — stated unambiguously (not the earlier "typed builder for all 181" wording)? [Clarity, Spec §FR-001 / Clarifications] — PASS: spec.md Clarifications explicitly names and flags the precision correction; FR-001/C-1/data-model Entity 2 state the split unambiguously; converged and user-confirmed through Gate A rounds 2-3 (plan.md Gate A log, RC-B). **SUPERSEDED 2026-07-16:** the Option-A `build_<Msg>`/`validate_<Msg>` app-subset builder leg described here was subsequently DEFERRED in its entirety to a follow-up feature (spec.md Clarifications → Session 2026-07-16) — no builder of any kind ships in this PR. The originally-audited clarity of the split is moot; the as-landed surface is read/reify/args/readback + runtime-validator for all 181, no app-subset distinction.
- [x] CHK006 Are the application-subset selection criteria defined precisely enough to derive the subset from the Orchestra `category→is_application` mapping without a pinned count? [Clarity, Spec §FR-001 / data-model Entity 5] — PASS: data-model Entity 5 + research R2b state the verified single-category rule (8 `category="Session"` frames = admin, all else = app), explicitly not pinned to a count, fails closed on an absent/empty category only (a present non-`Session` category classifies application — `Session` is EP303's sole admin/session-layer category). **SUPERSEDED 2026-07-16:** the app-subset these selection criteria were designed to feed (the `build_<Msg>` builder coverage predicate) is DEFERRED to a follow-up feature (spec.md Clarifications → Session 2026-07-16); data-model Entity 5 itself now states "this whole entity is moot for the current PR" — the `category→is_application` mapping is retained in the projection (RC-A) but has no consumer in this PR.
- [x] CHK007 Is the scope of the generated `validate_<Msg>` (required/group-presence only, dict-free) clearly distinguished from the separate runtime enum/type/domain validator? [Clarity, Spec §generated-api C-2] — PASS: contract C-2 draws the distinction explicitly (thin wrapper over `wire::validate_required<T>` vs. `dictionary_driven_validator`/`table_view` ctor arg/`wire_field_value_out_of_range`), matching data-model Entity 2 and spec AC-2. **SUPERSEDED 2026-07-16:** `validate_<Msg>` (the generated per-message builder-side validator distinguished here) is part of the DEFERRED builder tier (spec.md Clarifications → Session 2026-07-16) and does not ship in this PR; the shipped runtime `dictionary_driven_validator`/`table_view` path is unaffected.

## Requirement Consistency

- [x] CHK008 Is the "no `build_<Msg>` for admin/session frames" behavior documented as by-design (Option A) consistently across spec, data-model, and contracts — never as a coverage gap? [Consistency, Spec §FR-001 / FR-007 / data-model Entity 2] — PASS: FR-001, FR-007, contract V-2, and spec Edge Cases all use identical "by design (Option A), NOT a skip" language; no artifact frames it as a gap. **SUPERSEDED 2026-07-16:** as landed there is no `build_<Msg>` for ANY message (admin or app) — the whole builder tier is DEFERRED (spec.md Clarifications → Session 2026-07-16), superseding the narrower "admin-only exclusion" framing this item audited.
...
- [x] CHK011 Is behavioral parity with legacy tiers (build → serialize → read-back field-for-field) expressed as an objectively measurable acceptance criterion? [Measurability, Spec §C-2 / SC-002] — PASS: SC-002 + contract V-2 state a binary, objectively-checkable field-for-field equality / zero-skips assertion. **SUPERSEDED 2026-07-16:** the `build_<Msg>` → serialize → read-back leg is DEFERRED (spec.md SC-002 caveat, Clarifications → Session 2026-07-16); the delivered acceptance criterion covers only the universal reify/read-back leg (FR-007(b)), which remains objectively measurable as-is.
