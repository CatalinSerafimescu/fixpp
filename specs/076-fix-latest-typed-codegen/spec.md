...
2. **Given** a typed FIX Latest message with a missing required field or an invalid group presence, **When** its generated `validate_<Msg>` runs, **Then** validation fails identically to a legacy typed message — generated `validate_<Msg>` performs **required/group-presence checks only** (over the typed `Args`, dict-free), the same scope 067/069 emit. **And**, **Given** an out-of-domain enum or type error, **When** the serialized bytes are parsed and run through the separate runtime `dictionary_driven_validator` constructed with the caller-supplied `vlatest` `table_view`, **Then** the enum/type/domain violation is reported (`wire_field_value_out_of_range`, 075) — this is the runtime validation surface, NOT `validate_<Msg>`.
...
- **FR-001**: The codegen MUST emit the typed message layer for FIX Latest (EP303) into a distinct `fixpp::vlatest` namespace, collision-free with `fixpp::v50sp2` and every other existing tier, at the **same surface scope the merged 067/069 emitter produces** (Option A): (a) typed **read/reify/args classes and readback for ALL 181** messages (the `ir.ns`-keyed universal emitters), and (b) typed **`build_<Msg>` builder + `validate_<Msg>`** for the **application subset** of the 181 (the app-scoped `emit_builders` surface, exactly as 069 scopes builders to application messages). The application subset MUST be derived from the Orchestra `category→is_application` mapping (research R2b — verified single-category rule: the **8 `category="Session"` frames** `{0,1,2,3,4,5,A,n}` are admin, every other category is application), via a **vlatest-specific coverage predicate** that does NOT reuse the FIX44-family-specific `kOfficial33` / `kN002N003Excluded={BE,BF,BW,BX,BY}` sets (`emit_builders.cpp:58-76`) — BE/BF/BW/BX/BY are legitimate FIX-Latest application messages and MUST NOT be excluded. A witness MUST pin the admin complement to the exact set `{0,1,2,3,4,5,A,n}` (V-2b) so a business message silently misclassified as admin — which would drop its `build_<Msg>` with no other RED test — fails loudly. **[Gate-A sign-off item — see Clarifications]** this app-subset builder scope is a precision correction to earlier "typed builder for all 181" wording and is confirmed with the user at Gate A.
...
- **FR-007**: Closure verification MUST include differential round-trip vs the FIX Latest baseline (the `vlatest` runtime path), asserting field-level round-trip fidelity with zero skips **within each surface's scope**: (a) the **application subset** round-trips through the full typed **`build_<Msg>` → serialize → read-back** path (the builder surface exists only for the app-subset per FR-001); (b) **all 181** messages round-trip through the universal **reify/read-back** path (which is emitted for every message, not app-gated). No message in either surface's scope may be skipped. (Admin/session messages having no `build_<Msg>` is by design — FR-001 Option A — not a skip.)
