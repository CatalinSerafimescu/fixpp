...
- [X] T001 [P] Create the codegen golden directory `specs/076-fix-latest-typed-codegen/contracts/golden/` with a `README.md` documenting the inventory it will hold (goldens live under `specs/<id>/contracts/golden/`, cf. `specs/069-v44-all-families/contracts/golden/` — there is no `tools/codegen/golden/`). Bytes are added by T016; this task only scaffolds the location + inventory contract. *[superseded 2026-07-16 — as originally written this task planned an extended inventory covering the vlatest `{Fields,Messages,Validator,Reify,Builders}.hpp` + census `Manifest`, AND the extended legacy `v42/v44/v50sp2/vt11` + `_dispatch/` inventory. The as-built, landed inventory (see `contracts/golden/README.md` + `contracts/build-and-verification.md` V-4/V-7) is narrower: `vlatest_Messages.golden.hpp` ONLY (the 003/069 `Messages.hpp`-only precedent) — no extended golden, no `Builders.hpp` golden (builder tier deferred, spec.md Clarifications → Session 2026-07-16), no legacy/`_dispatch/` golden extension (additivity of those is proven by the OFF-vs-ON relative byte-diff walk instead).]*
