...
- [X] T016 [US3] Checked-in codegen golden under `specs/076-fix-latest-typed-codegen/contracts/golden/`: `vlatest_Messages.golden.hpp` (10 MB, `-text`-pinned via a co-located `.gitattributes`). **Descoped from the original inventory:** NO `Builders.hpp` golden (builder tier descoped), and NO `Fields/Validator/Reify` golden (goldened for *no* tier — matches the 003 `Messages.hpp`-only precedent; run-to-run determinism `AC-T1` self-compares them). The census `Manifest.txt` is intentionally NOT goldened (correctness pinned by V-1/V-1b, determinism by AC-T1 — see golden/README.md). Legacy V-7 baseline = the existing 003/069 goldens, unchanged. (contract V-4/V-7.)
...
- [ ] T021 [P] **V-5 fail-closed on unknown datatype** test in `tests/codegen/` — codegen over a synthetic Orchestra fragment carrying an unmapped datatype MUST fail closed (thrown, non-zero exit), never emit a mis-typed field (FR-010, contract V-5).
- [ ] T022 [P] **V-6 frozen-surface confirmation** — the existing C-ABI symbol-golden / ABI-golden gate and the Python surface show **zero** diff with the option ON or OFF (FR-008/SC-004, contract V-6; `ctest -L abi`). Existing gates — assert, do not author new ABI machinery.
- [ ] T023 **Constitution amendment (v0.7 → v0.8 MINOR — applied at merge, rides Gate A per Article XX fold)** — edit BOTH loci in one bump: (a) narrow **Article I §1** (lines 63/65) to reclassify FIX Latest **typed codegen** from post-1.0 to v1.0-delivered-by-076 (leaving only ApplExtID(1156)=303 + session negotiation post-1.0); (b) annotate **Article XVIII §2 line 339** that A-035..A-065 typed classes are delivered in `fixpp::vlatest` in v1.0 (only EP-field back-port into legacy dicts + ApplExtID on-wire differentiation remain post-1.0). Sync Impact Report MUST list BOTH loci (else internally contradictory post-merge). Do NOT run `/speckit-constitution` standalone (symlink-single-sourced) (plan Constitution Check).
- [ ] T024 [P] Update `spec/behaviors-and-limitations.md` and any Normative-References anchors for the delivered `fixpp::vlatest` typed tier (message-level `[FIX-Latest]` DocAbbrev granularity, per 074's disposition).
- [ ] T025 Run `quickstart.md` scenarios 1–7 end-to-end (generate ON, census two-leg, round-trip, OFF additive-clean, dispatch-exclusion, frozen surfaces, determinism golden).

### Mandatory close-out tasks (ALWAYS emit — Gate-B preconditions, Article XVII §8)

- [ ] T026 [P] **Catalogue close-out** — flip every feature-owned OFFICIAL row in `spec/feature-catalogue.md` to `done` with the PR/evidence ref: the 31 FIX Latest new-MsgType rows **A-035..A-065** (`feature-catalogue.md:425-455`) typed-codegen tier, and **D-011** (`:130`) typed-codegen tier; add/update the matching `spec/coverage-index.md` entries (MsgType accounting `:579-617`; D-011 `:704`). (T057 analog.)
- [ ] T027 **Feature-completeness audit (MUST be the FINAL task)** — assert against the merged tree that (i) every `tasks.md` row is `[X]` or carries an explicit waiver rationale; (ii) every spec **FR-** and **SC-** maps to a landed test AND a landed implementation; (iii) every feature-owned OFFICIAL catalogue row is `done` with a matching `coverage-index.md` entry. Record the verdict (100% or fully-waived) in `.specify/decisions/076-fix-latest-typed-codegen-verify.md` (`## Completeness` section) or a sibling `076-fix-latest-typed-codegen-completeness.md`. Hard `/gate-b` precondition (Article XVII §8 / pre-flight 4d). (T058 analog.)
