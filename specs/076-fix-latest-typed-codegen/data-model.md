...
> **⚠️ Post-Gate-A scope note (2026-07-16):** This document predates the builder descope and the Messages-only golden decision. The typed **`build_<Msg>`/`validate_<Msg>` builder tier** (Option A / RC-B app-subset) documented below as an entity/attribute of this feature is **deferred to a follow-up feature** (137 MB uncompilable `vlatest/Builders.hpp` from the non-deduped `emit_builders`); INV-4's generic "byte-stable vs the checked-in golden" (Entity 2) resolves, as landed, to `vlatest_Messages.golden.hpp` only — see `contracts/build-and-verification.md` V-4/V-7. What ships in this PR is the **read/reify/args/readback + runtime-validator tier for all 181** messages, plus the completeness census (Entities 3/3b). See `spec.md` Clarifications → Session 2026-07-16 for the builder descope. Builder-specific sentences below are marked `[DEFERRED 2026-07-16]` inline and retained for traceability — do not read them as delivered.
