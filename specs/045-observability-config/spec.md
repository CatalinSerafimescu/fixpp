...
5. **Given** a file whose OTLP sink references an unreadable or unparseable collector certificate, **When** loaded, **Then** loading fails attributing the failure to that sink/selector, and no session opens.
...
- An OTLP sink with an **empty endpoint**: the underlying sink defaults an empty endpoint to a localhost URL, but in a *file* an explicitly-empty endpoint is far more likely a mistake than intent; treated as a missing/empty required value, rejected fail-closed.
...
- A logger configured but the **engine never started**: configuration is frozen at load/open exactly as in step 1; constructing a logger from config has no observable side effect (sinks open lazily when the engine starts), so a failed multi-error load leaves nothing opened.
...
- **FR-006**: The loader MUST resolve the built-in **sink** kinds from *kind + parameters*: **file** (output directory, base file name, per-file size limit, retained-file count, fsync policy), **syslog** (identity string, facility — subject to platform availability per FR-013), and **OTLP** (collector endpoint, optional collector-TLS certificate, export timeout, max export batch size, max export retries).
...
- **FR-013**: A sink kind that is **real but not available on the current platform/build** (e.g. syslog on a non-POSIX build) MUST be rejected under the **invalid/contradictory selector** reason with a clear diagnostic naming the unavailable kind (Clarifications 2026-06-20); the loader MUST NOT silently skip it, silently drop it from the chain, conflate it with the recognized-but-deferred reason, or report it as an unknown enum.
- **FR-014**: When a sink's backing resource is invalid (an unreadable/unparseable OTLP collector certificate, an unwritable file-sink directory determinable at load), the loader MUST fail closed and attribute the failure to that sink/selector.
- **FR-015**: The loader MUST validate the entire logger configuration and fail closed before any session is opened; on any failure no logger reaches an opened state and nothing is left partially configured.
...
- **FR-023**: Any sensitive value reachable through a logger selector (e.g. credentials embedded in a collector endpoint or header) MUST be redacted from diagnostics and logs, consistent with step 1's redaction behavior.
...
- **SC-003**: For every defined logger failure class — unknown kind, missing required, out-of-range, platform-unavailable sink, unreadable certificate — a deliberately broken file is rejected before any session opens, with a correct per-key diagnostic and **zero** cases of silent no-op substitution or partial configuration.
