...
- **Group 2 (rider)** — harden the `ReconnectFsm` `credentials_rotated` callback-boundary seam: contain a callback exception during a reconnect attempt so it degrades gracefully rather than aborting the in-flight reconnect. (In the live `Session` path this callback is engine-owned and `noexcept`, so the throw is unreachable in production today; G2 hardens the standalone-FSM injection seam — see US2.)
...
### User Story 2 - A throwing credentials-rotated callback seam does not abort a reconnect (Priority: P2)

The `ReconnectFsm` invokes a `credentials_rotated` notification callback during an automatic reconnect attempt in which credentials were rotated. If that callback throws, the in-flight reconnect attempt must not be aborted by the exception; the reconnect proceeds (or degrades) per the existing reconnect policy as if the notification had completed.

**Production reachability (stated plainly — this is NOT an over-claimed arm):** in the live `Session` path the callback is engine-owned and `noexcept` — `Session::open()` injects a `noexcept` lambda that calls the `noexcept` `emit_event` (a pure ring-buffer write invoking no user code), so a throw CANNOT occur through the production path. The only throw-surface today is the standalone `ReconnectFsm` test-injection seam (`set_emit_credentials_rotated`). G2 therefore hardens that callback-boundary seam (defensive robustness if an injected callback — the test seam, or any future non-`noexcept` wiring — throws), explicitly NOT a production user-facing guarantee against an operator-registered throwing callback (no such reachable surface exists today).

**Why this priority**: Robustness of the reconnect callback-boundary seam. This is a graceful-degradation hardening, NOT a crash-prevention fix — an exception escaping the reconnect coroutine body is delivered to the coroutine's promise, not to `std::terminate`. It is lower priority than Group 1 because it does not change protocol behaviour and the unguarded case is already non-fatal; it is higher than Group 3 because it is a (small) production behaviour change rather than test-only.

**Independent Test**: Inject (via the standalone-FSM `set_emit_credentials_rotated` seam) a `credentials_rotated` callback that throws; drive a reconnect attempt in which credentials are rotated; assert the reconnect attempt completes its normal outcome (proceeds/retries per policy) rather than being abandoned because of the callback exception.

**Acceptance Scenarios**:

1. **Given** a reconnect attempt that rotated credentials and an injected `credentials_rotated` callback (FSM seam) that throws, **When** the reconnect attempt invokes the callback, **Then** the exception is contained at the callback boundary and the reconnect attempt continues to its policy-defined outcome (it is not abandoned by the callback throw).
...
- **FR-005**: A conforming inbound first-Logon (well-formed `SendingTime(52)` within the maximum-latency window) MUST establish the session with **emitted output byte-for-byte identical** to today's acceptor establishment path (no regression on the conforming path — the guard adds one parse + one `check_sending_time` on the conforming path but emits identical bytes).

**Group 2 — Reconnect credentials-rotated callback guard**

- **FR-006**: When the `ReconnectFsm` invokes the `credentials_rotated` notification callback (the injected FSM-seam callback), an exception thrown by that callback MUST be contained at the callback boundary so that it does not abort the in-flight reconnect attempt; the attempt MUST continue to its policy-defined outcome. (The live `Session`-path callback is engine-owned and `noexcept`, so this throw is unreachable in production today; FR-006 hardens the injection seam, not a reachable operator-callback guarantee.)
...
- **`credentials_rotated` callback**: the `ReconnectFsm` notification invoked on a reconnect attempt that rotated credentials; the Group 2 guard contains exceptions escaping it. In the live `Session` path it is engine-owned and `noexcept` (unreachable throw); the throwing case is the standalone-FSM injection seam.
...
- **`[FIX-SL §4.3.7]` Specifying application version (DefaultApplVerID(1137) on Logon for FIXT.1.1)** → catalogue **S-025** (shipped by 033; informative §5.3.3). Group 3 adds the missing session-level negative witnesses for the existing 033 `1137` reject path (no production change).
...
- **SC-003**: An acceptor receiving a conforming inbound first-Logon (well-formed `SendingTime(52)` within the window) establishes the session with **emitted output** identical to the pre-change behaviour — verified by a no-regression (capture-and-compare) check on the conforming establishment path.
