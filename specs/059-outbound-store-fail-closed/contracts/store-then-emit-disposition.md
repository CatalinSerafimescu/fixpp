...
- **Propagating, narrow guard `== dispatch_aborted` (1 — the residual):** `Session::send` (`:4051`, consuming `send_impl` `:4496`, the primary US1 app path). It missed the store class, so its guard is broadened to `dispatch_aborted || is_persistent_retain_fatal(err)` — the same durability-classified predicate as `store_then_emit`'s own gate (`[56,65)`, every `store_*` code except `store_cancelled`), not the closed 3-code set (gate-b/r1 FQ-1) — keeping app-veto (`app_do_not_send`/`app_payload_malformed`, `:4049-4050`) non-fatal (outside the store-code range). This is the one genuinely-new-in-059 call-site edit.
