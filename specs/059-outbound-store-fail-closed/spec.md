...
- Q: After a durable-store retain failure fails the session closed and the transient fault later clears, what must an in-process reconnect do (FR-007 / User Story 3)? → A: Reconcile from durable — on the fatal disconnect the session reseeds its outbound wire counter to the durable store counter via the existing targeted setter (`set_next_outbound`), so the next in-process reconnect resumes from the durable value. US3 is IN scope. This clean recovery is scoped to **plain (non-reset) persistent sessions**; under `reset_on_logon` the reconnect's durable reset overrides to 1, and under `bilateral_strict` (the default policy) the reconnect Logon is bounded by the pre-existing, deferred L-029-3 malformed-Logon limitation (a separate tracked item — 059 does not worsen it). See plan `/plan` D4.
...
1. **Given** a **plain (non-reset) persistent** session that failed closed on a durable retain failure and the fault has since cleared, **When** the session reconnects in-process, **Then** it resumes from the durable store's counter and successfully retains the next message (no repeating disconnect).
...
3. **Given** a `reset_on_logon` session, **When** it reconnects after the same fail-closed, **Then** the durable reset overrides the outbound counter to 1 and the reconcile is neutral (resume at 1).
4. **Given** a `bilateral_strict` session (the default policy), **When** it reconnects after the same fail-closed at seq `k>1`, **Then** the reconnect Logon carries `141=Y` with `34=k` — the **pre-existing, deferred L-029-3** malformed-Logon limitation (a separate tracked item); 059 does **not** worsen it (with or without the reconcile the counter is non-1), and clean recovery is NOT claimed for this policy.
...
- **Best-effort emit sites.** Several administrative emit paths (e.g. an outbound reject in reply to a malformed `ResendRequest`, and `Logout` emits) are intentionally best-effort — they ignore the emit result. Because the durable-retain-failure return is the **same shape** these sites already ignore on a transport-write failure, their behaviour is unchanged by this feature (a pre-existing disposition). The key guarantee still holds on every path: the un-retained frame is never transmitted (the fatal branch returns before the transmit step). Each such site is documented with its actual post-swallow continuation rather than re-engineered.
...
- **FR-001**: On a session backed by a **durable/persistent** store, an outbound message-retain failure MUST cause the session to fail closed — propagate the failure to the send caller as an error return (the same return channel the existing transport-write failure uses) and, on the primary application send path (`Session::send`), transition to disconnected — instead of the failure being discarded. The transition is owned by the send caller (parity with the existing transport-failure return), not fired from inside the retain step; the administrative best-effort emit sites are dispositioned per FR-006.
...
- **FR-004**: The fail-closed disposition MUST cover every outbound-retain failure class the store can return on the send path (durable I/O failure, out-of-order rejection, capacity exhaustion), classified by store durability rather than by error code. (The three named classes are the FileStore-reachable illustrative subset, not a closed set — a custom persistent store returning any other store-block error code MUST also fail closed; gate-b/r1 FQ-1.)
...
- **FR-006**: Every `store_then_emit` call site MUST be dispositioned by how it handles the retain-failure return — which is the **same return shape** the store-then-emit step already produces on a transport-write failure. Sites that propagate the return with a broad `if (!emit_r)` guard already fail closed. The one site that gates its transition specifically on the cancellation code (`Session::send`) MUST be broadened to also fail closed on the durable-retain-failure class. Sites that intentionally swallow the return (administrative best-effort emit paths) exhibit the **identical** post-swallow behaviour they already exhibit on the pre-existing transport-failure return — a pre-existing disposition; each MUST be documented with its actual continuation (the un-retained frame is never transmitted on any path), and left as-is (fixing it would be changing the pre-existing transport-failure disposition, out of 059 scope).
...
- **SC-006**: All 26 `store_then_emit` call sites are enumerated and classified: propagating-broad-guard (fail closed via `!emit_r`), the one propagating-narrow-guard site (`Session::send`, fail closed via the broadened guard), and the swallow-and-continue sites (documented as pre-existing transport-failure-parity best-effort, with each site's actual post-swallow continuation recorded).
...
- **A-3 (reconnect reconciliation mechanism)**: Per the 2026-07-03 clarification, the required in-session-reconnect behaviour is **reconcile-from-durable** (US3 in scope). The `/plan` design (D4) realises FR-007's "resume from the durable counter" with a **targeted outbound-only reseed** — the existing `SeqnumManager::set_next_outbound(durable_k)` setter (minted for the 032 initiator restore), done at disconnect time inside `store_then_emit`. It does **NOT** clear the shared `hydrated_` hydration latch (which would bypass the `bilateral_strict` suppression and re-seed inbound — cross-feature blast radius). US3 clean-recovery is scoped to plain persistent sessions; `reset_on_logon` overrides to 1; `bilateral_strict` (default) is bounded by pre-existing L-029-3.
- **A-4 (no public/wire/error-surface change)**: The change is a disposition of an already-returned error along an existing internal error channel; it introduces no new public API, no new wire behaviour, and no new error code visible to counterparties. Fail-closed reuses the existing session-fatal disconnect path. (Note: the pre-existing store-failure codes — `store_io_failure` / `store_seqnum_out_of_order` / `store_capacity_exhausted` — now become observable by the **local** `send()` caller on the persistent path, where today they are swallowed. This is intended — learning of the fail-closed is the point — and is consistent with the `dispatch_aborted` that `send()` already returns to the local caller on a transport failure. No *new* enumerator is added and nothing new is visible on the wire.)
