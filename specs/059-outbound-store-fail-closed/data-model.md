...
- **INV-059-6 (fail-closed via the shared error-return channel).** The persistent retain failure surfaces as an error return of the **same shape** as the existing transport-write failure, so every caller handles it exactly as it handles that pre-existing return. Census of the 26 `store_then_emit` sites (`9305e69`): **9 propagating-broad-guard** (`if(!emit_r)→Disconnected`: `:864 :1781 :1825 :2496 :2820 :3379 :3542 :4615 :4673`) fail closed unchanged; **1 propagating-narrow-guard** (`Session::send` `:4046`, gating on `== dispatch_aborted`) is broadened to the store-fatal class so the primary app path fails closed; **16 swallow-and-continue** (`:2130 :2344 :2656 :2680 :2742 :2869 :2921 :2959 :2982 :3176 :3233 :3687 :4902 :4963 :5221 :5264`) exhibit their **pre-existing** transport-failure continuation (documented best-effort — the un-retained frame is never transmitted). (FR-006; SC-006.)
