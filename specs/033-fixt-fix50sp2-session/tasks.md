...
> **⏸ PHASE DEFERRED → live self-run (T025–T028).** Live interop needs real TCP sockets; the sandbox silently SKIPs sockets → false pass (`feedback_codex_sandbox_blocks_sockets_false_pass`). All US3 work (harness cell registration, golden-writer redactor wiring, 2-pass golden capture, manifest flip) is the 032 close-out pattern — run with sockets allowed, self-verified, NOT marked done on confidence. FR-012/SC-004 + the live halves of SC-001/003/006 close here.
