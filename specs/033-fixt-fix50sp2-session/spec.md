...
4. **Given** a fixpp acceptor, **When** an incoming `FIXT.1.1` Logon omits `DefaultApplVerID(1137)` or advertises an application version fixpp does not support, **Then** fixpp rejects establishment with a well-defined, conformant response (reject/Logout) rather than establishing a mis-versioned session.
...
**Independent Test**: Run the previously-deferred FIXT interop cell against each reference engine as both initiator and acceptor; the cell establishes a FIXT.1.1/5.0SP2 session and the transcript matches a banked golden.
...
2. **Given** the matrix manifest, **When** the feature completes, **Then** the FIXT cell is no longer `deferred:fixt-routing` but carries a live disposition with a real status.

---

### Edge Cases

- **Application version mismatch**: an inbound FIXT Logon advertises an application version fixpp does not support → reject establishment conformantly; do not silently downgrade or establish mis-versioned.
- **Missing `1137` on a FIXT Logon**: an inbound `8=FIXT.1.1` Logon omits `DefaultApplVerID` → reject/Logout per spec (the field is required for FIXT establishment) rather than guessing a default.
...
- **FR-002**: The system MUST, for a FIXT-configured session, include `DefaultApplVerID(1137)` on the outbound Logon set to the configured application version (e.g. FIX.5.0 SP2).
- **FR-003**: The system MUST, as acceptor, read `DefaultApplVerID(1137)` from an inbound FIXT Logon and use the indicated application version as the session's default application dictionary for the duration of the session.
- **FR-004**: The system MUST reject FIXT session establishment (conformant reject/Logout, no Active session) when an inbound FIXT Logon omits `DefaultApplVerID(1137)` or advertises an application version the deployment does not support.
...
- **FR-006**: The system MUST allow an operator to configure, per session, that it speaks FIXT.1.1 with a specified default application version, independently of the existing FIX.4.x configuration path.
...
- **FR-008**: The system MUST, as acceptor, parse inbound `Username(553)`/`Password(554)` when present and make them available to the existing authorization decision path, while still permitting credential-free establishment when the deployment does not require them.
...
- **FR-010**: The system MUST tolerate (parse without failure) an inbound per-message `ApplVerID(1128)` on application messages of an established FIXT session, regardless of whether per-message application-version routing is implemented (see Assumptions / S-026 scope decision).
- **FR-011**: The system MUST NOT emit a populated `Password(554)` value into any persisted log, transcript, or golden artifact in clear text. *(Refined by clarify if a redaction convention already exists.)*
...
- **SC-004**: The previously `deferred:fixt-routing` interop cell passes live against both reference engines (QuickFIX-cpp and QuickFIX-J), in both roles, with banked goldens — i.e. the FIXT axis of the parity matrix is no longer deferred.
- **SC-005**: An inbound FIXT Logon that is missing `1137` or advertises an unsupported application version never results in an established (Active) mis-versioned session.

## Assumptions

- **Per-message `ApplVerID(1128)` routing (catalogue S-026) is OUT of scope for this feature** and deferred to a follow-on. The MVP negotiates a single session-wide default application version via `1137`; inbound per-message `1128` is tolerated (FR-010) but not used to switch dictionaries mid-session. *(This is the explicit open scope question flagged for `/speckit-clarify`; recorded here as the working default pending that decision.)*
- The target application version for this feature is **FIX.5.0 SP2** (the de-facto contemporary application version and the one the deferred interop cell names); the configuration mechanism is general enough to name a version, but 5.0 SP2 is the only application version this feature must establish and validate live.
