...
- Q: `verify_peer` multi-violation ordering — short-circuit first hit vs aggregate report → A: Short-circuit first hit in a documented 10-step evaluation order — (1) per-cert DER size → (2) RSA key lower bound (≥2048) → (3) RSA key upper bound (≤ `max_rsa_key_bits`) → (4) ECDSA curve (P-256/P-384) → (5) chain depth → (6) SAN cardinality → (7) X.509 version (v2/v3) → (8) expiration against effective clock → (9) Pinning lookup against `cfg.pinset_snapshot` under `mtls_pinned` → (10) CipherPolicy `is_allowed`. One `error::tls_*` variant per failed handshake (most rejection causes route through the `tls_handshake_failed` grouping variant with a diagnostic-field sub-reason per `[2g §6.6]`; only DoS-cap and pinning rejections use dedicated variants). Matches the OpenSSL `X509_V_ERR_*` norm; keeps the error envelope flat for the 2i C-ABI projection. Canonical step list at `contracts/security_profile.hpp:150-165`. Reference-engine sweep: no precedent — QuickFIX-cpp / QuickFIX-J / Fix8 ship no analogous DoS-bound surface. (Amended round 3 hand-edit 2026-05-23 — 8-step prose aligned with the 10-step canonical list.)
- Q: Pin lookup key for `Pinset::find` / `add` / `remove` — SHA-256-of-leaf-DER vs SPKI vs Subject DN → A: SHA-256 fingerprint of the leaf cert's DER encoding (32 bytes). Matches FIXS-RC1 §5 and design-doc §4.5 line 115. Stored as `std::array<std::byte, 32>` in the Pinset; lookup is SHA-256-keyed. **(Amended round 2 — round-2 NEW-P1-2 close)** The API ergonomics layer on top: `add` consumes a `Certificate const&` (the diagnostic envelope — subject_dn + SAN list — is PMR-copied from the cert at `add()` time per `[2g §4.3]` lines 482-487); only `remove` and `find` key on the bare SHA-256 fingerprint. The /clarify Q4 decision itself stands — the lookup key is unchanged SHA-256-of-leaf-DER; only the `add()` argument type changed from a bare fingerprint to a `Certificate` so the snapshot carries diagnostic context (SC-006). Pin rotation: the operator parses the new leaf cert into a `Certificate`, calls `add(new_leaf_cert)`, then `remove(sha256_of_old_leaf)`.
...
3. **Given** the operator calls `remove(old)` before `add(new)`, **When** a peer presents the new cert, **Then** the handshake fails with `error::tls_pin_mismatch` — the operator-induced ordering error is surfaced, not silently absorbed.
...
2. **Given** an operator-supplied `cert_source` whose `load_credentials()` returns `unexpected{tls_cert_load_failed}`, **When** the session is opened, **Then** session open fails with that exact error variant — no fallback to the file default.
...
4. **Given** the default `file_cert_source` constructed with a non-existent PEM path, **When** the factory `make_file_cert_source(...)` is called, **Then** it returns `unexpected{tls_cert_load_failed}` rather than throwing across the session-handling boundary.
...
3. **Given** a session opened in `mtls_ca` mode against a peer presenting an RSA-1024 leaf cert, **When** `verify_peer` runs, **Then** the handshake is refused with `error::tls_handshake_failed` whose diagnostic field carries sub-reason `"rsa_under_min"` per `[2g §6.6]` line 995 (the `tls_handshake_failed` grouping variant + sub-reason — see FR-025).
...
5. **Given** an `mtls_pinned` session whose pinset is non-null but empty (zero entries) at session open, **When** the session attempts to open, **Then** session open is refused at session-config time with the distinct variant `error::tls_pin_empty_at_open` — distinct from `tls_pin_mismatch` so operator logs separate "fixpp-config problem" from "peer-cert problem" (Clarifications 2026-05-23).
...
- **PEM password incorrect**: `make_file_cert_source(Config, …)` returns `unexpected{tls_cert_load_failed}`; no exception crosses the session-handling boundary.
- **Cert file unreadable** (permissions / missing): same — `unexpected{tls_cert_load_failed}`.
...
- **PMR allocator throws during cold-path `load_credentials`**: routed through `[2a §4.2]` `trap_throw`; surfaces as `error::tls_cert_load_failed`.
...
- **FR-004**: A file-based default impl (`file_cert_source`) MUST ship as the v1.0 reference — PEM/DER over a caller-supplied `file_cert_source::Config` per `[2g §4.2]` (the abstract base `cert_source` does NOT publish `Config`; `file_cert_source::Config` is impl-specific per data-model E-1 / E-1a — round-2 P1-1 propagation close), with an optional password callback for encrypted PEM.
...
- **FR-006**: `Pinset::add(Certificate const&)` and `Pinset::remove(std::array<std::byte, 32> const&)` MUST be separate explicit operations per `[2g §4.3]` lines 486-495 (Codex P1-2 + NEW-P1-4 close). `add` consumes a `Certificate` value and PMR-copies its diagnostic fields (subject_dn + SAN list) into the snapshot at add() time; the snapshot's per-pin diagnostic envelope is what the operator-visible logs / metrics inherit (SC-006). `remove` keys on the SHA-256 fingerprint (32 bytes) — the same `pin_fingerprint = std::array<std::byte, 32>` type `find` consumes. There MUST be NO atomic-swap or "set" shortcut per `[2g §6.5]` invariant 3. Old pins MUST remain usable for `find` until `remove` returns. A factory `make_pinset(Pinset::Config, std::pmr::memory_resource*) -> expected_t<std::shared_ptr<Pinset>>` MUST be provided as the [arch §6] rule-4 entry point for non-construction-time callers (2i C-ABI, future hot-reload), mirroring `make_file_cert_source`'s factory pattern per `[2g §4.3]` line 466 + `contracts/pinset.hpp:119`. (security, `[FIXS RC1 §5]`; Clarifications 2026-05-23.)
...
- **FR-013**: `SecurityProfile` MUST publish four enumerators per `[2g §4.5]` lines 650-656: the `unset = 0` sentinel plus the three user-visible values `mtls_ca`, `mtls_pinned`, and `one_way_ca`. The `unset` sentinel MUST be rejected at `make_ssl_ctx_config` with `error::tls_invalid_security_profile` and at `Session::open` with `error::invalid_session_config` per `[const §XII.5]`'s no-implicit-default rule. The `[[deprecated]]` attribute MUST be applied to the `one_way_ca` enumerator declaration itself (not in a comment) per `[const §XII.5]`. (security; Codex P1-4 close — the v0.1 "exactly three enumerators" wording silently rejected the `[2g §4.5]` sentinel-construction model; v0.2 rewrites to four enumerators with `unset` rejected at construction.)
...
- **FR-020a**: `verify_peer` MUST short-circuit on the first violation hit and surface exactly one `error::tls_*` variant per failed handshake (no aggregate report). The evaluation order MUST be (10-step canonical list per data-model E-14 + `contracts/security_profile.hpp` lines 156-165 + `[2g §6.5.1]` BINDING CONTRACT — round-2 P1-3 propagation close): (1) per-cert DER envelope size → (2) RSA key lower bound (≥ 2048) → (3) RSA key upper bound (≤ `max_rsa_key_bits`) → (4) ECDSA curve (P-256/P-384) → (5) chain depth → (6) SAN cardinality → (7) X.509 version → (8) expiration against `effective_clock` → (9) pinning (under `SecurityProfile::mtls_pinned`: scan `cfg.pinset_snapshot` for `peer_chain[0].sha256()` per `[2g §6.5.1]` — NEVER call `cfg.pinset->find/contains/snapshot` mid-verification) → (10) `CipherPolicy::is_allowed` on the negotiated suite. Matches OpenSSL `X509_V_ERR_*` semantics; keeps the 2i C-ABI projection flat. (security; Clarifications 2026-05-23.)
...
- **FR-025**: The `error::tls_*` variant family MUST be the **15-variant set re-emitted from `[2g §6.6]`** plus the one additive amendment from `/clarify` Q2 — 16 variants total (Codex P2-1 + NEW-P1-4 close). The binding set is, grouped by C-ABI coalescing target per `[2g §6.6]` lines 1006-1013:
    - **`FIXPP_ERR_TLS_CONFIG`**: `tls_cert_load_failed`, `tls_cert_parse_failed`, `tls_cipher_not_allowed`, `tls_invalid_security_profile`, `tls_sign_callback_unavailable`, and the `/clarify` Q2 amendment `tls_pin_empty_at_open` (distinct from `tls_pin_mismatch` — surfaces at session-open when `mtls_pinned` is paired with a non-null empty pinset).
    - **`FIXPP_ERR_TLS_HANDSHAKE`** (the C-ABI coalescing group; the `tls_handshake_failed` grouping variant + DoS-cap + pinning variants): `tls_handshake_failed` (the grouping variant — the diagnostic field carries sub-reasons `"expired"` / `"not_yet_valid"` / `"rsa_under_min"` / `"ecdsa_curve"` / `"chain_too_deep"` / `"x509_v1"` / `"sigalg_disallowed"`), `tls_rsa_key_too_large`, `tls_cert_der_too_large`, `tls_san_entries_exceeded`, `tls_pin_mismatch`.
    - **`FIXPP_ERR_TLS_PINSET`**: `tls_pin_not_found`, `tls_pin_already_present`, `tls_pinset_capacity_exhausted`.
    - **`FIXPP_ERR_TLS_RUNTIME`**: `tls_pinset_alloc_failed`.
    - **`FIXPP_ERR_CANCELLED`** (existing per `[const §XI.2]`): `tls_load_cancelled`.
  `verify_peer`'s short-circuit per FR-020a returns ONE variant per failed handshake. Most rejection causes route through the `tls_handshake_failed` grouping variant with a diagnostic-field sub-reason; only DoS caps and pinning use dedicated variants. (Codex P2-1 close — the v0.1 bundle re-derived a per-cause-variant set that disagreed with `[2g §6.6]` in 9 of 15 names AND silently dropped the `tls_handshake_failed` grouping variant the C-ABI coalescing scheme depends on; v0.2 transcribes `[2g §6.6]` verbatim and adds only the `tls_pin_empty_at_open` Q2 amendment.)
...
- **`verify_peer` multi-violation ordering** (resolved via Clarifications Session 2026-05-23): When a peer cert violates multiple bounds simultaneously, `verify_peer` MUST short-circuit on the first violation hit in this documented 10-step evaluation order (canonical per data-model E-14 + `contracts/security_profile.hpp` lines 156-165 + `[2g §6.5.1]` BINDING CONTRACT — round-2 P1-3 propagation close): (1) per-cert DER envelope size → (2) RSA key lower bound (≥ 2048 per `[FIXS §3.4]`) → (3) RSA key upper bound (≤ `max_rsa_key_bits`) → (4) ECDSA curve membership (P-256/P-384 only) → (5) chain depth ≤ `max_chain_depth` → (6) SAN list cardinality ≤ `max_san_entries` → (7) X.509 version (v2/v3 only) → (8) expiration against `effective_clock` → (9) pinning (under `SecurityProfile::mtls_pinned`: scan `cfg.pinset_snapshot` for `peer_chain[0].sha256()` per `[2g §6.5.1]`) → (10) `CipherPolicy::is_allowed` on the negotiated suite. One `error::tls_*` variant per failed handshake. Matches OpenSSL `X509_V_ERR_*` norm; keeps the 2i C-ABI projection flat (no list-of-codes type). FR-020a codifies this ordering.
