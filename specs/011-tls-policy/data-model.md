...
- **API** (re-emitted verbatim from `[2g §4.3]` lines 486-518 — Codex P1-2 + NEW-P1-4 close):
  ...
  - `[[nodiscard]] static core::expected_t<std::shared_ptr<Pinset>> make_pinset(Config cfg, std::pmr::memory_resource* mr) noexcept;` — `[arch §6]` rule-4 factory entry point per `[2g §4.3]` line 466; wraps construction in `[2a §4.2]` `trap_throw` so any PMR-allocation throw surfaces as `expected_t::unexpected{tls_pinset_alloc_failed}` — 2i C-ABI callers and future hot-reload paths NEVER see a thrown exception. Mirrors `make_file_cert_source` (E-1a). Direct-construction via `explicit Pinset(Config = {})` is also supported for engine-bootstrap-time callers per the `[arch §5.3]` carve-out. Surfaced in FR-006 (spec.md) per the 2026-05-24 /speckit-checklist-audit SPEC-FIXED.
- **Validation rules**:
