...
| P5 | the framer's carry buffer is constructed with capacity `>= max_bytes + 1` | the implementation itself (`engine.cpp:402`), as a value **derived from** the clamp bound. Not a caller obligation, but stated as a precondition of the postconditions below: without it S4/F1 do not hold. See I6. |
...
`engine.cpp:863-866` is `if (!read_r.has_value()) { transport->close(); continue; }` — every error
...
- **Not** which of the two cancellation-attributable errors an `Engine::stop()` produces. *(Added at
  ...
  `transport_read_cancelled` specifically. Today's sole caller is unaffected — `engine.cpp:863-866`
...
| **Blast radius** | the **two** production callers of `Transport::async_read_some` — `engine.cpp:413` (this contract) and `engine.cpp:542` (the session read pump). The pump's own termination comment (`engine.cpp:465-471`) already contracts for this behaviour; INV-4a is dispositioned **SAFE BY STRAND CONSTRUCTION** (research §D-2a.7) |
