...
| **Blast radius** | the **two** production callers of `Transport::async_read_some` — `engine.cpp:413` (this contract) and `engine.cpp:460` (the session read pump). The pump's own termination comment (`engine.cpp:383-389`) already contracts for this behaviour; INV-4a is dispositioned **SAFE BY STRAND CONSTRUCTION** (research §D-2a.7) |
