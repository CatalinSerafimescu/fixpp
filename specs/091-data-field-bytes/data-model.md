...
| `fixpp_session_close` | U at call; may observe the Logon-scan effect | its return code depends on "established at least once"; `interpret_logon` stops at a malformed count, so a required field after it is not read. **If** that makes the handshake refuse (not traced), close's result changes for that Logon | `version.h` history (conditional) |
| `fixpp_session_is_established` | U at call; may observe the Logon-scan effect | as `fixpp_session_close`: **if** the handshake refuses (not traced), `true` becomes `false` for that Logon | `version.h` history (conditional) |
