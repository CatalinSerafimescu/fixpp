...
| **XVII §1** Gate A: public C++ API + codegen layout | **yes, both triggers** | ✅ Gate A converged (loop 3 r3, owner-accepted without Codex) |
...
include/fix/c_api/version.h             # FR-019: FIXPP_C_ABI_VERSION_MINOR 8 → 9, re-authored trailing comment naming 091/fixpp#418 + the no-carrier effects (replay gap-fill, header/Logon scans; is_established/close as observers of the Logon refusal)
...
include/fix/c_api/session.h             # FR-019: BREAKING (1.9) notes on fixpp_session_send (incl. toApp not invoked on refusal) and fixpp_session_register_callback
...
0b. **C-ABI 1.9 BREAKING (FR-019)**, in the same PR as phase 0. Pattern: `.specify/495-493-486-dict-reify-copy.md`'s 1.8 procedure, re-derived for 1.9.
   ...
   - **C-ABI test, written first:** a synthetic XML dictionary declares a custom pair adjacently only inside a `<component>`. The test loads it with `fixpp_dict_load_from_xml`, writes a malformed instance through the C-ABI (Data with no Length before it), and calls `fixpp_msg_commit`.
     ...
     - **Setters:** for the three instances above, hand-written, as the construction names. Not `fixpp_msg_set_data`: on the unfixed loader the tag is unpaired and it refuses with `FIXPP_ERR_TYPE_MISMATCH`.
     - **`set_data` failure → different failure** (FR-019), in the same fixture: `fixpp_msg_set_data(5002, bytes, 0)` is deliberately `set_data`, because its unfixed-loader `FIXPP_ERR_TYPE_MISMATCH` is the RED; after FR-017 it asserts `FIXPP_ERR_WIRE_CONFORMANCE`. The same preconditions apply. Re-derive before writing it: in `fixpp_msg_set_data` the `len == 0` refusal must precede the `FIXPP_ERR_DICT_CONFIG` declaration check, else the GREEN value is that check's answer.
   ...
   - **Reader witnesses, written first** (FR-019), in the same file as the inbound-drop witness and over the same synthetic table. Each asserts **both** that `parse()` returns a value **and** the reader effect, so a parse refusal cannot pass as an absorbed field:
...
### Loop 3 round 2 — disagreements

None. No Opus-judged finding was marked Disagree. Recorded so later rounds do not re-raise them:
- **Supersedes loop 3 round 1's "no 1.9 notes on the additive setters"** for `fixpp_msg_set_data`
  and `fixpp_entry_set_data` only: their failure → different failure case is BREAKING (P2-2) and
  carries a note. `fixpp_msg_set_string`/`fixpp_entry_set_string` stay note-free (additive only).
- **P3-1 recorded as conditional, not asserted:** only `interpret_logon`'s stop at a malformed count
  was traced, not the handshake refusal it may cause, so `fixpp_session_is_established` /
  `fixpp_session_close` are named in the `version.h` history as conditional observers with no
  return-code change claimed.
- **P3-2 traced before asserting:** `send_impl` returns the carry refusal before its toApp
  `parse_and_dispatch_`, so the send note's toApp clause is a traced claim.
- **Appendix A carries no total and no line citations**; its row set is gated by the Phase 0b diff.

- Loop 3 round 2 applied 2026-09-24: Codex P1=0 P2=1 P3=0; Opus post-judging P1=0 P2=2 P3=2; rewrite adds the golden-equal C-ABI export classification appendix (data-model), msg_version/get_msg_type readers, failure→failure BREAKING rule for set_data. Reviews: research/reviews/codex_091-data-field-bytes_gate_a_L3_2_review.md, research/reviews/opus_091-data-field-bytes_gate_a_L3_2_adversarial_review.md.
