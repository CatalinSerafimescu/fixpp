...
- **Type-erased lock (`detail::shard_guard` + `atomic_shared_ptr.cpp`)**: the opaque RAII guard declared in the header; the 128-`std::mutex` shard table + the guard's lock/unlock bodies defined in `src/sync/atomic_shared_ptr.cpp`, compiled only on the fallback path. Keeps `<mutex>` and the `std::mutex` token out of the header.
