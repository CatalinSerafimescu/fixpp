...
- [ ] T021 [US2] Stand up the Tier-2 / opt-in (label-triggered, like `windows-msvc`) `linux-clang-libc++` CI lane: full library build + the concurrency-relevant test subset under available sanitizers on the fallback path; OTel-scoped per D-4; enable LSan where the host allows. [FR-011, E-6]
...
- **Polish (T024–T029)**: T024/T025/T026/T027 are [P] (different files). T028 after the lanes exist (US1/US2). T029 last (the completeness gate, /gate-b precondition).
