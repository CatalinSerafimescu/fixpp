...
This feature **discharges L-049-1** (wire the recorded `consumer_minor` into the live `translate_for_consumer` downgrade at `fixpp_engine_create`). **L-049-2 is DEFERRED (L-050-4), not discharged** — implement-time source contact found `[2i §4.3]` has no published session/app block (coalesced remediation-class design + hardcoded occupancy gate), so publishing the `FIXPP_ERR_SESSION_*` block would require editing the signed-off `[2i]` (forbidden by the Gate-A LEAVE / CHK030); per the user descope the 5 reachable session/app arms stay `FIXPP_ERR_UNKNOWN` and L-049-2 stays open (the published block defers to a dedicated `[2i §4.3]` amendment). The C-ABI version takes an additive **MINOR** bump (0.2.0 → 0.3.0), justified by the 21 new symbols alone.
...
**Scale/Scope**: **21** new exported functions (engine create/start/destroy = 3; session open/close/is_established/send/register_callback = 5; engine-config builder create/set_worker_threads/set_realtime_clock/destroy = 4; session-config builder create/set_comp_ids/set_begin_string/set_role/set_heartbeat_seconds/set_security/set_dictionary/set_reset_on_logon/destroy = 9) — authoritative count = the nm golden (tasks T014/T034) + 2 new opaque config-builder handle types + the internal `Application` trampoline + the internal event-loop owner. **No new published error block** (the session/app block is DEFERRED — L-050-4; the 5 reachable arms stay `FIXPP_ERR_UNKNOWN`, no `error.h`/`translate()`/occupancy delta). No new third-party dep, no codegen, no wire-format change.
...
- **Article X — ABI Policy (governing).**
  ...
  - §X.4 bounded enum + reserved ranges + stability + audit + occupancy → **no new error codes** (the `FIXPP_ERR_SESSION_*` block is DEFERRED — L-050-4; `[2i §4.3]` has no published session/app block, publishing needs an `[2i]` amendment out of scope). No `error_codes_v1.txt` append; the occupancy gate stays GREEN unchanged. **The downgrade still goes live** (FR-002 records `consumer_minor` at create); SC-004 witnesses it with a SYNTHETIC minor-3 code (no real code born at minor 3). Stability does not yet bind (major 0). **PASS.**
...
**Post-Phase-1 re-check**: still no violation. Phase 1 surfaced and resolved: (a) the close-thunk on-strand bridge (post + synchronous wait) is the one place a C thread blocks on the session domain — modelled in data-model E-6, TSan-gated; (b) the session/app block membership (which `session_*`/`app_*` variants are reachable through `session_open`/`session_send`) is enumerated in data-model E-4 — **but publication is DEFERRED (L-050-4, implement-time descope): all reachable session/app arms STAY `FIXPP_ERR_UNKNOWN`, no block is published, L-049-2 stays open.** No open mapping decision remains for Gate A beyond the send-reentrancy-class question (D-7) — **resolved at Gate A round 1 to `THREAD_SAFE`** (see `## Gate A`). The cross-thread blocking-bridge surface is **two** thunks (close AND send) — both TSan-gated (data-model E-6).
...
│   └── error-and-abi.md        # session/app block DEFERRED (L-050-4) + downgrade go-live (synthetic) + nm golden
└── checklists/
...
├── error.h         # UNCHANGED — FIXPP_ERR_SESSION_* block DEFERRED (L-050-4); no new codes
...
├── error.cpp       # UNCHANGED — no translate() re-point (block DEFERRED, L-050-4); session/app arms stay UNKNOWN, L-049-2 open
└── CMakeLists.txt  # EDIT — add engine.cpp/session.cpp/config.cpp to fixpp_capi_objects

tools/
├── abi_history/error_codes_v1.txt   # UNCHANGED — no append (block DEFERRED, L-050-4); occupancy gate stays green
...
- Round 3 applied 2026-06-24 — **orchestrator-inline (rewrite budget of 2 exhausted; per the user's "apply mechanical fixes yourself" authorization)**: Codex P1=0 P2=2; Opus census post-judging P1=0 P2=4 — all completeness additions to the E-4 reachable-arm set + the send-contract return list, **no design change, no new `fixpp_error_t`, no `translate()` re-point** (the "exactly 5 newly-published" invariant holds). A round-3 source census (Opus, independently confirmed by the orchestrator reading `src/session/session.cpp` + `seqnum_manager.cpp`) caught arms that R1/R2 both missed AND a **round-1-rewrite-introduced phantom**: the round-1 E-4 had listed `store_io_failure → STORE_RUNTIME` as send-reachable, but on the `Session::send` path `store_then_emit` **swallows** store I/O errors (I-07 logged-then-proceed, `session.cpp:4790`) — only `store_seqnum_overflow` (counter at `seqnum_max`, `assign_outbound`) is the reachable store-domain send arm. Edits: E-4 += `wire_frame_too_large` (30 → WIRE_LIMIT_EXCEEDED, existing), `store_io_failure`-row retargeted to `store_seqnum_overflow` (60 → STORE_RUNTIME, existing), `session_already_closed` (52) gains its send-path TOCTOU producer; send contract return-list += `APP_PAYLOAD_MALFORMED` + `WIRE_LIMIT_EXCEEDED` + `THREAD_SESSION_LIFECYCLE`; **US2/AC3 corrected** to witness `store_seqnum_overflow` (realizable: restore outbound counter at `seqnum_max`) instead of the I-07-swallowed I/O failure, and the swallow recorded as **L-050-3**. Reviews: research/reviews/codex_050-c-abi-session-send-recv_gate_a_3_review.md, research/reviews/opus_050-c-abi-session-send-recv_gate_a_3_adversarial_review.md. **Terminal-completeness statement (Opus census, all 6 producers traced to every return site): `engine_create` + config-builder setters → zero `core::error`; `register_session`/`start`/`send`/`close` fully traced; the reachable set is now exact.** _(The "exactly 5 newly-published" framing in this entry was LATER **SUPERSEDED** by the implement-time descope — see the Round-3 descope note below: those 5 arms stay `FIXPP_ERR_UNKNOWN`, publication deferred to L-050-4.)_
- **Round 3 — implement-time descope (user decision 2026-06-24).** `[2i §4.3]` (the master C-ABI error layout) has **no published session/app block** — `session_*`/`app_*` are the *unpublished* `FIXPP_ERR_UNKNOWN` bucket (`error.h:51`), the design uses **coalesced remediation-class groups** (not per-variant codes), and `tools/check_capi_occupancy.sh` has a **hardcoded `EXPECTED` map** derived from the `[2i §4.3]` master. So publishing 5 per-variant codes would require **editing the signed-off `[2i]`**, contradicting the LEAVE decision / CHK030 (`[2i]` not reopened). FR-015's premise (a "reserved `[2i §4.3]` session/app block") was **false**. **User chose DESCOPE:** leave the 5 reachable session/app arms (119/77/129/130/131) at `FIXPP_ERR_UNKNOWN`, defer the published block to a dedicated `[2i §4.3]` amendment (**L-050-4**); **L-049-2 stays open**. The **L-049-1 downgrade mechanism (FR-002 / SC-004) still ships** (witnessed via a synthetic minor-3 code). **Drops FR-015 / SC-005.** No new `error.h` codes, no `translate()` re-point, no `error_codes_v1.txt` append, occupancy gate stays green unchanged. Cross-artifact sweep applied to spec / data-model / research / contracts / tasks / checklist / plan.
...
- **L-049-2 discharge outcome (Round 2 — SUPERSEDED by the Round-3 descope, L-050-4).** Round 2 enumerated, from source (`src/capi/error.cpp` + `Engine::send`/`Session::send`/`Session::open`/`register_session`/`start` impls — see data-model E-4 / research D-8), the reachable arms that 049 left at `FIXPP_ERR_UNKNOWN` as exactly **5**: `session_invalid_argument` (119), `session_invalid_state_for_send` (77) — session-PROTOCOL block — plus `app_do_not_send` (129), `app_callback_threw` (130), `app_payload_malformed` (131) — app block. (131 was missing from the round-1 E-4 table; it is reachable because the 020 fail-closed opaque-payload validation runs on the byte-span `Engine::send` path — `session.cpp:4112-4330`.) **Round 2 planned to publish a `FIXPP_ERR_SESSION_*` block for these 5; the Round-3 implement-time descope (above) REVERSES that — `[2i §4.3]` has no such block to publish without editing signed-off `[2i]`, so all 5 stay `FIXPP_ERR_UNKNOWN`, nothing is published, and L-049-2 stays OPEN (L-050-4).** The reachability enumeration itself remains valid; only the publish-vs-stay-UNKNOWN disposition flipped.
