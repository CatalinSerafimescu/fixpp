...
**Evidence**: `Application` interface — `application.hpp:46–108`: 7 virtuals, 0 pure; `fromApp(const MessageView<access_mode::Index>&, const SessionId&) → expected_t<void>` (`application.hpp:84–88`, default accept). Threading: `application.hpp:67–68` — "fromApp is dispatched ON the session strand (single-thread confinement, INV-2)". `Application` is engine-wide via `EngineConfig::application` (`engine_config.hpp:179`, null → no callbacks), MUST outlive the Engine + every Session.
...
**Rationale**: the contract explicitly assigns the C-ABI thunk the job of serialising onto the session domain — so the thunk posts `Session::close` onto the strand and waits. This is the one place a C thread blocks on the io thread; it is the `FIXPP_REQUIRES_SESSION_LOCK` pattern and the primary TSan-gated surface ([[feedback_single_threaded_harness_masks_strand_races]] / [[feedback_session_executor_lifetime...]]). `close_mode`: v1.0 exposes **graceful** as default; a `force` flag (→ `close_mode::terminal`) is a thin optional parameter (research note — fold if cheap, else L-050-y).
...
## D-7 — `fixpp_session_send` reentrancy class: conservative `REQUIRES_SESSION_LOCK` default; `THREAD_SAFE` is a Gate-A question (OPEN → Gate A)

**Tension**: `Engine::send` is **any-thread-safe** (`engine.hpp:261–267`: enrollment gate + shared_ptr keepalive + re-entrant post). That would license `fixpp_session_send` = `FIXPP_THREAD_SAFE`, which is materially more useful for a C consumer with no strand handle (it can send from any thread). But `[2i §4.10]` line 1176 **illustratively** lists `fixpp_session_send` under `FIXPP_REQUIRES_SESSION_LOCK`.

**Decision (default)**: annotate `REQUIRES_SESSION_LOCK` (matches the published example, FR-017 as clarified) **unless Gate A blesses THREAD_SAFE**. Do not flip unilaterally — `[2i §4.10]` is the published taxonomy; the deviation is a one-line Gate-A decision with the `Engine::send` any-thread evidence above. Recorded here so Gate A rules explicitly.

**Note**: even if `send` stays annotated `REQUIRES_SESSION_LOCK`, the underlying `Engine::send` is safe to call from the C consumer's thread — the annotation is a *documented contract*, not an enforcement; the thunk routes through `Engine::send` (any-thread) regardless. The annotation question is purely "what do we promise the consumer."

---

## D-8 — `FIXPP_ERR_SESSION_*` block published now; discharges L-049-2 for reachable variants (DECIDED)

**Decision**: publish the real session-domain error block at its reserved `[2i §4.3]` slot; re-point `translate()`'s `session_*` (and reachable `app_*`) arms off the Feature-A `UNKNOWN` placeholder to the new codes; append to `error_codes_v1.txt` with **introducing_minor = 3**; pass the occupancy gate; amend `[2i §4.3]`. Variants with **no** reachable producing path through `session_open`/`session_send`/`engine_*` stay `UNKNOWN` and remain documented (residual L-049-2).

**Evidence**: Feature-A `translate()` (`src/capi/error.cpp`) maps `session_*`/`app_*`/`log_*`/`otel_*`/`out_of_memory` → `FIXPP_ERR_UNKNOWN` as a deliberate override (049 data-model E-3 / L-049-2). The producing functions: `Engine::send` returns `app_do_not_send` (129), `app_callback_threw`, `session_invalid_state_for_send` (77), `session_invalid_argument` (119) (`engine.hpp:248–254`); `register_session` returns `session_invalid_argument`/`invalid_session_config`; `Session::open`/`close` return `invalid_session_config`/`session_already_open`/`session_already_closed`/`executor_not_serialised`/`clock_not_set`. **data-model E-4 enumerates the exact reachable set** to publish; the rest stay UNKNOWN.
...
- **`fixpp_session_send` THREAD_SAFE annotation** — Gate-A decision (D-7).
