...
The C++ `error` variants **reachable** through Feature-B producing functions (`engine_create`/`session_open`/`engine_start`/`session_send`/`session_close`), enumerated from the **implementation** (`src/capi/error.cpp:91-216` for the current code; `src/session/engine.cpp:1486-1595` `Engine::send` → `src/session/session.cpp:4023-4330` `Session::send`/`send_impl`; `src/session/session.cpp:878-1063` `Session::open`; `engine.cpp` `register_session`/`start`), not from a header comment. Each reachable arm carries an explicit, mechanically-checkable label: **existing** (already published by 049, code shown) vs **stays `FIXPP_ERR_UNKNOWN`** (the session/app arms — publication **DEFERRED, L-050-4**; see below).

> **DESCOPE (L-050-4, user decision 2026-06-24).** `[2i §4.3]` has **no published session/app error block** (`session_*`/`app_*` are the unpublished `FIXPP_ERR_UNKNOWN` bucket, `error.h:51`; coalesced remediation-class design; the occupancy gate's `EXPECTED` map is hardcoded to the `[2i §4.3]` master). Publishing 5 per-variant codes would require editing the signed-off `[2i]` (forbidden by the Gate-A LEAVE / CHK030). So **0 codes are newly published by Feature B** — all reachable session/app arms (119/77/129/130/131) **stay `FIXPP_ERR_UNKNOWN`**, no `translate()` re-point, no `error.h` codes, no `error_codes_v1.txt` append, occupancy gate stays green unchanged. The published block defers to a dedicated `[2i §4.3]` amendment; **L-049-2 stays open**.

The split is driven by the slot's block in `error.hpp` and its current `translate()` arm:
- The **session-PROTOCOL block** (slots 66-77, 116-121) and the **app block** (slots 129-131) are the arms 049 left at `UNKNOWN` (`error.cpp:128-145`, `:212-215`) — these are what L-049-2 named; under the L-050-4 descope they **stay `UNKNOWN`** (publication deferred), so Feature B publishes **none** of them.
...
|---|---|---|---|---|
| `session_invalid_argument` | 119 | `UNKNOWN` (`:142`) | `register_session` dup / `send` id-not-registered (`engine.cpp:1544`) | **stays `FIXPP_ERR_UNKNOWN`** (publication deferred — L-050-4 / L-049-2 open) |
| `session_invalid_state_for_send` | 77 | `UNKNOWN` (`:138`) | `send` to a non-Active session (`engine.cpp:1516,1538,1557,1583` / `session.cpp:4033`) | **stays `FIXPP_ERR_UNKNOWN`** (publication deferred — L-050-4 / L-049-2 open) |
| `app_do_not_send` | 129 | `UNKNOWN` (`:212`) | `toApp` veto on `send` | **stays `FIXPP_ERR_UNKNOWN`** (publication deferred — L-050-4 / L-049-2 open) |
| `app_callback_threw` | 130 | `UNKNOWN` (`:213`) | `toApp` threw on `send` | **stays `FIXPP_ERR_UNKNOWN`** (publication deferred — L-050-4 / L-049-2 open) |
| `app_payload_malformed` | 131 | `UNKNOWN` (`:214`) | `send` with a malformed opaque frame (fail-closed validation pre-seqnum, `session.cpp:4112-4330`) | **stays `FIXPP_ERR_UNKNOWN`** (publication deferred — L-050-4 / L-049-2 open) |
...
- **0 codes are newly published by Feature B** — the published session/app block is deferred (**L-050-4**); all 5 reachable session/app arms (`session_invalid_argument` 119, `session_invalid_state_for_send` 77, `app_do_not_send` 129, `app_callback_threw` 130, `app_payload_malformed` 131) **remain `FIXPP_ERR_UNKNOWN`**. No arm is re-pointed off `UNKNOWN`; no new code is fixed in `error.h` / `error_codes_v1.txt`; the occupancy gate is unchanged.
...
- **Precise-enumeration mutation oracle (D-11)** — for an **existing-published** row the test asserts that arm still maps to its **existing** code (`THREAD_SESSION_LIFECYCLE` / `THREAD_CONFIG` / `STORE_RUNTIME` / `CANCELLED`) at the `translate()` level (a pure mapping assertion — no producer needed), i.e. it was NOT silently re-pointed. The "newly-published row → flip-off-`UNKNOWN` → RED" oracle **no longer applies** (L-050-4 — there are no newly-published arms); instead the test asserts the 5 deferred session/app arms (119/77/129/130/131) map to `FIXPP_ERR_UNKNOWN` (the documented deferred set). Neither is a "returns a published code" proxy (per [[feedback_completeness_gate_exact_set_not_subset]] / [[feedback_coverage_push_enshrines_bugs]]).
- **Stay UNKNOWN**: under the L-050-4 descope this set is **all the session/app arms** — the 5 reachable arms (119/77/129/130/131, publication deferred) PLUS the remaining session-PROTOCOL `session_*` variants (slots 66-76, 116-118, 120-121) + `log_*`/`otel_*`/`out_of_memory` → residual L-049-2, documented (L-049-2 stays open). The threading-block arms above are NOT in this set — they were already published, not UNKNOWN.
...
- `fixpp_engine_create(consumer_major, consumer_minor, …)`: `consumer_major != FIXPP_C_ABI_VERSION_MAJOR` (0) → `FIXPP_ERR_VERSION_MISMATCH` (5), no engine. Else record `consumer_minor` → `CapiEngine::consumer_minor` → every return-path code runs `translate_for_consumer(code, consumer_minor)` (D-9). The downgrade mechanism is **live** (discharges L-049-1). **No real error code is born at introducing_minor 3** (the session/app block is deferred — L-050-4); SC-004 witnesses the downgrade with a **synthetic** minor-3 code vs a minor-2 consumer (the mechanism still ships and is provable synthetically).
