...
- **FR-006a**: The **inbound parse / field-addressing** change is unconditional and MUST be treated as a correctness fix. `Session::open()` builds `inbound_tv_` from `as_table_view()` regardless of `validate_inbound_messages` (`src/session/session.cpp:992`), and the parser consumes it (066), so once groups register, a FIX40/41/42 session's reads of tags inside a repeating group become **membership-bounded** where they were previously absent or positionally-wrong. This MUST be pinned by a before/after behavior test with strict validation **off**, demonstrating the corrected read.
