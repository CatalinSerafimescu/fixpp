...
| FR-006, 006b | Strict-validation ON: the new group-required rejections equal the dictionary-derived enumeration. **P4's named-source leg additionally pinned by K6b** — on FIX42, `fixpp_msg_group_begin(t)` succeeds for exactly the bare store's registered tag set, both directions, so a divergent second structural realization inside the runtime tier cannot pass. *Location: `tests/dictionary/` (registration) + `tests/capi/` (cross-path)* |
