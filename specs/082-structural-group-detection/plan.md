...
| **XVI §4** — `/analyze` mandatory | Same trigger set | **DONE** | Run 2026-07-30, after `/speckit-tasks`; findings remediated in `spec.md`/`data-model.md`/`tasks.md` the same day. |
...
| **Appendix A** — mandatory-trigger reference (authoritative on conflicts, per its own preamble, `.specify/constitution.md:414-424`) | Listed trigger categories require **all four** controls | **4 of 4 satisfied** | 082 hits at least two Appendix-A rows: **Codegen layout** — "dictionary loader, multi-version coexistence" (`:424`; the driver change + the new `v42` split layout) and **Wire format / parser** — "offset-table semantics, framing rules, **validator changes**" (`:423`; FIX40/41/42 group-membership and 079 per-group enforcement become reachable, FIX43 tag 576 registers). The four controls: (1) `/clarify` — **DONE** (2026-07-29, plus a Gate A round-1 session 2026-07-30); (2) `/analyze` — **DONE** (2026-07-30, after `/speckit-tasks`; findings remediated the same day); (3) Codex Gate A — **CONVERGED at round 3 and user-signed-off 2026-07-30**; (4) **user `/plan` sign-off — GIVEN 2026-07-30.** The fourth was untracked in the reviewed revision, so nothing in the pipeline would have surfaced it; tracking it is what made it a decision the user could give. |

**Result: no article is violated once FR-020's annotation-only amendment lands. Nothing is pending the user — the Article XVIII §7 / Status-banner ratification and the Appendix-A user `/plan` sign-off were both given 2026-07-30 (spec § Open decisions OD-2, RESOLVED), and OD-1 resolved the same day to the fail-closed loader rejection (FR-023). All four Appendix-A controls are now discharged — `/analyze` ran 2026-07-30, after `/speckit-tasks`, with findings remediated the same day. Complexity Tracking table omitted — nothing to justify.**
...
tests/wire/ · tests/session/       # ungated parse-correction pins, strict OFF — FR-006a
                                   # + the strict-ON enforcement pin (the newly-reachable
                                   #   group-required rejections equal the dictionary-derived
                                   #   enumeration, set-equal both directions) — FR-006/FR-006b,
                                   #   SC-008a leg 2. Rides the existing validate_inbound_messages
                                   #   opt-in, so it belongs beside FR-006a's strict-OFF sibling.
