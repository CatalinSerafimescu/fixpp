...
- **Sentinel ambiguity — ambiguous in isolation, but its input is UNREACHABLE (FR-023).** The
  ...
  > **⚠️ AMENDED 2026-08-11 — the mechanism below is superseded; the residual exception NEEDS
  > RE-MEASUREMENT.** FR-023 no longer runs a 082-side literal scan: it is satisfied by 083's
  > `captured == 0` disposition (see `spec.md` FR-023 § AMENDED). That predicate is *"emitted no
  > first member"*, **not** *"has no literal child"*, and 083's per-context capture resolves
  > **through** nested components — so the three FIX50SP2 groups named below may no longer be
  > reachable at all, which would RETIRE this exception. Both readings are consistent with
  > `AllShippedContextsHaveADelimiterRecord` passing, so **neither is asserted here**: re-derive
  > `group_first_field(1499/1669/1919)` on FIX50SP2 before citing this paragraph, and update
  > `#208`'s scope with whatever it measures. Everything below is the pre-amendment text.
