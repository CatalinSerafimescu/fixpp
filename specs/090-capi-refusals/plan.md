...
| 1d | obligation 4 — *"remains subject to §1 review and to all four Appendix A controls"* | ⚠️ **FAIL** — this row is the **roll-up of rows 2–5** and takes the worst of them | **ONE** of the four controls (`/analyze`) is **OWED**; the user `/plan` sign-off is ✅ **DISCHARGED 2026-09-20** (row 5, pinned to `d12d2270`), Gate A is a **non-converged waiver**, and `/clarify` is an **adapted** discharge. ⚠️ **This cell read "two … are OWED" until the sign-off landed** — the VERDICT was unaffected, which is exactly why the stale justification survived the change that falsified it. Obligation 4 cannot be `PASS` while any of its four is outstanding |
...
`/analyze` (owed), ⚠️ **the user `/plan` sign-off — DISCHARGED 2026-09-20, see row 5** — `[const §XVII.7]`'s local pre-PR build (owed by
...
- **Owed before merge, carried forward so nothing below is re-discovered as a finding**:
  `/analyze` (row 4) · ⚠️ **the sign-off (row 5) is DISCHARGED and is NOT in this owed set** · the `[const §XVII.7]` local build and the
