...
| `/analyze` | ⚠️ **OWED.** It is a cross-artifact check over `spec.md` / `plan.md` / `tasks.md` and becomes runnable only once this file exists — scheduled at T083 |
...
- **Polish (Phase 7)**: depends on all four stories. Within it, T079's freeze re-baseline depends on **every** header edit (T008, T026, T027, T042, T056) having landed; T083 → T084 → T085 → T086 → T087 run in that order (`/speckit-analyze` → local build → the paired performance runs → `/simplify` → `/speckit-verify`), and T085 depends on T084's build approval; T091 and T092 are last, in that order.
...
- **Polish**: ⚠️ the four ledger rows **T067–T070 all write `spec/behaviors-and-limitations.md` and are therefore SEQUENTIAL** — only T067 carries `[P]`, against tasks in other files. T075 and T076 are `[P]` (different files); T078, T080, T081 and T091 are `[P]`.
...
- **Not that the seam REDs are executed.** Only Story 1's has been, inside fixpp#447 itself. Every other RED is **registered as NOT MEASURED** until T084's build gate opens.
