...
**Deferred follow-up (pre-v1.0; its OWN spec + own Gate A; NOT a 006 scope expansion) — `fixpp::sync::atomic_shared_ptr<T>`:** feature-detect P0718 (`__cpp_lib_atomic_shared_ptr` refined by a real compile probe — libc++'s macro state has lied) and, when absent (libc++), fall back to an **address-hash-sharded mutex/spinlock-guarded `shared_ptr`** for `drain_latch_ptr_` that still honors the I-23/I-13 acquire/release orderings. Do **not** transplant libstdc++'s internal split-reference `atomic<shared_ptr>` onto libc++'s `shared_ptr` (differing control block + `_LIBCPP_SHARED_PTR_TRIVIAL_ABI` ⇒ UB) — reimplement the contract on the public `shared_ptr` API only. New drain-path concurrency code ⇒ re-triggers Gate A per `[const §XVII.1]`, hence a follow-up rather than bolted into the converged 006 (006 ships on this waiver unchanged). Validation: a non-Tier-1 `linux-clang-libc++` lane; verify step-0 alias-vs-fallback probe assertion; drain/cancel_and_drain seams #19/#23/#24/#26/#29 under TSan on the fallback; native libstdc++/MSVC-STL path stays default (fallback guarded-out dead code, cannot regress Tier-1). Tracked: `spec/feature-catalogue.md` **NFR-017**; project memory `project_libcxx_atomic_shared_ptr_followup`.

> **DELIVERED 2026-06-21 by feature `046-atomic-shared-ptr` (FR-013/FR-004).** `drain_latch_ptr_` (and the other three production consumers — `pinset.snapshot_`, `transport_factory.cert_source_slot_`, `engine.reader_snapshot_`) were migrated to `fixpp::sync::atomic_shared_ptr<T>`. One refinement vs the plan above: the sharded-mutex lock is **type-erased into `src/core/sync/atomic_shared_ptr.cpp`** (header declares only an opaque `detail::shard_guard`), so the awaitable `async_mutex.hpp` stays `std::mutex`-free with **no constitutional amendment** — superseding the "re-triggers Gate A for a §XI.3 exemption" framing. See `specs/046-atomic-shared-ptr/`.
