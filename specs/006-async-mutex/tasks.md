...
- [X] T068 [P] Seam #18 `tests/sync/test_arm64_weak_memory.cpp` — weak-memory multi-threaded contention stress (2 tests: MT mutual-exclusion + drain-under-MT-contention) verifying I-01..I-31 (FR-013; SC-007). GREEN under the full Tier-1 matrix incl. TSan (the portable C++-memory-model check). Native Linux-ARM64 execution recorded **host-unavailable** (build host `uname -m`=x86_64) — to be noted in the verify doc; TSan is the SC-007 verification per the task's fallback clause.
- [X] T069 Memory-ordering audit complete: all 5 `memory_order_relaxed` sites confirmed non-synchronizing (refcount increment — synchronizing edge is the I-32 acq_rel decrement; CAS-loop initial-load + failure-order, re-validated by the CAS; pre-publication `phase_` init published by the subsequent `release` store / I-03). The 35 acq_rel + 38 acquire + 15 release sites carry the I-01..I-31 edges; TSan 30/30 across clang{debug,tsan,asan,ubsan}+gcc-release is the empirical confirmation no required pairing was weakened. FR-013 `static_assert`s (awaiter alignment ≥8, `waiter_record`≤256, awaiter ≤96, pointer round-trip, `atomic<uintptr_t>` lock-free) compile under clang-22 AND gcc-13 (SC-007).
- [X] T070 Sanitizer matrix GREEN serially: **`linux-clang-tsan` 30/30 (mandatory, 0 reports, non-waivable)** + `linux-clang-asan` 30/30 + `linux-clang-ubsan` 30/30 + `linux-gcc-release` 30/30, full `^sync_` set (`[const §IX.2]`/`[const §XVII.8]`; SC-002/SC-009). asan/ubsan/gcc-release dirs predated asio/1.36.0; resolved via the documented `conan install --output-folder` workaround.
- [X] T071 Real `[2f §6.3]` bench workloads authored (replaced Phase-1 placeholders): #1 uncontended async_lock + isolated unlock, #2 contended ping-pong suspend, #12 dispatch-vs-post delta; periodic post-trampoline bounds asio inline-resume recursion. Measured on `linux-clang-release`, committed to `bench/baselines/sync/async_mutex_baselines.json` with an explicit METHODOLOGY note (end-to-end coroutine+io_context wall time, NOT isolated-primitive — absolute figures sit above the isolated-CAS ceilings by construction; the ±5% gate is **SOFT/disabled** per `[const §VIII.2]`). `tools/bench_compare.py` run: baseline self-consistent within ±5% (exit 0, soft) (SC-005).
- [X] **T072 Coverage hardening (USER OBJECTIVE)** — CLOSED (user-accepted 2026-05-19) via the `[const §IX.1]`-sanctioned written justification note `specs/006-async-mutex/coverage-justification.md`. Built `linux-clang-coverage`, fresh per-PID profraw, full `^sync_` 30/30 GREEN, lcov DA cross-checked vs region-accurate `llvm-cov show` (agree: 83 zero lines / 81.56% raw; no profraw-staleness false-low — fast path 145–19 598×). All 83 enumerated + classified with evidence: **(A) 33 lines = verified `llvm-cov` multi-object anon-ns/inline mismatch artifact** — executed (callers covered 16–1 400× on the same profile: L620=1.4k, L1051=646, L788=145) but tooling-zeroed; excluded per `feedback_coverage_gate_lcov_basis` (NOT refactored — core-primitive churn for a tooling number is the prohibited anti-pattern). **(B) 3 lines = `std::terminate()` precondition** covered by seam #5 death test in a forked child whose profile is unmergeable (genuinely harness-unreachable; functionally GREEN 3/3 release). **(C) 47 lines = per-instantiation-dead (`906-913` `store_executor` false arm, `if constexpr`-dead for the 16 B shipped `any_io_executor`) + race-window defense-in-depth branches** whose observable contract is covered by the GREEN seam suite + TSan 30/30 + the T069 I-01..I-31 audit; pinning sub-ns interleavings = impossible-path test-bloat prohibited by `[const §IX.1]`. **Artifact-corrected line ≈ 88.9 %; genuinely-reachable-deterministic code ≈ 100 % (zero such untested).** Raw SC-009 floor not met purely due to (A)+(C) residue — justification note is the sanctioned `/speckit-verify` step-2 evidence. **Decision (user, 2026-05-19): accept justification-doc closure** — no core churn; the note is the `/speckit-verify` step-2 coverage evidence for the residue. Note (I1): `sync_lock_outside_session` excluded as declaration-contract-only (SC-009 benignity clause).
- [X] T073 Sweep performed (clang-format-22 + clang-tidy-22 with project `.clang-tidy` via the debug compile DB; cppcheck-class checks subsumed by clang-tidy). **Zero correctness-class defects.** Residue: (a) pure-style/advisory findings — NO CI clang-format/tidy step exists (tier1.yml) and merged converged 001/004 headers ship comparable advisory residue, so this is baseline-consistent, not a regression; (b) `performance-no-int-to-ptr` ×4 = the **spec-mandated** Lewis-Baker `uintptr_t` pointer-or-sentinel state encoding (NFR-016 / `[2f §6.2]`) — intrinsic to the mandated lock-free algorithm; (c) `bugprone-exception-escape`@611 = false positive (the `noexcept` resume runner calls only `noexcept` ops). No auto-fixes applied: they would either churn the Gate-A-converged core for style at sign-off or violate the NFR-016 algorithm (consistent with the user-affirmed no-core-churn decision). Header includes are Erratum-curated (IWYU: deliberate) (`[const §IX.4]`).
- [X] **T078 Consumer compile/link contract check — SC-008 / `[2e §3.1]` hand-off gate** — `tests/sync/test_consumer_contract_compile.cpp` authored + registered (`sync_consumer_contract_compile`), GREEN under `linux-clang-debug` + the full TSan/ASan/UBSan/gcc-release matrix. Positive asserts (constexpr-default-constructible via `constinit`, non-copy/move, exact contract return types for async_lock/unlock/cancel_and_drain/policy, `async_lock_via_session_executor(async_mutex&)` `[2f §4.3.2]` signature, guard movable/non-copyable/`sizeof==sizeof(void*)`) + negative U1/FR-015 asserts (no `try_lock`, no public engaged-guard ctor, no polymorphic/extension hook, no shared/recursive/timed alias) all enforced at compile time; links `fixpp_sync` (non-inline link-completeness). Contract clarified: `~async_mutex` is `noexcept(false)` by design so `is_nothrow_destructible` is intentionally false (asserted). Original task spec: compile-only consumer translation unit `tests/sync/test_consumer_contract_compile.cpp` (registered in `tests/sync/CMakeLists.txt`, T002) that includes **only** the shipped headers `<fixpp/core/sync/async_mutex.hpp>` + `<fixpp/session/async_lock_via_session_executor.hpp>` and asserts, via `static_assert`/SFINAE, that the shipped surface satisfies the `[2e §6.4]` writer-mutex contract shape and the declared session-side-helper surface — i.e. `2e`/`005`/`2g` can build against 2f. **Positive asserts**: `async_mutex` is `constexpr`-default-constructible, non-copyable, non-movable; `async_lock()/unlock()/cancel_and_drain()/policy()` are well-formed with the contract return types; `async_lock_via_session_executor(async_mutex&)` is declared with the `[2f §4.3.2]` signature; `async_lock_guard` is movable, non-copyable, `sizeof == sizeof(void*)`. **Negative asserts (U1 — FR-015 out-of-scope surface absent)**: no public `try_lock()` member; no `async_shared_mutex`/`async_recursive_mutex`/`async_timed_mutex` typedef/alias; `async_mutex` exposes no public engaged-guard ctor and no CRTP/`concept` extension hook (detect via `requires`/`std::is_*` traits). The TU links against `fixpp_sync` to confirm link-completeness of the shipped non-inline surface. Run GREEN under `linux-clang-debug`. Map: SC-008, FR-015, FR-001, FR-011.
- [X] T074 Completeness audit written: `specs/006-async-mutex/completeness-audit.md` — every FR-001..FR-016 + SC-001..SC-010 mapped to its covering impl + seam/evidence; **100% coverage, 0 waivers, 0 gaps**; tasks↔phases reconciled (all T001-T078 `[X]`; T072 via the user-accepted `[const §IX.1]` justification note). `/gate-b` precondition satisfied pending T076 GREEN (`feedback_feature_completeness_gate`; `[const §XVII.8]`).
- [X] T075 NFR-016 closure: `spec/feature-catalogue.md:228` (NFR-016 row) Status=done, /specify=v1.6, PR=#73, Tests=32 seams, Verified=GREEN; `spec/coverage-index.md:462` (NFR-016 supplemental) v1.6 sign-off with E-3 always-post + E-4 slot-allocator semantics; closure asserted (not mere presence) per gate-b/r1 F-1 fix 2026-05-19 (`[2f §4.1]`+`[arch §1.1]`→NFR-016; SC-010). Appendix D §D.1/§D.2/§D.3 cross-doc drop-ins to `[2d]` remain **queued at sign-off, NOT applied here** (research D-12) — unchanged.
- [X] T076 `/speckit-verify 006-async-mutex` run → `.specify/decisions/006-async-mutex-verify.md` (gitignored local-only per `project_gate_records_layout`). **Verdict GREEN**: 26 PASS / 0 FAIL / 3 SKIPPED-with-D-11-reason (ABI-nm·fuzz·abidiff non-applicable) / 1 SOFT (bench disabled `[const §VIII.2]`). Mandatory non-waivable TSan GREEN 30/30 across the full sync set; coverage closed via the user-accepted `[const §IX.1]` justification note. `gate-b-done` precondition (`[const §XVII.8]`) satisfied.
- [X] T077 Quickstart §1–§9 walked as the final acceptance pass (build/test/sanitizer-matrix/coverage/grep-gate/bench/verify all executed this session and consolidated in the verify record). **`local build: green on linux-clang-debug @ 2f2295a`** (for the PR body, `[const §XVII.7]`).
