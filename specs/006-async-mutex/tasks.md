...
- [X] T075 NFR-016 drop-in present: `spec/feature-catalogue.md:228` (NFR-016 row) + `spec/coverage-index.md:462` (linking entry) — verified in place (`[2f §4.1]`+`[arch §1.1]`→NFR-016; SC-010). Appendix D §D.1/§D.2/§D.3 cross-doc drop-ins to `[2d]` remain **queued at sign-off, NOT applied here** (research D-12) — unchanged.
