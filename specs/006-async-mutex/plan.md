...
| `[const §III.2]` | Conan, pinned deps | **CORRECTED 2026-05-18 (post-`/implement` precondition check):** the original "no new Conan row — ASIO already pinned" claim was **factually wrong** — asio was slated to "return in Phase 4" (conanfile.py comment) but 001–004 were non-async so it was never added; 006 is the first asio-consuming feature. `asio/1.36.0` (standalone, BSL-1.0) was added to `conanfile.py` as a **new pinned Conan row**, user-approved per `[const §III.2]` / the pugixml precedent (sign-off 2026-05-18). GTest/Benchmark unchanged. **Phase-4 Gate-A gap noted:** the doc-fidelity Gate-A passes (rounds 1–2) did not catch this false III.2 assertion; it was caught at the `/implement` toolchain-precondition check. No re-Gate-A required (dependency-pin addition with user sign-off; `[const §XVII.6]` trivial-diff class — new pinned dep, no design change). |
...
| `[const §XI.7]` | Threading-affecting features trigger all four mandatory controls | **Gate A mandatory — Threading/Concurrency + Error semantics triggers** per Appendix A. `/clarify` done (0 questions — design doc signed-off/Gate-A-converged); pipeline order per the canonical `[const §XVI.4]` row (`/analyze` post-`/tasks`); Codex Gate A before `/tasks`; user `/plan` sign-off. |
...
| `[const §XVI.4]` | `/analyze` mandatory post-`/plan` | **CANONICAL PIPELINE ORDER (single source of truth — all other mentions in this bundle cross-reference this row, do not restate):** `/plan` → Gate A → `/tasks` → `/analyze` → `/implement`. `/speckit-analyze` runs **after Gate A converges and after `/speckit-tasks`**, before `/speckit-implement` (Article XVI §4: `/analyze` is the drift check against constitution ↔ spec ↔ plan ↔ **tasks** — it must follow `/tasks`). |
...
├── CMakeLists.txt
├── fixtures/
│   ├── header_with_std_mutex_and_awaitable.hpp     # seam #14 — deliberate-violation fixture (gate MUST fire)
│   ├── header_without_violation.hpp                # seam #14 — non-violating fixture (gate MUST NOT fire)
│   └── header_transitive_awaitable_include.hpp     # seam #14 — std::mutex via transitive <asio/awaitable.hpp> include (post-preprocessing scope MUST catch it; [2f §9 seam #14] variant 3)
...
| #14 `std::mutex`-in-coroutine-context CI gate | `tests/sync/test_no_std_mutex_ci_gate.cpp` + `tests/sync/fixtures/header_with_std_mutex_and_awaitable.hpp` (violating) + `tests/sync/fixtures/header_without_violation.hpp` (non-violating) + `tests/sync/fixtures/header_transitive_awaitable_include.hpp` (transitive-include) | `[2f §9 seam #14]` (3 mandated fixture variants) / `[const §XV.9]` / `[2f §6.6]` |
...
- Round 1 applied 2026-05-18: Codex P1=1 P2=4 P3=1; Opus post-judging P1=2 P2=5 P3=2; rewrite addresses root causes RC#1 (boundary-fidelity / public try_lock removal), RC#2 (oracle-fidelity across the 3 awaitable-return oracles incl. the int-mr correction), RC#3 (quickstart constitution/verify re-sync + STL-availability probe + layering-owner disambiguation). Reviews: research/reviews/codex_006-async-mutex_gate_a_review.md, research/reviews/opus_006-async-mutex_gate_a_adversarial_review.md.

**Root causes + net effect (mirrors the Opus adversarial review's three-RC collapse — all findings are transcription-fidelity/cite-sync defects, not design defects; the v1.5 authority is sound and was not re-opened):**

- **RC#1 — boundary-fidelity (signed-off scope boundaries silently widened).** Collapses Codex P1 (public `try_lock()` reintroduced in `spec.md` FR-001), Codex P2 #4 ("or 2e implementation" owner divergence), and New N-P2-1 (§1.2 OWNS-list ambiguity left unresolved). Fix: `spec.md` FR-001 scopes `try_lock()` to `detail::`/friend-only with an explicit one-line note (+ FR-015 out-of-scope note + a `## Clarifications` Session-2026-05-18 bullet so the §1.2 OWNS line cannot be re-derived into a public method at `/tasks`); the session-helper owner string is unified to the verbatim authority ("the session-module spec, not this feature") across `data-model.md` E7 and `contracts/async_lock_via_session_executor.hpp` — "or 2e implementation" deleted everywhere (`spec.md` FR-011 / research.md D-10 already matched the authority and were left as-is).
- **RC#2 — oracle-fidelity (every `asio::awaitable<expected_t<…>>` return surface erased to `void`/`int`).** Collapses Codex P2 #1 (session-helper oracle), Codex P2 #2 (awaiter oracle), and New **N-P1-1 / P1** (the primary `contracts/async_mutex.hpp` class-surface oracle, with the `int mr = 0` parameter corruption). One uniform policy applied to all three oracles: **literal declarations with forward-declared `expected_t`/`asio::awaitable`** (the build header forward-declares these the same way). `contracts/async_mutex.hpp`: `async_lock` restored to `asio::awaitable<expected_t<async_lock_guard>> async_lock(std::pmr::memory_resource* mr = nullptr) noexcept` (the P1 `int mr` corruption corrected) and `cancel_and_drain` restored to `asio::awaitable<expected_t<void>>`; `contracts/async_lock_via_session_executor.hpp` restored to the §4.3.2 literal signature; `contracts/async_mutex_awaiter.hpp` restored the normative `slot_`/`result_` fields, the real `await_resume()` return type, the `asio::cancellation_type` `on_cancel` parameter, and the **84 B raw / ≈ 88 B padded / ≤ 96 B ceiling** byte budget (was the understated "≈ 73–81 B").
- **RC#3 — quickstart constitution/verify cite drift + STL-availability risk + cross-ref drift.** Collapses Codex P2 #3 (coverage 90/80 + `[const §IX.2]` miscite — 3 sites: §8 header, §8 gate+note, §9 verify step 2), Codex P3 (STL-availability risk record), and New N-P3-1 (Fuzz/abidiff cross-ref drift). Fix: all coverage gates → **≥95% line / ≥85% branch under `[const §IX.1]`** (matching `spec.md` SC-009; `[const §IX.2]` is the sanitizer clause); a named research.md D-4 STL-availability assumption + a quickstart §0 / verify step-0 compile probe for `std::atomic<std::shared_ptr<T>>` before `/implement`; quickstart §4 Fuzz/abidiff N/A aligned to research.md D-11 + `[const §VII.7]`/`[const §IX.5]` exactly as plan.md Constitution Check rows already do.

**Net effect:** both P1s closed (FR-001 public `try_lock()` removed; `contracts/async_mutex.hpp` oracle return-type + `mr` parameter corrected). No "Disagree" verdicts in the Opus review — every Codex finding was confirmed; none downgraded. `[const §VII.5]` "N/A-with-reason, NOT a waiver" framing was independently re-verified correct by the Opus review and left untouched. No signed-off design decision re-opened; every edit makes the bundle MATCH `.specify/2f-async-mutex.md` v1.5.

### Round 2 — applied 2026-05-18

- Round 2 applied 2026-05-18: Codex P1=0 P2=3 P3=0; Opus post-judging P1=0 P2=3 P3=1; rewrite addresses RC#4 (SC-009 coverage-scope cross-doc consistency incl. the session/ helper), RC#5 (seam #14 — 3 explicitly-named fixtures, no glob), RC#6 (pipeline-order canonicalization + N-P3-1 single-source hardening). Round-1 RC#1/RC#2/RC#3 independently re-verified fully closed by the round-2 Opus adversarial pass. Reviews: research/reviews/codex_006-async-mutex_gate_a_2_review.md, research/reviews/opus_006-async-mutex_gate_a_2_adversarial_review.md.

**Root causes + net effect (mirrors the round-2 Opus adversarial review's three-RC collapse — all residuals are cross-document fidelity defects in artifacts the round-1 rewrite did not touch; zero P1; no signed-off design decision re-opened; the v1.5 authority remains sound and was not re-litigated):**

- **RC#4 — SC-009 success-criterion coverage scope under-named the shipped module set.** Collapses Codex P2 Finding 1. `spec.md` SC-009 scoped the `[const §IX.1]` floor to `core/sync` only, contradicting `plan.md`'s `[const §IX.1]` Constitution-Check row and `quickstart.md` §8/verify-step-2, which all name `core/sync` **+ the declared `session/` helper`. Fix: `spec.md` SC-009 now reads the one canonical scope string used everywhere (`core/sync` (+ `session/` helper) — `async_mutex.hpp` (+ `.cpp` if out-of-line) plus the declared `include/fixpp/session/async_lock_via_session_executor.hpp`), with the Opus-noted declaration-only benignity qualifier (the session header is declaration-only here so its measured DA/BRDA contribution is nil — a zero-line header is not flagged uncovered). Scope statement now internally consistent across spec/plan/quickstart.
- **RC#5 — seam #14's third (transitive-include) fixture was prose-only, not bound to an explicitly named on-disk file.** Collapses Codex P2 Finding 2. Design-doc §9 seam #14 (`.specify/2f-async-mutex.md:1632`) mandates **three** concrete fixtures (violating / non-violating / transitive-include); `plan.md` Project Structure named only two and the seam-#14 map row used a `tests/sync/fixtures/` directory glob, breaking `plan.md`'s own no-glob promise and Article VI §4 bidirectional traceability. Fix: added `tests/sync/fixtures/header_transitive_awaitable_include.hpp` by explicit name to the Project Structure block, and replaced the directory glob in the seam-#14 mapping row with the explicit three-file list (violating / non-violating / transitive-include) so the seam→file binding is one-to-one and `/speckit-tasks` cannot drop the transitive variant.
- **RC#6 — the Gate-A Execution paragraph stated the pipeline order backwards (the strongest P2 — operative orchestrator instruction); + N-P3-1 single-source hardening.** Collapses Codex P2 Finding 3 + Opus New N-P3-1. `plan.md` Gate-A Execution paragraph ordered `/analyze` **before** `/tasks`, contradicting the canonical `[const §XVI.4]`/`[const §XI.7]` rows and the Phase-2 checklist, and Article XVI §4 (`/analyze` is the drift check **against** `tasks` — it must follow `/tasks`). Fix: the Gate-A Execution paragraph now defers to and restates the canonical `/plan` → Gate A → `/tasks` → `/analyze` → `/implement` order. N-P3-1: the canonical `[const §XVI.4]` Constitution-Check row is now the **single source of truth**; the four other pipeline-order mentions (`[const §XI.7]` row, Gate-A Execution paragraph, Phase-2 checklist, `spec.md` Assumptions, `quickstart.md` §10) cross-reference it rather than restate, so a future single-line edit cannot reintroduce the contradiction.

**Net effect:** zero P1; the three round-2 P2s closed (SC-009 scope consistent across spec/plan/quickstart; seam #14 = 3 explicitly-named fixtures, no glob; pipeline order canonical at `plan.md` Gate-A Execution + single-sourced) and the N-P3-1 hardening applied. No "Disagree" verdicts in the round-2 Opus review — all 3 Codex findings confirmed at P2; none downgraded (the declaration-only SC-009 benignity note was recorded, not used to downgrade). Round-1 RC#1/RC#2/RC#3 independently re-verified **fully closed** by the round-2 Opus adversarial pass; the round-1 rewrite introduced no new desync; the round-1 fixes were not modified. `[const §VII.5]` "N/A-with-reason, NOT a waiver" framing left exactly as-is. No signed-off design decision re-opened; every edit makes the bundle MATCH `.specify/2f-async-mutex.md` v1.5.
...
- [x] Pipeline order per the canonical `[const §XVI.4]` Constitution-Check row (single source of truth): `/plan` → Gate A → `/tasks` → `/analyze` → `/implement` (`/analyze` post-`/tasks`)
