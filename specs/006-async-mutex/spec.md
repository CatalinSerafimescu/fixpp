...
3. **Given** N coroutines queued on a held mutex, **When** the holder releases, **Then** the waiters are granted ownership one at a time with FIFO fairness within the drain cycle and each resumes on its own bound executor via one `post` hop (always-`post`ed per `[2f §4.6.2]` Erratum E-3 — never inline-`dispatch`ed — regardless of the mutex's `completion_policy`, which survives only as a semantic/intent knob).
...
On the v1.0 hot path the stable waiter node (`detail::waiter_record`, E-2) is drawn from the per-mutex `waiter_pool_` bounded arena and the frame-local awaiter is HALO-embedded, so a contended acquire performs **zero global `new`/`delete`**. Per `[2f §4.3.4]` Erratum E-4 (v1.6, source-verified): asio's `cancellation_slot` has **no allocator-binding hook**, so the cancellation-slot handler closure is owned by asio's per-thread recycling cache (zero global `new`/`delete` **in steady state, after a documented per-thread warm-up**; the one-time per-thread first-touch is `[2f §6.4]` bench-soft) — it is **not** drawn from the awaiter's inline buffer and `detail::slot_allocator` is **not** bound to the slot. When a caller cannot satisfy embedded storage, an explicit `async_lock(std::pmr::memory_resource* mr)` overload routes the `waiter_record` and type-erased completion storage to the caller-supplied resource — `core` never reaches into `session/` or an engine handle for memory.
...
1. **Given** the embedded-waiter path with HALO firing, **When** a contended acquire/cancel/drain cycle runs **in steady state (after a documented per-thread cancellation-recycler warm-up iteration per `[2f §4.3.4]` Erratum E-4)**, **Then** the allocation-counting harness reports zero global `new`/`delete` (the substantive `waiter_record` allocation is genuinely zero-global via the per-mutex `waiter_pool_`; the one-time per-thread asio cancellation first-touch is amortized and bench-soft, outside the measurement window).
...
- **FR-004**: Releasing ownership (guard destruction/`release()` → `unlock()`) MUST grant the next waiter with **FIFO fairness within a drain cycle** (LIFO list reversed on `unlock`), resuming the granted waiter on its **bound executor**. **`[2f §4.6.2]` Erratum E-3 (v1.6, TSan-diagnosed):** 2f waiter resumption is **always `post`ed** to the bound executor — never inline-`dispatch`ed — **regardless of `completion_policy` or `running_in_this_thread()`** (2f's resume site is intrinsically re-entrant; inline `dispatch` there was a TSan-diagnosed heap-UAF). `completion_policy` (`dispatch` default / `post` opt-in) is preserved only as a **semantic/intent knob** (constructor-set, `const`, queryable via `policy()`); for waiter resumption both policies `post`.
...
- **FR-009**: The embedded path MUST perform **zero global `new`/`delete`** on the v1.0 hot path (acquire/contend/cancel/drain) for the substantive allocation — the stable `detail::waiter_record` (E-2), drawn from the per-mutex `waiter_pool_` bounded arena. Per `[2f §4.3.4]` Erratum E-4 (v1.6, source-verified): the cancellation-slot handler closure is **not** drawn from the awaiter's inline buffer and `detail::slot_allocator` is **not** bound to the slot (asio's `cancellation_slot` exposes no allocator-binding hook); its storage is owned by asio's per-thread recycling cache and is zero global `new`/`delete` **only in steady state, after a documented per-thread warm-up** — the one-time per-thread first-touch is amortized and treated as `[2f §6.4]` bench-soft (observable, non-fatal). `detail::slot_allocator` is retained as the typed storage-policy wrapper for the `waiter_record` fallback (the allocation 2f *does* control), unit-verified by seam #21.
...
- **Awaiter (`detail::async_mutex_awaiter`)**: the frame-local suspension unit embedded in the caller's coroutine frame (HALO-eligible, ≤96 B). **`[2f §4.2]` Erratum E-2 (v1.6):** it is **no longer the intrusive node** — it keeps only `mutex_`, the cancellation `slot_`, the inline completion-handler `slot_storage_` plus its E-1 `invoke_fn_`/`destroy_fn_` type-erasure trampolines (**`[2f §4.2]` Erratum E-1 (v1.6):** the design's stored continuation `coro_` is **replaced by** the composed-operation completion handler placement-new'd into `slot_storage_` — there is **no `coro_` field**), and a `record_` attachment pointer. The intrusive identity (`next_`, the atomic phase machine `{queued, granted, cancelled}`, the owned terminal result, the bound-executor SBO, the `refcount_` reclamation token) lives on the **stable `detail::waiter_record`** (created only on the contended path, linked through `state_`/`next_drain_head_`, lifetime independent of the coroutine frame; reclamation = single-shot `refcount_`, invariant I-32).
...
- **`detail::slot_allocator`**: project-internal typed `Allocator`-shaped storage-policy wrapper. **`[2f §4.3.4]` Erratum E-4 (v1.6):** it is **not** bound to the cancellation slot (asio exposes no such hook) — it is the storage-decision carrier for the `waiter_record` fallback (the allocation 2f controls), unit-verified by seam #21. The cancellation-handler closure storage is owned by asio's per-thread recycler (zero global heap in steady state after per-thread warm-up; first-touch bench-soft).
...
- This feature realizes the **signed-off, Gate-A-converged design doc `.specify/2f-async-mutex.md` v1.5 + v1.6 errata E-1..E-4** (Goals §1, Scope §1.2, API §4, NFR §6, Test seams §9, Appendix A/D; errata recorded post-sign-off at `/implement`). Design decisions locked there (the six-item list, atomic algorithm, memory-ordering table, CAS-arbitration cancellation contract, destructive-move guard, `std::terminate()`-precondition + `cancel_and_drain()` shutdown shape, explicit-`mr` PMR fallback) are **decided and not re-opened**; the design doc **as amended by the v1.6 errata** wins on any conflict (consistent with the authority anchor above).
