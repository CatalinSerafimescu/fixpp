...
- `012-2h-transport` (`[2h]`/`[2g]`) — `Transport` (5 pure-virtuals) + **`TlsTransport::async_handshake(SslCtxConfig const& cfg [[clang::lifetimebound]]) -> awaitable<expected_t<handshake_result>>`** (`include/fixpp/transport/tls_transport.hpp:116-118` — note the cfg arg + lifetimebound: the caller MUST keep `ssl_cfg` alive past the `co_await`) with `handshake_result.peer_id : fixpp::tls::peer_identity` **owning** by value (`tls_transport.hpp:52-53`, held by the FSM per `tls_transport.hpp:40`); `TransportFactory::make(...)` (`:74`) + `TransportFactory::reload_credentials(...)` (`:95`) pure-virtual + the `cert_source_slot_` atomic-swap / `cert_source_snapshot()` contract (concrete-only at `:167-168`, **promoted to abstract pure-virtual by 014** — see §XIV.2); `ReconnectPolicy` schedule-array + jitter + `max_attempts` + `delay_for_attempt(n)` (`include/fixpp/transport/reconnect_policy.hpp:30`).
- `013-session-reconnect-binding` — the `ReconnectFsm` driver shell (`include/fixpp/session/reconnect_fsm.hpp`), `CompIdAuthorizationPolicy::authorize(peer_identity const&, std::string_view) const noexcept -> expected_t<bound_principal>` (`src/session/compid_authorization_policy.cpp:296`), the TLS-outcome `SessionEvent` variant union incl. **`credentials_rotated{old_sha256, new_sha256}`** (`include/fixpp/session/session_event.hpp:102-105`, *defined but never emitted* — DEFERRED-to-014 note at `session.hpp:274`), and the in-process `reload_credentials` control plane.
...
**Shipped-reality baseline** (every load-bearing claim re-verified against post-PR-#86 `main`; sections below reference this table rather than re-asserting baselines):

| Claim | Shipped reality | `file:line` |
|---|---|---|
| Authorize Logon gate (acceptor) | three-way guard: (1) override→authorize; (2) `else if (is_mtls)`→fail-CLOSED (emit `session_event_compid_authorization_failed`+Disconnected); (3) non-mTLS→skip | `src/session/session.cpp:953-1008` |
| Authorize Logon gate (initiator) | byte-for-byte symmetric three-way guard | `src/session/session.cpp:1757-1803` |
| `drive_reconnect_attempt` stub | `make()` then `(void)t` discard; uncalled by any production path | `src/session/reconnect_fsm.cpp:53-61` |
| `handshake_result.peer_id` | value-typed owning `peer_identity`, held by FSM by value | `include/fixpp/transport/tls_transport.hpp:52` (struct), `:40` (held-by-value note) |
| `async_handshake` signature | `async_handshake(SslCtxConfig const& cfg [[clang::lifetimebound]]) = 0` — **takes a cfg arg** (not nullary) | `include/fixpp/transport/tls_transport.hpp:116-118` |
| `cert_source_snapshot()` | **concrete-only** on `asio_tls_transport_factory`; **NOT** on the abstract `TransportFactory` (which has exactly 2 pure-virtuals: `make` `:74`, `reload_credentials` `:95`) | `include/fixpp/transport/transport_factory.hpp:167-168` (concrete); `:54-98` (abstract base) |
| FSM holds abstract factory ptr | `fixpp::transport::TransportFactory* factory_;` (forward-declared in header per §XV.9) | `include/fixpp/session/reconnect_fsm.hpp:152` |
| `credentials_rotated` | **defined-not-emitted** in 013 (no emit site anywhere); 014 emits | `include/fixpp/session/session_event.hpp:102-105` (def); `include/fixpp/session/session.hpp:274` (DEFERRED-to-014 note) |
| Error-slot boundary | max = `session_invalid_argument = 119`; slot 115 = `transport_accept_cancelled` (012 boundary); slot 70 = permanent hole; slot 74 = `session_test_request_unanswered` real meaning + too-high stand-in | `include/fixpp/core/error.hpp:627` (=119), `:588` (=115) |
| Slot-74 too-high callers | 3 sites `co_await seqnum_mgr_.check_inbound(...)` discard the code & disconnect (zero behavioural change on the 74→120 swap) | `src/session/session.cpp:904` (LogonSent), `:1261` (Active warm-up), `:1703` (LogonReceived); branch at `src/session/seqnum_manager.cpp:77` |
...
013 shipped the session-Phase-4 *control surface* with the **live-transport lifecycle stubbed**. 014 makes three of those behaviours real **at the session / reconnect-FSM / transport level**, then discharges five test/quality carry-forwards that were waived precisely because they needed a live TLS handshake (which the realized reconnect path now provides). It introduces **no new public type**, but it has **two public-surface deltas**: (1) one appended error-enum slot (`session_seqnum_too_high = 120`), and (2) the promotion of `cert_source_snapshot()` from the concrete `asio_tls_transport_factory` to a **pure-virtual on the abstract `TransportFactory`** (count 2/5 → 3/5, under the §XIV.2 cap of 5). Delta (2) is an explicit, Gate-A-blessed widening of 012's frozen `TransportFactory` contract — no NEW concept (the only concrete impl already has the method, and the FSM's documented rotation read needs it through the abstract pointer it holds); owned here, not smuggled, per re-emission discipline.

The three realizations (verified shipped reality, not plan prose):

1. **Live initiator reconnect (US1 / FR-001..005).** `ReconnectFsm::drive_reconnect_attempt()` today mints a transport via the factory and **immediately discards it** — `auto t = factory_->make(exec, ssl_cfg, nullptr); (void)t;` at `src/session/reconnect_fsm.cpp:53-61`, and the method is **not called by any production path**. 014 realizes it as the per-attempt **connect → handshake → identity-bind → rotation-emit → hand-back** coroutine wrapped in the bounded `ReconnectPolicy` retry loop: per attempt it reads the `cert_source` snapshot, builds the per-attempt `SslCtxConfig` (held in attempt scope across `make()` and `async_handshake`), calls `TransportFactory::make(...)`, drives `Transport::async_connect` then `TlsTransport::async_handshake(ssl_cfg)` (the stub skips both), captures `handshake_result`, runs the authorization decision, and on success **`co_return`s `expected_t<void>{}` (the unchanged 013-stub public return) and hands `{transport, handshake_result, bound_principal}` to the owning `Session` through the private `Session::install_reconnected_transport(...)` entry point** (no new public `reconnect_outcome` type — `reconnect_fsm_` is a direct value member of `Session`, so the FSM calls back into its owner directly; keeping the heavy `handshake_result`-carrying values off the `reconnect_fsm.hpp` public signature also avoids re-arming the §XV.9 awaitable-closure gate via a return type). `install_reconnected_transport` rebinds the outbound path (`transport_send_`) and re-drives Logon to Active (the FSM owns no emit/strand/transition path — those are `Session` members; data-model E-1). Failures (connect, handshake, **or authorization**) each consume one attempt and back off per the schedule until the cap → terminal-disconnected (an authorization failure on the reconnect path does NOT terminally Disconnect early — retry-to-cap per Q1). Cancellation (`cancellation_type::total`) aborts an in-flight attempt and releases the partially-constructed transport with no leak.

2. **Live identity → authorization (US2 / FR-006..008).** The CompID authorization decision is gated by an identical **three-way guard** at both Logon call sites (`src/session/session.cpp:953-1008` acceptor `LogonReceived`, `:1757-1803` initiator `LogonSent` peer-Logon-ack): (1) `logon_peer_identity_override` present → `authorize()` against the override identity; (2) `else if (is_mtls)` → **fail CLOSED** (emit `session_event_compid_authorization_failed` + Disconnected — landed in 013 Gate-B as the RC#A fail-closed-footgun fix, `session.cpp:944-948`/`:992-1006`/`:1790-1801`); (3) non-mTLS → skip (genuinely permissive). So under mTLS-without-override 013 *already* fails CLOSED — there is **no fail-OPEN hole** on that path. The problem 014 solves is narrower: on the live initiator path the mTLS gate has **no identity source**, so it can only ever fail-close (it cannot admit a legitimate peer at all). 014 changes the identity **source**: the real `handshake_result.peer_id` captured by `drive_reconnect_attempt` drives `authorize()`, making the already-fail-CLOSED mTLS gate *operable* with a live identity (admit on-list, fail-close off-list/absent) and removing the dependence on the test seam / fabricated stand-in on the initiator path. The `logon_peer_identity_override` seam **remains** as the binding-logic test seam (off-list / on-list / absent); acceptor-side live binding + seam removal + full T-041 closure are 015. Fail-closed/permissive semantics, canonical CN→SAN-DNS→SAN-URI→SHA-256 extraction order, and the `session_event_compid_authorization_failed` + `session_compid_unauthorized` event/code shapes are **inherited unchanged** from 013 (FR-019/020/022); the *FSM disposition* on the reconnect path differs (retry-to-cap, not terminal Disconnected — see Constraint "Reason-agnostic reconnect").
...
5. **FR-013** — (a) the `cert_source::load_credentials()` once-per-handshake counter witness (today **infeasible** with `mock_transport` per `tests/session/test_session_invariant_counter_witness.cpp:22-35` — zero loads; needs the real handshake) and (b) wire the `bench/transport/bench_tls_handshake_loopback.cpp` scaffold (012 RC#G; T029 TODO at `:16`/`:42-49`/`:52-67`) — CMake target **`bench_tls_handshake_loopback`** (`bench/transport/CMakeLists.txt:22`) — to a live loopback fixture → establishes the **new** handshake baseline. The scaffold's happy-path fixture is `tests/tls/fixtures/leaf_rsa2048.pem` (+ `ca.pem`) hard-coded at `:44`; that RSA-2048 leaf is the bench's happy path (the new ed25519/ed448/multi_san fixtures are for the FR-012/FR-014 *rejection* cells, not the bench) — so the fixture story names `leaf_rsa2048.pem` for SC-006/FR-013b to be mechanically checkable.
...
- **`fixpp::transport`** (012 — LOCKED except the one Gate-A-blessed widening below): `Transport` (5 pure-virtuals: `async_connect`/`async_read_some`/`async_write`/`close`/`cancel`), `TlsTransport` (+1: `async_handshake(SslCtxConfig const& cfg [[clang::lifetimebound]])` — **takes a cfg arg**, `tls_transport.hpp:116-118`), `handshake_result{peer_id, ...}` (`tls_transport.hpp:52`), `TransportFactory::make(asio::any_io_executor, tls::SslCtxConfig, std::pmr::memory_resource*) -> expected_t<unique_ptr<Transport>>` (`:74`) + `TransportFactory::reload_credentials(shared_ptr<cert_source>) noexcept` (`:95`) pure-virtual (count **2/5** from 013). `cert_source_snapshot()` ships **concrete-only** on `asio_tls_transport_factory` (`transport_factory.hpp:167-168`), NOT on the abstract base the FSM holds (`reconnect_fsm.hpp:152`) — **014 promotes it to a pure-virtual on the abstract `TransportFactory`** so the FSM's rotation read compiles through its abstract pointer: count **2/5 → 3/5**, still under the §XIV.2 cap. `ReconnectPolicy` (`reconnect_policy.hpp:30`: `schedule`/`max_attempts`/`delay_for_attempt(n)` + jitter — already constructed at `ReconnectFsm` ctor; 014 walks it). The `[2h]` in-flight-exclusivity + `cancellation_type::total → transport_*_cancelled` contracts are BINDING and consumed verbatim.
...
- **OpenSSL** — same row 011/012 pinned; 014 uses it only to compute the SHA-256 of our own leaf DER for `credentials_rotated` (no new abstraction; **no new `cert_source` pure-virtual** — fingerprint is computed from the already-returned `local_credentials` leaf). The only §XIV.2 movement is the `TransportFactory::cert_source_snapshot()` promotion (2/5 → 3/5, under cap — see above); `cert_source` itself is untouched.
...
- **`[const §XIV.2]` pluggable-interface caps RESPECTED (one promotion, under cap)** — `Transport` stays 5/5, `TlsTransport` stays +1 as shipped, `cert_source` is untouched; **`TransportFactory` moves 2/5 → 3/5** by promoting `cert_source_snapshot()` from the concrete `asio_tls_transport_factory` to a pure-virtual on the abstract base (so the FSM's documented rotation read compiles through its abstract `TransportFactory*` pointer). Still under the 5 cap. This is no NEW concept (the method already exists on the only production concrete impl); the test-local `TransportFactory` subclasses each gain a trivial override returning their held source. The leaf-fingerprint is still computed from the already-returned `local_credentials`, **not** via a new `cert_source` method.
...
- **`[const §XII]` security** — the mTLS Logon gate already fails CLOSED under 013 (the three-way guard's `else if (is_mtls)` arm); 014 introduces NO fail-CLOSED from scratch and there is no fail-OPEN hole on the mTLS path. 014 only swaps the identity *source* (test-seam/stub → live `handshake_result.peer_id`) on the initiator path, making the already-fail-CLOSED gate *operable* with a real identity (admit on-list, fail-close off-list/absent). The `sigalg_disallowed` cell asserts the `[const §XII.3]` allow-list rejects an Ed25519/Ed448 leaf.
- **`[const §XV.9]` forward-declare discipline preserved under the `cert_source_snapshot()` promotion** — promoting the method to the abstract `TransportFactory` does **NOT** require `reconnect_fsm.hpp` to `#include` the full `transport_factory.hpp` definition: the call stays in `reconnect_fsm.cpp` (which already includes it), and the header keeps its forward-declare `namespace fixpp::transport { class TransportFactory; }` (`reconnect_fsm.hpp:40-48`). `#include`-ing the factory header here would transitively drag `tls/pinset.hpp`'s `std::shared_mutex` into the `<asio/awaitable.hpp>` closure of every consumer (`session.hpp`) — the exact `8e2d362` regression class per `[[feedback_awaitable_header_mutex_include_edge]]`. The naïve "the FSM now calls a base method, so include the base header" reading is explicitly forbidden.
...
**Scale/Scope**: small, surgical — **no new public type, no new header**, but **two public-surface deltas** (error slot 120 + the `TransportFactory::cert_source_snapshot()` pure-virtual promotion). Estimated footprint:
- **`include/fixpp/core/error.hpp`** — **+1** enum variant (`session_seqnum_too_high = 120`) + the slot-74 hole/meaning comment update.
- **`include/fixpp/transport/transport_factory.hpp`** — **promote** `cert_source_snapshot()` to a pure-virtual on the abstract `TransportFactory` (2/5 → 3/5); the concrete `asio_tls_transport_factory` override already exists (`:167-168`). ~3 lines (move the declaration to the base + `= 0`).
- **`src/transport/transport_factory.cpp`** + the **test-local `TransportFactory` subclasses** — implement/confirm the now-abstract `cert_source_snapshot()` override (the asio impl already has its body at `:193`). Because the method becomes pure-virtual, **every** test-local factory subclass that overrides `make`/`reload_credentials` (e.g. `tests/session/test_reconnect_happy_path.cpp`, `tests/session/test_reload_credentials_in_flight.cpp`) MUST add a trivial `cert_source_snapshot()` override returning its held source — else it won't compile. ~a few lines per subclass.
- **`src/session/reconnect_fsm.{hpp,cpp}`** — realize `drive_reconnect_attempt`: the per-attempt connect/handshake/identity/rotation pipeline + the `ReconnectPolicy` retry-to-cap loop + cancellation/release + the FSM-held `{last_active_source_ : shared_ptr<cert_source>, last_active_fp_ : array<byte,32>}` rotation-detect state (strong-ref owning member per `[[feedback_weak_ptr_cache_needs_owning_context]]`). **Also correct the inherited-false 013 doc-comment** at `reconnect_fsm.hpp:20-23` and `:76-82` (it describes `factory_->cert_source_snapshot()` as if the abstract base already had the method — true only after this promotion). Keep the forward-declare of `TransportFactory` (`:40-48`); do **not** `#include transport_factory.hpp` into the header (§XV.9). ~250–400 lines.
- **`src/session/session.cpp`** — at `:958`/`:1762` (inside the three-way guard at `:953-1008`/`:1757-1803`) add the live-identity arm: the FSM-held `handshake_result.peer_id` becomes the authorize source on the initiator live path (keep the `logon_peer_identity_override` seam arm and the `else if (is_mtls)` fail-CLOSED arm); wire `drive_reconnect_attempt`'s outcome (rebind `transport_send_`, store peer_id, re-drive Logon). ~120–200 lines.
...
| XII | Security & TLS | mTLS gate already fails CLOSED in 013 (three-way guard); 014 introduces NO fail-CLOSED from scratch and there is no fail-OPEN hole on the mTLS path — it swaps the identity *source* (test-seam/stub → live `peer_id`) so the gate becomes *operable* with a real identity on the initiator path; `sigalg_disallowed` cell exercises the `[const §XII.3]` allow-list; no coalescing of the `tls_*` master-enum | ✓ (central — see Phase-0 §R2/§R5) |
| XIV.2 | Pluggable ≤5 pure-virtuals | `TransportFactory` **2/5 → 3/5** — promote `cert_source_snapshot()` from concrete to abstract pure-virtual (under cap; no NEW concept — the production concrete impl already has it; test-local factory subclasses add a trivial override). `Transport` 5/5, `TlsTransport` +1 shipped, `cert_source` untouched; leaf fingerprint computed from `local_credentials`, not a new `cert_source` method | ✓ (under cap; Gate-A-blessed widening) |
...
**No constitution violations to track.** The ABI append (slot 120) respects `[const §X.4]`; the `TransportFactory::cert_source_snapshot()` promotion (2/5 → 3/5) is under the §XIV.2 cap and is an explicit Gate-A-blessed widening (no NEW concept); the deliberate initiator-only binding asymmetry is a *clarified, documented* boundary (Q2), not a half-restructure defect.
...
│   ├── transport/
│   │   └── transport_factory.hpp      # PROMOTED — cert_source_snapshot() moves from concrete asio_tls_transport_factory (:167-168) to a pure-virtual on the abstract TransportFactory (2/5 → 3/5, under §XIV.2 cap)
│   └── session/
│       ├── reconnect_fsm.hpp          # EXTENDED — drive_reconnect_attempt realized signature/contract; FSM-held rotation-detect state {last_active_source_, last_active_fp_} (strong-ref member). DOC-FIX :20-23 / :76-82 — they describe factory_->cert_source_snapshot() as if the abstract base had it (true only after the promotion). KEEP the TransportFactory forward-declare (:40-48); do NOT #include transport_factory.hpp (§XV.9)
│       └── session.hpp                # (comment at :274 "credentials_rotated DEFERRED to 014" resolved; reload_credentials forwarding unchanged)
│
├── src/
│   ├── transport/
│   │   └── transport_factory.cpp      # asio impl override of cert_source_snapshot() already exists (:193); becomes the override of an abstract pure-virtual (no body change). Test-local TransportFactory subclasses add a trivial override (return held source)
│   └── session/
│       ├── reconnect_fsm.cpp          # REALIZED — replaces the make()+`(void)t` stub (:53-61) with connect→handshake→authorize→rotation-emit→hand-back + ReconnectPolicy retry-to-cap loop + cancellation/release
│       ├── session.cpp                # EXTENDED — inside the three-way guard at :953-1008 / :1757-1803, add the live-identity arm: authorize SOURCE = FSM-held live handshake_result.peer_id on the initiator path (keep the override seam arm + the else-if(is_mtls) fail-CLOSED arm); install reconnect outcome (rebind transport_send_, re-drive Logon)
...
**Structure Decision**: 014 is an in-place realization of stubbed behaviour in the existing `session` module plus carry-forward witnesses in `transport`/`bench`/`fuzz` tests. No new module, no new public type, no `tools/check_layers.py` change. There are **two** genuine public-surface deltas: (1) `error::session_seqnum_too_high = 120` (captured in `contracts/error_slots.hpp`), and (2) the promotion of `TransportFactory::cert_source_snapshot()` to a pure-virtual on the abstract base (2/5 → 3/5; documented in `contracts/realized-behavior.md` C4 — a Gate-A-blessed widening of 012's frozen contract, no NEW concept).

## Complexity Tracking

> No `Constitution Check` violations → **no entries**. The deliberate initiator-only identity-binding asymmetry is a clarified, documented 014/015 boundary (Clarifications Q2), not an unjustified complexity; the single error-slot append is standard `[const §X.4]` ABI hygiene; the `cert_source_snapshot()` promotion (2/5 → 3/5) is under the §XIV.2 cap and is a documented Gate-A-blessed widening.

## Gate A

- Round 1 applied 2026-05-29: Codex P1=2 P2=2 P3=0; Opus post-judging P1=2 P2=3 P3=3; rewrite addresses Root cause #1 (security baseline re-based on the shipped mTLS three-way guard), #2 (promote cert_source_snapshot() to abstract TransportFactory, 2/5→3/5, fix §XIV.2 + inherited 013 header prose), #3 (shipped-reality table + credentials_rotated baseline), plus the handoff/async_handshake-signature/Q1-disposition cluster. Reviews: research/reviews/codex_014-transport-active-binding_gate_a_review.md, research/reviews/opus_014-transport-active-binding_gate_a_adversarial_review.md
- Round 2 applied 2026-05-29: Codex P1=0 P2=1 P3=1; Opus post-judging P1=0 P2=1 P3=2; rewrite adopts handoff option (a) — drive_reconnect_attempt keeps its public expected_t<void> return + private Session::install_reconnected_transport handoff (no reconnect_outcome public type; preserves reconnect_fsm.hpp header-minimalism / §XV.9), and aligns E-1's TlsTransport cast to the shipped single-dynamic_cast discipline (tls_transport.hpp:61-67). Reviews: research/reviews/codex_014-transport-active-binding_gate_a_2_review.md, research/reviews/opus_014-transport-active-binding_gate_a_2_adversarial_review.md

### Round 1 — disagreements

- Codex P2-4 (handshake-bench path `bench/transport/` "does not exist", move to `bench/tls/`) — OVERRULED by Opus on source: `bench/transport/bench_tls_handshake_loopback.cpp` exists exactly as the bundle states; `bench/tls/` has no handshake bench. Path unchanged; only the P3 residual (name CMake target + reconcile `leaf_rsa2048.pem` fixture) applied.
