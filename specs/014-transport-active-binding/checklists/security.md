...
- [x] CHK008 Are the authorization-failure event/code shapes (`session_compid_unauthorized` + `session_event_compid_authorization_failed`) required to be identical between the inherited 013 open-Logon path and the 014 reconnect path? [Consistency, Spec §FR-007] — PASS: FR-007 states "The fail-closed/permissive semantics, canonical extraction order, and event/code *shapes* are inherited unchanged from 013 (FR-019/FR-020/FR-022)." It then explicitly notes the *FSM disposition* differs (retry-to-cap on reconnect vs terminal Disconnected on open path), while keeping the event/code shapes constant. Consistent.
