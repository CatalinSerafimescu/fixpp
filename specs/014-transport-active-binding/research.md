...
**Decision.** The live identity is `handshake_result.peer_id` returned by **`TlsTransport::async_handshake(SslCtxConfig const& cfg [[clang::lifetimebound]])`** (`include/fixpp/transport/tls_transport.hpp:116-118`, returning `asio::awaitable<core::expected_t<handshake_result>>`). `handshake_result` is a value-typed POD with `fixpp::tls::peer_identity peer_id` that **owns** its SAN/CN material (`tls_transport.hpp:52-53`), held **by value** across the session lifetime by the FSM (`tls_transport.hpp:40`). `drive_reconnect_attempt` issues `async_connect` then `async_handshake(ssl_cfg)` — passing the per-attempt `SslCtxConfig` it built for `make()`, which it MUST keep alive across the `co_await` (the arg is `[[clang::lifetimebound]]`) — captures `handshake_result`, and threads `peer_id` to the existing `authorize()` call sites.

**Rationale.** This surface was shipped by 012 specifically for this consumer; no new transport API is needed for the identity (the `cert_source_snapshot()` promotion in R3 is a separate, rotation-observability widening). The stub today (`reconnect_fsm.cpp:53-61`) calls only `factory_->make(...)` and never `async_connect`/`async_handshake`, so the peer identity is simply never produced — 014 calls the two steps the stub skips.
...
**Decision.** Compute the SHA-256 over **our own** loaded leaf-certificate DER (from `cert_source::load_credentials() -> local_credentials`, `cert_source.hpp:32-41`) using OpenSSL, into the existing `std::array<std::byte,32>` fields. Rotation detection lives in the **FSM**, not the factory: `ReconnectFsm` holds two owning members — `last_active_source_ : std::shared_ptr<cert_source>` and `last_active_fp_ : std::array<std::byte,32>`. On each `drive_reconnect_attempt`: read `factory_->cert_source_snapshot()` **through the abstract `TransportFactory*` the FSM holds** (`reconnect_fsm.hpp:152`); because that method ships **concrete-only** (`transport_factory.hpp:167-168`) and is absent from the abstract base, 014 **promotes it to a pure-virtual on the abstract `TransportFactory`** (2/5 → 3/5, under the §XIV.2 cap — see plan §XIV.2 + contracts C4). If the snapshot differs from `last_active_source_` (i.e. `reload_credentials` staged a new source), invoke the `Session`-injected strand-bound `emit_credentials_rotated_` callback with `{old=last_active_fp_, new=fp(new source)}` **before** `make()` (the FSM owns the snapshot read; `Session` owns the strand-bound emit — the FSM has no `SessionEvent` emit path), then update both members. First-ever attempt (`last_active_source_ == nullptr`) is the initial load, **not** a rotation → no event. No-op rotation (`new fp == old fp`) **still** emits (FR-011).

**Rationale.** Keeps the detect/compute inside 014's `session` module with **no new `cert_source` pure-virtual** and the strand-bound emit on the `Session` that owns it. The only §XIV.2 movement is the `TransportFactory::cert_source_snapshot()` promotion (2/5 → 3/5, under cap), which the FSM legitimately needs through its abstract pointer — no NEW concept (the only concrete impl already has the method). Holding `last_active_source_` as a **strong-ref owning FSM member** (not a `weak_ptr`) is the explicit fix for `[[feedback_weak_ptr_cache_needs_owning_context]]`: the old source must survive to fingerprint it on the next rotation. The emit-before-`make()` ordering and the no-op-still-emits rule are inherited verbatim from 013 FR-032.
...
| R3 | Real leaf fingerprint | OpenSSL SHA-256 over our `local_credentials` leaf; FSM-held strong-ref rotation state + `Session`-injected strand-bound emit callback; promote `cert_source_snapshot()` to abstract `TransportFactory` pure-virtual (2/5 → 3/5, under cap); no new `cert_source` pure-virtual |
