...
- **FR-003**: Every failed reconnect attempt MUST consume one attempt and be retried per the backoff schedule until the cap is reached, at which point the session transitions to the FSM's terminal disconnected outcome (no infinite retry). This is **reason-agnostic**: every failure cause — factory `make()` failure, connection failure, TLS handshake failure, or authorization failure (off-list / absent identity under a binding policy) — MUST count as exactly one attempt and be retried to the same cap. No fail-fast and no distinct cap or terminal cause for any specific failure kind.
...
- **FR-006**: When a handshake yields an authenticated peer identity (`handshake_result.peer_id`), the live identity MUST be supplied to `CompIdAuthorizationPolicy::authorize()` — there must be no fabricated/stand-in identity on the live path. On the initiator live path the identity source precedence is: (1) the real `handshake_result.peer_id` from the completed TLS handshake (when available on the live reconnect path); (2) the `logon_peer_identity_override` seam (retained for binding-logic tests, per FR-008); (3) the inherited 013 arms (`else if (is_mtls)` fail-CLOSED / non-mTLS permissive skip). The live handshake arm (1) is inserted ahead of the seam arm (2); it does NOT replace arms (2) or (3). (Design records E-2/C2 document the implementation.)
...
- **FR-009**: After `reload_credentials` has staged a new `cert_source`, the system MUST emit `SessionEvent::credentials_rotated{old_sha256, new_sha256}` on the session strand at the next `drive_reconnect_attempt`, immediately BEFORE the new `cert_source_snapshot()` is passed to `make()` (before the first handshake on the rotated source), per 013 FR-032. The **first-ever credential load** (when no prior `cert_source` was active — `last_active_source_ == nullptr` in the FSM's rotation-detect state, E-3) is NOT a rotation and MUST NOT emit a `credentials_rotated` event; the FSM silently sets its rotation-detect baseline and proceeds to `make()`.
...
- **SC-001**: An initiator session whose transport drops re-establishes a live, TLS-authenticated session with no operator intervention in 100% of cases reachable by the loopback-TLS integration test (i.e., where the peer becomes reachable and the handshake succeeds within the configured cap), within the configured backoff schedule. "Reachable" is bounded by the test fixture: drop + re-available peer within the `ReconnectPolicy` cap → always reconnects; cap exceeded → terminal Disconnected (covered by SC-002).
...
- **SC-004**: Across N ≥ 3 consecutive failed-then-succeeded and cancelled-mid-handshake attempts, the system leaks no transport, socket, or memory, as verified under the sanitizer matrix.
