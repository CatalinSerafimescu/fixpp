...
- [ ] T007 **[RELOCATED → lands atomically with T012 in US1]** — `operator[]` cannot build `GroupT{span,ctx}` until the generated `G_n` has the `entry_context` ctor (T012); splitting them breaks the build mid-phase (symmetric-interface-halves rule). Thread the `entry_context` through the read chain (depends on T002–T006):
