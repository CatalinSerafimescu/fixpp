...
- Q: Cost model for grouped-entry reads — lazy (pay-per-access) vs eager (materialized at parse time)? → A: No spec-level constraint. `/speckit-plan` selects the mechanism and its cost model freely, judged on correctness + FR-004 (zero per-access heap allocation + lifetime safety); neither lazy nor eager is mandated by the spec. If Phase-0 analysis shows one materially regresses the group-heavy hot path, that is a plan-level tradeoff to surface at `/plan` sign-off, not a spec requirement. **(Refined below — the hot-path posture is now constrained.)**
- Q: Is typed reading of a repeating group NESTED inside another group entry in v1.0 scope? → A: **YES.** Verified 2026-07-05 that both reference engines (QuickFIX C++ and QuickFIX/J) expose repeating groups as fully recursive typed group classes, and MassQuote's core data (BidPx/OfferPx/BidSize/OfferSize) lives in the nested `NoQuoteEntries` — read per-instance-typed by both. `field_value(tag)` on the whole message returns only the FIRST occurrence, so it cannot substitute for per-instance nested reads. FR-002 stands (P1). **Added constraint (refines the cost-model answer above):** one-level (non-nested) group-entry SCALAR reads MUST stay cheap — no per-entry sub-index build on the common one-level path (e.g. MarketData `NoMDEntries`); a per-entry sub-index may be built only when a caller DESCENDS into a nested group, and then lazily/bounded. This protects the MarketData hot path while delivering nested parity. Exact mechanism (span-scan scalars + lazy sub-view on nested descent, vs lazy-per-group materialization) remains a `/plan` decision under this constraint.
- Q: (Gate-A scope boundary, recorded during US2 implementation) Does 062 deliver full **multi-entry** nested reads end-to-end, or only the nested-read mechanism? → A: **Mechanism + single-entry-per-occurrence only; multi-entry deferred to prerequisite 063.** Implementing US2 surfaced TWO **pre-existing** defects that block the multi-entry nested case end-to-end: **Defect A** — the dictionary loader registers repeating-group membership globally, first-XML-occurrence-wins, keyed by NumInGroup tag (`xml_loader.cpp:476-516`), so a tag reused with differing membership (e.g. FIX44 tag 295 `NoQuoteEntries`) resolves to the wrong variant; **Defect B** — `OffsetTable::group()`'s flat `seen_in_instance` boundary walk (`offset_table.cpp:419-437`) is NOT nesting-aware, so an outer instance containing a nested group with >1 entries is truncated at the 2nd nested entry. BOTH are UPSTREAM of 062's changed surface: 062 holds the whole-frame `OffsetTable::build()` guard AND `group_slice.len` **UNCHANGED** (RC1) — it deliberately does not touch outer-slice extent computation. 062 therefore delivers and unit-proves the recursive nested-read **MECHANISM** (span-scan scalars + lazy dict-aware sub-view + collision-free slice-identity cache) and **single-entry-per-occurrence** nested reads over generated flyweights (the depth-3 non-first-occurrence + dict-aware-trailing witnesses PASS); the **multi-entry** case (>1 nested entry within one outer occurrence — MassQuote `NoQuoteEntries` per-instance BidPx/OfferPx) and the real-`Dictionary::as_table_view()` case are deferred to prerequisite **063 "nested group-parse correctness"** (consolidates Defects A+B; `research/G19-fix-fpml-iso20022/research/findings/dict-group-tag-collision-2026-07-05.md`). This bounds FR-002 (see its scope note) and SC-001, and is recorded as `L-062-*` limitation rows.
...
- **FR-002**: Entry accessors for **nested** repeating groups MUST work recursively (an entry's `group<c, G_c>()` returns a usable `group_view` whose entries are themselves readable). **(Scope, per Clarifications 2026-07-05: 062 delivers the recursive nested-read MECHANISM + single-entry-per-occurrence nested reads, proven over generated flyweights; the MULTI-ENTRY nested case and real-dictionary nested reads are blocked by pre-existing UPSTREAM Defects A/B outside 062's RC1-frozen surface — see `OffsetTable::build()`/`group_slice.len` UNCHANGED — and are deferred to prerequisite 063. FR-002's mechanism requirement is met at the 062 layer; end-to-end multi-entry correctness is a 063 deliverable — `L-062-*`.)**
...
- **SC-001**: Typed reads of repeating-group entries (single-level and nested) compile and return exact wire values over generated flyweights — demonstrated for at least 2 distinct grouped messages including one with a nested group. **(Per Clarifications 2026-07-05: satisfied by NewOrderList `orders` (one-level) + the depth-3 non-first-occurrence and dict-aware-trailing nested witnesses (single-entry-per-occurrence). The multi-entry `NestedQuoteEntriesPerInstancePrices` witness is honestly `GTEST_SKIP()`'d pending 063 — see FR-002 scope note.)**
