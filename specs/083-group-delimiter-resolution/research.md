...
**Recorded residual, not elided** *(re-stated at round 3 — the residual shrank, and by how much matters)*: after Phase 4 this file holds **three** instance-boundary rules — dictionary-sourced and nesting-aware at `:656`; wire-derived but nesting-aware at **both** positions at `:454` (C-8.0c); wire-derived and **flat** at `:561`. The disagreement FR-021 exists to remove is therefore narrowed to **one** site, `:561`, whose exposure is a cap check and never wrong returned data. That is why **C-8.5 still does not claim to "discharge part of #180"** — and the reason is now stronger than when this paragraph was written: 083 lands **neither** of L-063-4's two legs. Leg 1 (*"make the splitter nesting-aware"*) is **NOT DONE — DESCOPED with measured evidence** (T054's zero-population measurement across all ten dictionaries; `tasks.md` T059 is `[~]`); leg 2 (*"fold the redundant flat cap loop into the same traversal"*) is **OUTSTANDING, now tracked as fixpp#214**. **`#180` itself was already CLOSED by 072 on 2026-07-13 and is not reopened here.** *(Corrected 2026-08-02, Gate B r3, fixpp#216 P1 — previously read "this feature lands only the first … #180 stays open", carrying both the withdrawn C-8.5-leg-done claim and the corrected `#180` state.)*
