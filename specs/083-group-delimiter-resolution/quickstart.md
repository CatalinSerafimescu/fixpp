...
| 4 — consumers | FR-019a **class (a)** — the five groups whose global value moves (W-12) | old opening tag rejects, new accepts — pinned, not merely described. Class **(b)** (cause-1 tags, same severity) is pinned by **W-11** on the row above; class **(c)** is a widening, disclosed not pinned. *(Row re-labelled 2026-07-31, `/speckit-analyze` re-run — "five disclosed delimiter moves" read as the whole disclosure when it is one of three classes.)* |
