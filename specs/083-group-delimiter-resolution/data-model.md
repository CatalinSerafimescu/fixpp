...
**A correct split of a truncated extent is still wrong**, which is why FR-021b's agreement invariant below could not have held on the **485 contexts** *(corrected 2026-08-02, Gate B r2 — superseding the earlier "262"; see `spec.md:342-346` / SC-016)* whose post-fix delimiter is a nested group's count tag until C-8.0c landed.
...
**Hard constraint (FR-018a)**: no exported C ABI signature changes — the ABI is GA-frozen at 1.5.0. The affected function is file-static; this is a behaviour change reachable through the frozen surface, disclosed via FR-019, not a surface change. The disclosure must **enumerate all three FR-019a classes** (C-9.6) — (a) the five groups whose *global* value moves, (b) the cause-1 tags whose lookup becomes context-sensitive in divergent contexts, (c) the three unregistered groups + six nested children that become buildable — because that enumeration is what lets the SC-007 audit tell an intended new rejection from a regression, and it can only do so for classes it actually lists. Classes (a) and (b) are the SC-007 exceptions; (c) is a widening. *(Widened 2026-07-31 at the `/speckit-analyze` re-run, F1: this paragraph still read "the five groups whose delimiter moves" after the checklist audit corrected that scope in `spec.md`, `contracts/capi_group_grammar.md`, `tasks.md` and `plan.md` — this file was missed by that sweep, leaving the same wrong-population statement live in the entity a designer reads.)* Witnesses W-11/W-11a/W-11b/W-12/W-13 in `tests/capi/capi_group_delimiter_ctx_test.cpp` — W-11a is the only one that distinguishes the FR-018b route from the constitutionally-barred per-commit rebuild, which is behaviourally identical.
