...
**Amendment discovered at /speckit-implement T006** (2026-05-23): the first-draft claim "all members are copyable" was incorrect. `SessionConfig` has a `std::unique_ptr<MessageStoreFactory> store_factory` member at `session_config.hpp:127`, and `MessageStoreFactory` explicitly deletes copy AND move (`include/fixpp/session/message_store_factory.hpp:36-39`), making the whole `SessionConfig` non-copyable AND non-movable. Resolution (orchestrator decision, /speckit-implement Phase 3 escalation): change the `store_factory` field from `std::unique_ptr<MessageStoreFactory>` to `std::shared_ptr<MessageStoreFactory>` (per FR-001a). Rationale: the 005 `unique_ptr` choice was about polymorphic ownership through indirection (per the comment at `message_store_factory.hpp:9-12` — "preserving the class identity, the deleted move/copy, the virtual destructor"), NOT about forbidding sharing. `MessageStoreFactory` is a stateless interface — only one virtual method `make()` returning a freshly-minted `unique_ptr<MessageStore>` — so factory sharing across Sessions is meaningful and the per-Session uniqueness invariant (one MessageStore per Session, satisfied by each Session calling `make()` on the shared factory) is preserved. The amendment is a small 005 design change (factory ownership: unique→shared); documented in T027b Gate A inheritance addendum. Call sites that assign `cfg.store_factory = std::make_unique<...>(...)` continue to work via the implicit `unique_ptr<T>&& → shared_ptr<U>` move-conversion (zero caller-side change).
