...
| `fsm_visit_write_idx_` | (absent) | `std::uint32_t fsm_visit_write_idx_ = 0;` | FR-004 / D-2 / `[gate-b/r1 F-13]` | New private member added by `gate-b/r1` round-1 fix. Monotonic write-index distinct from `fsm_visit_count_`; modular `% 16` arithmetic cycles 0..15 indefinitely so the ring keeps rotating after the saturating count caps. Without this separation, combining write-position with the saturating count froze slot 15 after the 255th transition. |
| `record_state_transition_(fsm_state) noexcept` | (absent) | private method | FR-004 / D-2 | New private helper. Records the transition into the ring buffer, then performs the assignment to `fsm_state_`. Called from every `fsm_state_ = X;` site in `src/session/session.cpp` (/speckit-analyze verified **37 sites**, line ranges in plan §Scale/Scope). |
