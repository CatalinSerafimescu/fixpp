...
- [X] T001 Verify branch state: confirm `git branch --show-current` returns `010-session-cfg-lifetime`, confirm `git log --oneline ba2222d..HEAD` shows zero 010 commits yet (untouched base), confirm `git status` is clean. If any check fails, STOP and surface to user.
- [X] T002 [P] Baseline test run: `ctest --test-dir build/linux-clang-debug -R 'session_|tc_'` against the inherited HEAD `ba2222d`. Capture pass/fail tally. Expected: all GREEN (009 closed at GREEN, the only test currently skipped is `session_coverage_adversarial` under ASan per the W-5 carry-forward — that's the test FR-003 re-enables). If any unexpected session test is RED at baseline, STOP and surface — that's a pre-existing 005/009 issue this slice did not introduce.
...
- [X] T005 Add the `record_state_transition_()` definition + route every `fsm_state_ = X;` site through it in `src/session/session.cpp` (research D-2):
  ...
  - Replace every `fsm_state_ = X;` site (/speckit-analyze verified **37 sites** in the post-009 tree, spread across `Session::open`, the message-handler `switch` cascade, the close path, and the logout-timer handler — NOT the ~10 first-draft estimate) with `record_state_transition_(X);`. Re-grep at task time with `grep -cE '\bfsm_state_\s*=\s' src/session/session.cpp` — expected count = 37 pre-edit. Search pattern post-edit: `grep -nE '\bfsm_state_\s*=\s' src/session/session.cpp` MUST return **exactly one matching line** (the assignment inside the helper body itself; in-class initializer at session.hpp:385 is initialization, not an assignment, and the grep does not match it).
...
- [X] T006 [US1] **Two-part task** discovered at /speckit-implement first attempt (2026-05-23): the by-value `Session::cfg_` requires `SessionConfig` to be copy-constructible, but `SessionConfig::store_factory` was a `std::unique_ptr<MessageStoreFactory>` with deleted move/copy on the factory itself, making the whole `SessionConfig` neither copy- nor move-constructible. **Part (a) — type amendment (FR-001a):** flip `include/fixpp/session/session_config.hpp:127` from `std::unique_ptr<MessageStoreFactory> store_factory;` to `std::shared_ptr<MessageStoreFactory> store_factory;`. Update the comment on the field (was "// unique ownership") to "// shared ownership; stateless factory shared across Sessions; each Session calls make() to mint its own MessageStore". Update the include comment at session_config.hpp:26 (was "// unique_ptr member ⇒ complete type") to "// shared_ptr member ⇒ complete type (still needs complete type for ABI / deleter generation)". Update the explanatory comment block at `include/fixpp/session/message_store_factory.hpp:9-12` from "unique_ptr<MessageStoreFactory>" to "shared_ptr<MessageStoreFactory>" with a brief note pointing at 010 FR-001a + Gate A addendum. **Part (b) — hygiene gate:** add `#include <type_traits>` (if not already present — it likely IS already, from the abandoned first attempt) and add `static_assert(std::is_copy_constructible_v<SessionConfig>, "SessionConfig must be copy-constructible per 010 W-5 by-value Session::cfg_ membership");` immediately after the class definition. Verify the build still compiles (the static_assert will pass post-amendment). Call sites that assign `cfg.store_factory = std::make_unique<FileStoreFactory>(...)` continue to work unchanged via implicit `unique_ptr<T>&& → shared_ptr<U>` move-conversion (tests at `tests/session/test_file_store_flush_for_session_close.cpp:155,315` use this pattern; no edit needed there).
- [X] T007 [US1] Write `tests/session/cfg_lifetime_safety_test.cpp` (TDD red per FR-001 / FR-002 / SC-001):
...
- [X] T012 [US2] Write `tests/session/logon_received_observability_test.cpp` (TDD red per FR-004 acceptance scenario 1):
  ...
  - Add to `tests/session/CMakeLists.txt` via `add_threading_test(session_logon_received_observability logon_received_observability_test.cpp)` (mirror the existing pattern of other session tests). Verify the test compiles; will pass once T005 is in (the helper records every transition). If T005 didn't already make this GREEN, surface — the helper has a defect.
- [X] T013 [P] [US2] Write `tests/session/session_send_invalid_state_test.cpp` (TDD red per FR-005 acceptance scenarios 1+2):
  ...
  - Add to `tests/session/CMakeLists.txt` via `add_threading_test(session_send_invalid_state session_send_invalid_state_test.cpp)`. Verify the test compiles and FAILS pre-fix (the existing impl returns `session_invalid_logon`).
- [X] T014 [US2] Replace the one `co_return std::unexpected(error::session_invalid_logon);` site in `Session::send` in `src/session/session.cpp` (line 1151 — /speckit-analyze verified exactly **one** site exists in `Session::send`, not two as the first-draft estimated; re-grep `session_invalid_logon` in session.cpp at task time to confirm count = 1 inside `Session::send`) with `co_return std::unexpected(error::session_invalid_state_for_send);` (FR-005). Verify T013 GREEN.
- [X] T015 [US2] Safety-net grep on existing tests for `error::session_invalid_logon` assertions at the `Session::send` non-Active site (FR-005 AC3): `grep -rn 'session_invalid_logon' tests/session/`. /speckit-analyze verified the expected count of send-path-specific updates needed = **0** (existing tests assert `EXPECT_FALSE(result.has_value())` only at this site, never the specific variant; the new T013 tests provide AC3 coverage going forward). If the grep returns any send-path-specific variant assertion (vs the FSM Logon-refusal path tests, which legitimately keep `session_invalid_logon`), surface and update; otherwise mark T015 complete with the documented expected-zero-changes outcome. Verify the full `session_|tc_` ctest set GREEN.
...
- [X] T016 [US3] Enumerate the FSM matrix cells from the 005 data-model + the existing `switch (fsm_state_)` cascade in `src/session/session.cpp` (research D-4). Output: an inline comment table at the top of the new test file `tests/session/fsm_matrix_witness_test.cpp` listing every (state, event) cell with its design-required outcome: `transition_to(X)` / `reject_and_disconnect` / `ignore_no_op`. /speckit-analyze verified the event alphabet at `include/fixpp/session/session_fsm.hpp:52-67` enumerates **15 events** (not 8-10); raw matrix = 6 × 15 = 90, minus design-forbidden ≈ **55-70 reachable cells**. Cross-check against `specs/005-session-establishment-fsm/data-model.md` — if any impl-handled cell is missing from the 005 data-model OR vice-versa, **open a blocking issue for the orchestrator and STOP — do not proceed to T017 until the discrepancy is resolved**, as writing tests for undocumented cells would contradict FR-010 (no 005 design amendment). The `/speckit-analyze` step has already run by task time; the escalation target is the parent Opus orchestrator, not another /analyze invocation.
- [X] T017 [US3] Write `tests/session/fsm_matrix_witness_test.cpp` (TDD red per FR-006 + SC-002):
  ...
  - Estimated **~700-900 LoC** post-/speckit-analyze (15-event alphabet × 6 states minus design-forbidden ≈ 55-70 cells, ~12-15 LoC per cell incl. setup + assertion). If this exceeds the Sonnet-invocation LoC cap per `[[feedback_phase_implementer_sonnet_runaway_scope]]`, split T017 into per-state batches (one Sonnet invocation per FSM state's row — 6 batches total). Verify compile + at least one cell GREEN at write time (positive-control sanity); the rest verified at T020.
- [X] T018 [P] [US3] Write `tests/session/admin_builder_distinct_now_test.cpp` (TDD red per FR-007 + SC-006):
...
- [X] T019 [P] [US3] Write `tests/session/admin_emit_mixed_path_test.cpp` (TDD red per FR-008 + SC-005):
...
- [X] T021 [US4] Write `tests/session/initiator_transport_throw_test.cpp` (TDD red per FR-009 + SC-007):
...
- [X] T025 [P] Static analysis sweep on touched files: clang-tidy + clang-format + cppcheck + IWYU per `[const §IX.4]`; `[const §XV.9]` mutex-in-awaitable grep gate (`tests/sync/check_no_std_mutex_corpus` GREEN). Touched files per plan.md Project Structure (session.hpp, session.cpp, error.hpp, session_config.hpp + the new test files). Capture findings; ride-along fixes only (no broader cleanup per `[[feedback_phase_implementer_sonnet_runaway_scope]]` surgical-changes discipline).
- [X] T026 Invariant I-25 grep gate: confirm every `fsm_state_ =` assignment in `src/session/session.cpp` is INSIDE the body of `record_state_transition_()` (the only legitimate site). Search: `grep -nE '\bfsm_state_\s*=\s' src/session/session.cpp` — expected output = exactly one line (the assignment inside the helper). Note: the in-class default initializer at `include/fixpp/session/session.hpp:385` (`fsm_state fsm_state_ = fsm_state::NotConnected;`) is initialization, not an assignment-operator call, and the grep does NOT match it — this is correct behavior. If any other site appears in `src/session/session.cpp`, surface as a regression and add the missing helper-routing.
- [X] T027 Spec / catalogue / coverage-index bookkeeping per `[[feedback_pipeline_mark_done_step]]`:
...
- [X] T027b Gate A inheritance addendum (per FR-012 option ii + plan.md `[const §XVII.1]` row): synthesize `library/.specify/decisions/010-session-cfg-lifetime-gatea.md` (local-only, gitignored per `[const §XVII.8]`). Content: a short YAML-frontmatter + body record asserting `## Gate A` decision = `inherited-from-005-with-addendum`, citing (a) the 005 Gate A round-3 sign-off date 2026-05-18, (b) the /speckit-clarify 2026-05-23 ruling on FR-001 W-5 = Option A, (c) research D-1 / D-2 / D-3 plan-level micro-decisions, (d) the FR-001a `SessionConfig::store_factory` amendment from `unique_ptr<MessageStoreFactory>` to `shared_ptr<MessageStoreFactory>` discovered at /speckit-implement T006 (research D-1 first-draft "all members copyable" claim was incorrect; orchestrator-resolved 2026-05-23 via the user-approved A1 option; documented above), (e) the rationale text "All new and amended public symbols (`Session::fsm_visit_history()` + `error::session_invalid_state_for_send = 77` + the `SessionConfig::store_factory` ownership change from unique to shared) fall under `[const §XVII.1]` trivial-feature skip — observation-only accessor + next-slot error variant + an ownership-model refinement that preserves the 005 polymorphic-indirection intent (the `MessageStoreFactory` interface remains stateless with one virtual `make()` method; the per-Session MessageStore uniqueness invariant is preserved because each Session calls `make()` to mint its own store). The C-ABI surface is itself deferred per `[const §X.2]` + 2i, so the new error-variant slot has no live downstream C consumer; the store_factory ownership change is C++-side only and never appears on the C ABI surface." This file is the explicit Gate A evidence the `/gate-b` pre-flight check verifies (per `gate-b.md` Pre-flight step 4c).
- [X] T028 Feature-completeness audit per `[const §XVII.8]` + `[[project_005_phase8_completeness_false_pass]]` + `[[feedback_simplify_pass_catches_9th_burn]]` — audit **test BODIES** (not file names): for each FR-001 through FR-009, identify the test file + the specific assertions that BIND the runtime contract; verify no SUCCEED-placeholders, no test that passes for the wrong reason, no end-state assertion that matches via the wrong matrix row. Cross-reference SC-001 through SC-009 the same way. **Also record T024's bench gate disposition** (pass / fail / waiver rationale per `[const §VIII.2]`) and **T023's W-1..W-4 carry-forward outcome** (cleared / re-waived per the T023 branch logic). Output a `## Completeness` section in the /speckit-verify record OR a sibling `010-session-cfg-lifetime-completeness.md` per the verify-record's choice.
- [X] T029 Run `/speckit-verify 010-session-cfg-lifetime` per `[const §XVII.8]`. Expected verdict: GREEN (the W-5 + bundled-deferral closure is purely additive coverage + a surgical refactor — no regressions expected). If YELLOW: list waivers (likely auto-clear of W-1..W-4 OR re-waive with rationale per `[[feedback_codecov_patch_vs_lcov_da_brda_gate]]` + PR #73 precedent). RED is unexpected; surface and STOP.
...
- Phase 7: T022 + T023 + T024 + T025 are different commands and can run [P]. T026 + T027 + T027b + T028 sequential. T029 last.
...
**Risk tracking**: the matrix witness file (T017, **~700-900 LoC** post-/speckit-analyze re-estimate) is the largest single deliverable; expect to split T017 into per-state batches (one Sonnet invocation per FSM state's row — 6 batches total) to stay inside the Sonnet-invocation LoC cap per `[[feedback_phase_implementer_sonnet_runaway_scope]]`.
