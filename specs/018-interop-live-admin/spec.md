...
- **SC-005**: The release-prep matrix run including G1 completes deterministically (no hangs, no flakes across a configured repeat count) within a quantified budget: the G1 admin cells add ≈ (4 scenario groups × 2 roles = 8 base cells, each bounded by its self-deadline) to the 016 matrix; the dominant per-cell wall time is the US2 ~5s idle window and the US3 recovery reconnect cycles. The repeat count is fixed at **3** (matching the 016 ROADMAP flake-gate figure); the G1 increment to the release-prep matrix budget MUST stay within the 016 ROADMAP allotment (cite the ROADMAP figure in tasks). No cell may rely on `ioc.run()` to terminate (FR-010).
