...
static_assert(std::is_destructible_v<async_mutex>
              && !std::is_nothrow_destructible_v<async_mutex>,
...
static_assert(std::is_same_v<
    decltype(std::declval<async_mutex&>().async_lock()),
...
static_assert(std::is_same_v<
    decltype(std::declval<async_mutex&>().cancel_and_drain()),
    asio::awaitable<expected_t<void>>>,
    "cancel_and_drain() must return asio::awaitable<expected_t<void>>");
static_assert(std::is_same_v<
    decltype(std::declval<async_mutex&>().unlock()), void>,
    "unlock() must return void");
static_assert(std::is_same_v<
    decltype(std::declval<const async_mutex&>().policy()), completion_policy>,
...
static_assert(std::is_same_v<
    decltype(fixpp::session::async_lock_via_session_executor(
...
template <class Ns> concept declares_shared_variant = requires {
    typename Ns::async_shared_mutex;
};
template <class Ns> concept declares_recursive_variant = requires {
    typename Ns::async_recursive_mutex;
};
template <class Ns> concept declares_timed_variant = requires {
    typename Ns::async_timed_mutex;
};
...
static_assert(!declares_shared_variant<sync_surface_probe>,
    "FR-015: no async_shared_mutex");
static_assert(!declares_recursive_variant<sync_surface_probe>,
    "FR-015: no async_recursive_mutex");
static_assert(!declares_timed_variant<sync_surface_probe>,
    "FR-015: no async_timed_mutex");
