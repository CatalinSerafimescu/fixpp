...
TEST(AtomicSharedPtrLinearizability, SpotCheck) {
    ...
    EXPECT_LT(shortest_stagger_ns, 1'000'000)
        ...
        << " us for a window of at most 60 us — the requested window is being rounded up to "
           ...
           "test asked for (cross_thread_realtime_edges="
        << cross_thread_realtime_edges << ")";
            ...
            for (int i = 0; i < kIters; ++i) {
                ...
                if (op == 0) {
                    ...
                    (void)ptr.compare_exchange_strong(
                        expected, std::make_shared<int>(val_dist(rng)), std::memory_order_seq_cst,
                        std::memory_order_relaxed);
                }
            }
        });
...
TEST(AtomicSharedPtrManyInstancesIsolation, NoCrossInstanceCorruption) {
    ...
    for (int t = 0; t < kThreads; ++t) {
        workers.emplace_back([&, t]() {
            ...
            for (int i = 0; i < kIters && !invalid_observation.load(std::memory_order_relaxed);
                 ++i) {
                const int idx = idx_dist(rng);
                const int val = val_dist(rng);
                atoms[static_cast<std::size_t>(idx)]->store(std::make_shared<int>(val),
                                                            std::memory_order_relaxed);
...
TEST(AtomicSharedPtrRandomizedStress, BoundedMixedOpNoErrors) {
    const int kIters = read_env_int("FIXPP_ASP_STRESS_ITERS", 2000);
    const int kThreadCount = static_cast<int>(std::max(4U, std::thread::hardware_concurrency()));
        ...
        workers.emplace_back([&, t]() {
            ...
            for (int i = 0; i < kIters; ++i) {
                ...
                if (op == 0) {
                    ...
                    auto expected = ptr.load(std::memory_order_relaxed);
                    (void)ptr.compare_exchange_weak(expected, std::make_shared<int>(val_dist(rng)),
                                                    std::memory_order_acq_rel,
