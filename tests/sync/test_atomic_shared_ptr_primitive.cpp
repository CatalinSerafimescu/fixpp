...
concept AtomicSharedPtrLike = requires(AtomicLike a, const AtomicLike ca, std::shared_ptr<int> sp,
                                       std::shared_ptr<int>& expected, std::memory_order order) {
    AtomicLike{};
    AtomicLike{sp};
...
#if FIXPP_ATOMIC_SHARED_PTR_NATIVE_ACTIVE
    static_assert(
        std::is_same_v<fixpp::sync::atomic_shared_ptr<int>, std::atomic<std::shared_ptr<int>>>,
...
TEST(AtomicSharedPtrLockFreeReporting, LockFreeContract) {
    ...
    // implementations.
    RecordProperty("native_is_lock_free", ptr.is_lock_free() ? "true" : "false");
    SUCCEED() << "Native path: is_lock_free()=" << std::boolalpha << ptr.is_lock_free()
              << " (informational)";
#else
    AtomicIntPtr ptr;
    EXPECT_FALSE(ptr.is_lock_free()) << "Fallback must report is_lock_free()==false";
