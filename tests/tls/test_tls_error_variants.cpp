...
SslCtxConfig make_mtls_ca_cfg(
    std::shared_ptr<fixpp::core::Clock> clk = nullptr)
{
    if (!clk) {
        clk = std::make_shared<stub_clock>(
...
// within SAN/DER caps).
Certificate make_valid_ecdsa_cert(
...
TEST(TlsErrorVariants, InvalidSecurityProfile) {
    auto result = make_ssl_ctx_config(SecurityProfile::unset,
                                      std::make_shared<stub_cs>(),
                                      std::make_shared<stub_clock>(
                                          std::chrono::system_clock::now()),
                                      nullptr, nullptr);
...
TEST(TlsErrorVariants, PinEmptyAtOpen) {
    ...
    auto result = make_ssl_ctx_config(
        ...
        std::make_shared<stub_cs>(),
        std::make_shared<stub_clock>(std::chrono::system_clock::now()),
        *pinset_r, nullptr);
    ASSERT_FALSE(result.has_value());
...
TEST(TlsErrorVariants, PinMismatch) {
    ...
    auto cfg_r = make_ssl_ctx_config(
        SecurityProfile::mtls_pinned,
        std::make_shared<stub_cs>(),
        std::make_shared<stub_clock>(now),
        pinset, nullptr);
