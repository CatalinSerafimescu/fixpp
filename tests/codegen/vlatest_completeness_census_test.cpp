...
const std::set<std::string>& non_derivable_datatype_quirks() {
    static const std::set<std::string> s = {
        "TagNum", "XID", "XIDREF", "MultipleCharValue", "MultipleStringValue", "LocalMktTime",
    };
    return s;
}
        ...
        if (name == "fixr:fieldRef") {
            ...
            if (!inserted) {
                throw std::runtime_error(
                    "duplicate occurrence key (group counter) in own walk: msg=" + msg_type +
                    " path=" + join_path(stack) + " tag=" + std::to_string(counter_tag));
            }
...
Census build_census() {
    ...
    const std::string xml_path =
        std::string(FIXPP_DICT_DATA_DIR) + "/orchestra/OrchestraFIXLatest.xml";
...
TEST(VlatestCompletenessCensus, StructuralKeyExactSetEquality) {
    ...
    EXPECT_TRUE(only_expected.empty())
        << describe(only_expected, "in manifest but NOT in raw-XML walk");
    ...
    for (const auto& [msg, expected_count] : c.expected.occurrence_count) {
        EXPECT_EQ(actual_counts[msg], expected_count)
            << "occurrence-count mismatch for msg_type=" << msg;
    }
}
...
TEST(VlatestCompletenessCensus, DatatypeTokenBestEffortConsistency) {
    ...
    for (const auto& [key, actual_token] : c.actual.dtype) {
        ...
        if (actual_token != it->second) {
            std::ostringstream oss;
            oss << "msg=" << std::get<0>(key) << " path=" << std::get<1>(key)
                << " tag=" << std::get<2>(key) << " derived=" << actual_token
                << " manifest=" << it->second;
