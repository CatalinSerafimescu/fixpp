...
struct frame_view_access {
    static frame_view make(std::byte const* frame, std::size_t frame_len,
...
[[nodiscard]] inline core::expected_t<frame_view>
make_frame_view(std::span<const std::byte> buf,
                detail::generation_token gen = {}) noexcept {
    ...
    if (p9 == std::string_view::npos) {
        return core::expected_t<frame_view>{
            std::unexpect, core::error::wire_invalid_body_length};
    }
    if (s[p9] == SOH) {
    ...
    if (p10 == std::string_view::npos) {
        return core::expected_t<frame_view>{
            std::unexpect, core::error::wire_checksum_mismatch};
    }
    std::size_t checksum_digit = p10 + 1;  // the '1' of "10="
    std::size_t body_len = checksum_digit - body_off;

    return frame_view_access::make(buf.data(), buf.size(), body_off,
                                   body_len, gen);
}
