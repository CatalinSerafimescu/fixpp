...
/// open), so plain remove_all suffices. Windows does NOT: a live FileStore or FileSink
...
/// destroyed.
///
/// CONTRACT: callers MUST destroy/close every FileStore/FileSink over `p` before calling
/// this (scope the store/sink, or reset its owning pointer). This helper only
