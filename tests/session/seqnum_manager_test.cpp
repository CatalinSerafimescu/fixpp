...
//   4. Too-high inbound → session_seqnum_too_high (slot 120, 014 FR-016; replaces the
//      slot-74 stand-in; slot 70 session_seqnum_gap_unrecoverable deleted pre-v1.0 per
//      013 T006a). I-4: no ResendRequest.
...
// check_inbound() returns session_seqnum_too_high (120) for too-high (014 FR-016,
// replacing the former slot-74 session_test_request_unanswered stand-in).
...
TEST_F(SeqnumManagerTest, TooHighInboundIsSessionFatal) {
    ...
    EXPECT_EQ(r.error(), fixpp::core::error::session_seqnum_too_high)
        << "Expected session_seqnum_too_high (slot 120, 014 FR-016 / E-4); "
           "replaces slot-74 stand-in (session_test_request_unanswered was "
