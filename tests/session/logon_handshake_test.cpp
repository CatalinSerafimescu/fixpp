...
protected:
    ...
    fixpp::core::expected_t<void> open_sync(fixpp::session::Session& s) {
        ...
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, std::chrono::milliseconds{200},
                                                        "LogonHandshakeTest::open_sync")) {
            fixpp::test_support::cancel_and_drain_or_report(ioc, *clock,
                                                            "LogonHandshakeTest::open_sync");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss << "LogonHandshakeTest::open_sync";
    ...
    fixpp::core::expected_t<void> feed_sync(fixpp::session::Session& s,
        ...
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, std::chrono::milliseconds{200},
                                                        "LogonHandshakeTest::feed_sync")) {
            fixpp::test_support::cancel_and_drain_or_report(ioc, *clock,
                                                            "LogonHandshakeTest::feed_sync");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss << "LogonHandshakeTest::feed_sync";
            return std::unexpected(fixpp::test_support::kWindowMissSentinel);
        }
        return fut.get();
    }
