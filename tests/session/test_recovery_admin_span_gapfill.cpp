...
protected:
    ...
    fixpp::core::expected_t<void> run_open(fixpp::session::Session& s) {
        ...
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, 100ms,
                                                        "RecoveryAdminSpanGapfillTest::run_open")) {
            fixpp::test_support::cancel_and_drain_or_report(
                ioc, *clock, "RecoveryAdminSpanGapfillTest::run_open");
    ...
    fixpp::core::expected_t<void> feed(fixpp::session::Session& s,
        ...
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, 100ms,
                                                        "RecoveryAdminSpanGapfillTest::feed")) {
...
protected:
    ...
    fixpp::core::expected_t<void> run_open(fixpp::session::Session& s) {
        ...
        if (!fixpp::test_support::run_window_then_ready(
                ioc, fut, 100ms, "RecoveryGapfillNearCapacityTest::run_open")) {
            fixpp::test_support::cancel_and_drain_or_report(
                ioc, *clock, "RecoveryGapfillNearCapacityTest::run_open");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss
                          << "RecoveryGapfillNearCapacityTest::run_open";
            return std::unexpected(fixpp::test_support::kWindowMissSentinel);
        }
        return fut.get();
    }
    ...
    fixpp::core::expected_t<void> feed(fixpp::session::Session& s,
        ...
        auto fut = asio::co_spawn(ioc, s.on_inbound_frame(frame), asio::use_future);
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, 100ms,
                                                        "RecoveryGapfillNearCapacityTest::feed")) {
            fixpp::test_support::cancel_and_drain_or_report(
                ioc, *clock, "RecoveryGapfillNearCapacityTest::feed");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss
                          << "RecoveryGapfillNearCapacityTest::feed";
            return std::unexpected(fixpp::test_support::kWindowMissSentinel);
        }
