...
class suspending_collecting_visitor final
...
public:
    ...
    on_frame(seqnum_t seq, std::span<const std::byte> frame
             [[clang::lifetimebound]]) noexcept override
    {
    ...
    auto fut = asio::co_spawn(pool.get_executor(),
        [&store]() -> asio::awaitable<void> {
            ...
            for (int i = 0; i < kFrames; ++i) {
                ...
                auto expected = make_test_frame(static_cast<seqnum_t>(i + 1),
                                                direction_t::outbound);
...
class recursive_store_visitor final
    : public fixpp::session::retrieve_visitor
{
public:
    ...
    asio::awaitable<fixpp::core::expected_t<visit_result>>
    on_frame(seqnum_t seq, std::span<const std::byte> frame) noexcept override
    {
        ...
        auto r = co_await store_.store(
            next_seq,
            std::span<const std::byte>(next_frame),
            dir_);
    ...
    auto fut = asio::co_spawn(pool.get_executor(),
        [&store]() -> asio::awaitable<void> {
            ...
            auto r = co_await store.store(1,
                std::span<const std::byte>(frame1), direction_t::inbound);
            ...
            // Use a timeout via future to detect deadlock (if test hangs > 10s, it's deadlocked)
...
public:
    ...
    asio::awaitable<fixpp::core::expected_t<visit_result>>
    on_frame(seqnum_t seq, std::span<const std::byte>) noexcept override
    {
        ++frames_seen_;
