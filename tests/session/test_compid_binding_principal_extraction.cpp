...
protected:
    ...
    fixpp::core::expected_t<void> run_open(fixpp::session::Session& s) {
        ...
        if (!fixpp::test_support::run_window_then_ready(
                ioc, fut, 100ms, "CompidPrincipalExtractionSessionTest::run_open")) {
            fixpp::test_support::cancel_and_drain_or_report(
                ioc, *clock, "CompidPrincipalExtractionSessionTest::run_open");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss
                          << "CompidPrincipalExtractionSessionTest::run_open";
    ...
    fixpp::core::expected_t<void> feed(fixpp::session::Session& s,
        ...
        if (!fixpp::test_support::run_window_then_ready(
                ioc, fut, 100ms, "CompidPrincipalExtractionSessionTest::feed")) {
            fixpp::test_support::cancel_and_drain_or_report(
                ioc, *clock, "CompidPrincipalExtractionSessionTest::feed");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss
                          << "CompidPrincipalExtractionSessionTest::feed";
            return std::unexpected(fixpp::test_support::kWindowMissSentinel);
        }
        return fut.get();
    }
