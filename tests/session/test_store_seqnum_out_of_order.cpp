...
TEST(StoreSeqnumOutOfOrder, SkipAheadRejected) {
    ...
    auto fut = asio::co_spawn(
        ...
        [&store]() -> asio::awaitable<void> {
            ...
            auto r =
                co_await store.store(5, std::span<const std::byte>(frame5), direction_t::outbound);
            EXPECT_FALSE(r.has_value()) << "store(seq=5) when next==1 must fail";
...
TEST(StoreSeqnumOutOfOrder, ZeroSeqnumRejected) {
    ...
    auto fut = asio::co_spawn(
        ...
        [&store]() -> asio::awaitable<void> {
            ...
            auto r =
                co_await store.store(0, std::span<const std::byte>(frame0), direction_t::inbound);
...
TEST(StoreSeqnumOutOfOrder, MutexReleasedAfterRejection_NextStoreSucceeds) {
    ...
    auto fut = asio::co_spawn(
        ...
        [&store]() -> asio::awaitable<void> {
            ...
            EXPECT_TRUE(r2.has_value()) << "store(seq=1) after rejected out-of-order must succeed";
...
TEST(StoreSeqnumOutOfOrder, OutOfOrderInboundDoesNotAffectOutbound) {
    ...
    auto fut = asio::co_spawn(
        ...
        [&store]() -> asio::awaitable<void> {
            ...
            auto r1 = co_await store.store(1, std::span<const std::byte>(frame_o1),
            ...
            auto r2 =
                co_await store.store(5, std::span<const std::byte>(frame_i5), direction_t::inbound);
...
TEST(StoreSeqnumOutOfOrder, ConcurrentOutOfOrderAndValidStore) {
    ...
    auto fut_invalid = asio::co_spawn(
        pool.get_executor(),
        [store, frame5, &got_out_of_order]() mutable -> asio::awaitable<void> {
            auto r =
                co_await store->store(5, std::span<const std::byte>(frame5), direction_t::outbound);
            if (!r.has_value() && r.error() == fixpp::core::error::store_seqnum_out_of_order) {
...
TEST(StoreSeqnumOutOfOrder, NextSeqnumAtMaxReturnsOverflow) {
    ...
    run_on_pool(pool, [&store]() -> asio::awaitable<void> {
        ...
        EXPECT_EQ(*r2, seqnum_max) << "counter must remain at seqnum_max after overflow (no wrap)";
    });
}
