...
protected:
    ...
    fixpp::core::expected_t<void> open_sync(fixpp::session::Session& s) {
        ...
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, 200ms,
                                                        "SendInvalidStateTest::open_sync")) {
            fixpp::test_support::cancel_and_drain_or_report(ioc, *clock,
                                                            "SendInvalidStateTest::open_sync");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss << "SendInvalidStateTest::open_sync";
    ...
    fixpp::core::expected_t<void> feed_sync(fixpp::session::Session& s,
        ...
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, 200ms,
                                                        "SendInvalidStateTest::feed_sync")) {
            fixpp::test_support::cancel_and_drain_or_report(ioc, *clock,
                                                            "SendInvalidStateTest::feed_sync");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss << "SendInvalidStateTest::feed_sync";
            return std::unexpected(fixpp::test_support::kWindowMissSentinel);
        }
        return fut.get();
    }
...
TEST_F(SendInvalidStateTest, SendInDisconnected_ReturnsInvalidStateForSend) {
    ...
    // close(terminal) → Disconnected.
    {
        ...
        if (!fixpp::test_support::run_window_then_ready(
                ioc, close_fut, 200ms, "SendInDisconnected_ReturnsInvalidStateForSend")) {
            fixpp::test_support::cancel_and_drain_or_report(
                ioc, *clock, "SendInDisconnected_ReturnsInvalidStateForSend");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss
                          << "SendInDisconnected_ReturnsInvalidStateForSend";
            return;
        }
        (void)close_fut.get();
    }
