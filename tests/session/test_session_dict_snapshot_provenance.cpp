...
struct ProvenanceFixture {
    ...
    void feed(Session& sess, std::span<const std::byte> frame) {
        ...
        if (!fixpp::test_support::run_window_then_ready(ioc, fut, 200ms,
                                                        "ProvenanceFixture::feed")) {
            fixpp::test_support::cancel_and_drain_or_report(ioc, *clock, "ProvenanceFixture::feed");
            ADD_FAILURE() << fixpp::test_support::kWindowMiss << "ProvenanceFixture::feed";
            return;
        }
        (void)fut.get();
    }
