...
TEST_F(DurableBeforeTransmitTest, InboundStoreBeforeFromAdminDispatch) {
    ...
    for (char c : full) {
        logon_frame.push_back(static_cast<std::byte>(c));
    }
...
TEST_F(DurableBeforeTransmitTest, OutboundStoreBeforeTransportSend) {
    ...
    // LogonReceived (matrix), so we must reach Active before close().
    {
        ...
        for (char c : full) {
            logon_ack.push_back(static_cast<std::byte>(c));
        }
    ...
    ASSERT_FALSE(order.empty()) << "Expected at least one outbound frame (Logout) to be emitted";
    ...
    for (std::size_t i = 0; i < order.size(); ++i) {
        if (order[i] == "transport_send") {
            ...
            EXPECT_EQ(order[i - 1], "store_out")
                ...
                << " must be IMMEDIATELY preceded by store_out (got '" << order[i - 1] << "')";
            ++pair_count;
        }
    }
