...
TEST(FirstFrameTotalCancelTls, LegA_JoinedHelper_CancellationAttributable) {
    ...
    asio::co_spawn(
        ...
        [&]() -> asio::awaitable<void> {
            ...
            entered_read = true;
            result =
                co_await read_first_frame_bounded(*pair.server, buf, clock, kDeadline, kMaxBytes);
        },
...
TEST(FirstFrameTotalCancelTls, LegB_DirectRead_ExactCancelled) {
    ...
    EXPECT_FALSE(watchdog_fired)
        ...
        << "1000ms. See leg A's message for the mechanism (asio_tls_transport.cpp's "
           "async_read_some reset_cancellation_state call).";
