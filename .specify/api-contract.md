...
> **Status:** Draft v0.1 — Phase 2, awaiting Codex Gate A review and user sign-off.
...
- C++ public API of `core`, `wire` accessors, `dictionary`, `session`, `transport` interface, `tls`, `log` interface, `tap` interface, `service` plugin interfaces.
- The C ABI (`<fix/c_api.h>` + split sub-headers).
...
- The in-process logger's choice of implementation (`quill` vs in-house) per `[2k]` / `[arch §11]` — pending TS-13 benchmark spike.
...
│       ├── engine.h         # 2i — fixpp_engine_t lifecycle
│       ├── session.h        # 2i — fixpp_session_t lifecycle
│       ├── dict.h           # 2i — fixpp_dict_t accessors
│       ├── store.h          # 2i / 2e — fixpp_store_t opaque type
│       ├── log.h            # 2k — log emit / sink registration
│       └── otel.h           # 2k — provider plumbing
└── fixpp/
...
Per `[const §X.3]` / `[2a]`: `fixpp_decimal_t` is a 16-byte / 8-byte-aligned PoD `(int64 mantissa, int8 exponent)` with `INT64_MIN` mantissa as the invalid sentinel and 7 trailing `_reserved` bytes ignored on read in v1.0 (forward-compat slot under future `FIXPP_C_ABI_DECIMAL_RESERVED_USED`). Helpers: `FIXPP_DECIMAL_INITIALIZER`, `FIXPP_DECIMAL_INVALID`, `fixpp_decimal_init()`, `fixpp_decimal_equal()`. The build-time engine-wide default type is selected via the `FIXPP_DECIMAL_T` macro, and a mismatch is **link-fatal** via `decimal_alias_sentinel<T>::tag` per `[2a]`.

### 7.4 `fixpp_error_t` numeric block layout

Per `[2i §1.1]` / `[2i §4.3]`. The numeric value of any **published** variant is frozen for life of `FIXPP_C_ABI_VERSION_MAJOR == 1`; new variants append at unused slots; an audit trail at `tools/abi_history/error_codes_v1.txt` is checked-in append-only and verified by `tools/check_capi_occupancy.sh` + `abidiff` per `[const §IX.5]`.

| Block | Range | Owner | Occupancy at v1.0 |
|---|---|---|---|
| Architectural sentinels + 2i-introduced | `[0, 99]` | 2i | 11 occupied (3 architectural: SUCCESS, UNKNOWN, VERSION_MISMATCH; 8 introduced: INVALID_HANDLE, TYPE_MISMATCH, TAG_NOT_FOUND, INDEX_OUT_OF_RANGE, BUFFER_TOO_SMALL, CAPI_CONFIG_INVALID, NULL_HANDLE, …) |
| `FIXPP_ERR_WIRE_*` | `[100, 199]` | 2b | 20 |
| `FIXPP_ERR_DICT_*` | `[200, 299]` | 2c | 13 |
| `FIXPP_ERR_THREAD_*` | `[300, 399]` | 2d | 9 |
| `FIXPP_ERR_STORE_*` | `[400, 499]` | 2e | 10 |
| `FIXPP_ERR_SYNC_*` | `[500, 599]` | 2f | 4 |
| `FIXPP_ERR_TLS_*` | `[600, 699]` | 2g | 15 |
| `FIXPP_ERR_TRANSPORT_*` | `[700, 799]` | 2h | 22 |
| `FIXPP_ERR_DECIMAL_*` | `[800, 899]` | 2a | 3 |
| `FIXPP_ERR_CTRL_*` | `[900, 999]` | 2j | 2 (`CTRL_CONFIG = 900`, `CTRL_RUNTIME = 901`); remainder reserved for 2j growth |
| `FIXPP_ERR_LOG_*` | `[1000, 1099]` | 2k | 7 |
| (reserved) | `[1100, 1199]` | reserved | 0 |
| `FIXPP_ERR_BINDING_*` | `[1200, 1299]` | 2m | 5 (`SUBINTERPRETER = 1201`, `CALLBACK_REENTRANT_CLOSE = 1204`, …) |
| (reserved) | `[1300, 1399]` | reserved (post-v1.x growth) | 0 |
| (reserved) | `[1400+]` | reserved | 0 |

**Cancellation translation rule** per `[const §XI.2]` / `[2i §4.9]`: every `*_aborted` / `*_cancelled` from sibling docs maps to `FIXPP_ERR_CANCELLED = 1` at the C ABI.

**Forward-compat** per `[arch §5.3]`: TO-consumer out-of-range mapped to `FIXPP_ERR_UNKNOWN`; FROM-consumer opaque pass-through tolerance (not actively rejected).

### 7.5 Exception trap split

Per `[2i §4]`: construction-vs-steady-state split — `guarded_call_construction` whitelists exactly three entry points (`fixpp_engine_create`, `fixpp_session_create`, `fixpp_dict_create`) where a C++ exception is trapped and translated to a sentinel error; `guarded_call_steady` is `std::abort` per `[const §VIII.5]` invariant-violation rule.
...
|---|---|---|---|---|---|
| 2a | Decimal type + `decimal_traits<T>` | `fixpp::core::decimal<T>`, `pod_decimal`, `decimal_traits<T>`; C ABI `fixpp_decimal_t`, `FIXPP_DECIMAL_T`, `FIXPP_DECIMAL_INITIALIZER`, `FIXPP_DECIMAL_INVALID`, `fixpp_decimal_init`, `fixpp_decimal_equal` | `[800, 899]` (3 occupied) | W-009 | [`decisions/2a-decimal.md`](../../decisions/2a-decimal.md) |
| 2b | Wire parser + offset-table | `fixpp::wire::Parser`, `OffsetTable`, `Writer`, `Validator`, `Framer`; lifetime contract via `[[clang::lifetimebound]]` | `[100, 199]` (20 occupied) | W-001..W-014, OSS-006/008/013, parse/serialise/validate behaviour for A-001..A-034, M-001..M-012, P-001..P-008, C-001..C-003, R-001..R-005, N-001..N-003 | [`decisions/2b-wire.md`](../../decisions/2b-wire.md) |
| 2c | Dictionary codegen + dialect overlay | `fixpp::dict::Dictionary`, `XmlLoader`, `DialectOverlay`, `ComponentRef`, `FieldRef`, `GroupRef`; generated `fixpp::v42::*` / `v44::*` / `v50sp2::*` / `vt11::*` | `[200, 299]` (13 occupied) | D-001..D-011, OSS-001/010; typed-message classes for A-001..A-013 + M/P/C/R/N (codegen scope) | [`decisions/2c-codegen.md`](../../decisions/2c-codegen.md) |
| 2d | Application threading + `Clock` | `fixpp::core::Clock` (+ `system_clock_source`, mock); `fixpp::core::session_executor`; `fixpp::current_trace_context`; default per-session strand on user-supplied `asio::any_io_executor` | `[300, 399]` (9 occupied) | NFR-015 (NEW); cross-cuts S-035, S-003/S-004, LOG-001..004, OBS-001..003 | [`decisions/2d-threading.md`](../../decisions/2d-threading.md) |
| 2e | MessageStore async API + QuickFIX-shim | `fixpp::session::MessageStore` (4 PV awaitable surface); `MemoryStore`, `FileStore`; `MessageStoreFactory` | `[400, 499]` (10 occupied) | S-011/012/013/014, OSS-002, COM-009 | [`decisions/2e-msgstore.md`](../../decisions/2e-msgstore.md) |
| 2f | Awaitable mutex `async_mutex` | `fixpp::sync::async_mutex` (≤56 B padded; `cancel_and_drain()`); `async_lock(mr)` PMR overload; session helper `async_lock_via_session_executor` | `[500, 599]` (4 occupied) | NFR-016 (NEW) | [`decisions/2f-async-mutex.md`](../../decisions/2f-async-mutex.md) |
| 2g | TLS `cert_source` + Pinset rotation | `fixpp::tls::cert_source` (2 PV); `credentials` value-type with optional HSM `sign_callback`; `file_cert_source`; `Pinset` add-then-remove with `pin_snapshot`; `consteval CipherPolicy` + `is_allowed()` runtime accessor; `SecurityProfile` enum | `[600, 699]` (15 occupied) | T-006/007/008/011/013; T-039/040 split with 2h | [`decisions/2g-tls.md`](../../decisions/2g-tls.md) |
| 2h | Transport interface + ASIO TCP/TLS | `fixpp::transport::Transport` (5 PV at cap); `TlsTransport` sub-interface (`async_handshake`); `Listener`; `TransportFactory`; `Endpoint`; `ReconnectPolicy::defaults()`; value-typed `handshake_result`; `asio_tls_transport` default | `[700, 799]` (22 occupied) | T-001..T-005, T-009/010/012; T-039/040 split with 2g | [`decisions/2h-transport.md`](../../decisions/2h-transport.md) |
| 2i | C ABI message rep + `fixpp_error_t` | Opaque handles (`fixpp_engine_t`, `fixpp_session_t`, `fixpp_msg_t`, `fixpp_dict_t`, `fixpp_store_t`); `fixpp_msg_get_*` / `_set_*`; `fixpp_msg_clone`; `fixpp_strerror`; `fixpp_version`; `fixpp_error_t` numeric layout (§7.4); split `<fix/c_api/*.h>` headers | (owner of the layout itself) | CA-001..CA-010 | [`decisions/2i-capi.md`](../../decisions/2i-capi.md) |
| 2j | Control-plane interface + gRPC default | `fixpp::service::ControlPlane` (3 PV + 2 reserved); `ControlPlaneConfig`; `ControlPlaneFactory`; `grpc_control_plane` over UDS / named pipe; proto schema `service/proto/fixpp_control.proto` (7 RPCs) | `[900, 999]` (2 occupied: `CTRL_CONFIG = 900`, `CTRL_RUNTIME = 901`) | SVC-001 (sole), SVC-004 (sole), SVC-005 (NEW pluggable interface) | [`decisions/2j-controlplane.md`](../../decisions/2j-controlplane.md) |
| 2k | Async logger + OTel observability | `fixpp::log::Logger` (zero-alloc producer; bounded MPSC; drain thread); `Sink` (4 PV); `Record`; `Level`; `LogConfig`; default sinks `FileSink`, `OtlpLogSink`, `SyslogSink`; `fixpp::otel::TracerProvider` / `MeterProvider`; `SessionSpans`; macros `FIXPP_SLOG`, `FIXPP_ELOG`, `FIXPP_LOG0`; `Session::get_trace_context() const noexcept [[nodiscard, clang::lifetimebound]]` | `[1000, 1099]` (7 occupied) | LOG-001..004 (sole), OBS-001..003 (sole) | [`decisions/2k-log-otel.md`](../../decisions/2k-log-otel.md) |
| 2l | Session-tap consumer | `fixpp::tap::TapConsumer = std::variant<NoTap, RingBufferTap, Iox2Tap, SyncCallbackTap>`; `TapConfig`; `TapRecord`; `TapShmRecord` (iceoryx2 cross-process shape — Provisional per §3.2); iceoryx2 service-name format `fixpp/<beginstring>/<sender>/<target>[/<qualifier>]` | (no own block) | SVC-002, SVC-003 (cross-cut); S-036, NFR-014, COM-008 | [`decisions/2l-tap.md`](../../decisions/2l-tap.md) |
| 2m | SWIG/Python binding shape | `fixpp` Python module (CPython 3.10–3.13, single-interpreter); `fixpp.FixppError` block-mapped exception hierarchy; opaque-handle Python wrappers reject `__reduce_ex__`; manylinux 2_28 wheel via `cibuildwheel` + `auditwheel repair` | `[1200, 1299]` (5 occupied) | PY-001..PY-005 | [`decisions/2m-pybind.md`](../../decisions/2m-pybind.md) |
...
- Adding a new sub-header under `<fixpp/<module>/...>` for a new module that respects the layering rules in `[arch §2]`.
...
**End of v0.1 draft. Awaiting Codex Gate A review and user sign-off.**
