...
**Tension:** `[2d §4.5]` v0.4 publishes `SessionConfig` with `cert_source` and `security_profile` but no `pinset` field. v0.1 of this doc relied on the field implicitly without queueing the amendment; RC#3 / N-P1-1 close requires the explicit field declaration.
