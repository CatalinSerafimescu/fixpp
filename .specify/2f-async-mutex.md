...
> - `[2d §4.7]` Cancellation propagation API — two-phase close + per-mode effect table; the `async_mutex::lock` row is 2f's inherited contract. **Appendix D §D.2 rewrites that row to surface `expected_t::unexpected{sync_lock_aborted}` at the 2f boundary, mapped to `FIXPP_ERR_CANCELLED` at the C ABI** (RC#4 fix; consistent with `[2d §6.5]`'s `cancellable_dispatch → dispatch_aborted` precedent).
...
The `async_mutex::lock` row is 2f's inherited contract; **Appendix D §D.2** rewrites the row to surface `expected_t::unexpected{sync_lock_aborted}` at the 2f boundary (mapped to `FIXPP_ERR_CANCELLED` at the C ABI), consistent with `[2d §6.5]`'s `cancellable_dispatch → dispatch_aborted` precedent (RC#4 fix).
...
> `await_suspend(std::coroutine_handle<>)`), while §3/§6.1 (the `[arch §5.1]` Implication-for-2f paragraph) require
...
> source-confirmed by Opus (`include/fixpp/core/sync/async_mutex.hpp`'s `async_mutex_awaiter` node type;
> its `on_cancel` method defers; its drain walker derefs the frame).
>
> **Resolution (binding for `/implement`; the design doc remains authoritative as
...
> **── Erratum E-5 (2026-06-22, feature `048-async-mutex-strand-reap`; Gate-A-converged 3 rounds; supersedes the unmerged 047 converging-loop / PR #143) ──**
>
> **Decision (strand-local reap).** `cancel_and_drain()`'s advertised contract is **narrowed to the strand-serialized (single-thread-per-mutex) topology** — the only topology its production consumers use (the 2 drain consumers, `Session::write_gate_` and `SeqnumManager::mutex_`, run on the per-session strand; cancellation emission is itself marshaled onto that strand, `src/session/engine.cpp`'s cancellation-emission dispatch). Ordinary cross-thread `async_lock`/`unlock` contention (the §1.1 cross-domain seam) stays **supported**; ONLY drain-overlap with another thread's acquire/cancel/unlock is narrowed (UNDEFINED — INV-2). The cross-thread "convergence" machinery that §4.7.2/§4.7.3 (drain-latch ownership, the §4.7.3 invariants I-1..I-8) and the §1.1 / §4.1 member budget describe is **REMOVED**: `drain_latch_ptr_` (the `std::atomic<std::shared_ptr<detail::drain_latch_state>>`) + the entire nested `detail::drain_latch_state` type (`concurrent_channel`, `signal_release`/`signal_abort`/`notify`/`async_wait`, `released_`/`aborted_`/`in_flight_resumptions_`), the `active_acquirers_count_` in-flight epoch, the `draining_`↔counter Dekker handshake, and the reaper's own cancellation slot + abort path are all deleted. The §3.1 `drain_latch_state` row, the §4.5 cancellation-result table's drain-abort row, the §4.7.3 I-1..I-8 invariants, and the §1.1/§4.1 member enumeration (`active_acquirers_count_` + `drain_latch_ptr_`) are **factually superseded** by this erratum.
>
> **New `cancel_and_drain()` (binding).** A synchronous strand-local single-pass reap inside a quiescence loop: disable own cancellation (teardown runs to completion — the drain is now UNINTERRUPTIBLE, no `sync_lock_aborted` drain return); set `draining_`; a reentrant call on the strand AWAITS `draining_complete_` and returns the terminal result (never eager-ok); then `for(;;){` reap both lists (CAS each parked waiter queued→cancelled, `result_=unexpected{sync_lock_aborted}`, `schedule_record_resume` which is the SOLE incrementer of the new strand-local `in_flight_resumers_`); break iff (`active_holders_count_==0 ∧ in_flight_resumers_==0 ∧ both lists empty in one pass`); else `co_await asio::post(executor)` `}`; finalize CAS `state_ locked_no_waiters→not_locked` THEN `draining_complete_.store(release)`. Two NEW plain strand-local mutex members replace the latch: `std::atomic<std::uint32_t> in_flight_resumers_` (the barrier that keeps the mutex alive until every posted resumer has dereferenced `record->mutex_` — prevents the immediate-destroy UAF) and `std::atomic<bool> draining_complete_` (reentrant-completion flag, replaces the channel subscriber protocol). `active_holders_count_` is retained (relaxed). RETAINED unchanged: the per-waiter `phase_` `{queued,granted,cancelled}` CAS arbitration, `on_cancel`, E-3 always-post resumption, `state_`/`next_drain_head_` encoding, `unlock()`'s walk.
>
> **DRAIN PRECONDITION.** Prompt termination rests on a documented precondition — no current holder is blocked on an unbounded operation (holders release promptly), satisfied by the 2 callers (write_gate: socket closed before the drain so the in-flight write completes; seqnum: never held across real async I/O) — NOT an O(holder-count) bound.
>
> **OOM (FR-003).** `inherited_slot.assign` is wrapped to fail closed (`sync_lock_alloc_failed`, the EXISTING variant — no new error/ABI) via the posted runner; `reaper_slot.assign` is eliminated with the reaper park. The resume `asio::post` (pre-existing) and the drain holder-yield `asio::post` (replaces the removed `async_wait`) remain pre-existing OOM-terminate sites — a non-allocating-completion redesign was evaluated and DROPPED as unproven (no asio preflight seam; cf. E-4); deferred as **L-048-1**.
>
> **ABI / layout.** No public or C-ABI surface change (`sync_lock_alloc_failed`/`sync_lock_drained`/`sync_lock_aborted` all pre-exist; the C-ABI mapping is unchanged). BUT `sizeof(async_mutex)` CHANGES (drops `drain_latch_ptr_`+`drain_latch_state`+`active_acquirers_count_`, adds `in_flight_resumers_`+`draining_complete_`) — a C++ object-layout change ⇒ header-recompile of the by-value embedders (`seqnum_manager`/`memory_store`/`file_store`); a compile-time `sizeof`/`alignof` layout golden is added. Removing `drain_latch_ptr_` drops the libc++ `atomic<shared_ptr>` portability-fallback consumer set 4→3 (046/PR #142 rebases on this).
>
> **Why (rationale).** The 047 converging-loop (the prior attempt to make the cross-thread machinery correct) left a residual multi-threaded lost-wake (W-B1: 3/25 standalone). The machinery is dead production capability (the drain is strand-confined) AND broken; removing it eliminates the orphan by construction and the maintenance burden. Witnessed GREEN across debug + ASan + TSan-libstdc++ (`sync_drain_strand_local_reap`/`_immediate_destroy`/`_reentrant_during_active`/`_onstrand_cancel`/`_predrain_holder`/`_awaitable_cancellation` + the `layout_golden`).
>
> **Scope / Gate note.** E-5 amends Gate-A-converged 2f §1.1 / §3.1 / §4.1 / §4.5 / §4.7.2 / §4.7.3 / §4.7.4 / NFR-016. Feature `048-async-mutex-strand-reap`; Gate A converged 3 rounds / 2 rewrites (Codex gpt-5.5 + Opus adversarial); records `specs/048-async-mutex-strand-reap/` + B-048-1/2 + L-048-1/2.
...
- Add **NFR-016** to `library/spec/feature-catalogue.md` (one row, mirroring the NFR-015 row format from `feature-catalogue.md`'s NFR-015 row):
