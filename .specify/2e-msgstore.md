...
| `MessageStoreFactory` ownership shape | C++ pointer type | `[arch §5.6]` (frozen at session open) + `[2d §4.5]` (held by `SessionConfig`). 2e returns `std::unique_ptr<MessageStore>` from `make()`; the factory itself is `std::unique_ptr<MessageStoreFactory>` on `SessionConfig`. **`[2d §4.5]` v0.4 holds it as `std::shared_ptr<MessageStoreFactory>` — round-2 root cause #2 close: orchestrator applies Appendix D §D.1 drop-in at 2e sign-off (per 2d v0.4 / 2c v1.3 sibling-doc-edit precedent).** | §4.4, Appendix D §D.1. |
...
public:
    ...
    // per-mode effect table lands the new row at 2e
...
public:
    ...
    make(std::string_view sender_comp_id,
         ...
         asio::any_io_executor file_io_executor) noexcept = 0;  // engine-resolved EngineConfig::file_io_executor per §4.3.2
};
...
- `library/spec/coverage-index.md`'s row (`§4.8 | Message recovery | Y | S-014 | —`) is amended to **`S-011, S-012, S-013, S-014`** (Codex P2-10 / per Opus confirm) with a note that 2e discharges the **store-side** API + default impls and Phase-4 owns the FSM.
- `[arch §11]` Q3 disposition is updated from `Phase 2 validates [SYN §3.2 Q7]` to the live FR-039 wording in `architecture.md`'s Q3 disposition row (Path B verdict, no runtime adapter; documented incompatibility + migration recipe + `quickfix_compat::cfg_loader` config-translation surface; disposition applied by `008-message-store` Phase-4 Gate A convergence). v0.2's Path A subset wrapper retired in round 2 per Codex C-R2-P2-1 escalation; v0.3 verdict is Path B only.
...
| N2-P3-1 (Opus new — Appendix D §D.1 drop-in not byte-exact) | P3 (NEW) | Editorial line-edit | v0.2 Appendix D §D.1 used a post-edit-form drop-in that didn't exactly match 2d v0.4 §4.5's `store_factory` field alignment. v0.3 Appendix D §D.1 rewrites the drop-in as a `diff -u`-style "before / after" form per the 2d v0.4 Appendix D §D.1 / 2c v1.3 Appendix D §3 precedent — quotes the actual 2d v0.4 lines verbatim, then shows the post-edit form, so the orchestrator's apply step is mechanical. | Appendix D §D.1 |
...
| Codex P2-10 (coverage handoff misses S-011/S-012/S-013) | P2 | Coverage-index amendment captured in §11 hand-off: the `§4.8 | Message recovery` row changes from `S-014` to `S-011, S-012, S-013, S-014`. | §11 hand-off |
...
**Tension:** `[2d §4.5]` v0.4 declares `std::shared_ptr<MessageStoreFactory> store_factory;` as a `SessionConfig` plugin-override field. Per N1 (round 1), `[arch §5.6]`'s mid-session-swap ban implies unique ownership of the factory (no shared-store-across-sessions, no second `shared_ptr` alive past `~Session`). 2e §4.4 ships `make()` returning `std::unique_ptr<MessageStore>` regardless; the field-type edit is the sibling-doc amendment owed by 2d at 2e sign-off.

**Before** (current `2d-threading.md` v0.4 text, `[2d §4.5]`'s `store_factory` field):
...
The diff is a single-token swap on the `store_factory` field (`std::shared_ptr` → `std::unique_ptr`) plus a one-line trailing comment on the same line. Column alignment matches `[2d §4.5]` v0.4's existing two-space gutter; the orchestrator's apply step is mechanical.
...
**One-paragraph contract on the hook's cancellation/error semantics (drop-in addition — append immediately after the per-mode effect table, before the existing Notes section that follows it):**
...
**Before** (current `2e-msgstore.md` v0.4 text, `[2e §4.4]` within the `MessageStoreFactory` block — the existing comment block above the method is not modified):
...
…and the matching default-factory declarations for `MemoryStoreFactory` and `FileStoreFactory`:
...
**Effective:** as of `008-message-store` Phase-4 Gate A convergence (2026-05-20). **Pre-applied at this rewrite** — the live `[2e §4.4]` block has been replaced in-place with the 5-param "After" form above, shipping inside the `008-message-store` PR bundle (parallel to the FR-037/038/039 hybrid-ownership pattern recorded in `008-message-store` spec.md Clarifications Q3 + research.md D-8). This contrasts with D.1/D.2 which were pre-applied at 2e v0.4 sign-off and shipped through `007-threading-clock`'s merge. This Appendix D §D.3 entry is retained as the byte-exact diff record of the amendment.
...
**Before** (current `2e-msgstore.md` v0.4 text, no validation contract — `[2e §4.3]` `FileStore::Config` block and `[2e §6.3.1]` filename composition carry no MUST-reject clause on CompID values; `[2e §4.4]` `FileStoreFactory::make()` (post-§D.3) carries no validation hook before file open / lock take):
...
**Before** (current `2e-msgstore.md` v0.4 text — `[2e §6.3.5]` platform-portability table carries `flock` / `LockFileEx` as the advisory-lock primitive with no filesystem-type qualifier; `[2e §10 Q4]` asserts the advisory lock closes the two-engine race unconditionally; FR-013 / I-16 / `contracts/file_store_factory.hpp`'s `make()` docstring repeat the unconditional shape):
...
- `specs/008-message-store/contracts/file_store_factory.hpp` (make() docstring) — cross-references §D.5.
...
**Before** (current `2e-msgstore.md` v0.4 text, no deleter contract — `[2e §4.4]` declares `make()` returning `expected_t<std::unique_ptr<MessageStore>>` with no statement on object-allocation discipline; `[2e §6.1.1]` + `[2e §8]` + FR-026/FR-027 talk only about internal-storage allocation):
