...
An **acquire** load of `read_sequence_` in step 2 is REQUIRED (#402). ⚠️ **Corrected #402 — was `relaxed`.** The old rationale (a stale, under-advanced read only makes the ring look fuller ⇒ at worst an early `drop_newest` drop) is about LIVENESS and was the wrong axis: it says nothing about the ordering that REUSING a slot requires. The acquire pairs with the drain's release store so that a producer claiming `i + capacity` cannot overwrite a slot the drain is still copying (a TORN record). See the MPSC ring protocol header in `src/log/logger.cpp`. ⚠️ This paragraph previously read: "A relaxed load of `read_sequence_` in step 2 is safe: a stale (under-advanced) read makes the ring look fuller than it is, at worst causing an early drop — correct under `drop_newest`." Do not restore it. On `block`: the producer spins `std::this_thread::yield()` until the step-3 check clears and the CAS succeeds; see caveat in `overflow_policy::block` doc-comment above.

**P1-2 fix (v0.3):** `read_sequence_` is `std::atomic<std::uint64_t>`, not plain `uint64_t`. Multiple producer threads read `read_sequence_` in the fullness check; a plain `uint64_t` read across threads is a data race under the C++17 memory model (TSan fires). ⚠️ **#402: the producer load must be `memory_order_acquire`, not `relaxed`.** This paragraph said relaxed "is sufficient — stale reads cause early drops, not corruption". That is a LIVENESS argument and it was the wrong axis: the hazard is not a stale fullness read, it is SLOT REUSE. Once the drain has copied slot `i` and release-stored `read_sequence_`, a producer may claim `i + capacity` and overwrite the same slot; without the acquire nothing orders the drain's reads of the old generation against those writes, and the drain can copy a **torn** record. Corruption, not an early drop.
...
**TS-2 — MPSC queue overflow (drop_newest).** Construct a `Logger` with `capacity=1`, `on_overflow=drop_newest`. Emit 100 records from one thread (drain thread sleeping). Assert: `Logger::drop_count() == 99`, exactly 1 record in the ring (the FIRST-enqueued record, since drop_newest preserves the oldest in-flight records), drain thread processes exactly 1 record. Run under TSan to verify no data race on `read_sequence_` (which is `std::atomic<uint64_t>` — producers load with `memory_order_acquire` (**#402; was `relaxed`**), drain thread stores with `memory_order_release`). ⚠️ **TS-2 as specified cannot detect the #402 race**: `capacity=1` with the drain asleep fills the ring but never RECYCLES a slot, and slot reuse on wraparound is the only thing the acquire protects. A witness needs sustained wraparound against a LIVE drain — see `tests/log/test_file_sink_backpressure.cpp`.
...
|---|---|---|
| **LOG-001** | Async logging core: `Logger` with lock-free MPSC ring (256-byte `Record` slots, Vyukov MPSC sequence counter; `read_sequence_` is `std::atomic<uint64_t>` with **acquire** producer loads / release drain store (#402; was relaxed); cache-line padded with `alignas(64)`), dedicated drain thread, `FIXPP_SLOG`/`FIXPP_ELOG`/`FIXPP_LOG0` macros, `drop_newest`/`block` overflow, separate `drop_count()` and `timeout_drop_count()` counters. | §4.1, §4.2, §4.3, §6.1, §6.2 |
...
> ⚠️ **THE ROW ABOVE IS THE ORIGIN OF #402, AND IT IS LEFT AS WRITTEN BECAUSE THAT IS THE POINT.** A real contradiction was found between step 2 (`acquire`) and the rationale (`relaxed is safe`) — and it was resolved by changing the STEP to match the PROSE. That was the wrong side: the step was right and the rationale was the outlier, because the rationale reasoned about fullness/liveness while the step was carrying the slot-reuse edge. The defect shipped and stayed latent for the reason recorded at TS-2 above — no test recycled a ring slot against a live drain. **Reconciling a contradiction by picking the cheaper side is how a correct memory order gets weakened; establish which side the argument actually supports first.**
