...
Sync Impact Report — v0.2 → v0.3 (2026-06-17)
  ...
  Modified principles:
    - Article XII §5 (Session requires an explicit SecurityProfile) — reopened the closed SecurityProfile set (was {mtls_ca, mtls_pinned, one_way_ca}; design-doc amendment required to reopen per §XVI.3) and appended a fourth profile kind `insecure_plain_tcp` (NO TLS, plain TCP). Encoded: opt-in-only / never-implicit-default (the `unset`-rejected rule is unchanged); the `[[deprecated]]` construction-site diagnostic §5 prescribes for `one_way_ca`, applied at the session-layer enumerator the operator selects; §1–§4 (TLS-mechanism rules) are inapplicable/vacuously satisfied on this profile because no TLS context is built; §7 (app-layer EncryptMethod(98) ban) is UNCHANGED and still applies. Added a sentence clarifying §1–§4 are conditioned on a TLS profile being selected and do not assert TLS-is-always-present.
