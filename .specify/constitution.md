<!--
Sync Impact Report — v1.0 → v1.1 (2026-09-15) — PENDING (ratified on Gate A convergence + user sign-off)
  Bump: MINOR, per Article XX §4. No banned pattern is added and no perf budget is tightened. The
    amendment narrows WHEN a C-ABI compatibility promise applies, and no consumer relies on one yet:
    as of this amendment fixpp has no GitHub Release and publishes to no package index.
  Modified: Article X — adds §7. Before the first public release a breaking C-ABI change bumps MINOR,
    is declared BREAKING, and updates every in-repository consumer in the same PR. The compatibility
    promise starts at the first public release, where the C-ABI version resets to 1.0.0 with the
    introducing_minor rebase; from then on a breaking change requires a MAJOR bump.
  ...
  Consistency edits in this PR: include/fix/c_api/version.h freeze comment (+ tools/capi_freeze.sha256),
    tools/check_capi_freeze.sh header, brain/components/c-api.md, CLAUDE.md.
  Modified: Article XX §3 — an amendment is recorded in its Sync Impact Report (and in CHANGELOG.md
    when §4 applies), replacing `_log.md`. No decision log of that name ever existed: the only file
    called _log.md (research/_log.md) is the Phase 1 research log, and no amendment since v0.1 wrote
    to one. Every amendment has been recorded here instead, so this corrects a dead pointer and drops
    no obligation anyone met. First raised as 082 Gate B finding F-7 (P3,
    research/reviews/opus_pr261_1_triage.md).
...
7. **Before the first public release, a breaking C-ABI change is allowed but must be declared.** The *first public release* is the first GitHub Release of this repository (which triggers tier1's wheel `gh release upload`), or any other publication of fixpp outside this repository and its CI artifacts. Until then the C ABI has no external consumers and its version is not a compatibility promise; the 0→1 freeze (`FIXPP_C_ABI_VERSION_MAJOR == 1`) fixes the surface's shape and review discipline only. A **breaking change** is a removed or renamed symbol, a changed signature, or a call sequence on an existing symbol whose result changes (it newly fails, newly succeeds, or produces different output). Before the first public release, a breaking change:
   - bumps `FIXPP_C_ABI_VERSION_MINOR`, not MAJOR, so the error-code downgrade frame of §4 stays continuous;
   - is marked **BREAKING** in the symbol's doc comment, in the PR description, and in the behaviors-and-limitations delta;
...
2. **Amendment process:**
   ...
   - User signs off.
3. **Every amendment is recorded in a Sync Impact Report** at the top of this document: its date, the version bump and its §4 classification, the clauses changed, the rationale, the review and sign-off that ratified it, and the PR or feature it rode. A backwards-incompatible amendment is also entered in `CHANGELOG.md` (§4). Feature-level decisions are recorded in `.specify/decisions/` and in the feature's own spec bundle, not here.
