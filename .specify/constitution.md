<!--
Sync Impact Report — v2.0 → v2.1 (2026-09-23) — PROPOSED (Codex Gate A, then user sign-off, pending)
  Bump: MINOR, per Article XX §4 — not backwards-incompatible. No banned pattern is added and no perf
    budget tightens. Applying v1.0's reading (a change that makes a previously mergeable PR fail is a
    tightening): §XVI.6 already assigned implementation to a sub-agent ("Sonnet executes; Opus reviews
    increments"), so a PR the orchestrator implemented did not conform under v2.0 either, and a PR that
    a Sonnet sub-agent implemented still conforms under v2.1. The amendment removes the model name from
    the constitution and states the orchestrator's exclusion that §XVI.6 already implied.
  Modified: Article XVI §6 — "Sonnet executes; Opus reviews increments" becomes: a clean-context
    implementer sub-agent executes; the orchestrator (the main session) reviews increments and does not
    implement. Which model runs the implementer is configuration (the agent definition), not a
    constitutional term.
  Modified: Article XVII §4 — "Neither Sonnet nor Opus auto-invokes Codex" becomes "No Claude agent
    auto-invokes Codex". Same rule, without a model list that goes stale.
  Modified: Article XVII §5 — "Sonnet fixes accepted items" becomes "the implementer sub-agent fixes
    accepted items; the orchestrator does not".
  Modified: Article XX §5 — the illustrative hand-off rule names "the implementer" instead of Sonnet.
  Rationale: the owner moved the implementer and Gate B fixer from Sonnet to the Opus alias
    (2026-09-23). A transcript audit found that the longest Gate B loops ran with the orchestrator as
    fixer and no independent judge, and that the fixer model did not decide whether a fix
    re-introduced the defect it repaired. Naming a model in the constitution made every model change
    an amendment; naming the role does not.
  Affected catalogue rows: none.
  Affected specs and docs (changed): .specify/pipeline.md (flow diagram step 14, new disposition [L]),
    .specify/close-out.md §2 row 15, .specify/codex-review.md, .claude/skills/speckit-implement/SKILL.md
    (step 5a: the orchestrator never implements task bodies; between-phase comment-claim lint),
    CONTRIBUTING.md. Parent-repository counterparts (agent definition, /gate-b, the edit guard) are
    outside this repository.
  Review and sign-off: pending — Codex Gate A on this PR, then the owner's sign-off.
-->
<!--
...
6. **`/implement` is one task at a time, TDD red-green-refactor.** A clean-context implementer sub-agent executes; the orchestrator (the main session) reviews increments and does not implement. Which model runs the implementer is configuration (the agent definition), not a constitutional term.
...
4. **User invokes Codex.** No Claude agent auto-invokes Codex; the gates are user-driven (`codex:codex-rescue` agent or local Codex CLI). The PR description links to the Gate A outcome and the Gate B outcome.

5. **Findings triage:** Opus triages; the implementer sub-agent fixes accepted items (the orchestrator does not); user signs off feature completion at `/specify` boundaries and at module close.
