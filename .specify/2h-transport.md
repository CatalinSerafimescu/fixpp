...
| `Listener::async_accept` | Engine-scoped — does NOT respond to per-session graceful close (the Listener outlives many `Transport` instances). For a per-engine graceful close, treat as terminal. | Same — engine-scoped; per-session graceful phase 2 does not affect the Listener. | Cancels with `transport_accept_cancelled` when engine-level `Listener::cancel()` fires (Listener-owned cancel is engine-scoped). |
...
| `transport_already_connected` | §4.1 — `async_connect` called twice on the same Transport instance before close()/cancel(). Also covers in-flight `async_handshake` overlap (handshake is one-shot per Transport lifetime, like connect — a second `async_handshake` while one is in flight surfaces this variant). | Programmer error — fix the FSM. |
