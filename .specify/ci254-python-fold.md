...
| **N6** | P3 | **Applied** — §3.2 adds the **1457/1457 byte-identical compile-command** evidence and the `CMakeLists.txt:333–335` `add_subdirectory` gating as **E-31**, and §9 item 7 now carries the **mechanism** (⚠️ **SUPERSEDED at Gate B round 1, F7** — this row recorded it as "exactly one ccache miss per leg per run, measured 1461/1 by #247"; #247's measurement was taken against a restore-only reader whose cache could not contain the SWIG wrapper, so the recurring reading does not survive. The mechanism as it now stands is **one additional cacheable edge**, missing on the first compatible run and hitting thereafter) that v0.1 asserted without one |
...
historical evidence at every one of the three sites they appear** — §3.2 (`:85`), risk R1 (`:618`) and
E-8 (`:778`), each now carrying the 3053-edge withdrawal in place. That is the closure: *removed where
...
found `:618` was the one site still stating the figures with no caveat, and found this closure
