...
> 1. **The workflow-level Gate-A trigger fires on the design doc, not on the CMake edit.** `TRIGGER_RE='^(include/|src/|codegen/|\.specify/[^/]+\.md$)'` (`tier1.yml`) does not list `bindings/`. Committing this file matches `^\.specify/[^/]+\.md$`, which is also **explicitly excluded** from the pure-doc auto-waive. So `gate-a-done` / `gate-a-waived` is required either way.
...
>
> **Cites (process / constitutional — see §11 for the normative-reference disposition):** `[const §XVII.1]` (*"Any new design document under `.specify/` … qualifies by default"* — verified in `.specify/constitution.md`; why this doc exists at all), `[const §XVII.7]` (local pre-PR build gate, incl. `pytest bindings/python/tests/` **if the change touches `bindings/python/`**; see §8 PG-9, which is now literally triggered), `[const §XVII.8]` (verify/label evidence rule), `[const §IX.1]` (*"no uncovered error path without an explicit assessment — that is the enforced gate; the percentage is the target"*), `[const §IX.2]` (Tier 1 must run ASan/UBSan/TSan on every PR), `[const §IX.6]` (Tier 1 = *"…Python pytest…"*, i.e. pytest is a required Tier-1 constituent, not an optional lane), `[const §VII.2]` (*"Python tests: pytest against the SWIG bindings"*), `[const §VI.5]` (Normative References, §11).
>
...
| NG-7 | **Do not change `continue-on-error`, `fail-fast`, or the per-leg concurrency groups.** | The matrix already keys its concurrency group on `matrix.preset` (`tier1.yml`), which is correct for six legs that each grow a python tail. |
| NG-8 | **Do not change the default of the new install control, and do not gate it on `SKBUILD`.** | `FIXPP_INSTALL_PYTHON` defaults **ON** (§4.5). An `if(DEFINED SKBUILD)` guard is the tempting one-liner and it **regresses feature 056**: LAY-1 / D-4 / T006 deliberately fixed the **in-tree `cmake --install` path too**, not only the wheel path (`specs/056-python-wheel-packaging/{tasks.md T006, research.md's flat-install-destination paragraph, checklists/packaging.md CHK002}`). Only CI sets it OFF. |

## 3. Current state

### 3.1 The two shapes

| | `linux` matrix job (`tier1.yml`) | `python-bindings` job (`tier1.yml`) |
|---|---|---|
| legs | **6** presets: `linux-clang-{debug,release,asan,ubsan,tsan}`, `linux-gcc-release` | **4**: `none`, `asan`, `tsan`, `ubsan` |
| configure | `cmake --preset <preset>` | `cmake --preset linux-clang-debug` **+** `FIXPP_ENABLE_<SAN>=ON` **+** `BUILD_SHARED_LIBS=ON` **+** `FIXPP_PYTHON_SANITIZER=<san>` **+** `-B build/<san>-py` **+** a `CMAKE_TOOLCHAIN_FILE` override |
| build dir | `build/<preset>` (preset `binaryDir`, `CMakePresets.json`) | `build/linux-clang-debug` (`none`) / `build/linux-clang-<san>-py` (the three sanitizer legs) |
| Conan profile | `conan/profiles/<preset>` | `conan/profiles/<conan_profile>` — aligned per leg by #243/PR #251 |
| tests | **ctest**: 350 tests, 30.5 min on ubsan. **No pytest** for the bindings (pytest *is* installed — the `decimal_compare_oracle` CTest entry needs it) | **pytest** only: 82 tests, **11.8 s**. No ctest |
| `cmake --install` | **yes, on all six legs** — `fixpp::consumer::install-witness` stage-installs (`tests/consumer/run_consumer_witness.cmake`) | **never** — the deleted job runs no install and no CPack |
| CPack | `fixpp-package` on both Release legs, uploaded | none |
| timeout | `240` | `180` |
| gating | `proceed` (both gate labels) — **not** path-gated | `proceed` **and** `python_touched` |
| free-disk step | yes | yes — a duplicate |
| ccache | full install + restore + **save on push** | `none` leg only, **restore-only**, sharing the matrix debug leg's key + a restore-check probe + a stats/liveness step |
...
- **Command-level, not just graph-level, evidence exists and v0.1 omitted it.** #244 part 1 measured **1457/1457 compile commands byte-identical** between the matrix `linux-clang-debug` tree and the python tree once the build dirs are named the same, and established that `FIXPP_BUILD_PYTHON=ON` *"adds exactly one compile edge and perturbs nothing else — it only gates `add_subdirectory(bindings/python)"*. The gating is three lines at `CMakeLists.txt` (verified). This is stronger than the edge-set diff for the debug shape, and it is the **mechanism** behind §9's ccache claim: **one additional cacheable edge** per leg (the SWIG wrapper) — a miss on the first compatible run, hits thereafter. ⚠️ **Not "one miss per run"** (Gate B round 1, F7): #247's 1461/1 measurement was taken against a *restore-only reader* consuming a C++ matrix cache that structurally **could not contain** the wrapper, so the miss was an artefact of the old topology, not a recurring cost of the new one. See §9 item 7; Appendix A, **E-31**.
- **The hand-assembled configuration is cache-variable-identical to the preset.** Flattening `CMakePresets.json`, `linux-clang-debug` + `FIXPP_ENABLE_UBSAN=ON` + `BUILD_SHARED_LIBS=ON` differs from the `linux-clang-ubsan` preset in **`CMAKE_TOOLCHAIN_FILE` alone**, which `tier1.yml` already overrides. No `linux-*` preset carries an `environment` block or a top-level `toolchainFile`. ⇒ **§4.3's "use the preset" is a readability change, not a behavioural one.** Stated deliberately flat: it is not sold as a correctness fix. It establishes equality of *declared cache variables*, not of generated compiler/link commands, CMake policy state, discovered Python variables, or install scripts — E-5 is tagged **`D+I`** accordingly.
  (Verified in this worktree: `CMakePresets.json` — the `asan`/`ubsan`/`tsan` presets already carry `BUILD_SHARED_LIBS=ON`, so the python legs' `-DBUILD_SHARED_LIBS=ON` is and always was redundant — and was never applied to the other three presets.)
...
|---|---:|---|---|
| `push:main` / `workflow_dispatch` / `release` | **316 runner-min** (asan 113.1 · ubsan 99.5 · tsan 95.8 · none 7.6) | apt `swig`+`python3-dev` × 6 · 5 ninja edges × 6 · pytest wall × 6 · ⚠️ **no recurring ccache term** — one cache warm-up on the first compatible run only (§9 item 7, F7) | **≈ −(316 − added)**, added is small but **not zero and not yet measured on all six legs** |
...
The matrix job's existing install step (`tier1.yml`) gains **`swig`** and **`python3-dev`**:
...
Today the sanitizer identity is spelled three times per python leg — the preset name, `FIXPP_ENABLE_<SAN>=ON`, and `FIXPP_PYTHON_SANITIZER=<san>` — plus a fourth time in `san_opts` and a fifth in the runtime basename. Two of those (`preset`, `FIXPP_ENABLE_*`) collapse for free: the preset already carries `FIXPP_ENABLE_<SAN>` and `BUILD_SHARED_LIBS` (`CMakePresets.json`). The remaining three are **derived from `matrix.preset` once** and consumed by expression, never re-spelled.
...
No `-B` and no `CMAKE_TOOLCHAIN_FILE` override: the preset's `binaryDir` is `build/<preset>` (`CMakePresets.json`) and each preset's `CMAKE_TOOLCHAIN_FILE` already points at `build/<preset>/conan_toolchain.cmake`, which is exactly where `conan install … -of build/${{ matrix.preset }}` (`tier1.yml`) puts it. Both overrides existed only to service the `-py` build dirs, which cease to exist.
...
- Carried over **verbatim** from `tier1.yml`, except: `PYTHONPATH` becomes `${{ github.workspace }}/build/${{ matrix.preset }}/lib`, and the three matrix-driven values come from `steps.pysan.outputs.*` instead of `matrix.*`.
- The `libclang_rt` resolution moves unchanged, including both naming schemes (`libclang_rt.<base>-x86_64.so`, then `$(clang -print-runtime-dir)/libclang_rt.<base>.so`) and the **`ubsan` → `ubsan_standalone`** mapping. (Note for the reader of the diff: `bindings/python/CMakeLists.txt` already links the asan/ubsan module against a *shared* sanitizer runtime with an rpath, so `LD_PRELOAD` is belt-and-braces for `import` and load-bearing for the *subprocess children* of the watchdog/canary tests. It is preserved as-is; this change is not the place to test that hypothesis — §10 OQ-6.)
- ⚠️ **Sanitizer options stay per-invocation.** `tier1.yml` scopes them as `env <opts> LD_PRELOAD="$RT" pytest`, and `PYTHONPATH` is set on the step, not the job. **`san_opts` must NOT be promoted to a job-level `env:`** during implementation — that would silently change the C++ ctest run in the same job. Verified: nothing set for pytest reaches ctest or vice versa today.
...
- **Placement:** immediately after the two ctest steps and **before** the packaging block. Rationale: (a) the C++ ctest signal is produced before any python step can stop the job, so a python failure never preempts it; (b) a red python leg then uploads no packages, which matches the existing intent recorded — *"Runs only after a green test step, so a red lane uploads nothing."*
- On the three `none`-mapped legs the sanitizer step is skipped and vice versa; the two are jointly exhaustive over the six legs by construction of the §4.3.2 table (the same "mutually exclusive by `if:`, jointly covering" shape the workflow already uses).
...
- `tier1.yml` runs `cmake --build --target fixpp-package` on **both** `linux-gcc-release` and `linux-clang-release`; uploads `packages-linux-{clang,gcc}-release` with `if-no-files-found: error`. These are the **shipped C++ deliverable**.
- `tests/consumer/run_consumer_witness.cmake` runs `cmake --install "${FIXPP_MAIN_BUILD_DIR}" --prefix "${_stage}"` and is registered on **all six** legs (`tier1.yml`, asserted `expected=1`). So *every* leg executes the install rules after this change, not only the Release ones.
- **L-056-4** (`spec/behaviors-and-limitations.md`) records as delivered behaviour: *"the `packages-linux-{clang,gcc}-release` artifacts are the **C++** consumer deliverable and contain no Python."* Repeated **verbatim in three more files** — `tier1.yml`, `bindings/python/pyproject.toml`, `bindings/python/cibw-before-all.sh` — one of which is the very note §4.8 promises to *keep and re-point*. Shipping the accidental payload would make a note this change deliberately preserves **false**.
- **The existing witnesses cannot see it.** `tests/packaging/run_package_contents_witness.cmake` is a MUST-BE-PRESENT list plus a **7-pattern denylist**, an archive-format extension set, an archive count derived from the shipped `fixppTargets.cmake`, and a raw-listing `usr/` prefix check. A root-level `_fixpp.so` / `fixpp.py` / `_fixpp_data/` matches **none of the seven**. The consumer witness checks for *specific* leaks (`fixpp/_dispatch/`, `fixpp/vt11/`) and for *required* content — there is no allowlist. **Both are structurally blind to added root-level files.** So a `ctest -L 'packaging|consumer'` 9/9 green with bindings ON is a **false-green**, and v0.1's AC-3 could not have discharged R6.
...
guarding the four `install()` calls in `bindings/python/CMakeLists.txt`, plus one reporting line:
...
⚠️ **OFF does not affect the tests.** In-tree pytest runs from `PYTHONPATH=build/<preset>/lib`, staged by the `POST_BUILD` copy, **not** by `install()` — stated in the file's own comment. Turning the install rules off on the matrix legs therefore removes nothing the pytest step depends on.

**The `FIXPP_PY_INSTALL_DIR` branch — three reachable outcomes, and the design must name which one CI is in.** The current logic is:
...
| absolute SITEARCH, **not writable** (e.g. `/usr/lib/python3/dist-packages` from `python3-dev`) | `cmake --install` fails → `run_consumer_witness.cmake` `FATAL_ERROR` → **all six legs RED at ctest** | never reached |
...
staging root, not `${stage}${prefix}`. ⚠️ **Do not copy `tests/consumer/run_consumer_witness.cmake`,
...
count), not merely that ctest was green; `tier1.yml`'s `expected=1` on the consumer witness is
...
| 4 | **job end**, after `Upload packages`, `if: always()` | the operative high-water mark — two steps, below | +2 steps |
...
|---|---|---|---|
| 1 | `.github/workflows/tier1.yml` | the job itself | **DELETE** |
| 2 | `tier1.yml` | `gate-precheck`'s `PY_RE` comment names `python-bindings` as one of the gated jobs, and the four clang profiles as its build recipe | **REWRITE** — the gated set is now the wheel jobs alone; `.specify/` note that #253 owns the literal (NG-4) |
| 3 | `tier1.yml` | *"~1748 instrumented executables … the runner's ~14 GB free-disk ceiling"* | **REWRITE** — the **ceiling moved** (145 G / 87 G pre / 114 G post, 2026-08-08); the 14 GB figure was **accurate when written** (run `28184395001` died at edge 1651/1748); correct the count to **330 executables / 3627 ninja edges (1460 compiles + 1460 scans + 330 links)** |
| 4 | `tier1.yml` | *"pytest … is registered in every preset, **not just the python-bindings build**"* — three copies; after the fold the bindings **are** built in every preset, so the sentence inverts | **REWRITE ×3** |
| 5 | `tier1.yml` | the deleted job's concurrency group | **DELETE** with the job |
| 6 | `tier1.yml` | duplicate of row 3 with `~1987` | **DELETE** with the job |
| 7 | `tier1.yml` | `ccache-hitrate … (python-bindings none)` annotation text | **DELETE** with the job |
| 8 | `tier1.yml` | the wheel-vs-legs note: *"ADDITIVE (CI-8): does NOT touch the python-bindings sanitizer matrix"* + *"the `python-bindings` legs above are a TEST VEHICLE that ships nothing"* + *"`packages-linux-{clang,gcc}-release` … carries no Python"* | **KEEP and RE-POINT** — the test vehicle becomes the six `linux` matrix legs. ⚠️ The *second* half (**no Python in the packages**) stays **TRUE** — that is what §4.5 preserves. This paragraph is the NG-3 record that stops the wheel collapse being re-proposed; it must survive the deletion of the job it points at |
| 9 | `tier1.yml` | the pin's step name — *"Regression pin — tier1.yml python-bindings policy (#251)"* | **REWRITE** with the pin (§5b) |
| 10 | `tier1.yml` | `tier1-required`'s `needs:` list | **REWRITE** — 8 names → 7 (§5) |
| 11 | `tier1.yml` | `tier1-required`'s comment enumerating `python-bindings` as an independently gating job | **REWRITE** — §5.1's site list is **six**, not five; this is the sixth |
| 12 | `tier1.yml` | latency note citing 113.1 / 134.8 for `python-bindings (asan)` | **DELETE** with the assertions (§5) |
| 13 | `tier1.yml` | the `pb=` capture, the diagnostic `echo`, and both `::error::` branches | **DELETE** (§5.1) |
| 14 | `ci/test-tier1-python-policy.sh` (7 sites:) | the whole pin — header, extraction, three assertions, `EXPECTED_NEEDS` | **REWRITE** (§5b) |
| 15 | `ci/test-ccache-scripts.sh` | *"#248 is specifically about extracting TIER 1's in-workflow ccache probes (the ~170 lines of `run:` script around `python-bindings`) … those `run:` blocks are UNCHANGED by this PR"* — this is **#248's scope definition living in a script header**, and #254 deletes those blocks | **REWRITE** — a live operational instruction, not a historical note |
| 16 | `.github/workflows/cache-cleanup.yml` | *"tier1's third site, python-bindings `none`, is `save: false` and never writes"* — this sentence **is** the rationale for the whole state-based reclaim design; after #254 there is no third site | **REWRITE** — live operational rationale |
| 17 | `bindings/python/pyproject.toml` | L-056-4 verbatim, second axis | **RE-POINT** the vehicle noun; the no-Python half stays true |
| 18 | `bindings/python/cibw-before-all.sh` | L-056-4 verbatim, second axis | **RE-POINT** likewise |
| 19 | `spec/behaviors-and-limitations.md` (**L-056-4 itself**) | *"Those bindings are a **test vehicle only — not a byte of them ships**; likewise the `packages-linux-{clang,gcc}-release` artifacts are the **C++** consumer deliverable and contain no Python."* | **RE-POINT** — the vehicle is now the six matrix legs; add a `#254` note that the property is **preserved by `FIXPP_INSTALL_PYTHON=OFF`**, i.e. it stayed true by construction rather than by accident |

| 20 | `bindings/python/tests/conftest.py` | describes the **current CI vehicle** for these tests (the `python-bindings` matrix) | **RE-POINT** — the vehicle is the six `linux` legs. Promoted out of table B by **Appendix D P3(3)** |
| 21 | `bindings/python/tests/test_gil_release_canary.py` | same, **and** already stale on its own terms — it names the matrix as none/asan/tsan; the ubsan lane landed at #159 | **RE-POINT** — two fixes in one line; label them separately in the diff so the pre-existing staleness is not read as introduced by #254 |

⚠️ **Rows 20/21 are NOT safe to land before the deletion.** Together with rows 15 (`ci/test-ccache-scripts.sh`) and 16 (`cache-cleanup.yml`), every one of these four re-points asserts a **post-deletion** state — *"there is no third ccache site"*, *"the vehicle is the six matrix legs"*. Landing them while the `python-bindings` job still exists makes the repo assert something **false**, which is strictly worse than stale (`feedback_stale_anchor_repoint_to_a_plausible_twin_is_worse_than_stale`). **They ship in the same commit as the deletion, not before it.**
...
| site | disposition |
|---|---|
| `bindings/python/tests/conftest.py` | ⚠️ **SUPERSEDED by Appendix D P3(3) → RE-POINT** (moved to table A as row 20). Round 2 was right: this does not merely *incidentally* name the matrix, it describes the **current CI vehicle**, and after the fold that description is false |
| `bindings/python/tests/test_gil_release_canary.py` | ⚠️ **SUPERSEDED by Appendix D P3(3) → RE-POINT** (table A row 21). It is *also* already stale on its own terms — it says the matrix is none/asan/tsan and the ubsan lane landed at #159 — so the re-point fixes two things; say which is which in the diff so the pre-existing staleness is not read as introduced here |
| `CMakeLists.txt` | **LEAVE** — an MSVC `/bigobj` comment mentioning the matrix as context |
| `spec/coverage-index.md` (the `PY-001 DELIVERED` and `PY-002 + PY-003 DELIVERED` entries) · `spec/feature-catalogue.md` (rows `PY-001`, `PY-002`) · `spec/behaviors-and-limitations-closed.md` (`L-054-2`, and the 053 `msg_get_string` argout codec waiver) · `spec/behaviors-and-limitations.md` (`B-075-5`) — all cited by ID, not line, per the #310 gate | **LEAVE — historical.** These record what the PY-001/002/003 features delivered *at the time*, keyed to closed L-rows. Rewriting them would falsify a record |
| `CLAUDE.md`'s PR #251 / PR #247 bullets · `CLAUDE-history.md`'s PR #159 bullet | **LEAVE — historical.** Merged-PR narratives for #251/#247/#159 |
...
`tier1-required` (`tier1.yml`) is the required check. It lists `python-bindings` in `needs:` **and** asserts on `needs.python-bindings.result` in both branches of the `python_touched` split.
...
|---|---|---|---|---|
| `release` | — | early exit, gate n/a | unchanged | unchanged (early exit precedes everything) |
...
**The six sites that move as one unit.** "Assertion deleted" above is **six** edits inside `tier1-required` (v0.1 said five and missed the enumerating comment), and a partial application is worse than none: the `needs:` entry, the `pb='${{ needs.python-bindings.result }}'` capture, the diagnostic `echo` that prints `python-bindings=$pb`, the two branch conjuncts, and the explanatory comment enumerating `python-bindings` as an independently gating job. Drop the conjuncts but keep the capture and the echo, and the log names a job that no longer exists; drop the capture but keep the echo, and the diagnostic prints an empty interpolation. The adjacent latency comment goes with them (§4.8 row 12).
...
- The python build and pytest now ride **inside `linux`**, which `tier1-required` asserts `== success` **unconditionally** for every non-release event.
- `linux` has **no `python_touched` gate** — only `proceed`. So the bindings' signal moves from *conditionally required* (required only when the diff touched a python-relevant path) to *unconditionally required*.
...
- `python_touched` itself stays alive and asserted — the `python-wheel-{build,test}` branches still read it. It simply governs the wheel jobs alone, which is #253's requested split arrived at by deletion (NG-4).
...
7. **Mutants, per `feedback_verification_grep_must_be_proven_nonzero_on_the_unfixed_tree`** — an assertion never shown to fail is not evidence. At minimum:
   ...
   - Each mutant keeps the existing `cmp -s`-based **no-op guard** so a replacement that fails to apply cannot read as a pass, and each keeps its `grep`-the-reason check so a mutant cannot pass the pin for the wrong reason.
...
9. The pin's header comment is rewritten to describe what it now pins. A pin whose self-description is stale is the r3 failure mode of PR #251's own review loop.
...
The whole `python-bindings` job, `tier1.yml` (~555 lines).
...
|---|---|---|
| `Conan restore disposition (observation, not a gate)` | | the Job-Summary disposition line **for the four deleted legs only** |
| `ccache restore check (did the matrix debug leg's cache land?)` | | the cross-job restore probe |
| `ccache stats (none leg — restore-only)` | | the `none` leg's hit-rate annotation |
...
**Including all of #244 part 1's python-side machinery**: the restore-only `ccache-action`, the restore-check probe, and the stats/liveness step.

One consequence worth a line in the PR body: this retires **the repo's only cross-job ccache key share** (the deleted job's restore-only `ccache-action` step; its own note calls it *"the first observation in this repo of ccache-action sharing a key ACROSS jobs"*). After #254 every ccache call site owns its own key again — a simplification of the cache model, not merely a deletion.
...
| **R1** | **Disk.** The bindings land on the leg that already died of ENOSPC. | **Measured, and now instrumented at four points.** 114 G free post-reclaim (CI, pre-build) vs a 32.96 GiB tree (local); bindings add ~126 MiB, `_fixpp.so` 120.83 MiB of it (96 %) — ⚠️ **the `~126 MiB` total and the derived 96 % come from an uncontrolled 3053-edge reconfigure and are retained here only as annotated historical evidence; the independently checkable figure is `_fixpp.so` = 120.83 MiB (§3.2's derivation-hygiene note, E-8)**; ctest's `Testing/` output is 20 KB. ⚠️ **No "×N margin" headline** — the two terms are CI and local respectively (§3.2). **NG-1 keeps the reclaim step** — the margin *is* the reclaim. | Low. The **job-end** measurement (§4.7 point 4) is the operative number and it fails closed if the instrument cannot run. Self-asserting if it ever bites (Build reds). ⚠️ **Fallback, from the issue itself:** if the ubsan leg's job-end headroom is not comfortably positive, **#254 is scoped down to the legs that fit.** |
...
| **R3** | **Path gating.** Merged, `python_touched` no longer skips the bindings work on PRs. | **Quantified, and the cost is real.** On `push:main` it costs nothing — but ⚠️ **that is a tautology, not a measurement**: `tier1.yml` sets `PYTHON_TOUCHED=true` unconditionally for every non-PR event (E-19, retagged **I**). The decision-relevant rate is the **PR-side** one: **11 of the last 30 merged PRs match `PY_RE` (37 %)** (E-30). On the other **63 %** the merged legs pay `+(swig+python3-dev apt) + 5 edges + pytest wall`, × 6, against a deleted job that cost **0**. ⚠️ **There is deliberately no recurring `+ 1 ccache miss` term here** — v0.2 carried one; the SWIG wrapper misses once, on the first compatible run, and hits thereafter (§9 item 7, F7). PR-side pytest wall: **11.61 s** (ubsan) / **5.51 s** (gcc-release) / **5.31 s** (clang-release) — ⚠️ **three of six configurations only**, no debug, asan or **tsan** number, and tsan's subprocess watchdog/canary tests are exactly where extrapolating is unsafe. | Low in absolute minutes, **net negative on 63 % of PRs**, and it buys the §5.2 strengthening (the bindings become unconditionally required). Stated as a trade, not a saving. |
...
| **R5** | **Toolchain setup on legs that do not need it.** | **Nearly nil, and smaller than the issue assumed.** The matrix job **already** runs `actions/setup-python@v6` and `pip install pytest pyyaml` on every leg — pytest is required by the `decimal_compare_oracle` CTest entry. New: `swig` + `python3-dev` on an existing apt line (§4.2). | Seconds per leg. |
...
| **R8** | **`-Werror` on the SWIG wrapper**, newly compiled on `linux-clang-release` (which inherits `FIXPP_WERROR=ON` from `_base`, `CMakePresets.json`; `linux-gcc-release`/`linux-gcc-debug` set it OFF). | **Nil, by inspection, re-verified.** `fixpp_maybe_werror` (`cmake/Helpers.cmake`) is applied **per target**, and `bindings/python/CMakeLists.txt` never calls it — `fixpp_py` gets no `-Werror` under any preset. Verified by grep over that file: zero occurrences. ⚠️ **Superseded by #417 (2026-09-13):** `fixpp_apply_werror_to_all_targets` (`cmake/Helpers.cmake`) now applies `fixpp_maybe_werror` to every compiled target, `fixpp_py` included, so the wrapper IS built with `-Werror` wherever `FIXPP_WERROR=ON`. The risk is no longer nil by construction; a warning in the wrapper now fails that leg. | Nil. Recorded so a reviewer does not have to re-derive it. |
...
| Concurrency-group collisions | **None.** The matrix keys on `tier1-<pr\|ref>-${{ matrix.preset }}`; the deleted job keyed on `…-python-bindings-<san>`. Deletion removes a group |
| Cache-key collisions | **None.** The deleted site was **restore-only** on the matrix debug leg's key (`CCACHE_DIR=/tmp/fixpp-ccache-linux-clang-debug`). Deleting it removes a **reader**, never a writer |
...
| `linux-clang-coverage`, tier 2, tier 3 | **Untouched.** `coverage` is a separate job with its own preset; `cmake/Codegen.cmake` already passes `-DFIXPP_BUILD_PYTHON=OFF` to the bootstrap codegen build |
...
| `release` | **By inspection**, unchanged: the early exit precedes every python assertion | No release is being cut for this PR |
...
| **CO-5** | **Actions-cache archive sizes and eviction effect** — the OPEN half of **R2-P2-4**, made executable. §9 item 7 says the pool effect is *small and POSITIVE, and UNMEASURED*; this is the row that measures it. | `gh api -H "Accept: application/vnd.github+json" "/repos/CatalinSerafimescu/fixpp/actions/caches?per_page=100"`, taken **twice**: once on the last pre-merge `push:main` run and once after the first post-merge `push:main` run has completed all six legs. Compare `size_in_bytes` for the six `ccache-linux-<preset>-*` keys (the tier-1 writers) and record the delta per key; the expected sign is **positive** and the expected magnitude is **≥ one SWIG wrapper object**. **Eviction is identified** as a key present in the before-listing and absent from the after-listing, or a `total_active_caches_size_in_bytes` at the 10 GB ceiling — either is a finding to report, not a footnote. **Owner:** whoever merges #254. **Completion criterion:** both listings and the per-key delta table written into `.specify/decisions/pr258-python-fold-gateb.md`. ⚠️ **This row is a PREREQUISITE for closing #254** — the issue does not close on "CI was green" (Gate B round 3, Codex finding 8). |
...
1. **The install path, on all six legs.** `fixpp::consumer::install-witness` stage-installs on every leg; with the bindings ON and the control absent, an absolute-unwritable `Python3_SITEARCH` would `FATAL_ERROR` in `run_consumer_witness.cmake` and red **six** legs at ctest, while an absolute-writable one would silently pollute the runner's interpreter and evade any prefix diff. §4.5 closes all three branches; §4.5.3 asserts it per leg.
2. **The two Release legs' uploaded artifacts.** `packages-linux-{clang,gcc}-release` are the shipped C++ deliverable and `linux-clang-release` runs **no packaging ctest tier at all** (`-L packaging` is gated to `linux-gcc-release`) — so that artifact has no content witness of any kind. Bounded by §4.5, not by the existing witnesses.
...
6. **Conan/GHCR caches: unaffected.** The matrix legs keep `-pr conan/profiles/<preset>` and their own seeds; deleting a **restore-only** consumer removes a reader, never a writer, so no seeded package is invalidated and nothing needs re-seeding.
...
8. **This PR's own gate posture.** See the header's *"What the scope class means for gate labelling"*: the workflow trigger fires on `^\.specify/[^/]+\.md$`, which is also excluded from the pure-doc auto-waive; and the CMake edit makes the "CI-only, no product code" waiver rationale used on #227/#245/#247/#250/#251 **inapplicable**. `.specify/**` sits in `paths-ignore` for **`push:`** only, never `pull_request` — so the doc suppresses no PR-side signal.
...
| `[const §VII.2]` | | *"Python tests: pytest against the SWIG bindings"* — the binding test framework |
| `[const §IX.1]` | | *"…that is the enforced gate; the percentage is the target"* — used **by analogy** in §4.7 for the disk floor, not as a coverage claim |
| `[const §IX.2]` | | *"Sanitizers — Tier 1 (every PR, Linux/Clang): ASan, UBSan, TSan must all run and pass"* — why a silently-uninstrumented leg is a constitutional false-green (§4.3.2) |
| `[const §IX.6]` | | Tier 1 enumerates *"…Python pytest…"* — pytest is a required Tier-1 constituent, not an optional lane |
| `[const §XVII.1]` | | *"Any new design document under `.specify/` … qualifies by default"* — why this doc goes through Gate A |
| `[const §XVII.7]` | | local pre-PR build gate; **path-conditioned** on `bindings/python/` — met literally once §4.5 lands (PG-9) |
| `[const §XVII.8]` | | `/speckit-verify` record requirement; noted for the Gate B precondition, not exercised by this doc |
...
| E-4 | edge composition of the tree | **1460 compiles + 1460 scans + 330 links = 3627** | M | same run. ⇒ the `~1748` (`tier1.yml`) and `~1987` comments contradict each other and are **both wrong** |
| E-5 | cache-variable delta, hand-assembled ubsan vs the `linux-clang-ubsan` preset | **`CMAKE_TOOLCHAIN_FILE` only** | **D+I** | flattened `CMakePresets.json` (**derivation** over **inspected** tracked JSON — retagged from `M+I` in v0.1); the override is at `tier1.yml`. ⚠️ Covers *declared cache variables*, not generated compiler/link commands, policy state, discovered Python variables, or install scripts |
...
| E-19 | `python_touched` on non-PR events | **`true` unconditionally** — 12 of the last 12 runs, **by construction** | **I** | `tier1.yml`: `if [ "$EV" = "pull_request" ]; then … else PYTHON_TOUCHED=true; fi`. ⚠️ **Retagged M→I in v0.2.** The "12 of 12" is the workflow's `else` branch restated as data, not a measurement of anything |
| E-20 | R5 — setup already present | `actions/setup-python@v6` + `pip install pytest pyyaml` on **every** matrix leg | I | `tier1.yml`; pytest required by `decimal_compare_oracle` |
| E-21 | apt delta | **`swig` + `python3-dev`** | I | matrix vs python. ⚠️ corrects the brief's *"only `swig` is new"* |
| E-22 | `BUILD_SHARED_LIBS` across presets | ON for `linux-clang-{asan,ubsan,tsan}`; **not set** for `linux-clang-{debug,release}`, `linux-gcc-release` | I | `CMakePresets.json` ⇒ the python legs' `-DBUILD_SHARED_LIBS=ON` was redundant on three legs and **never applied to the other three** |
| E-23 | `tier1-required` shape | `needs:` 8 names; `python-bindings` asserted (`success`) (exactly `skipped`); enumerating comment | I | ⇒ §5. **Both** branches red if left behind — corrects the brief's "the false branch". **Six** sites, not five |
| E-24 | the regression pin | `ci/test-tier1-python-policy.sh` — `jobs["python-bindings"]`; needs-census; step-parameterisation; mutants A/C1/C2/D/E; invoked from `ci-script-pins` at `tier1.yml`; `ci-script-pins` **ungated** and asserted | I | ⇒ §5b. **Absent from the brief's edit list** |
| E-25 | R6 — install rules | four `install()` calls, **unconditional** under `FIXPP_BUILD_PYTHON=ON`; destination `${Python3_SITEARCH}` **or** `"."` chosen by the preceding `if(SKBUILD)` block | I | `bindings/python/CMakeLists.txt`. Reached by: consumer stage-install on **all six** legs (`tier1.yml`, `tests/consumer/run_consumer_witness.cmake`), `fixpp-package` on both Release legs, upload; packaging tier gated to `linux-gcc-release` |
| E-26 | R8 — no `-Werror` on the wrapper | `fixpp_maybe_werror` is per-target (`cmake/Helpers.cmake`) and **never called** in `bindings/python/CMakeLists.txt` | I | grep over that file: 0 occurrences. `linux-clang-release` inherits `FIXPP_WERROR=ON` (`CMakePresets.json`); `linux-gcc-release` sets it OFF — neither reaches `fixpp_py`. ⚠️ **Superseded by #417 (2026-09-13):** the option now reaches `fixpp_py` through `fixpp_apply_werror_to_all_targets`; re-derive from `cmake/Helpers.cmake`, not from this row |
| E-27 | test-step source | `Run Python tests` `tier1.yml`; `Run Python tests under sanitizer` (runtime resolution, `ubsan → ubsan_standalone`, `LD_PRELOAD`, `PYTHONPATH`, per-invocation `san_opts`) | I | brief's line reference for the sanitizer step (**1572**) confirmed |
...
| **E-29** | net runner-minute arithmetic | `net = 316 − (apt swig+python3-dev × 6) − (5 ninja edges × 6) − (pytest wall × 6)` | **D** | E-11 minus E-18/E-31 terms. ⚠️ Every subtrahend is small; **the label is what was wrong in v0.1, not the magnitude**. And the whole expression is conditional on the event class — E-30. ⚠️ **v0.2 carried a fourth subtrahend, `− (1 ccache miss × 6)`; it is REMOVED, not merely annotated** — §9 item 7 forbids carrying a per-run miss into the net arithmetic, and this row is the arithmetic it was forbidding (Gate B round 2, R2-N1: the F7 restatement had landed at 1 of the 4 sites round 1 named, and this was the load-bearing one) |
| **E-30** | PR-side `PY_RE` match rate | **11 of the last 30 merged PRs (37 %)** | M | `gh pr list --state merged --limit 30` × `gh pr view --json files`, each file list tested against `PY_RE` (`tier1.yml`), 2026-08-09. ⇒ on **19/30 (63 %)** of merged PRs the deleted job cost **0** and #254 is **net positive** runner-minutes. ⚠️ Computed with **today's** `PY_RE` — see OQ-9 |
| **E-31** | command-level merge evidence + the ccache prediction | **1457/1457 compile commands byte-identical**; `FIXPP_BUILD_PYTHON=ON` *"adds exactly one compile edge and perturbs nothing else"*; measured on run `31256807981`: **1461 hits / 1 miss over 1462 cacheable calls** | M | #244 part 1 / PR #247 (`CLAUDE.md`'s PR #247 bullet, issue #254 body). Gating mechanism verified in the root `CMakeLists.txt`'s `if(FIXPP_BUILD_PYTHON) add_subdirectory(bindings/python) endif()` guard — **I**. ⇒ this is the **mechanism** behind §9.7's cache claim, which v0.1 asserted without one |
...
| **E-33** | L-056-4 invariant site count | **4** — `spec/behaviors-and-limitations.md`, `tier1.yml`, `bindings/python/pyproject.toml`, `bindings/python/cibw-before-all.sh` | M+I | two-axis census, 2026-08-09: `grep -rn 'python-bindings'` **and** `grep -rn 'no Python\|TEST VEHICLE\|not a byte of them ships'`, excluding `specs/`. ⚠️ **The second axis is not optional**: L-056-4's own home file does not contain the string `python-bindings` and is invisible to the first |
...
| **#3(d)** | **P2** | CONFIRM | **Applied** — §4.7 delivers the issue's **four `df` points**, the operative one at **job end** (after `Upload packages`) rather than after Build, because the consumer install-witness, the packaging sub-project builds and CPack all land later. The issue's **fallback** ("scope down to the legs that fit") is recorded under **R1**, which v0.1 omitted entirely |
...
| **N6** | P3 | **Applied** — §3.2 adds the **1457/1457 byte-identical compile-command** evidence and the `CMakeLists.txt` `add_subdirectory` gating as **E-31**, and §9 item 7 now carries the **mechanism** (⚠️ **SUPERSEDED at Gate B round 1, F7** — this row recorded it as "exactly one ccache miss per leg per run, measured 1461/1 by #247"; #247's measurement was taken against a restore-only reader whose cache could not contain the SWIG wrapper, so the recurring reading does not survive. The mechanism as it now stands is **one additional cacheable edge**, missing on the first compatible run and hitting thereafter) that v0.1 asserted without one |
...
| **R2-P2-4** | "Actions-cache pool strictly reduced" / "no effect on the 10 GB budget" are **false**: deleting the restore-only job removes a *reader*, but enabling the binding adds one cacheable SWIG wrapper object to each of the six *writers*. | Restate as: writer set unchanged, one reader removed, **contents change by ≥ the wrapper object**. Drop "strictly reduced" and "no effect". Record archive sizes / eviction effects at close-out.  **Two clauses, tracked separately (Gate B round 2, finding 4a — a single ✅ over a two-clause requirement is machine-invisible):** ✅ **DONE (restatement)** — §9 item 7 restated in v0.3; the two false phrases are gone and the residual is named as UNMEASURED-at-close-out rather than predicted. ⬜ **OPEN (close-out measurement)** — archive sizes and eviction effects cannot exist before the six merged legs have run. **Executable as §8.3 CO-5** (Gate B round 3, Codex finding 8: an OPEN marker pointing at a decision record that does not exist yet is prose, not a gate), which names the API query, the six keys, the before/after comparison, how eviction is identified, the owner and the completion criterion. **#254 must not be closed on "CI was green".** |
...
P3s, also binding but cheap — **all four CLOSED (1) at round 2, (2)–(4) at round 1**: **(1)** ✅ *done,
and the scope is stated rather than implied* — **the `~126 MiB` total and the derived "96 %" are
removed from `tier1.yml`** (the only place they appeared outside this doc, and the workflow never
carried the "96 %" at all, contrary to round 1's F2), **and retained inside this doc as annotated
historical evidence at every one of the three sites they appear** — §3.2, risk R1 and
E-8, each now carrying the 3053-edge withdrawal in place. That is the closure: *removed where
it could be quoted as a current measurement, annotated where it is the record of what was measured.*
⚠️ **The requirement's original wording — "drop the total and the derived 96 %" — is deliberately NOT
executed literally**, because deleting the figures would also delete the evidence that they were
withdrawn; the caveats are what annotate the hazard at the point of use. Gate B round 2 (finding 5)
found was the one site still stating the figures with no caveat, and found this closure
asserting a scope it had not achieved while being immediately followed by the unqualified requirement
sentence it contradicts. Both are now fixed. The independently checkable figure remains
`_fixpp.so` = 120.83 MiB. **(2)** ✅ *done* — PG-2 restated in §8. PG-2's pytest rule was wrong on two counts — pytest exits
...
touched-file count was **9** excluding this doc, not 8; and `bindings/python/tests/conftest.py` +
`test_gil_release_canary.py` describe the **current** CI vehicle (the latter already stale — it
