...
| E-26 | R8 — no `-Werror` on the wrapper | `fixpp_maybe_werror` is per-target (`cmake/Helpers.cmake`) and **never called** in `bindings/python/CMakeLists.txt` | I | grep over that file: 0 occurrences. `linux-clang-release` inherits `FIXPP_WERROR=ON` (`CMakePresets.json`); `linux-gcc-release` sets it OFF — neither reaches `fixpp_py` |
