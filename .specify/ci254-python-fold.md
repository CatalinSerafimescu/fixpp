...
| **PG-2** | **Every `linux-*` leg — including both Release legs — runs pytest with a non-vacuous test count.** | The pytest step itself, on all six legs, with `continue-on-error` off. ⚠️ **Assert the count, not the exit code** — an empty pytest selection exits 0. **One rule for all six legs** (v0.1 carried two, `82 passed` on some and "nonzero" on others): the step asserts `N passed` with **N ≥ the count `pytest --collect-only -q` reports in the same step**. No hardcoded `82` — the repo adds pytests regularly (053 → 054 → 055 → 056), and a hardcoded count turns a normal test addition into a red CI criterion. |
...
7. **Actions-cache pool: strictly reduced, with a mechanism.** The deleted ccache call site was `save: false`, so the pool's *write* set is unchanged and one reader disappears. The added cost per surviving leg is **exactly one ccache miss per run** — the SWIG wrapper edge — which #247 measured directly (1461 hits / 1 miss over 1462 cacheable calls) and which the 1457/1457 byte-identical command comparison predicted (E-31). No effect on the 10 GB budget #240/#241 are measured against, in either direction.
...
P3s, also binding but cheap: **(1)** drop the `~126 MiB` total and the derived "96 %" — the
uncontrolled 3053-edge reconfigure cannot support them; keep only the independently measured
`_fixpp.so` = 120.83 MiB. **(2)** PG-2's pytest rule is wrong on two counts — pytest exits **non-zero**
when it collects nothing (it is not `ctest -L`'s vacuous success), and "passed ≥ collected" fails on
any legitimate skip/xfail; use `--collect-only` > 0, then trust pytest's own status. **(3)** the
...
omits UBSan), so they are re-point targets, not history. **(4)** E-30 must record the PR-number
