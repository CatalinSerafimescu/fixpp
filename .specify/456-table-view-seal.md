...
| `include/fixpp/dict/dictionary_snapshot.hpp` | *"`Dictionary::as_table_view()` … stays public and keeps returning a mutable `table_view` BY VALUE … so this type does not make `table_view` itself immutable."* | **rewrite.** Every clause is false afterwards. This is the shipped public header whose whole purpose is to explain the #215 boundary. |
...
| `include/fixpp/dict/table_view.hpp` | the class contract's *"Constructed ONCE at session/validator setup time … Immutable after construction"* | **amend** — it stops being an aspiration the type does not enforce and becomes a statement of what the type is. Naming `table_view_builder` here is the pointer a reader needs. |
| `include/fixpp/dict/dictionary.hpp` | `as_table_view()`'s *"the returned table_view is immutable and must not be rebuilt on the per-message hot path"* | **amend** — "immutable" is now enforced rather than asserted; say which. |
